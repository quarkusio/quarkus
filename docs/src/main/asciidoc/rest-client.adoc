...
A request filter can also wrap the stream the request entity is written to, for example to compress it, by replacing the entity stream:
...
public class GzipClientRequestFilter implements ClientRequestFilter {
    ...
    public void filter(ClientRequestContext requestContext) throws IOException {
        requestContext.setEntityStream(new GZIPOutputStream(requestContext.getEntityStream()));
        requestContext.getHeaders().putSingle("Content-Encoding", "gzip");
    }
}
----

The entity stream applies to entities serialized by a `MessageBodyWriter`; `File`, `Path`, `InputStream`, `Multi` and multipart entities are streamed as they are, and replacing the entity stream for such a request fails with an `IllegalStateException`.
