 ...
 * Telemetry SPI, so we need to rely on the Metrics SPI. Right now, it is not possible to register multiple
 * <code>VertxMetrics</code>, meaning that only a single one is available per Quarkus instance. To avoid clashing with
 * other extensions that provide Metrics data (like the Micrometer extension), we only register the
 * {@link OpenTelemetryVertxMetricsFactory} if the <code>VertxHttpServerMetrics</code> is not available in the runtime.
 */
public class OpenTelemetryVertxMetricsFactory implements VertxMetricsFactory {
    ...
    public static class OpenTelemetryVertxServerMetrics implements HttpServerMetrics<MetricRequest, Object, Object> {
        @Override
