...
method and `server.port` into `Server.port()` method. The configuration property name to look up is constructed from
the prefix and the method name, separated by a dot (`.`) character.
