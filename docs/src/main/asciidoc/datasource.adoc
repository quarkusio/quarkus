...
we resolve the JDBC driver to be used automatically, so you don't need to configure it yourself.
If you want to use a database kind that is not part of this list, just use `other` or whatever name you prefer.

[NOTE]
====
Although it is possible to use any JDBC driver in JVM mode, it is unlikely that it will work when compiling your application to a native executable.

If you plan to do the latter, we recommend you to use the existing JDBC Quarkus extensions (or contribute one for your driver).
====
...
the extension does not support compiling the embedded database engine into native images.
