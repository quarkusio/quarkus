...
public class ProcessUtil {
    ...
    /**
     * Launches and returns a {@link Process} built from the {@link ProcessBuilder builder}. Before launching the
     * process, this method checks if inherit IO is disabled and if so, streams both the {@code STDOUT} and
     * {@code STDERR} of the launched process using {@link #streamToSysOutSysErr(Process)}. Else, it launches the
     * process with {@link ProcessBuilder#inheritIO()}
     *
     * @param builder
     *        The process builder
     * @param shouldRedirectIO
     *        Whether {@link java.lang.ProcessBuilder.Redirect#INHERIT} can be used for launching the process
     *
    ...
    /**
     * Launches and returns a {@link Process} built from the {@link ProcessBuilder builder}. Before launching the
     * process, this method checks if inheritIO is disabled and if so, streams (only) the {@code STDOUT} of the launched
     * process using {@link #streamOutputToSysOut(Process)} (Process)}. Else, it launches the process with
     * {@link ProcessBuilder#inheritIO()}
     *
     * @param builder
     *        The process builder
     * @param shouldRedirectIO
     ...
     * @throws IOException
     */
    public static Process launchProcessStreamStdOut(final ProcessBuilder builder, boolean shouldRedirectIO)
            throws IOException {
        if (!shouldRedirectIO) {
            return builder.inheritIO().start();
        }
    ...
    /**
     * This is a convenience method which internally calls both the {@link #streamOutputToSysOut(Process)} and
     * {@link #streamErrorToSysErr(Process)} methods
     *
     * @param process
    ...
    /**
     * Streams the {@link Process process'} {@code STDOUT} to the current process' {@code System.out stream}. This
     * creates and starts a thread to stream the contents. The {@link Process} is expected to have been started in
     * {@link java.lang.ProcessBuilder.Redirect#PIPE} mode
     *
     * @param process
    ...
    /**
     * Streams the {@link Process process'} {@code STDERR} to the current process' {@code System.err stream}. This
     * creates and starts a thread to stream the contents. The {@link Process} is expected to have been started in
     * {@link java.lang.ProcessBuilder.Redirect#PIPE} mode
     *
     * @param process
    ...
    /**
     * Streams the {@link Process process'} {@code STDERR} to the given {@code printStream}. This creates and starts a
     * thread to stream the contents. The {@link Process} is expected to have been started in
     * {@link java.lang.ProcessBuilder.Redirect#PIPE} mode
     *
     * @param process
     *        The process whose STDERR needs to be streamed.
