...
NOTE: This section is not an explanation of the reactive model. Refer to that xref:quarkus-reactive-architecture.adoc[the {product-name-first} Reactive Architecture] for more details.

Under the hood, {product-name} uses a reactive engine.
This engine provides the efficiency and concurrency to cope with modern, containerized, and cloud-native applications.

For example, when you use RESTEasy Reactive or gRPC, {product-name} can invoke your business logic on I/O threads.
...
When using the imperative model, {product-name} associates a worker thread to each processing unit (like an HTTP request or a gRPC invocation).
...
In {product-name}, when you execute reactive code, you run in a _Context_, representing the execution thread (event loop or worker thread).
...
IMPORTANT: Most {product-name} clients do this automatically, invoking the continuation in the proper context.
...
In {product-name}, access to the local data is restricted to duplicated contexts.
...
Vert.x web will create a new duplicated context for each http web request; {product-name} RESTEasy Reactive will mark such contexts as safe.
...
In general, {product-name} invokes reactive code on duplicated contexts.
...
- Funqy
- {product-name} scheduler and Quartz
...
In {product-name}, the CDI request scope is stored in a duplicated context, meaning it gets automatically propagated alongside the reactive processing of a request.
...
- xref:quarkus-reactive-architecture.adoc[{product-name} Reactive Architecture]
- https://vertx.io/docs/vertx-core/java/#_reactor_and_multi_reactor[Vert.x Reactor and Multi-Reactor documentation]
- xref:logging.adoc[{product-name} logging]