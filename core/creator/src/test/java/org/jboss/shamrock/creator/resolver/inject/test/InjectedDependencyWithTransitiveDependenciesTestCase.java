 ...
 */
public class InjectedDependencyWithTransitiveDependenciesTestCase extends CollectDependenciesBase {
    ...
    protected void setupDependencies() throws Exception {
        ...
        final TsArtifact deployment = new TsArtifact("deployment")
                .addDependency(transitive2)
                .addDependency(transitive3);
        install(deployment, "runtime");

        installAsDep(
                TsArtifact.jar("extension"),
                newJar().addFile(
                        PropsBuilder.init(BootstrapConstants.PROP_INJECT_DEPS, deployment.toString()).build(),
