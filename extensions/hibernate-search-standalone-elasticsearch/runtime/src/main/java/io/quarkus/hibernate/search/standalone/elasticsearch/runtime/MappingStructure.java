...
public enum MappingStructure {
    ...
    /**
     * Entities indexed through Hibernate Search are the root of a document. With this structure: * An indexed entity
     * "owns" other entities it references through associations, which *cannot* be updated independently of the indexed
     * entity; in particular they cannot be passed to
     * {@link org.hibernate.search.mapper.pojo.standalone.work.SearchIndexingPlan#addOrUpdate(Object)}. * Therefore,
     * when an entity changes, Hibernate Search doesn't need to resolve other entities to reindex, which means in
     * particular that associations between entities can be uni-directional: specifying the inverse side of associations
     * through `@AssociationInverseSide` *is not required*. See also
