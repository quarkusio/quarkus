...
public class EnforcingPlatformForConditionalDepsTest extends QuarkusGradleWrapperTestBase {
    ...
    private static final String CONDITIONAL_GROUP_ID = "org.enforcing.conditional";
    ...
    public void conditionalDevDependenciesWithoutConstraints() throws Exception {
        assertConditionalDependencies(
                ":runner:quarkusGenerateDevAppModel",
                "runner",
                "ext-a:1.0-SNAPSHOT",
    ...
    public void enforcePinnedVersion() throws Exception {
        assertConditionalDependencies(
                ":runner-enforced:quarkusGenerateDevAppModel",
                "runner-enforced",
                "ext-a:1.0-SNAPSHOT",
                "ext-a-deployment:1.0-SNAPSHOT",
    ...
    public void excludeConditionalDependencyFromConsumer() throws Exception {
        assertConditionalDependencies(
                ":runner-excluded-1:quarkusGenerateDevAppModel",
                "runner-excluded-1",
                "ext-a:1.0-SNAPSHOT",
                "ext-a-deployment:1.0-SNAPSHOT");
    }
    ...
    public void excludeConditionalDependencyFromBom() throws Exception {
        assertConditionalDependencies(
                ":runner-excluded-2:quarkusGenerateDevAppModel",
                "runner-excluded-2",
                "ext-a:1.0-SNAPSHOT",
                "ext-a-deployment:1.0-SNAPSHOT",
                "dev-mode-only-lib:9.4.0");
     ...
     * asserts that the resolved set of artifacts belonging to {@link #CONDITIONAL_GROUP_ID} matches exactly the
     * expected coordinates (artifactId:version) in any order.
     *
     * @param task the Gradle task to invoke (e.g. :runner:quarkusGenerateDevAppModel)
     ...
     */
    private void assertConditionalDependencies(String task, String moduleDirName, String... expectedArtifacts)
            throws Exception {
        var projectDir = getProjectDir(CONSUMER_PROJECT_PATH);
        runGradleWrapper(projectDir, "clean", task);
        ...
        var conditionalArtifacts = ((Collection<? extends Dependency>) appModel.getDependencies()).stream()
                .filter(d -> CONDITIONAL_GROUP_ID.equals(d.getGroupId()))
