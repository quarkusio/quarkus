...
| All output
|`quarkus.otel.experimental.otlp.default.enable`
|false
|If true, the default OTLP exporter is created even when another exporter is already present for
that signal, allowing both to coexist. This is a *build* time property.
...
By default, if a custom exporter is present in the CDI container — for example one contributed by a
Quarkiverse extension, or a bean defined directly in the application — Quarkus does not create its
built-in OTLP exporter for that signal. This lets you fully replace the default exporter with your
own without any leftover output.

If instead you want the default OTLP exporter to keep exporting *in addition to* any other exporters
that are present, set:
...
This is a *build* time property. With it enabled, telemetry is sent both to the default OTLP endpoint
and to any other configured exporters, so you can, for example, combine a Quarkiverse exporter with
the default collector without having to choose one or the other.
