...
{product-name-first} introduces an intermediate abstraction called `Credentials Provider`,
...
not yet implemented in {product-name} (e.g. Azure Key Vault).
...
Implementing a custom credentials provider is the only option when a vault product is not yet supported in {product-name}, or if credentials need to be retrieved from a custom store.
