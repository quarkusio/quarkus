...
:summary: This guide explains how to implement gRPC services in your {product-name-first} application.
...
{product-name} generates a few implementation classes for services declared in the `proto` file:
...
[[health]]
== Health Check
For the implemented services, {product-name} gRPC exposes health information in the following format:
[source,protobuf]
----
syntax = "proto3";
...
Additionally, if {product-name} SmallRye Health is added to the application, a readiness check for
the state of the gRPC services will be added to the MicroProfile Health endpoint response, that is `/q/health`.

== Reflection Service

{product-name} gRPC Server implements the https://github.com/grpc/grpc/blob/master/doc/server-reflection.md[reflection service].
...
NOTE: {product-name} exposes both the reflection service `v1` and `v1alpha`.
...
public class HelloServiceTest implements Greeter {
    ...
    void shouldReturnHello() {
        ...
        client.sayHello(HelloRequest.newBuilder().setName("{product-name}").build())
                .subscribe().with(reply -> message.complete(reply.getMessage()));
        assertThat(message.get(5, TimeUnit.SECONDS)).isEqualTo("Hello {product-name}");
    }
}
----

== Trying out your services manually
In the dev mode, you can try out your gRPC services in the {product-name} Dev UI.
