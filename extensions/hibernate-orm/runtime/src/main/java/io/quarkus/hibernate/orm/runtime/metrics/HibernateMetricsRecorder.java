...
public class HibernateMetricsRecorder {
        ...
        return new Consumer<MetricsFactory>() {
            ...
            public void accept(MetricsFactory metricsFactory) {
                ...
                for (String puName : jpaConfig.getPersistenceUnits()) {
                    SessionFactory sessionFactory = jpaConfig.getEntityManagerFactory(puName)
     ...
     *        Quarkus MetricsFactory for generic metrics registration
     * @param puName
    ...
    void registerMetrics(MetricsFactory metricsFactory, String puName, Statistics statistics) {
        ...
        // Session statistics
        createStatisticsCounter(metricsFactory, "hibernate.sessions.open", "Global number of sessions opened", puName,
                statistics, Statistics::getSessionOpenCount);
        createStatisticsCounter(metricsFactory, "hibernate.sessions.closed", "Global number of sessions closed", puName,
                statistics, Statistics::getSessionCloseCount);

        // Transaction statistics
        createStatisticsCounter(metricsFactory, "hibernate.transactions",
                "The number of transactions (see result for success or failure)", puName, statistics,
        ...
        createStatisticsCounter(metricsFactory, "hibernate.flushes",
                "Global number of flush operations executed (either manual or automatic).", puName, statistics,
        ...
        createStatisticsCounter(metricsFactory, "hibernate.statements",
                "The number of prepared statements (see status for prepared or closed)", puName, statistics,
        ...
        Arrays.stream(statistics.getSecondLevelCacheRegionNames())
                ...
                .forEach(regionName -> {
                    ...
                    createStatisticsCounter(metricsFactory, "hibernate.second.level.cache.requests",
                            "The number of requests made to second level cache (see result for hit or miss)", puName,
                            regionStatistics, CacheRegionStatistics::getHitCount, "result", "hit", "region",
                            regionName);
                    createStatisticsCounter(metricsFactory, "hibernate.second.level.cache.requests",
                            "The number of requests made to second level cache (see result for hit or miss)", puName,
                            regionStatistics, CacheRegionStatistics::getMissCount, "result", "miss", "region",
                            regionName);
                    createStatisticsCounter(metricsFactory, "hibernate.second.level.cache.puts",
                            "The number of entities/collections put in the second level cache", puName,
                            regionStatistics, CacheRegionStatistics::getPutCount, "region", regionName);
                });

        // Entity Information
        createStatisticsCounter(metricsFactory, "hibernate.entities.loads", "Global number of entity loads", puName,
                statistics, Statistics::getEntityLoadCount);
        createStatisticsCounter(metricsFactory, "hibernate.entities.updates", "Global number of entity updates", puName,
                statistics, Statistics::getEntityUpdateCount);
        createStatisticsCounter(metricsFactory, "hibernate.entities.inserts", "Global number of entity inserts", puName,
                statistics, Statistics::getEntityInsertCount);
        createStatisticsCounter(metricsFactory, "hibernate.entities.deletes", "Global number of entity deletes", puName,
                statistics, Statistics::getEntityDeleteCount);
        createStatisticsCounter(metricsFactory, "hibernate.entities.fetches", "Global number of entity fetches", puName,
                statistics, Statistics::getEntityFetchCount);

        // Collections
        createStatisticsCounter(metricsFactory, "hibernate.collections.loads", "Global number of collections loaded",
                puName, statistics, Statistics::getCollectionLoadCount);
        createStatisticsCounter(metricsFactory, "hibernate.collections.updates", "Global number of collections updated",
                puName, statistics, Statistics::getCollectionUpdateCount);
        createStatisticsCounter(metricsFactory, "hibernate.collections.deletes", "Global number of collections removed",
        ...
        createStatisticsCounter(metricsFactory, "hibernate.collections.recreates",
                "Global number of collections recreated", puName, statistics, Statistics::getCollectionRecreateCount);
        createStatisticsCounter(metricsFactory, "hibernate.collections.fetches", "Global number of collections fetched",
                puName, statistics, Statistics::getCollectionFetchCount);

        // Natural Id cache
        createStatisticsCounter(metricsFactory, "hibernate.natural.id.requests",
                "The number of natural id cache requests (see result for hit or miss)", puName, statistics,
                Statistics::getNaturalIdCacheHitCount, "result", "hit");
        createStatisticsCounter(metricsFactory, "hibernate.natural.id.cache.puts",
                "The number of cacheable natural id requests put in cache", puName, statistics,
        ...
        createStatisticsCounter(metricsFactory, "hibernate.natural.id.executions",
                "The number of natural id query executions", puName, statistics,
                Statistics::getNaturalIdQueryExecutionCount);
        createTimeGauge(metricsFactory, "hibernate.query.natural.id.executions.max",
                "The maximum query time for natural id queries executed against the database", puName, statistics,
                Statistics::getNaturalIdQueryExecutionMaxTime);

        // Query statistics
        createStatisticsCounter(metricsFactory, "hibernate.query.executions", "The number of query executions", puName,
                statistics, Statistics::getQueryExecutionCount);
        createTimeGauge(metricsFactory, "hibernate.query.executions.max",
                "The maximum query time for queries executed against the database", puName, statistics,
                Statistics::getQueryExecutionMaxTime);

        // Query Cache
        createStatisticsCounter(metricsFactory, "hibernate.cache.query.requests",
                "The number of query cache requests (see result for hit or miss)", puName, statistics,
        ...
        createStatisticsCounter(metricsFactory, "hibernate.cache.query.puts",
                "The number of cacheable queries put in cache", puName, statistics, Statistics::getQueryCachePutCount);
        createStatisticsCounter(metricsFactory, "hibernate.cache.query.plan",
                "The number of query plan cache requests (see result for hit or miss)", puName, statistics,
        ...
        createStatisticsCounter(metricsFactory, "hibernate.cache.update.timestamps.requests",
                "The number of update timestamps cache requests (see result for hit or miss)", puName, statistics,
        ...
        createStatisticsCounter(metricsFactory, "hibernate.cache.update.timestamps.puts",
                "The number of update timestamps put in cache", puName, statistics,
    ...
    <T> void createStatisticsCounter(MetricsFactory metricsFactory, String metricName, String description,
        ...
        createBuilder(metricsFactory, metricName, description, puName, tags).buildCounter(statistics, f);
    }

    void createTimeGauge(MetricsFactory metricsFactory, String metricName, String description, String puName,
            Statistics statistics, Function<Statistics, Long> f, String... tags) {

        createBuilder(metricsFactory, metricName, description, puName, tags).unit(TimeUnit.MILLISECONDS.toString())
    ...
    MetricsFactory.MetricBuilder createBuilder(MetricsFactory metricsFactory, String metricName, String description,
        ...
        MetricsFactory.MetricBuilder builder = metricsFactory.builder(metricName).description(description)
