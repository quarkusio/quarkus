...
/**
 * Request filter which can be used to customize requests such as the verification JsonWebKey set and token grant
 * requests which are made from the OIDC adapter to the OIDC provider.
 * <p/>
...
public interface OidcRequestFilter {
     ...
     */
    record OidcRequestContext(HttpRequest<Buffer> request, Buffer requestBody,
     ...
     *        the request context which provides access to the HTTP request headers and body, as well as context
     *        properties.
     */
     ...
     *
     * @param request
     *        HTTP request that can have its headers customized
     * @param requestBody
     *        request body, will be null for HTTP GET methods, may be null for other HTTP methods
     * @param contextProperties
     *        context properties that can be available in context of some requests
     *
     * @deprecated use {@link #filter(OidcRequestContext)}
     */
    @Deprecated(forRemoval = true)
    default void filter(HttpRequest<Buffer> request, Buffer requestBody,
            OidcRequestContextProperties contextProperties) {
