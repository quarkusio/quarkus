    ...
    static void selectAuthMechanism(RoutingContext routingContext, String authMechanism) {
        if (requestAlreadyAuthenticated(routingContext, authMechanism)) {
            throw new AuthenticationFailedException("""
                    Request has been authenticated with different authentication mechanism before the '%s' authentication
                    mechanism was selected with an annotation. Most often, this will happen when you configure
                    an HTTP permission check that requires authentication (like 'roles-allowed' policy).
                    Please set the 'quarkus.http.auth.permission."permissions".applies-to=JAXRS' to all the HTTP permissions
                    applied only on Jakarta REST endpoints that selects the mechanism with the HttpAuthMechanism annotation.
                    """.formatted(authMechanism));
        }
