...
public class GraphQLFederationComplexTest extends AbstractGraphQLTest {
    ...
    static QuarkusUnitTest test = new QuarkusUnitTest()
            .withApplicationRoot((jar) -> jar.addClasses(FooApi.class, Foo.class, Bar.class)
                    .addAsResource(
                            new StringAsset("quarkus.smallrye-graphql.schema-include-directives=true\n"
    ...
    public void checkServiceDeclarationInSchema() {
        RestAssured.given().get("/graphql/schema.graphql").then().body(containsString("type _Service {"));
    }

    @Test
    public void checkFederationDirectivesInSchema() {
        RestAssured.given().get("/graphql/schema.graphql").then().body(containsString("id: Int! @external"))
    ...
    public void resolvePerFederation() {
        ...
        RestAssured.given().when().accept(MEDIATYPE_JSON).contentType(MEDIATYPE_JSON).body(request).post("/graphql")
                .then().assertThat().statusCode(200).and().body(CoreMatchers.is(
