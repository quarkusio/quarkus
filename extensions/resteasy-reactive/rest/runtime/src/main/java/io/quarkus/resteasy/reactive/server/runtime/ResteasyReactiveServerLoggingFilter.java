...
public class ResteasyReactiveServerLoggingFilter implements ContainerRequestFilter, ContainerResponseFilter {
    ...
    // Bounds on how much of a multipart request body we'll buffer for logging: a floor so a small
    // bodyLimit (a display-truncation character count) doesn't also cripple multipart parsing, and
    // a ceiling so a large bodyLimit can't make us buffer an unbounded amount of body.
    private static final long MIN_MULTIPART_BUFFER_BYTES = 1024;
    private static final long MAX_MULTIPART_BUFFER_BYTES = 10 * 1024;
    ...
    public void filter(ContainerRequestContext requestContext) throws IOException {
            ...
            if (requestContext.hasEntity()) {
                ...
                if (isMultipartMediaType(mediaType)) {
                    ...
                    if ("chunked".equalsIgnoreCase(transferEncoding)) {
                        ...
                        long maxBuffer = Math.min(Math.max(logging.bodyLimit(), MIN_MULTIPART_BUFFER_BYTES),
                                MAX_MULTIPART_BUFFER_BYTES);
        ...
        // Register a headers-end handler to capture streaming/SSE response starts, since
        // ContainerResponseFilter is bypassed entirely for those. For normal responses,
        // ContainerResponseFilter usually logs first, but the shared AtomicBoolean below makes
        // whichever of the two runs first win the log atomically — no assumption about ordering
        // between this Vert.x callback and the JAX-RS filter chain is required.
        RoutingContext routingContext = currentVertxRequest.getCurrent();
        if (routingContext != null) {
            AtomicBoolean responseLogged = new AtomicBoolean(false);
            routingContext.put(RESPONSE_LOGGED_KEY, responseLogged);
    ...
    public void filter(ContainerRequestContext requestContext, ContainerResponseContext responseContext) {
        ...
        // Atomically claim the log for this response. If the headers-end handler registered in the
        // request filter already won (e.g. headers were flushed before this filter ran), skip.
        RoutingContext routingContext = currentVertxRequest.getCurrent();
        AtomicBoolean responseLogged = routingContext != null ? routingContext.get(RESPONSE_LOGGED_KEY) : null;
        if (responseLogged != null && !responseLogged.compareAndSet(false, true)) {
            return;
        }
