...
The violations of such a rejected request are not logged, since they are a client error, but they can be seen in the console by enabling the debug logs of the builtin mappers, which is useful in dev mode:

[source,properties]
----
quarkus.log.category."io.quarkus.hibernate.validator".level=DEBUG
----

If the application code needs to handle `ValidationException` in some custom way,
