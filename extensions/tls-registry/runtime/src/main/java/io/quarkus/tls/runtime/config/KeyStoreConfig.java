...
public interface KeyStoreConfig {
     ...
     * <p>
     * When configuring the keystore with PEM files, multiple CRT/Key must be given. When configuring the keystore with
     * a JKS or a P12 file, it selects one alias based on the SNI hostname. In this case, all the keystore password and
     * alias password must be the same (configured with the {@code password} and {@code alias-password} properties. Do
     * not set the {@code alias} property.
     */
    @WithDefault("false")
    boolean sni();

    /**
     * The credential provider configuration for the keys store. A credential provider offers a way to retrieve the key
     * store password and alias password. Note that the credential provider is only used if the password / alias
     * password are not set in the configuration.
     */
    ...
    default void validate(InstanceHandle<KeyStoreProvider> provider, String name) {
        if (provider.isAvailable() && (pem().isPresent() || p12().isPresent() || jks().isPresent())) {
            throw new IllegalStateException("Invalid truststore '" + name
        ...
        if (pem().isPresent() && (p12().isPresent() || jks().isPresent())) {
            throw new IllegalStateException("Invalid keystore '" + name
                    + "' - The keystore cannot be configured with PEM and PKCS12 or JKS at the same time");
        }

        if (p12().isPresent() && jks().isPresent()) {
            throw new IllegalStateException("Invalid keystore '" + name
                    + "' - The keystore cannot be configured with PKCS12 and JKS at the same time");
        }
    }
