...
public interface ServerSslConfig {
    ...
    /**
     * Enables Server Name Indication (SNI), an TLS extension allowing the server to use multiple certificates. The
     * client indicate the server name during the TLS handshake, allowing the server to select the right certificate.
     */
    @WithDefault("false")
    boolean sni();
