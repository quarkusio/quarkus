...
The health check validates a connection of the pool.
When all connections of the pool are in use, no connection is available for the validation and the datasource is reported as `DOWN`, even if the database is reachable.
To let the health check open a new connection in that case, which may temporarily exceed `quarkus.datasource.jdbc.max-size`, set:
