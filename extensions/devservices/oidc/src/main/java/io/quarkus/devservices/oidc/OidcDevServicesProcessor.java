...
public class OidcDevServicesProcessor {
    ...
    private static void passwordTokenEndpoint(RoutingContext rc) {
        ...
        if (clientId == null || clientId.isEmpty()) {
            LOG.warn("client_id is not present, denying token request");
    ...
    private static void refreshTokenEndpoint(RoutingContext rc) {
        ...
        if (clientSecret == null || clientSecret.isEmpty()) {
            LOG.warn("client_secret is not present, denying token request");
            invalidTokenResponse(rc);
            return;
        }
        ...
        if (userAndRoles == null) {
            LOG.warnf("Received invalid refresh_token, denying token refresh: %s", refreshToken);
            invalidTokenResponse(rc);
            return;
        }
