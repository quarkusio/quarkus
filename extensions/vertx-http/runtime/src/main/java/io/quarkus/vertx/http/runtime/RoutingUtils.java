...
public final class RoutingUtils {
    ...
    /**
     * Get the normalized and decoded path: - normalize based on RFC3986 - convert % encoded characters to their non
     * encoded form (using {@link URIDecoder#decodeURIComponent}) - invalid if the path contains '?' (query section of
     * the path)
     *
     * @param ctx
     *        the RoutingContext
    ...
    /**
     * Enabled compression by removing CONTENT_ENCODING header as specified in Vert.x when the media-type should be
     * compressed and config enable compression.
     *
     * @param config
     ...
     * @param path
     */
    public static void compressIfNeeded(VertxHttpBuildTimeConfig config, Set<String> compressMediaTypes,
            RoutingContext ctx, String path) {
