...
public interface SwaggerUiConfig {
     ...
     * <p>
     * The value `/` is not allowed as it blocks the application from serving anything else. By default, this value will
     * be resolved as a path relative to `${quarkus.http.non-application-root-path}`.
     */
    @WithDefault("swagger-ui")
    String path();

    /**
     * If this should be included every time. By default, this is only included when the application is running in dev
     * mode.
     */
    @WithDefault("false")
    ...
    /**
     * The urls that will be included as options. By default, the OpenAPI path will be used. Here you can override that
     * and supply multiple urls that will appear in the TopBar plugin.
     */
     ...
     * that are shown. Can be Boolean to enable or disable, or a string, in which case filtering will be enabled using
     * that string as the filter expression. Filtering is case-sensitive matching the filter expression anywhere inside
     * the tag.
     */
    ...
    /**
     * Apply a sort to the operation list of each API. It can be 'alpha' (sort by paths alphanumerically), 'method'
     * (sort by HTTP method) or a function (see Array.prototype.sort() to know how sort function works). Default is the
     * order returned by the server unchanged.
     */
    ...
    /**
     * Apply a sort to the tag list of each API. It can be 'alpha' (sort by paths alphanumerically) or a function (see
     * Array.prototype.sort() to learn how to write a sort function). Two tag name strings are passed to the sorter for
     * each pass. Default is the order determined by Swagger UI.
     */
    ...
    /**
     * Set to {@code false} to deactivate syntax highlighting of payloads and cURL command. Can be otherwise an object
     * with the {@code activate} and {@code theme} properties.
     */
    ...
    /**
     * MUST be a function. Function to intercept remote definition, "Try it out", and OAuth 2.0 requests. Accepts one
     * argument requestInterceptor(request) and must return the modified request, or a Promise that resolves to the
     * modified request.
     */
    Optional<String> requestInterceptor();

    /**
     * If set, MUST be an array of command line options available to the curl command. This can be set on the mutated
     * request in the requestInterceptor function.
     */
    Optional<List<String>> requestCurlOptions();

    /**
     * MUST be a function. Function to intercept remote definition, "Try it out", and OAuth 2.0 responses. Accepts one
     * argument responseInterceptor(response) and must return the modified response, or a Promise that resolves to the
     * modified response.
     */
    Optional<String> responseInterceptor();

    /**
     * If set to true, uses the mutated request returned from a requestInterceptor to produce the curl command in the
     * UI, otherwise the request before the requestInterceptor was applied is used.
     */
    Optional<Boolean> showMutatedRequest();

    /**
     * List of HTTP methods that have the "Try it out" feature enabled. An empty array disables "Try it out" for all
     * operations. This does not filter the operations from the display.
     */
    Optional<List<HttpMethod>> supportedSubmitMethods();

    /**
     * By default, Swagger UI attempts to validate specs against swagger.io's online validator. You can use this
     * parameter to set a different validator URL, for example for locally deployed validators (Validator Badge).
     * Setting it to either none, 127.0.0.1 or localhost will disable validation.
     */
    Optional<String> validatorUrl();

    /**
     * If set to true, enables passing credentials, as defined in the Fetch standard, in CORS requests that are sent by
     * the browser.
     */
    ...
    /**
     * OAuth only activated for the accessCode flow. During the authorization_code request to the tokenUrl, pass the
     * Client Password using the HTTP Basic Authentication scheme - Used in the initOAuth method.
     */
    Optional<Boolean> oauthUseBasicAuthenticationWithAccessCodeGrant();

    /**
     * OAuth only applies to authorization code flows. Proof Key for Code Exchange brings enhanced security for OAuth
     * public clients - Used in the initOAuth method.
     */
    ...
    /**
     * Pre-authorize Basic Auth, programmatically set Username for a Basic authorization scheme - Used in the
     * preauthorizeBasic method.
     */
    Optional<String> preauthorizeBasicUsername();

    /**
     * Pre-authorize Basic Auth, programmatically set Password for a Basic authorization scheme - Used in the
     * preauthorizeBasic method.
     */
    Optional<String> preauthorizeBasicPassword();

    /**
     * Pre-authorize ApiKey Auth, programmatically set DefinitionKey for an API key or Bearer authorization scheme -
     * Used in the preauthorizeApiKey method.
     */
    Optional<String> preauthorizeApiKeyAuthDefinitionKey();

    /**
     * Pre-authorize ApiKey Auth, programmatically set ApiKeyValue for an API key or Bearer authorization scheme - Used
     * in the preauthorizeApiKey method.
     */
