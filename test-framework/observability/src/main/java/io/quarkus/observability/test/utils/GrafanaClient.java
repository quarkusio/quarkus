...
public class GrafanaClient {
    ...
    private <T> void handle(String path, Function<HttpRequest.Builder, HttpRequest.Builder> method,
            HttpResponse.BodyHandler<T> bodyHandler, BiConsumer<HttpResponse<T>, T> consumer) {
        try {
            String credentials = username + ":" + password;
            String encodedCredentials = Base64.getEncoder().encodeToString(credentials.getBytes());

            HttpClient httpClient = HttpClient.newHttpClient();
            HttpRequest.Builder builder = HttpRequest.newBuilder().uri(URI.create(url + path)).header("Authorization",
    ...
    private String grafana() {
        ...
        handle("/config/grafana", HttpRequest.Builder::GET, HttpResponse.BodyHandlers.ofString(), (r, b) -> {
            ref.set(b);
        });
        return ref.get();
    }
    ...
    public User user() {
        ...
        handle("/api/user", HttpRequest.Builder::GET, HttpResponse.BodyHandlers.ofString(), (r, b) -> {
            try {
    ...
    public QueryResult query(String query) {
        ...
        handle("/api/datasources/proxy/1/api/v1/query?query=" + query, HttpRequest.Builder::GET,
                HttpResponse.BodyHandlers.ofString(), (r, b) -> {
                    try {
    ...
    public TempoResult traces(String service, int limit, int spss) {
        ...
        String path = "/api/datasources/proxy/uid/tempo/api/search?q=%7Bresource.service.name%3D%22" + service
                + "%22%7D&limit=" + limit + "&spss=" + spss;
        handle(path, HttpRequest.Builder::GET, HttpResponse.BodyHandlers.ofString(), (r, b) -> {
            try {
