...
=== Checksum validation of registry catalogs

Because registry catalogs are continuously re-deployed snapshots, a client may read `maven-metadata.xml` and its checksum from two different generations of the catalog while the registry is publishing an update, and report a checksum mismatch for a download that is in fact intact. Catalog artifacts are therefore resolved with checksum validation disabled. This applies to catalog resolution only — the repositories a project resolves its dependencies from still validate checksums according to the Maven settings.
