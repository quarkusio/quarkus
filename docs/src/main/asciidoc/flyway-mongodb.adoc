...
The `flyway-database-nc-mongodb` connector executes every migration script
by spawning a `mongosh` subprocess, so `mongosh` must be installed and
available on the `PATH` of the process that runs Flyway. This is true at
both development time and at runtime (including JVM and native modes).

Install `mongosh` by following the
https://www.mongodb.com/docs/mongodb-shell/install/[official instructions]
for your operating system.

If `mongosh` is not available, Flyway will fail when applying a migration
with an error from the NC connector indicating that the shell could not be
started.
