...
Elasticsearch is a well known full text search engine and NoSQL datastore that is widely used.

In this guide, we will see how you can get your REST services to use Elasticsearch.

Quarkus provides two ways to access Elasticsearch: via the lower level `RestClient` or via the `RestHighLevelClient` we will call them
...
If you want to use the high level client instead, replace the `elasticsearch-rest-client` by the `elasticsearch-rest-high-level-client`.
