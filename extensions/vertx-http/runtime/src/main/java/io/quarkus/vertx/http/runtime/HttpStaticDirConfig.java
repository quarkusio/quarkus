...
public interface HttpStaticDirConfig {
    ...
    /**
     * Enable Quarkus to serve static files from a local filesystem directory (outside of Java resources)
     * <p>
    ...
    default String normalizedPath() {
        String d = path().orElseThrow(() -> new IllegalArgumentException(
                "quarkus.http.static-dir.path must be set when path serving is enabled")).trim();
    ...
    default String normalizedEndpoint() {
        String p = endpoint().orElseThrow(() -> new IllegalArgumentException(
                "quarkus.http.static-dir.endpoint must be set when endpoint serving is enabled")).trim();
