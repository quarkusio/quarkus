...
[NOTE]
====
The Quarkus CycloneDX extension primarily focuses on JSON format, as the underlying CycloneDX library prioritizes JSON support. While XML format is also supported, JSON is recommended for optimal compatibility and feature support.
====
...
The format of the embedded SBOM is controlled by the `quarkus.cyclonedx.format` configuration option:

[source,properties]
----
quarkus.cyclonedx.format=xml  # Generates XML format embedded SBOM
----

[NOTE]
====
If `quarkus.cyclonedx.format` is set to `all`, the embedded SBOM will use JSON format, as only a single embedded SBOM can be included in the application.
====
...
By default, generated SBOMs (both JSON and XML) are not pretty-printed. To enable pretty-printing:
...
This setting applies to both JSON and XML formats.
