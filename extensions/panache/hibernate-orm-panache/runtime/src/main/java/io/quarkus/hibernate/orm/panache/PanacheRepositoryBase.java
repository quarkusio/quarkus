 ...
 * this repository will gain you the exact same useful methods that are on {@link PanacheEntityBase}. Unless you have a
 * custom ID strategy, you should not implement this interface directly but implement {@link PanacheRepository} instead.
 * </p>
 *
...
public interface PanacheRepositoryBase<Entity, Id> {
    ...
    /**
     * Persist the given entity in the database, if not already persisted. Then flushes all pending changes to the
     * database.
     *
     * @param entity
     *        the entity to persist.
    ...
    /**
     * Returns true if the given entity is persistent in the database. If yes, all modifications to its persistent
     * fields will be automatically committed to the database at transaction commit time.
     *
     * @param entity
     *        the entity to check
     *
    ...
    /**
     * Find all entities of this type, in the given order. This method is a shortcut for
     * <code>findAll().stream()</code>. It requires a transaction to work. Without a transaction, the underlying cursor
     * can be closed before the end of the stream.
     *
     * @return a {@link Stream} containing all results, without paging
     *
     * @see #streamAll()
