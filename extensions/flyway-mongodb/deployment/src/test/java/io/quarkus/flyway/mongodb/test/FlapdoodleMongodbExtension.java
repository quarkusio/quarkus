...
/**
 * Starts a single embedded MongoDB instance, listening on {@link #MONGO_PORT}, that is shared by every test annotated
 * with {@code @ExtendWith(FlapdoodleMongodbExtension.class)}.
 * <p>
 * The instance is stored in the root {@link ExtensionContext} store, so it is started by the first test that needs it
 * and stopped once the whole test run is over. Tests point Quarkus at it via {@link #MONGO_CONNECTION_STRING}.
 */
