...
public interface ResteasyReactiveConfig {
    ...
    /**
     * The amount of memory that can be used to buffer input before switching to blocking IO, up to
     * {@code Long.MAX_VALUE} bytes.
     */
    @WithDefault("10k")
    ...
    /**
     * When one of the quarkus-rest-jackson or quarkus-rest-jsonb extension are active and the result type of an
     * endpoint is an application class or one of {@code Collection}, {@code List}, {@code Set} or {@code Map}, we
     * assume the default return type is "application/json" if this configuration is enabled.
     */
    @WithDefault("true")
    ...
    /**
     * An advanced option that can be set when they RESTEasy Reactive should NOT reply with 404 when it does not match
     * the URL path and instead just pass control onto the next Vert.x handler (if any)
     */
    @WithDefault("false")
    boolean resumeOn404();
