...
== Accessing the cluster from `@QuarkusIntegrationTest`

In a `@QuarkusIntegrationTest` (including native tests), the tested artifact runs in a separate process, so a `KubernetesClient` can't be obtained through CDI injection like in a `@QuarkusTest`. Instead, build one from the Dev Services connection properties exposed through `io.quarkus.test.common.DevServicesContext`.

The `quarkus-kubernetes-client-test-utils` artifact provides a helper for this:

[source, xml]
----
<dependency>
    <groupId>io.quarkus</groupId>
    <artifactId>quarkus-kubernetes-client-test-utils</artifactId>
    <scope>test</scope>
</dependency>
----

Declare a `KubernetesClientDevServicesTestHelper` field on your test class - Quarkus automatically injects the Dev Services connection properties into it, the same way it does for a plain `DevServicesContext` field:
...
WARNING: If `MyResourceIT` extends a `@QuarkusTest` class (as above, to reuse its test methods), that base class must **not** use `@Inject` to populate the `client` field. `@QuarkusIntegrationTest` scans the whole class hierarchy for `@Inject` fields and fails fast if it finds one, since there is no CDI container to satisfy it in a separate-process test. Use a plain field instead, and look it up with `Arc.container().select(KubernetesClient.class).get()` in the base class's own `@BeforeEach`:
+
[source, java]
----
@QuarkusTest
...
----
+
The subclass's `setUpClient()` above then overrides this one (same name and signature), so only the Dev Services variant runs under `@QuarkusIntegrationTest`.
