...
[source,text]
----
Redundant metrics collection detected: both Micrometer (registries: PrometheusMeterRegistry) and OpenTelemetry are active. Consider using the quarkus-micrometer-opentelemetry extension or disabling one system (e.g., setting quarkus.otel.metrics.enabled=false).
----
