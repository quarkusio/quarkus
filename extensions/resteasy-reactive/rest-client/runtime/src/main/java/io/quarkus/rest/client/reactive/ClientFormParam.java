...
/**
 * Used to specify a form parameter that should be sent with the outbound request. When this annotation is placed at the
 * interface level of a REST client interface, the specified form parameter will be sent on each request for all methods
 * in the interface. When this annotation is placed on a method, the parameter will be sent only for that method. If the
 * same form parameter is specified in an annotation for both the type and the method, only the parameter value
 * specified in the annotation on the method will be sent.
 * <p>
 ...
 * contain no arguments or a single <code>String</code> argument. The String argument is the name of the form parameter.
 * <p>
 ...
 * <code>@ClientFormParam</code> annotation with a <code>value</code> attribute that references a method that does not
 ...
 * to false, then the implementation will not send this form parameter if the method throws an exception.
 * <p>
...
public @interface ClientFormParam {
     ...
     *         default) or just skip this form parameter (false)
     */
