...
If you already have your {product-name-first} project configured, you can add the `rest-client-reactive-jackson` extension
...
[NOTE]
====
When the `quarkus-rest-client-reactive-jackson` extension is installed, {product-name} will use the `application/json` media type
...
[TIP]
====
The `QuarkusRestClientBuilder` interface is a {product-name}-specific API to programmatically create clients with additional configuration options. Otherwise, you can also use the `RestClientBuilder` interface from the Microprofile API:
...
This section shows how to replace your client with a mock. If you would like to get a more in-depth understanding of how mocking works in {product-name}, see the blog post on https://quarkus.io/blog/mocking/[Mocking CDI beans].
...
The xref:rest-client.adoc#using-a-mock-http-server-for-tests[Wiremock section of the {product-name} - Using the REST Client]
