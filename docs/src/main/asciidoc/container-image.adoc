...
=== Running against a Kubernetes or OpenShift cluster

Instead of running the built container image locally with Docker/Podman, `@QuarkusIntegrationTest` can deploy it to
a real Kubernetes or OpenShift cluster, run the tests against it, and tear the deployment down afterwards. This
reuses the manifest generated by the `quarkus-kubernetes` extension (`kubernetes.yml`) or the `quarkus-openshift`
extension (`openshift.yml`), so make sure the relevant extension is present - see
xref:deploying-to-kubernetes.adoc[Deploying to Kubernetes and OpenShift] for how manifest generation and
`deployment-target` selection works.

To enable it, configure the registry the built image should be pushed to before being deployed - that's the only
thing that needs to be set explicitly. Which property you set determines both that this launch mode activates *and*
which target is used: `quarkus.test.kubernetes.registry` targets the `kubernetes.yml` manifest, while
`quarkus.test.openshift.registry` targets `openshift.yml`. Only one of the two should be set. This registry is
intentionally independent of `quarkus.container-image.registry`, since the registry reachable from a test cluster is
usually not the same as the one used for production pushes:

[source,properties]
----
quarkus.test.kubernetes.registry=my-test-registry.example.com
----

The application is deployed using whichever kubeconfig context is active on the machine running the tests.

The following properties are available under both `quarkus.test.kubernetes.*` and `quarkus.test.openshift.*` -
each is resolved entirely independently per target, the same way `quarkus.kubernetes.*` and `quarkus.openshift.*`
never share values despite having the same shape. Setting e.g. `quarkus.test.kubernetes.exposure` has no effect
when targeting OpenShift; use `quarkus.test.openshift.exposure` for that:

include::{generated-dir}/config/quarkus-core_quarkus.test.kubernetes.adoc[opts=optional, leveloffset=+1]

include::{generated-dir}/config/quarkus-core_quarkus.test.openshift.adoc[opts=optional, leveloffset=+1]

NOTE: `external` exposure needs the Route/Ingress host to actually resolve to the test cluster from wherever the
test JVM runs. An OpenShift `Route` is assigned a real, resolvable hostname on the cluster's own router domain
automatically, so this usually just works. A Kubernetes `Ingress` has no such default - you must set
`quarkus.kubernetes.ingress.host` explicitly to something that resolves to the cluster's ingress controller (e.g.
a real DNS entry, or the https://nip.io/[nip.io] wildcard trick shown in xref:stork-kubernetes.adoc[]), or the
launcher will fail to find a usable host. The launcher also assumes the router/ingress controller is reachable on
the standard port (443 for TLS, 80 otherwise) - if it isn't (e.g. a rootless container runtime that can't bind a
port below 1024, so the ingress controller ends up exposed on a high port instead), override it with
`quarkus.test.kubernetes.external-port`/`quarkus.test.openshift.external-port`.

This launch mode assumes the image is built and pushed by one of `container-image-docker`, `container-image-jib`
or `container-image-podman` (i.e. `quarkus.container-image.build=true`). Building directly on the cluster with the
OpenShift S2I builder (`container-image-openshift`) is not supported by this launch mode.

NOTE: Dev Services are not available to the deployed application - they start containers on the machine running
the tests, which the cluster generally can't reach (this is true even for a local cluster like minikube or CRC,
and always true for a remote one). If your application relies on Dev Services when run locally, either deploy the
equivalent service into the cluster yourself and point the application at it via configuration, or use a real,
externally reachable instance of that service for this launch mode. Automatically deploying Dev Services into the
cluster may be added in a future release.

When pushing to OpenShift's own internal image registry (e.g. its externally-exposed default route), two
extra properties are usually needed:

* `quarkus.container-image.group` must match the target OpenShift project/namespace name, since the internal
  registry namespaces images per-project and rejects a push whose group doesn't match a project you have access to.
* `quarkus.docker.additional-args=--provenance=false`, since modern Docker's default builder attaches a BuildKit
  provenance/attestation manifest to every image, which this registry's implementation rejects with an HTTP 500 on
  push.

TIP: For local testing, both minikube and CRC (OpenShift Local) ship a usable registry out of the box:

* minikube: `minikube addons enable registry`, then `kubectl port-forward --namespace kube-system service/registry 5000:80 &`
  to expose it on `localhost:5000` (the port-forward is also what makes the minikube node itself resolve
  `localhost:5000` back to that registry, so the pushed image can be pulled from within the cluster too).
* CRC: the cluster's own internal registry is exposed via a route - run `oc registry info` to get its hostname,
  then `docker login -u developer -p "$(oc whoami -t)" <that hostname>` before pushing to it.

TIP: Pushing to a local insecure registry (plain HTTP, or HTTPS with an untrusted certificate) with Podman fails
by default, since Podman verifies TLS for any registry host. Set
`quarkus.test.kubernetes.insecure-registry=true` (or `quarkus.test.openshift.insecure-registry=true`) to push with
`--tls-verify=false` instead. This has no effect when pushing with Docker, which relies on daemon-level
`insecure-registries` configuration rather than a per-command flag.
