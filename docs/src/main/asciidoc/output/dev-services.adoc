...
{product-name-first} supports the automatic provisioning of unconfigured services in development and test mode. We refer to this capability
as Dev Services. From a developer's perspective this means that if you include an extension and don't configure it then
{product-name} will automatically start the relevant service (usually using https://www.testcontainers.org/[Testcontainers] behind the scenes) and wire up your
application to use this service.

All this functionality is part of the {product-name} `deployment` modules, so does not affect the production application in any
...
This page lists all the Dev Services that {product-name} supports.
...
{product-name} provides Dev Services for all databases it supports. Most of these are run in a container, except
...
N.B. if you opt in for this feature, {product-name} will not reset the state of the database between runs unless you explicitly configure it to.
