...
public class JsonObjectScalarTest extends AbstractGraphQLTest {
    ...
    @RegisterExtension
    static QuarkusUnitTest test = new QuarkusUnitTest().withApplicationRoot((jar) -> jar.addClasses(ObjectApi.class)
    ...
    public void testJsonObject() {
        ...
        RestAssured.given().body(query).contentType(MEDIATYPE_JSON).post("/graphql/").then().assertThat()
                .statusCode(200).body("data.echo.field1", equalTo(1)).body("data.echo.field2", equalTo("2"));
    }

    @Test
    public void testJsonSchemaDefinition() {
        RestAssured.given().get("/graphql/schema.graphql").prettyPeek().then().assertThat()
                .body(containsString("scalar JSON")).body(containsString("echo(input: JSON): JSON")).statusCode(200);
    }
