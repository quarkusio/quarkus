...
:summary: Learn more about what we call a Platform in the {product-name-first} world.

The {product-name} extension ecosystem consists of the {product-name} extensions developed and maintained by the community, including the {product-name} core development team. While the {product-name} ecosystem (sometimes also referred to as the "{product-name} universe") includes all the {product-name} extensions ever developed, there is also a concept of a {product-name} platform.

== Quarkus Platform

The fundamental promise of a {product-name} platform is any combination of the {product-name} extensions the platform consists of can be used in the same application without causing any conflict for each other.
Each organization creating their {product-name} platform may establish their own criteria for the extensions to be accepted into the platform and the means to guarantee the compatibility between the accepted extensions.

=== Quarkus Platform Artifacts

Each {product-name} platform is defined with a few artifacts.

=== Quarkus Platform BOM

Each {product-name} Platform is expected to provide a Maven BOM artifact that
...
* includes all the {product-name} extension artifacts (the runtime and the deployment ones) the platform consists of
...
{product-name} applications that want to include extensions from a {product-name} platform will be importing the {product-name} platform BOM.

[[platform-descriptor]]
=== Quarkus Platform Descriptor

{product-name} platform descriptor is a JSON artifact that provides information about the platform and its extensions to the {product-name} tools. E.g. https://code.quarkus.io/ and the {product-name} command line tools consult this descriptor to list, add and remove extensions to/from the project on user's request.
This artifact is also used as a {product-name} platform identifier. When {product-name} tools need to identify the {product-name} platform(s) used in the project, they will analyze the dependency version constraints of the project (the effective list of the managed dependencies from the `dependencyManagement` section in Maven terms) looking for the platform descriptor artifact(s) among them. Given that the platform descriptors are included into the {product-name} platform BOMs, every {product-name} application will inherit the platform descriptor artifact from the imported platform BOM(s) as a dependency version constraint (managed dependency in Maven terms).

To be able to easily identify {product-name} platform descriptors among the project's dependency constraints, the platform descriptor Maven artifact coordinates should follow the following naming convention:

* the `groupId` of the descriptor artifact should match the `groupId` of the corresponding {product-name} Platform BOM;
* the `artifactId` of the descriptor artifact should be the `artifactId` of the corresponding {product-name} Platform BOM with the `-quarkus-platform-descriptor` suffix;
* the `classifier` of the descriptor artifact should match the `version` of the corresponding {product-name} Platform BOM;
* the `type` of the descriptor artifact should be `json`;
* the `version` of the descriptor artifact should match the `version` of the  corresponding {product-name} Platform BOM.
...
<6> this parameter is optional, it allows to override some metadata from the {product-name} extension descriptors found in every runtime extension artifact from which the platform descriptor is generated
...
A {product-name} platform may provide its own default values for some configuration options.

{product-name} is using https://github.com/smallrye/smallrye-config[SmallRye Config] for wiring application configuration. A {product-name} platform may be used as another source of configuration in the hierarchy of the configuration sources dominated by the application's `application.properties`.
...
* the `groupId` of the properties artifact should match the `groupId` of the corresponding {product-name} Platform BOM;
* the `artifactId` of the properties artifact should be the `artifactId` of the corresponding {product-name} Platform BOM with the `-quarkus-platform-properties` suffix;
...
In case an application is importing more than one {product-name} platform and those platforms include their own platform properties artifacts, the content of those platform properties artifacts will be merged to form a single set of properties that will be used for the application build.
...
----
  ...
  <name>Acme - {product-name} - BOM</name>
  <packaging>pom</packaging>

  <dependencyManagement>
    <dependencies>
      <!-- Acme {product-name} platform properties -->
      <dependency>
