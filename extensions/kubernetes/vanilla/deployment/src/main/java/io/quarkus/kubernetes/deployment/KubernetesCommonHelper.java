...
public class KubernetesCommonHelper {
    ...
    private static final String[] PROMETHEUS_ANNOTATION_TARGETS = { "Service", "Deployment", "DeploymentConfig" };
    ...
    public static Optional<Project> createProject(ApplicationInfoBuildItem app,
        ...
        return createProject(app, customProjectRoot, outputTarget.getOutputDirectory().resolve(String
                .format(OUTPUT_ARTIFACT_FORMAT, outputTarget.getBaseName(), packageConfig.computedRunnerSuffix())));
    }
    ...
    public static Optional<Project> createProject(ApplicationInfoBuildItem app,
        ...
        try {
            ...
            BuildInfo buildInfo = new BuildInfo(name, app.getVersion(), "jar", project.getBuildInfo().getBuildTool(),
                    project.getBuildInfo().getBuildToolVersion(), artifactPath.toAbsolutePath(),
                    project.getBuildInfo().getClassOutputDir(), project.getBuildInfo().getResourceDir());

            return Optional.of(
                    new Project(customProjectRoot.isPresent() ? customProjectRoot.get().getRoot() : project.getRoot(),
     ...
     * Creates the configurator build items.
     */
    public static Optional<Port> getPort(List<KubernetesPortBuildItem> ports, PlatformConfiguration config,
            String targetPort) {
        return combinePorts(ports, config).values().stream().filter(distinct(Port::getName))
                .filter(p -> p.getName().equals(targetPort)).findFirst();
    }
    ...
    public static Map<String, Port> combinePorts(List<KubernetesPortBuildItem> ports, PlatformConfiguration config) {
        ...
        allPorts.putAll(
                ports.stream().map(p -> new PortBuilder().withName(p.getName()).withContainerPort(p.getPort()).build())
                        .collect(Collectors.toMap(Port::getName, Function.identity(), (first, second) -> first))); // prevent
                                                                                                                                                                                                                                          // dublicate
                                                                                                                                                                                                                                          // keys

        activePorts.putAll(verifyPorts(ports).entrySet().stream()
        ...
        ports.forEach(port -> {
            ...
            if (enabled) {
                ...
                Optional<Integer> value = Optional.ofNullable(configuration.ports().get(port.getName()))
                        .map(p -> p.containerPort()).filter(OptionalInt::isPresent).map(OptionalInt::getAsInt);
                ...
                PropertyUtil.printMessages(String.format("The container port %s", port.getName()), target,
                        kubernetesPortProperty, port.getSource());
            }
        });
    }
    ...
    public static List<DecoratorBuildItem> createDecorators(Optional<Project> project, String target, String name,
            Optional<KubernetesNamespaceBuildItem> namespace, PlatformConfiguration config,
            Optional<MetricsCapabilityBuildItem> metricsConfiguration,
            Optional<KubernetesClientCapabilityBuildItem> kubernetesClientConfiguration,
            List<KubernetesAnnotationBuildItem> annotations, List<KubernetesLabelBuildItem> labels,
            Optional<ContainerImageInfoBuildItem> image, Optional<KubernetesCommandBuildItem> command,
            Optional<Port> port, Optional<KubernetesHealthLivenessPathBuildItem> livenessProbePath,
        ...
        result.addAll(
                createAnnotationDecorators(project, target, name, config, metricsConfiguration, annotations, port));
    ...
    private static Collection<DecoratorBuildItem> createRbacDecorators(String name, String target,
            PlatformConfiguration config, Optional<KubernetesClientCapabilityBuildItem> kubernetesClientConfiguration,
        ...
        for (Map.Entry<String, RbacConfig.RoleConfig> roleFromConfig : config.rbac().roles().entrySet()) {
            ...
            result.add(new DecoratorBuildItem(target, new AddRoleResourceDecorator(name, roleName,
                    role.namespace().orElse(null), role.labels(), toPolicyRulesList(role.policyRules()))));
        ...
        Targetable
                .filteredByTarget(rolesFromExtensions, target).map(
                        role -> new DecoratorBuildItem(target,
                                new AddRoleResourceDecorator(name, role.getName(), role.getNamespace(),
                                        Collections.emptyMap(), role.getRules().stream().map(RBACUtil::from).toList())))
                .forEach(result::add);

        // Add cluster roles from configuration
        for (Map.Entry<String, RbacConfig.ClusterRoleConfig> clusterRoleFromConfig : config.rbac().clusterRoles()
                .entrySet()) {
            ...
            result.add(new DecoratorBuildItem(target, new AddClusterRoleResourceDecorator(name, clusterRoleName,
                    clusterRole.labels(), toPolicyRulesList(clusterRole.policyRules()))));
        ...
        Targetable.filteredByTarget(clusterRolesFromExtensions, target)
                .map(role -> new DecoratorBuildItem(target, new AddClusterRoleResourceDecorator(name, role.getName(),
                        Collections.emptyMap(), role.getRules().stream().map(RBACUtil::from).toList())))
                .forEach(result::add);
        ...
        Targetable.filteredByTarget(roleBindingsFromExtensions, target).map(rb -> new DecoratorBuildItem(target,
                new AddRoleBindingResourceDecorator(name,
                        Strings.isNotNullOrEmpty(rb.getName()) ? rb.getName() : name + "-" + rb.getRoleRef().getName(),
                        rb.getNamespace(), rb.getLabels(), rb.getRoleRef(), rb.getSubjects())))
                .forEach(result::add);
        ...
        for (Map.Entry<String, RbacConfig.RoleBindingConfig> rb : config.rbac().roleBindings().entrySet()) {
            ...
            if (roleBinding.subjects().isEmpty()) {
                ...
                subjects.add(new Subject(null, SERVICE_ACCOUNT, effectiveServiceAccountName,
                ...
                for (Map.Entry<String, RbacConfig.SubjectConfig> s : roleBinding.subjects().entrySet()) {
                    ...
                    subjects.add(new Subject(subject.apiGroup().orElse(null), subject.kind(), subjectName,
            ...
            result.add(new DecoratorBuildItem(target, new AddRoleBindingResourceDecorator(name, rbName, null, // todo:
                    // should
                    // namespace
                    // be
                    // providable
                    // via
                    // config?
                    roleBinding.labels(), new RoleRef(roleName, clusterWide), subjects.toArray(new Subject[0]))));
        }

        // Add cluster role bindings from extensions
        Targetable.filteredByTarget(clusterRoleBindingsFromExtensions, target).map(rb -> new DecoratorBuildItem(target,
                new AddClusterRoleBindingResourceDecorator(name,
                        Strings.isNotNullOrEmpty(rb.getName()) ? rb.getName() : name + "-" + rb.getRoleRef().getName(),
                        rb.getLabels(), rb.getRoleRef(), rb.getSubjects())))
                .forEach(result::add);

        // Add cluster role bindings from configuration
        for (Map.Entry<String, RbacConfig.ClusterRoleBindingConfig> rb : config.rbac().clusterRoleBindings()
                .entrySet()) {
            String rbName = rb.getValue().name().orElse(rb.getKey());
            ...
            result.add(new DecoratorBuildItem(target,
                    new AddClusterRoleBindingResourceDecorator(name, rbName, clusterRoleBinding.labels(),
                            new RoleRef(clusterRoleBinding.roleName(), true), subjects.toArray(new Subject[0]))));
        }
        ...
        if (config.rbac().roleBindings().isEmpty()) {
            if (defaultRoleName != null) {
                ...
                requiresServiceAccount = true;
                result.add(new DecoratorBuildItem(target, new AddRoleBindingResourceDecorator(name, name, null, // todo:
                        // should
                        // namespace
                        // be
                        // providable
                        // via
                        // config?
                        Collections.emptyMap(), new RoleRef(defaultRoleName, defaultClusterWide), new Subject(null,
                                SERVICE_ACCOUNT, effectiveServiceAccountName, effectiveServiceAccountNamespace))));
                ...
                requiresServiceAccount = true;
                result.add(new DecoratorBuildItem(target,
                        new AddRoleBindingResourceDecorator(name, name + "-" + DEFAULT_ROLE_NAME_VIEW, null, // todo:
                                // should
                                // namespace
                                // be
                                // providable
                                // via
                                // config?
                                Collections.emptyMap(), new RoleRef(DEFAULT_ROLE_NAME_VIEW, true),
                                new Subject(null, SERVICE_ACCOUNT, effectiveServiceAccountName,
                                        effectiveServiceAccountNamespace))));
            }
        }
        ...
        if (requiresServiceAccount) {
            ...
            result.add(new DecoratorBuildItem(target, new AddServiceAccountResourceDecorator(name,
                    effectiveServiceAccountName, effectiveServiceAccountNamespace, Collections.emptyMap())));
        }
        ...
        if (effectiveServiceAccount.wasSet() || requiresServiceAccount) {
            result.add(new DecoratorBuildItem(target,
                    new ApplyServiceAccountNameDecorator(name, effectiveServiceAccountName)));
        }
    ...
    public static KubernetesEffectiveServiceAccountBuildItem computeEffectiveServiceAccount(String name, String target,
        ...
        for (KubernetesServiceAccountBuildItem sa : serviceAccountsFromExtensions) {
            ...
            decorators.produce(new DecoratorBuildItem(target,
                    new AddServiceAccountResourceDecorator(name, saName, sa.getNamespace(), sa.getLabels())));
        ...
        for (Map.Entry<String, RbacConfig.ServiceAccountConfig> sa : config.rbac().serviceAccounts().entrySet()) {
            ...
            decorators.produce(new DecoratorBuildItem(target, new AddServiceAccountResourceDecorator(name, saName,
                    sa.getValue().namespace().orElse(null), sa.getValue().labels())));
    ...
    private static Collection<DecoratorBuildItem> createLabelDecorators(String target, String name,
        ...
        labels.forEach(l -> result
                .add(new DecoratorBuildItem(l.getTarget(), new AddLabelDecorator(name, l.getKey(), l.getValue()))));
    ...
    /**
     * If user defines a custom command via configuration, this is used. If not, it will use the one from other
     * extensions.
     *
     * @param target
     *        The deployment target (e.g. kubernetes, openshift, knative)
     * @param name
     *        The name of the resource to accept the configuration
     * @param config
     *        The {@link PlatformConfiguration} instance
     * @param command
     *        Optional command item from other extensions
     */
    ...
    /**
     * If user defines arguments via configuration, then these will be merged to the ones from other extensions. If not,
     * then only the arguments from other extensions will be used if any.
     *
     * @param target
     *        The deployment target (e.g. kubernetes, openshift, knative)
     * @param name
    ...
    public static List<DecoratorBuildItem> createInitContainerDecorators(String target, String applicationName,
        ...
        List<AddEnvVarDecorator> envVarDecorators = decorators.stream()
                ...
                .map(d -> d.getDecorator(AddEnvVarDecorator.class)).filter(Optional::isPresent).map(Optional::get)
                .toList();

        List<AddMountDecorator> mountDecorators = decorators.stream()
                .filter(d -> d.getGroup() == null || d.getGroup().equals(target))
                .map(d -> d.getDecorator(AddMountDecorator.class)).filter(Optional::isPresent).map(Optional::get)
                .toList();
        ...
        items.stream().filter(item -> item.getTarget() == null || item.getTarget().equals(target)).forEach(item -> {
            io.dekorate.kubernetes.config.ContainerBuilder containerBuilder = new io.dekorate.kubernetes.config.ContainerBuilder()
                    .withName(item.getName()).withImage(item.getImage())
                            ...
                            new ApplicationContainerDecorator<ContainerBuilder>(applicationName, item.getName()) {
                                ...
                                public void andThenVisit(ContainerBuilder builder) {
                                    ...
                                    for (Map.Entry<String, String> e : item.getEnvVars().entrySet()) {
                                        ...
                                        builder.addNewEnv().withName(e.getKey()).withValue(e.getValue()).endEnv();
            ...
            result.add(new DecoratorBuildItem(target,
                    new AddInitContainerDecorator(applicationName,
                            containerBuilder.addAllToEnvVars(item.getEnvVars().entrySet().stream()
                                    .map(e -> new EnvBuilder().withName(e.getKey()).withValue(e.getValue()).build())
                                    .collect(Collectors.toList())).build())));
        });
        return result;
    }

    public static List<DecoratorBuildItem> createInitJobDecorators(String target, String applicationName,
        ...
        List<NamedResourceDecorator<?>> volumeDecorators = decorators.stream()
                ...
                .map(d -> (NamedResourceDecorator<?>) d.getDecorator()).collect(Collectors.toList());
        ...
        List<AddImagePullSecretDecorator> imagePullSecretDecorators = decorators.stream()
                .filter(d -> d.getGroup() == null || d.getGroup().equals(target))
                .map(d -> d.getDecorator(AddImagePullSecretDecorator.class)).filter(Optional::isPresent)
                .map(Optional::get).toList();

        List<ApplyServiceAccountNameDecorator> serviceAccountDecorators = decorators.stream()
                .filter(d -> d.getGroup() == null || d.getGroup().equals(target))
                .map(d -> d.getDecorator(ApplyServiceAccountNameDecorator.class)).filter(Optional::isPresent)
                .map(Optional::get).toList();
        ...
        items.stream().filter(item -> item.getTarget() == null || item.getTarget().equals(target)).forEach(item -> {
            ...
            for (final AddImagePullSecretDecorator delegate : imagePullSecretDecorators) {
                result.add(new DecoratorBuildItem(target,
                        new NamedResourceDecorator<PodSpecBuilder>("Job", item.getName()) {
                            @Override
            ...
            if (item.isSharedEnvironment()) {
                for (final AddEnvVarDecorator delegate : envVarDecorators) {
                    result.add(new DecoratorBuildItem(target,
                            new NamedResourceDecorator<ContainerBuilder>("Job", item.getName()) {
                                @Override
                                public void andThenVisit(ContainerBuilder builder, ObjectMeta meta) {
                                    delegate.andThenVisit(builder);
                                    // Currently, we have no way to filter out provided env vars.
            ...
            result.add(new DecoratorBuildItem(target, new CreateJobResourceFromImageDecorator(item.getName(),
                    item.getImage(), item.getCommand(), item.getArguments())));
        });
        return result;
    }
     ...
     *        The deployment target (e.g. kubernetes, openshift, knative)
     * @param name
     *        The name of the resource to accept the configuration param namespace The optional namespace to apply
     *        to the resource
     * @param config
     *        The {@link PlatformConfiguration} instance
     */
    ...
    private static List<DecoratorBuildItem> createPodDecorators(String target, String name,
        ...
        List<DecoratorBuildItem> result = new ArrayList<>();
        config.imagePullSecrets().ifPresent(l -> l
                .forEach(s -> result.add(new DecoratorBuildItem(target, new AddImagePullSecretDecorator(name, s)))));
        ...
        config.nodeSelector().ifPresent(n -> result
                .add(new DecoratorBuildItem(target, new AddNodeSelectorDecorator(name, n.key(), n.value()))));

        config.initContainers().entrySet().forEach(e -> result.add(
                new DecoratorBuildItem(target, new AddInitContainerDecorator(name, ContainerConverter.convert(e)))));

        config.getSidecars().entrySet().forEach(e -> result
                .add(new DecoratorBuildItem(target, new AddSidecarDecorator(name, ContainerConverter.convert(e)))));
        ...
        if (config.securityContext().isAnyPropertySet()) {
            result.add(
                    new DecoratorBuildItem(target, new ApplySecuritySettingsDecorator(name, config.securityContext())));
        }

        return result;
    }

    private static List<DecoratorBuildItem> createAppConfigVolumeAndEnvDecorators(String target, String name,
            PlatformConfiguration config) {
        ...
        config.appSecret().ifPresent(s -> {
            result.add(new DecoratorBuildItem(target, new AddSecretVolumeDecorator(
                    new SecretVolumeBuilder().withSecretName(s).withVolumeName("app-secret").build())));
            result.add(new DecoratorBuildItem(target, new AddMountDecorator(
        ...
        config.appConfigMap().ifPresent(s -> {
            result.add(new DecoratorBuildItem(target, new AddConfigMapVolumeDecorator(
    ...
    private static List<DecoratorBuildItem> createMountAndVolumeDecorators(String target, String name,
            PlatformConfiguration config) {
        List<DecoratorBuildItem> result = new ArrayList<>();

        config.mounts().entrySet().forEach(e -> result
                .add(new DecoratorBuildItem(target, new AddMountDecorator(ANY, name, MountConverter.convert(e)))));
        ...
        config.configMapVolumes().entrySet().forEach(e -> result.add(
                new DecoratorBuildItem(target, new AddConfigMapVolumeDecorator(ConfigMapVolumeConverter.convert(e)))));

        config.emptyDirVolumes().ifPresent(volumes -> volumes.forEach(e -> result.add(
                new DecoratorBuildItem(target, new AddEmptyDirVolumeDecorator(EmptyDirVolumeConverter.convert(e))))));

        config.pvcVolumes().entrySet().forEach(e -> result
                .add(new DecoratorBuildItem(target, new AddPvcVolumeDecorator(PvcVolumeConverter.convert(e)))));
        ...
        config.azureFileVolumes().entrySet().forEach(e -> result.add(
                new DecoratorBuildItem(target, new AddAzureFileVolumeDecorator(AzureFileVolumeConverter.convert(e)))));

        config.azureDiskVolumes().entrySet().forEach(e -> result.add(
                new DecoratorBuildItem(target, new AddAzureDiskVolumeDecorator(AzureDiskVolumeConverter.convert(e)))));
        return result;
    }

    private static List<DecoratorBuildItem> createAnnotationDecorators(Optional<Project> project, String target,
            String name, PlatformConfiguration config, Optional<MetricsCapabilityBuildItem> metricsConfiguration,
            List<KubernetesAnnotationBuildItem> annotations, Optional<Port> port) {
        List<DecoratorBuildItem> result = new ArrayList<>();

        annotations.forEach(a -> result.add(
                new DecoratorBuildItem(a.getTarget(), new AddAnnotationDecorator(name, a.getKey(), a.getValue()))));
        ...
        metricsConfiguration.ifPresent(m -> {
            ...
            if (port.isPresent() && path != null) {
                if (config.prometheus().generateServiceMonitor()) {
                    result.add(new DecoratorBuildItem(target,
                            new AddServiceMonitorResourceDecorator(config.prometheus().scheme().orElse("http"),
                                    config.prometheus().port().orElse(String.valueOf(port.get().getContainerPort())),
                                    config.prometheus().path().orElse(path), 10, true)));
                }

                if (config.prometheus().annotations()) {
                    result.add(new DecoratorBuildItem(target,
                            new AddAnnotationDecorator(name, config.prometheus().scrape().orElse(prefix + "/scrape"),
                                    "true", PROMETHEUS_ANNOTATION_TARGETS)));
                    result.add(new DecoratorBuildItem(target, new AddAnnotationDecorator(name,
                    ...
                    result.add(new DecoratorBuildItem(target,
                            new AddAnnotationDecorator(name, config.prometheus().port().orElse(prefix + "/port"),
                                    "" + prometheusPort, PROMETHEUS_ANNOTATION_TARGETS)));
                    result.add(new DecoratorBuildItem(target,
                            new AddAnnotationDecorator(name, config.prometheus().scheme().orElse(prefix + "/scheme"),
                                    "http", PROMETHEUS_ANNOTATION_TARGETS)));
                }
            }
        });
    ...
    /**
     * Create a decorator that sets the port to the http probe. The rules for setting the probe are the following: 1. if
     * 'http-action-port' is set, use that. 2. if 'http-action-port-name' is set, use that to lookup the port value. 3.
     * if a `KubernetesPorbePortBuild` is set, then use that to lookup the port. 4. if we still haven't found a port
     * fallback to 8080.
     *
     * @param name
     ...
     * @param probeKind
     *        The probe kind (e.g. readinessProbe, livenessProbe etc)
     * @param portName
     *        the probe port name build item
     * @param ports
     *        a list of kubernetes port build items
     *
    ...
    public static DecoratorBuildItem createProbeHttpPortDecorator(String name, String target, String probeKind,
            ProbeConfig probeConfig, Optional<KubernetesProbePortNameBuildItem> portName,
            List<KubernetesPortBuildItem> ports, Map<String, PortConfig> portsFromConfig) {
        ...
        String httpPortName = probeConfig.httpActionPortName()
                .or(() -> portName.map(KubernetesProbePortNameBuildItem::getName)).orElse(HTTP_PORT);
        ...
        if (probeConfig.httpActionPort().isPresent()) {
            ...
            port = ports.stream().filter(p -> httpPortName.equals(p.getName())).map(KubernetesPortBuildItem::getPort)
        ...
        if (probeConfig.httpActionScheme().isPresent()) {
            ...
            scheme = SCHEME_HTTPS;
        } else if (portName.isPresent() && portName.get().getScheme() != null
        ...
        // Applying to all deployments to mimic the same logic as the rest of probes in the method
        // createProbeDecorators.
        return new DecoratorBuildItem(target,
                new ApplyHttpGetActionPortDecorator(ANY, name, httpPortName, port, probeKind, scheme));
    }

    /**
     * Create the decorators needed for setting up probes. The method will not create decorators related to ports, as
     * they are not supported by all targets (e.g. knative) Port related decorators are created by `applyProbePort`
     * instead.
     *
    ...
    private static List<DecoratorBuildItem> createProbeDecorators(String name, String target, ProbeConfig livenessProbe,
            ProbeConfig readinessProbe, ProbeConfig startupProbe,
    ...
    private static Optional<DecoratorBuildItem> createLivenessProbe(String name, String target,
            ProbeConfig livenessProbe, Optional<KubernetesHealthLivenessPathBuildItem> livenessPath) {
        if (livenessProbe.hasUserSuppliedAction()) {
            ...
            return Optional.of(new DecoratorBuildItem(target, new AddLivenessProbeDecorator(name, ProbeConverter
                    .builder(name, livenessProbe).withHttpActionPath(livenessPath.get().getPath()).build())));
        }
        return Optional.empty();
    }

    private static Optional<DecoratorBuildItem> createReadinessProbe(String name, String target,
            ProbeConfig readinessProbe, Optional<KubernetesHealthReadinessPathBuildItem> readinessPath) {
        if (readinessProbe.hasUserSuppliedAction()) {
            ...
            return Optional.of(new DecoratorBuildItem(target, new AddReadinessProbeDecorator(name, ProbeConverter
                    .builder(name, readinessProbe).withHttpActionPath(readinessPath.get().getPath()).build())));
        }
        return Optional.empty();
    }
    ...
    private static Optional<DecoratorBuildItem> createStartupProbe(String name, String target, ProbeConfig startupProbe,
        ...
        if (startupProbe.hasUserSuppliedAction()) {
            ...
            return Optional.of(new DecoratorBuildItem(target, new AddStartupProbeDecorator(name, ProbeConverter
                    .builder(name, startupProbe).withHttpActionPath(startupPath.get().getPath()).build())));
        }
        return Optional.empty();
    }
