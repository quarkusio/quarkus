...
public class OrOperatorTemplateExtensionTest {
    ...
    static final QuarkusUnitTest config = new QuarkusUnitTest().withApplicationRoot((jar) -> jar
            ...
            .addAsResource(new StringAsset("{@io.quarkus.qute.deployment.typesafe.ItemWithName itemWithName}\n"
                    ...
                    + "{item.name.or(itemWithName.name).toUpperCase}" + "{items.or(items2).length}" + // test arrays
                    "{otherItem.id.or(itemWithName.id).longValue()}" + // tests boxed type
                    "{otherItem.getPrimitiveId().or(itemWithName.getPrimitiveId()).longValue()}" + // tests primitive
                                                                                                   // type
                    "{/for}\n"), "templates/item.html"));
    ...
    public void test() {
        ...
        assertEquals(expected + items.length + OtherItem.ID + OtherItem.PRIMITIVE_ID,
                item.data(ITEM, new Item(ITEM_NAME.toUpperCase(), new OtherItem()), ITEM_WITH_NAME, itemWithName, ITEMS,
                        items, ITEMS_2, null).render().trim());

        // ids comes from ItemWithName, name comes from ItemWithName.Name and toUpperCase is regular method
        Item[] items2 = new Item[2];
        assertEquals(expected + items2.length + itemWithName.getId() + itemWithName.getPrimitiveId(),
                item.data(ITEM, new Item(null, (OtherItem) null), ITEM_WITH_NAME, itemWithName, ITEMS, null, ITEMS_2,
                        items2).render().trim());
    }
