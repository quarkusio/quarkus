...
public class SmallRyeGraphQLClientProcessor {
    ...
    void buildClientModel(CombinedIndexBuildItem index, SmallRyeGraphQLClientRecorder recorder,
        ...
        if (!index.getIndex().getAnnotations(GRAPHQL_CLIENT_API).isEmpty()) {
            ...
            RuntimeValue<ClientModels> modelRuntimeClientModel = recorder
                    .getRuntimeClientModel(normalizeClientModels(clientModels));
    ...
    /**
     * Normalizes ClientModels to make sure that generated bytecode is stable.
     * <p>
     * We do not sort clientModelMap here because its order is stable.
     * The operationMap for each model does not have a stable iteration order due to
     * {@link io.smallrye.graphql.client.model.MethodKey} not having a stable hash code.
     * <p>
     * We enforce the map order here.*
     */
    private static ClientModels normalizeClientModels(ClientModels clientModels) {
        Map<String, ClientModel> sortedClientModelMap = new LinkedHashMap<>();
        clientModels.getClientModelMap()
                .forEach((key, clientModel) -> {
                    Map<MethodKey, String> sortedOperationMap = new LinkedHashMap<>();
                    clientModel.getOperationMap().entrySet().stream()
                            .sorted(Map.Entry.comparingByKey(SmallRyeGraphQLClientProcessor::compareMethodKeys))
                            .forEach(operation -> sortedOperationMap.put(operation.getKey(), operation.getValue()));
                    clientModel.setOperationMap(sortedOperationMap);
                    sortedClientModelMap.put(key, clientModel);
                });
        clientModels.setClientModelMap(sortedClientModelMap);
        return clientModels;
    }

    private static int compareMethodKeys(MethodKey left, MethodKey right) {
        int result = left.getMethodName().compareTo(right.getMethodName());
        if (result != 0) {
            return result;
        }
        Class<?>[] leftParameterTypes = left.getParameterTypes();
        Class<?>[] rightParameterTypes = right.getParameterTypes();
        result = Integer.compare(leftParameterTypes.length, rightParameterTypes.length);
        if (result != 0) {
            return result;
        }
        for (int i = 0; i < leftParameterTypes.length; i++) {
            result = leftParameterTypes[i].getName().compareTo(rightParameterTypes[i].getName());
            if (result != 0) {
                return result;
            }
        }
