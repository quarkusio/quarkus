...
Guidelines are provided to help you to contribute clear and consistent content that is also sourced in the required diataxis structure and composition of {product-name-first} documentation.

== AsciiDoc syntax

{product-name} docs use AsciiDoc syntax.
...
Content from this repository is published to the https://quarkus.io/guides/[{product-name}.io website].
...
Your titles and headings must also follow the specific guidance for the {product-name} content types, as outlined in the following table:

.Title guidance for different {product-name} content types
[cols="15%,25%a,30%,30%"]
|===
...
|Security and authentication mechanisms in {product-name}
|Discovering Reactive SQL Clients In {product-name}
...
|Secure your {product-name} application with WebAuthn authentication
|Applying WebAuthn Authentication In {product-name}
...
|Create a {product-name} application in JVM mode by using the quick start example
|Creating an App
|===
...
{product-name} documentation uses a flat hierarchy.
...
Documents related to writing and contributing to {product-name} docs share the `doc-` prefix, for example.
...
Use the {product-name} templates to ensure that your content is consistent with the preferred {product-name} documentation structure and style.
...
The first sentence of the abstract provides a summary and gets automatically added to the tile on the link:https://quarkus.io/guides/[{product-name} guides homepage].
...
See xref:{doc-guides}/doc-concept.adoc[{product-name} documentation content types] for more information about content types and organization.
====
...
{product-name} documentation is built from source in a few different environments.
...
[source,asciidoc]
----
xref:{doc-guides}/doc-concept.adoc[{product-name} Documentation concepts] <1>
----
<1> The cross-reference starts with `xref:`, uses a cross-reference source attribute(`\{doc-guides}`), and provides a readable description: `[{product-name} Documentation concepts]`.
...
----
For more details about anchored IDs, see the xref:doc-contribute-docs-howto.adoc#anchors-howto[Cross-reference in-file and cross-file content by using anchors] section of the "Contribute to {product-name} documentation" guide.
...
* Add the definite article “the” and “{product-name}” before the title of the guide, unless the word “{product-name}" is already in the title.
...
|====
|Correct|Incorrect
|For more information, see the xref:validation.adoc#configuring-the-validatorfactory[Configuring the ValidatorFactory] section of the {product-name} "Validation with Hibernate Validator" guide.|For more information, see the 'Configuring the ValidatorFactory' section of the xref:validation.adoc[VALIDATION WITH HIBERNATE VALIDATOR guide].
|For more information, see the xref:cache.adoc#generating-a-cache-key-with-cachekeygenerator[Generating a cache key with CacheKeyGenerator] section of the {product-name} "Application Data Caching" guide.|For more information, see xref:cache.adoc#generating-a-cache-key-with-cachekeygenerator[Generating a cache key with CacheKeyGenerator].
|
For more information, see the xref:security-openid-connect-web-authentication/.adoc#proof-of-key-for-code-exchange-pkce[PKCE-related section] of the {product-name} "OpenID Connect (OIDC) Authorization Code Flow Mechanism" guide.|For more information, see xref:security-openid-connect-web-authentication.adoc#proof-of-key-for-code-exchange-pkce[OpenID Connect (OIDC) Authorization Code Flow Mechanism].
...
The {product-name} documentation build copies source files enumerated in `*-examples/yaml` files into a flattened structure in the `target/asciidoc/examples` directory (from the project root).
...
{product-name} documentation is grouped into the following categories.
...
| architecture | {product-name} runtime and ecosystem architecture
...
| contributing | Guidance and references to help you contribute to {product-name}.
| core | Information about how the {product-name} works
| data | Topics related to using data sources with {product-name}
...
|\{quarkus-base-url}|{quarkus-base-url}| {product-name} GitHub URL common base prefix.
|\{quarkus-clone-url}|{quarkus-clone-url}| {product-name} URL for `git clone` referenced by the documentation.
|\{quarkus-archive-url}|{quarkus-archive-url}| {product-name} URL to main source archive.
|\{quarkus-blob-url}|{quarkus-blob-url}| {product-name} URL to main blob source tree; used for referencing source files.
|\{quarkus-tree-url}|{quarkus-tree-url}| {product-name} URL to main source tree root; used for referencing directories.
|\{quarkus-issues-url}|{quarkus-issues-url}| {product-name} URL to the issues page.
|\{quarkus-images-url}|{quarkus-images-url}| {product-name} URL to set of container images delivered for {product-name}.
