...
* `dev`, if defined.
* External consumers should rely only on `api` or `deployment-spi` to be loose-coupled (See <<coupling-api-and-spi>>.).
...
==== deployment module
* `io.<quarkus|quarkiverse>.<extension-name>.deployment`: Internal build logic (processors, build steps, internal build items). Public APIs must reside in the `deployment-spi` module. May include subpackages (excluding `api`).
