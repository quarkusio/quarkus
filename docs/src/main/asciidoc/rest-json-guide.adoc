...
You might already have understood that, when using GraalVM, you need to register your classes for reflection.

But we did not register `Fruit` for reflection in any way and everything is working fine.

The fact is that Quarkus does some magic when it is capable of inferring the serialized types from the REST methods.
...
Quarkus does some magic to further simplify things when running your application as a native image.
