...
Now, lets create our first *GraphQL* API:

Edit the `org.acme.microprofile.graphql.FilmResource` class as the following:
...
Enter the following to into `GraphiQL` to retrieve two films.
...
GraphQL API can be expanded without breaking previous queries enabling easy
API `evolution`.

* adding the `smallrye-graphql` Quarkus extension to your project using the
