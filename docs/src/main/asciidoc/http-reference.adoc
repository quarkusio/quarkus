...
By default Quarkus will serve static files in the `META-INF/resources` directory of your application. This location
...
Additionally, Quarkus supports deployment of static files defined in a directory other than `META-INF/resources`.
This is particularly useful when you want to leverage the power of a Quarkus backend alongside a frontend application (outside your Quarkus backend repository).
In this workflow, you can configure your Quarkus backend using the configuration properties below to serve static files from the directory that contains your web application contents or the result of a frontend framework build process.

[source,properties]
----
quarkus.http.static.web-content.endpoint=/web <1>
quarkus.http.static.web-content.index=index.html <2>
quarkus.http.static.web-content.path=/path/to/directory <3>
----

1. Configure a `/web` endpoint named `web-content` that will serve static web resources.
2. Initialise the directory path containing static content for the `/web` endpoint.
3. Set `index.html` as the default landing page for the `/web` endpoint.
