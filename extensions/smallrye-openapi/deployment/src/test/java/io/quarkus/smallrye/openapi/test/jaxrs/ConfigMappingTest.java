...
public class ConfigMappingTest {
    ...
    public void testOpenApiPathAccessResource() {
        RestAssured.given().queryParam("format", "JSON").when().get(OPEN_API_PATH).then().log().body().and()
                .body("openapi", Matchers.equalTo("3.0.2"))
                .body("servers[0].url", Matchers.startsWith("http://www.server"))
                .body("servers[1].url", Matchers.startsWith("http://www.server"))
                .body("info.title", Matchers.equalTo("My API"))
                .body("info.description", Matchers.equalTo("My Description"))
                .body("info.termsOfService", Matchers.equalTo("Some terms"))
                .body("info.contact.name", Matchers.equalTo("My Name"))
                .body("info.contact.url", Matchers.equalTo("http://www.foo.bar"))
                .body("info.contact.email", Matchers.equalTo("my.email@provider.com"))
                .body("info.license.name", Matchers.equalTo("Some License"))
                .body("info.license.url", Matchers.equalTo("http://www.somelicense.com"))
                .body("info.version", Matchers.equalTo("1.2.3")).body("paths.'/greeting/goodbye'.get.operationId",
