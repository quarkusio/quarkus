...
ID and access tokens may have to decrypted or their headers preprocessed for the verification process to start and succeed.
...
|quarkus.oidc.token.decrypt-id-token |true| Decrypt ID token
|quarkus.oidc.token.decrypt-access-token |false| Decryption access token
|quarkus.oidc.token.customizer-name || Customizer name
|====

OIDC providers usually issue signed ID and access tokens but may also additionally encrypt these tokens which Quarkus needs to decrypt to verify them. Use the `quarkus.oidc.token.decryption-key-location` property to point to a JWK or PEM decryption key file and choose which token types must be decrypted, the ID token with `quarkus.oidc.token.decrypt-id-token`, or the access token with `quarkus.oidc.token.decrypt-access-token`.

ID token decryption is enabled by default when the decryption key location is configured and the ID token sequence contains 5 parts separated by dot character. Access token decryption is not enabled by default because opaque access tokens which do not contain inner-signed JWT tokens may have 5 parts separated by dot character by chance.
