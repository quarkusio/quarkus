 ...
 * <p>
 * In particular, we don't want to actually stop the containers when they have been flagged for reuse, and when the
 * Testcontainers configuration has been explicitly set to allow container reuse. To enable reuse, add
 * {@literal testcontainers.reuse.enable=true} in your {@literal .testcontainers.properties} file, to be stored in your
 * home.
 *
...
public final class ContainerShutdownCloseable implements Closeable {
    ...
    /**
     * @param container
     *        the container to be eventually closed
     * @param friendlyServiceName
    ...
    @Override
    public void close() {
        if (TestcontainersConfiguration.getInstance().environmentSupportsReuse() && container.isShouldBeReused()) {
            LOG.infof(
