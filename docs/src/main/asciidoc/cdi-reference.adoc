...
One of the limitations of GraalVM is the usage of https://www.graalvm.org/{graalvm-major-minor-version}/reference-manual/native-image/Reflection/[Reflection, window="_blank"].
...
TIP: The list is sorted by priority as defined by `io.quarkus.arc.InjectableBean#getPriority()`. Higher priority goes first. In general, the `@javax.annotation.Priority` and `@io.quarkus.arc.Priority` annotations can be used to assign the priority to a class bean, producer method or producer field.
