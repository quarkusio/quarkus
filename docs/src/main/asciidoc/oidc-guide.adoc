...
The solution is located in the `using-openid-connect` {quickstarts-tree-url}/using-openid-connect[directory].
...
If you plan to consume this application from a Web Application running on a different domain, you will need to configure CORS (Cross-origin resource sharing). Please read the link:http-reference.html#cors-filter[HTTP CORS documentation] for more details.
...
[source,bash]
----
./target/using-openid-connect-runner
----
