...
/**
 * Allows executing commands from the {@code geo} group. See <a href="https://redis.io/commands/?group=geo">the geo
 * command list</a> for further information about these commands. Geo-localized items are tuples composed of longitude,
 * latitude and the member. The member is of type {@code &lt;V&gt}. Each key can store multiple items.
 *
 * @param <K>
 *        the type of the key
 * @param <V>
...
public interface ReactiveGeoCommands<K, V> extends ReactiveRedisCommands {
     ...
     *         latitude (x,y) of each member name passed as argument to the command. Non-existing elements are reported
     ...
     * representing a geospatial index to fetch members matching a given maximum distance from a poUni<Integer> Group:
     * geo Requires Redis 3.2.0
     *
     * @param key
     *        the key
     * @param longitude
    ...
    /**
     * Execute the command <a href="https://redis.io/commands/georadius">GEORADIUS</a>. Summary: Query a sorted set
     * representing a geospatial index to fetch members matching a given maximum distance from a poUni<Integer>. It also
     * stores the results in a sorted set. Group: geo Requires Redis 3.2.0
     *
     * @param key
     *        the key
     * @param longitude
     ...
     **/
    @Deprecated
    Uni<Long> georadius(K key, double longitude, double latitude, double radius, GeoUnit unit,
     ...
     * 6.2.0
     *
     * @return the list of {@code GeoValue&lt;V>&gt;}. The populated data depends on the parameters configured in
     *         {@code args}.
     **/
