...
Quarkus provides a dev-mode (explained in more detail link:maven-tooling#dev-mode[here] and link:gradle-tooling#dev-mode[here]) which greatly aids
...
The `quarkus-hibernate-orm` extension will run the `import.sql` script in `src/main/resources` when Quarkus is running in dev-mode. More details can be found link:hibernate-orm#dev-mode[here].
...
More information can be found link:datasource#dev-services[here].

=== Swagger UI

The `quarkus-smallrye-openapi` extension will expose the Swagger UI when Quarkus is running in dev-mode. Additional information can be found link:openapi-swaggerui#dev-mode[here].

=== GraphQL UI

The `quarkus-smallrye-graphql` extension will expose the GraphiQL UI when Quarkus is running in dev-mode. More details can be found link:smallrye-graphql#ui[here].

=== Health UI

The `quarkus-smallrye-health` extension will expose the Health UI when Quarkus is running in dev-mode. link:smallrye-health#ui[This] section provides additional information.

=== Mock mailer

The `quarkus-mailer` extension will enable an in-memory mock mail server when Quarkus is running in dev-mode. See link:mailer-reference#testing[this] for more details.
...
Furthermore, the purpose built ClassLoader that comes with the link:maven-tooling#fast-jar[fast-jar] package type ensures that class lookup is done as fast as possible while also keeping
