...
passing them as parameters everywhere. Many {product-name-first} extensions require those contextual objects to operate
...
make those {product-name} extensions work properly in reactive/async settings. It works by capturing those contextual
...
[NOTE]
====
The injected `ManagedExecutor` uses the {product-name} thread pool.
====
...
The following contexts are available in {product-name} either out of the box, or depending on whether you include
their extensions:
