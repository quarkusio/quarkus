...
To set up a multi-module maven project we need to adjust the configuration given above slightly. Add or replace the following elements in your root `pom.xml` of the parent module:
...
<1> Set the packaging to `pom` as the parent module doesn't include any source code.
<2> Set the version as a property to reuse the value in submodules.
<3> Optionally use a BOM file to omit the version of the different Quarkus dependencies in your submodules.
<4> Remove the `build` step from the plugin, we will add this to one of the submodules.
<5> Set up the submodules

Next create the `pom.xml` files for the submodules in their respective subfolders:
[source,xml,subs=attributes+]
----
<project>
    [...]
    ...
    <artifactId>webapp</artifactId> <2>
    <build>
        <plugins>
            <plugin> <3>
                <groupId>io.quarkus</groupId>
...
<1> Link to the parent and make sure it's using the correct version by using the `revision` property.
<2> Name the submodule.
<3> One of the submodules needs to include the Quarkus Maven plugin with the build step. Other modules can omit this plugin.
<4> Include dependencies on other submodules and again make sure it's using the correct version by referencing the parent version.

[WARNING]
====
To enable Dependency Injection to work across submodules we need to add an empty `beans.xml` file in every submodules in the `src/main/resources/META-INF` directory of that module.
====

Check the link:https://github.com/quarkusio/quarkus-quickstarts/tree/master/maven-multi-module-quickstart[quickstarts] for a full example.
