...
public final class JsonRpcCodec {
    ...
    private static final Map<Class<?>, Method> GET_MAP_CACHE = new ConcurrentHashMap<>();
    ...
    public JsonRpcRequest readRequest(String json) {
        ...
        if (parsed instanceof Map) {
            ...
            try {
                Method getMap = GET_MAP_CACHE.computeIfAbsent(parsed.getClass(), clazz -> {
                    try {
                        return clazz.getMethod("getMap");
                    } catch (NoSuchMethodException e) {
                        throw new IllegalArgumentException("Cannot convert parsed JSON to Map: " + clazz, e);
                    }
                });
