...
public interface WebAuthnRunTimeConfig {
    ...
    /**
     * ResidentKey https://www.w3.org/TR/webauthn-2/#dictdef-authenticatorselectioncriteria This enum is used to specify
     * the desired behaviour for resident keys with the authenticator.
     */
    ...
    /**
     * The origins of the application. The origin is basically protocol, host and port. If you are calling WebAuthn API
     * while your application is located at {@code https://example.com/login}, then origin will be
     * {@code https://example.com}. If you are calling from {@code http://localhost:2823/test}, then the origin will be
     * {@code http://localhost:2823}. Please note that WebAuthn API will not work on pages loaded over HTTP, unless it
     * is localhost, which is considered secure context. If unspecified, this defaults to whatever URI this application
     * is deployed on. This allows more than one value if you want to allow multiple origins. See
     ...
     * hardening reasons. By default, all transports should be allowed. If your application is to be used by mobile
     * phone users, you may want to restrict only the {@code INTERNAL} authenticator to be allowed. Permitted values
     * are:
     * <ul>
     ...
     * </ul>
     * For security reasons your application may choose to restrict to a specific attachment mode. If omitted, then any
     * mode is permitted.
     */
     ...
     * user is present during the authentication ceremonies, but in some cases, applications may want to reduce the
     ...
     * cryptographic verification of the authenticator hardware. Attestation implies that the privacy of the users may
     * be exposed and browsers might override the desired configuration on the user's behalf. Valid values are:
     * <ul>
     * <li>{@code NONE} - no attestation data is sent with registration</li>
     * <li>{@code INDIRECT} - attestation data is sent with registration, yielding anonymized data by a trusted CA</li>
     ...
     * support at least the following 2 algorithms: {@code ES256} and {@code RS256}. Applications may require stronger
     * keys and algorithms, for example: {@code ES512} or {@code EdDSA}. Note that the use of stronger algorithms, e.g.:
     * {@code EdDSA} may require Java 15 or a cryptographic {@code JCE} provider that implements the algorithms. See
     * https://www.w3.org/TR/webauthn-1/#dictdef-publickeycredentialparameters
     */
     ...
     * TODO: don't enable this yet and with upcoming Leve3 version vert.x may break this config see
     * https://github.com/vert-x3/vertx-auth/issues/535
     */
    ...
    /**
     * Root CRLs for attestation verification of authenticators. These are the root CRLs from FIDO Alliance. The CRLs
     * are required to ensure that the metda data service can be trusted.
     */
    ...
    interface RelyingPartyConfig {
        /**
         * The id (or domain name of your server, as obtained from the first entry of <code>origins</code> or looking at
         * where this request is being served from)
         */
    ...
    /**
     * The inactivity (idle) timeout When inactivity timeout is reached, cookie is not renewed and a new login is
     * enforced.
     */
    @WithDefault("PT30M")
    ...
    /**
     * How old a cookie can get before it will be replaced with a new cookie with an updated timeout, also referred to
     * as "renewal-timeout". Note that smaller values will result in slightly more server load (as new encrypted cookies
     * will be generated more often); however, larger values affect the inactivity timeout because the timeout is set
     * when a cookie is generated. For example if this is set to 10 minutes, and the inactivity timeout is 30m, if a
     * user's last request is when the cookie is 9m old then the actual timeout will happen 21m after the last request
     * because the timeout is only refreshed when a new cookie is generated. That is, no timeout is tracked on the
     * server side; the timestamp is encoded and encrypted in the cookie itself, and it is decrypted and parsed with
     * each request.
     */
    @WithDefault("PT1M")
    ...
    /**
     * Max-Age attribute for the session cookie. This is the amount of time the browser will keep the cookie. The
     * default value is empty, which means the cookie will be kept until the browser is closed.
     */
    Optional<Duration> cookieMaxAge();

    /**
     * Set to <code>true</code> if you want to enable the default registration endpoint at
     * <code>/q/webauthn/register</code>, in which case you should also implement the
     * <code>WebAuthnUserProvider.store</code> method.
     */
    @WithDefault("false")
    Optional<Boolean> enableRegistrationEndpoint();

    /**
     * Set to <code>true</code> if you want to enable the default login endpoint at <code>/q/webauthn/login</code>, in
     * which case you should also implement the <code>WebAuthnUserProvider.update</code> method.
     */
    @WithDefault("false")
