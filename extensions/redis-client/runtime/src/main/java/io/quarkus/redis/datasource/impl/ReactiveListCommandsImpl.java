...
import io.quarkus.redis.datasource.api.list.LPosArgs;
import io.quarkus.redis.datasource.api.list.Position;
...
public class ReactiveListCommandsImpl<K, V> extends AbstractListCommands<K, V> implements ReactiveListCommands<K, V> {
    ...
    public Uni<Long> lpos(K key, V element, LPosArgs args) {
        return super._lpos(key, element, args)
                .map(this::decodeLongOrNull);
    }
    ...
    public Uni<List<Long>> lpos(K key, V element, int count, LPosArgs args) {
        return super._lpos(key, element, count, args)
                .map(this::decodeListOfLongs);
    }

    @Override
    public Uni<Long> lpush(K key, V... elements) {
        return super._lpush(key, elements)
                .map(Response::toLong);
    ...
    public Uni<Long> lpushx(K key, V... elements) {
        return super._lpushx(key, elements)
                .map(Response::toLong);
    }

    @Override
    public Uni<List<V>> lrange(K key, long start, long stop) {
        return super._lrange(key, start, stop)
                .map(this::decodeListV);
    }

    @Override
    public Uni<Long> lrem(K key, long count, V element) {
        return super._lrem(key, count, element)
                .map(Response::toLong);
