...
public class MetricDotNames {
    ...
    static final DotName COUNTED_ANNOTATION = DotName
    ...
    static final DotName METERED_ANNOTATION = DotName
    ...
    static final Set<DotName> individualMetrics = new HashSet<>(Arrays.asList(CONCURRENT_GAUGE_ANNOTATION,
            COUNTED_ANNOTATION, GAUGE_ANNOTATION, METERED_ANNOTATION, SIMPLY_TIMED_ANNOTATION, TIMED_ANNOTATION));
    ...
    static final DotName METRIC = DotName.createSimple("org.eclipse.microprofile.metrics.Metric");
     ...
     * @param annotations
     *
     * @return true if the map of all annotations contains any MP Metrics annotations
     */
    ...
    /**
     * @return true for known metrics subsystem classes that should not be inspected for lifecycle constraints, etc.
     */
     ...
     * @param classInfo
     *
     * @return true if the specified class is either a REST endpoint or has a singleton/application scope.
     */
    static boolean isSingleInstance(ClassInfo classInfo) {
        ...
        return classInfo.annotationsMap().containsKey(REST_CONTROLLER)
                || classInfo.annotationsMap().containsKey(JAXRS_PATH) || BuiltinScope.APPLICATION.equals(beanScope)
