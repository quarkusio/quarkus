...
{product-name-first} provides first class support for using Liquibase as will be explained in this guide.
...
Liquibase support relies on the {product-name} datasource config.
...
Now you can start your application and {product-name} will run the Liquibase's update method according to your config:
...
----
<1> The {product-name} extension provides a factory to initialize a Liquibase instance
<2> Inject the {product-name} liquibase factory if you want to use the liquibase methods directly
...
{product-name} will already have run the migrate operation.
...
<2> Inject Liquibase for named datasources using the {product-name} `LiquibaseDataSource` qualifier
