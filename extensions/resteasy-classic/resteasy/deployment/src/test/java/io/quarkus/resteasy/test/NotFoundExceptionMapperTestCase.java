...
public class NotFoundExceptionMapperTestCase {
    ...
    public void testResourceNotFound() {
        ...
        RestAssured.when().get("/not_found").then().statusCode(404);
    }

    @Test
    public void testHtmlResourceNotFound() {
        // test the exception mapper provided in dev mode, if no accept, will just return a plain 404
        RestAssured.given().accept(ContentType.HTML).when().get("/not_found").then().statusCode(404)
                .contentType(ContentType.HTML).body(containsString("/index.html")) // check that index.html is displayed
    ...
    public void testJsonResourceNotFound() {
        ...
        RestAssured.given().accept(ContentType.JSON).when().get("/not_found").then().statusCode(404)
    ...
    public void shouldDisplayNewAddedFileIn404ErrorPage() {
        ...
        RestAssured.get("/index2.html").then().statusCode(200).body(containsString(CONTENT)); // check that index2.html
                                                                                              // is live reloaded

        RestAssured.given().accept(ContentType.HTML).when().get("/api").then() // try to load unknown path
                .statusCode(404).body(containsString("index2.html")); // check that index2.html is displayed
    }
    ...
    public void shouldNotDisplayDeletedFileIn404ErrorPage() {
        ...
        RestAssured.get("/test.html").then().statusCode(200).body(containsString(TEST_CONTENT)); // check that test.html
                                                                                                 // is live reloaded

        test.deleteResourceFile(META_INF_RESOURCES + "test.html"); // delete test.html file

        RestAssured.given().accept(ContentType.HTML).when().get("/test.html").then() // try to load static file
                .statusCode(404).body(not(containsString("test.html"))); // check that test.html is not displayed
