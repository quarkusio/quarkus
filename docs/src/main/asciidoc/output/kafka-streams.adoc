...
:summary: This guide demonstrates how your {product-name-first} application can utilize the Apache Kafka Streams API to implement stream processing applications based on Apache Kafka.

This guide demonstrates how your {product-name} application can utilize the Apache Kafka Streams API to implement stream processing applications based on Apache Kafka.
...
[NOTE]
====
The {product-name} extension for Kafka Streams allows for very fast turnaround times during development by supporting the {product-name} Dev Mode (e.g. via `./mvnw compile quarkus:dev`).
...
Also specify the following settings in your {product-name} `application.properties`:
...
If you already have your {product-name} project configured, you can add the `smallrye-reactive-messaging-kafka` extension
...
The two channels are mapped to Kafka topics using the {product-name} configuration file `application.properties`.
...
This creates the `aggregator` project with the {product-name} extension for Kafka Streams and with the Jackson support for RESTEasy Reactive.

If you already have your {product-name} project configured, you can add the `kafka-streams` extension
to your project by running the following command in your project base directory:
...
----
<1> The `@RegisterForReflection` annotation instructs {product-name} to keep the class and its members during the native compilation. More details about the `@RegisterForReflection` annotation can be found on  the xref:writing-native-applications-tips.adoc#registerForReflection[native application tips] page.
...
the {product-name} extension will take care of configuring, starting and stopping the actual Kafka Streams engine.
...
The Kafka Streams extension is configured via the {product-name} configuration file `application.properties`.
...
`topics` is specific to {product-name}: the application will wait for all the given topics to exist before launching the Kafka Streams engine.
...
Instead of running them directly on the host machine using the {product-name} dev mode,
...
The `Dockerfile` created by {product-name} by default needs one adjustment for the `aggregator` application in order to run the Kafka Streams pipeline.
...
The {product-name} extension for Kafka Streams enables the execution of stream processing applications
...
This guide has shown how you can build stream processing applications using {product-name} and the Kafka Streams APIs,
...
Refer to the {product-name} guides on xref:telemetry-micrometer.adoc[Micrometer], xref:smallrye-metrics.adoc[SmallRye Metrics], and xref:smallrye-health.adoc[SmallRye Health] to learn more.
