...
:summary: This reference guide explains how to develop command line applications with {product-name-first}.
...
. Implement `QuarkusApplication` and have {product-name} run this method automatically
. Implement `QuarkusApplication` and a Java main method, and use the Java main method to launch {product-name}
...
The simplest possible command mode application with access to {product-name} APIs might appear as follows:
...
----
<.> The `@QuarkusMain` annotation tells {product-name} that this is the main entry point.
<.> The `run` method is invoked once {product-name} starts, and the application stops when it finishes.
...
public class JavaMain {
    ...
    public static void main(String... args) {
        {product-name}.run(HelloWorldMain.class, args);
    }
}
----
...
and `quarkus.package.main-class` is not specified then {product-name} will automatically generate a main class
...
. Shut down {product-name} and exit the JVM after the main method returns

Shutdown is always initiated by the application main thread returning. If you want to run some logic on startup,
and then run like a normal application (i.e. not exit) then you should call `{product-name}.waitForExit` from the main
thread (A non-command mode application is essentially just running an application that just calls `waitForExit`).

If you want to shut down a running application and you are not in the main thread, then you should call `{product-name}.asyncExit`
