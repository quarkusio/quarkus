...
TIP: Remember that it's possible to quote property names when they contain `.`. Indeed, without quoting, Quarkus wouldn't
be able to tell where your property ends when parsing the property's "path". So if you need to reference a
