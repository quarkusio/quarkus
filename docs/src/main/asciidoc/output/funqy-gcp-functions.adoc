...
within your project, then you will need to choose the function within your {product-name-first} `application.properties`:
...
that will bootstrap {product-name}.
====
...
{product-name} provides built-in support for testing your Funqy Google Cloud functions via the `quarkus-test-google-cloud-functions` dependency.
