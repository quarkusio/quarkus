...
The {product-name-first} Security framework supports multiple authentication mechanisms, which you can use to secure your applications.
You can also combine authentication mechanisms.

[TIP]
====
Before you choose an authentication mechanism for securing your {product-name} applications, review the information provided.
====

== Overview of supported authentication mechanisms

Some supported authentication mechanisms are built into {product-name}, while others require you to add an extension.
All of these mechanisms are detailed in the following sections:

* <<built-in-authentication-mechanisms>>
* <<other-supported-authentication-mechanisms>>

The following table maps specific authentication requirements to a supported mechanism that you can use in {product-name}:
...
{product-name} Security provides the following built-in authentication support:
...
You can secure your {product-name} application endpoints with the built-in HTTP Basic authentication mechanism.
For more information, see the following documentation:

* xref:security-basic-authentication.adoc[Basic authentication]
** xref:security-basic-authentication-howto.adoc[Enable Basic authentication]
* xref:security-jpa.adoc[{product-name} Security with Jakarta Persistence]
...
{product-name} provides form-based authentication that works similarly to traditional Servlet form-based auth.
Unlike traditional form authentication, the authenticated user is not stored in an HTTP session because {product-name} does not support clustered HTTP sessions.
...
{product-name} provides mutual TLS (mTLS) authentication so that you can authenticate users based on their X.509 certificates.

To use this authentication method, you must first enable SSL/TLS for your application.
For more information, see the xref:http-reference.adoc#ssl[Supporting secure connections with SSL]  section of the {product-name} "HTTP reference" guide.
...
The information from the client certificate can be used to enhance {product-name} `SecurityIdentity`.
For example, you can add new roles after checking a client certificate subject name, and so on.
For more information about customizing `SecurityIdentity`, see the link:{url-quarkusio-guides}security-customization#security-identity-customization[Security identity customization] section in the {product-name} "Security tips and tricks" guide.

[[other-supported-authentication-mechanisms]]
== Other supported authentication mechanisms

{product-name} Security also supports the following authentication mechanisms through extensions:
...
For more information, see the  xref:security-webauthn.adoc[Secure a {product-name} application by using the WebAuthn authentication mechanism] guide.
...
The {product-name} `quarkus-oidc` extension provides a reactive, interoperable, multitenant-enabled OIDC adapter that supports Bearer token and Authorization Code Flow authentication mechanisms.
...
After the user is redirected back to {product-name}, the mechanism completes the authentication process by exchanging the provided code that was granted for the ID, access, and refresh tokens.
...
[NOTE]
====
Using the {product-name} OIDC extension, both the Bearer token and Authorization Code Flow authentication mechanisms use <<smallrye-jwt-authentication>> to represent JWT tokens as MicroProfile JWT `org.eclipse.microprofile.jwt.JsonWebToken`.
====

==== Additional Quarkus resources for OIDC authentication

For more information about OIDC authentication and authorization methods that you can use to secure your {product-name} applications, see the following resources:

[options="header"]
|====
|OIDC topic |{product-name} information resource
...
|Securing {product-name} with commonly-used OpenID Connect providers|xref:security-openid-connect-providers.adoc[Configuring well-known OpenID Connect providers]
...
[NOTE]
====
To enable the {product-name} OIDC extension at runtime, set `quarkus.oidc.tenant-enabled=false` at build time.
...
This filter can be registered with MicroProfile REST client implementations injected into the current {product-name} endpoint, but it is not related to the authentication requirements of this service endpoint.
For example, it can be a public endpoint or be protected with mTLS.

[IMPORTANT]
====
In this scenario, you do not need to protect your {product-name} endpoint by using the {product-name} OpenID Connect adapter.
====
...
This filter can be registered with MicroProfile REST client implementations injected into the current {product-name} endpoint, which must be protected by using the {product-name} OIDC adapter.
...
`quarkus-elytron-security-oauth2` provides an alternative to the {product-name} `quarkus-oidc` Bearer token authentication mechanism extension.
`quarkus-elytron-security-oauth2` is based on `Elytron` and is primarily intended for introspecting opaque tokens remotely.
For more information, see the {product-name} xref:security-oauth2.adoc[Using OAuth2] guide.

[[oidc-jwt-oauth2-comparison]]
== Choosing between OpenID Connect, SmallRye JWT, and OAuth2 authentication mechanisms

Use the following information to select the appropriate token authentication mechanism to secure your {product-name} applications.
...
For example, you can combine the built-in Basic and the {product-name} `quarkus-oidc` Bearer token authentication mechanisms.

[IMPORTANT]
====
You cannot combine the {product-name} `quarkus-oidc` Bearer token and `smallrye-jwt` authentication mechanisms because both mechanisms attempt to verify the token extracted from the HTTP Bearer token authentication scheme.
====
...
* xref:security-architecture.adoc[{product-name} Security architecture]
* xref:security-identity-providers.adoc[Identity providers]
