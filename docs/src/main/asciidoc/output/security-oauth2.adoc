...
:categories: security
:summary: This guide explains how your {product-name-first} application can utilize OAuth2 tokens to provide secured access to the Jakarta REST endpoints.
:extension-name: Elytron Security OAuth2

This guide explains how your {product-name} application can utilize OAuth2 tokens to provide secured access to the Jakarta REST (formerly known as JAX-RS) endpoints.
...
OpenID Connect extension has to be used if the {product-name} application needs to authenticate the users using OIDC Authorization Code Flow. For more information, see the xref:security-oidc-code-flow-authentication.adoc[OIDC code flow mechanism for protecting web applications] guide.
...
2019-03-28 14:31:37,315 INFO  [io.quarkus] (main) {product-name} 0.20.0 started in 0.006s. Listening on: http://[::]:8080
...
<1> The `start` method is invoked by {product-name} before any test is run and returns a `Map` of configuration properties that apply during the test execution.
<2> Launch Wiremock.
...
<1> Use the previously created `MockAuthorizationServerTestResource` as a {product-name} test resource.
