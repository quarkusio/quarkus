...
public class JsonRpcRouter {
    ...
    public JsonRpcMethod findMethod(String methodName) {
        JsonRpcMethod m = runtimeMethodsMap.get(methodName);
        if (m != null) {
            return m;
        }
        m = runtimeSubscriptionMap.get(methodName);
        if (m != null) {
            return m;
        }
        m = deploymentMethodsMap.get(methodName);
        if (m != null) {
            return m;
        }
        m = deploymentSubscriptionsMap.get(methodName);
        if (m != null) {
            return m;
        }
        m = recordedMethodsMap.get(methodName);
        if (m != null) {
            return m;
        }
        return recordedSubscriptionsMap.get(methodName);
    }
