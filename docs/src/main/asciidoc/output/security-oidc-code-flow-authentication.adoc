...
To protect your web applications, you can use the industry-standard OpenID Connect (OIDC) Authorization Code Flow mechanism provided by the {product-name-first} OIDC extension.

== Overview of the OIDC authorization code flow mechanism

The {product-name} OpenID Connect (OIDC) extension can protect application HTTP endpoints by using the OIDC Authorization Code Flow mechanism supported by OIDC-compliant authorization servers, such as link:https://www.keycloak.org[Keycloak].
...
The following diagram outlines the Authorization Code Flow mechanism in {product-name}.

.Authorization code flow mechanism in {product-name}
image::authorization_code_flow.png[alt=Authorization Code Flow, width="60%", align=center]
. The {product-name} user requests access to a {product-name} web-app application.
. The {product-name} web-app redirects the user to the authorization endpoint, that is, the OIDC provider for authentication.
...
. The OIDC provider authenticates the user credentials entered and, if successful, issues an authorization code then redirects the user back to the {product-name} web-app with the code included as a query parameter.
. The {product-name} web-app exchanges this authorization code with the OIDC provider for ID, access, and refresh tokens.

The authorization code flow is completed and the {product-name} web-app uses the tokens issued to access information about the user and grants the relevant role-based authorization to that user.
The following tokens are issued:

* ID token: The {product-name} `web-app` application uses the user information in the ID token to enable the authenticated user to log in securely and to provide role-based access to the web application.
* Access token: The {product-name} web-app might use the access token to access the UserInfo API to get additional information about the authenticated user or to propagate it to another endpoint.
* Refresh token: (Optional) If the ID and access tokens expire, the {product-name} web-app can use the refresh token to get new ID and access tokens.
...
You can use this same configuration to override a discovered endpoint URL if that URL does not work for the local {product-name} endpoint and a more specific value is required. 
...
{product-name} OIDC, specifically the `quarkus.oidc.runtime.OidcProviderClient` class,  authenticates to the OIDC provider when the authorization code must be exchanged for the ID, access, and refresh tokens, or when the ID and access tokens need to be refreshed or introspected.
...
In {product-name} Security, `quarkus-oidc` supports a non-standard `client_secret_post_jwt` authentication method, which you can configure as follows:
...
In our case, this is the {product-name} application.

{product-name} sets this parameter to the current application request URL by default. 
For example, if a user is trying to access a {product-name} service endpoint at `http://localhost:8080/service/1` then the `redirect_uri` parameter is set to `http://localhost:8080/service/1`. 
...
In such cases, a `quarkus.oidc.authentication.redirect-path` property has to be set, for example, `quarkus.oidc.authentication.redirect-path=/service/callback`, and {product-name} will set the `redirect_uri` parameter to an absolute URL such as `http://localhost:8080/service/callback`, which will be the same regardless of the current request URL.
...
If the user authentication fails at the OIDC authorization endpoint, then the provider will redirect the user back to {product-name} with `error` and `error_description` parameters instead of `code`.
...
Note that `AccessTokenCredential` is used if the access token issued to the {product-name} `web-app` application is opaque (binary) and can not be parsed to a `JsonWebToken` or if the inner content is necessary for the application.
...
RefreshToken is used by Quarkusn OIDC to refresh the current ID and access tokens as part of its <<session-management,session management>> process.
...
The verification process of OIDC authorization code flow tokens follows the Bearer token authentication token verification and introspection logic. For more information, see the xref:security-oidc-bearer-token-authentication.adoc#token-verification-introspection[Token Verification And Introspection] section of the "{product-name} OpenID Connect (OIDC) Bearer token authentication" guide.

[NOTE]
--
With {product-name} `web-app` applications, only the `IdToken` is verified by default because the access token is not used to access the current {product-name} `web-app` endpoint and is intended to be propagated to the services expecting this access token.
If you expect the access token to contain the roles required to access the current {product-name} endpoint (`quarkus.oidc.roles.source=accesstoken`) then it will also be verified.
--
...
While PKCE is of primary importance to public OpenID Connect clients, such as SPA scripts running in a browser, it can also provide an extra level of protection to {product-name} OIDC `web-app` applications. 
With PKCE, {product-name} OIDC `web-app` applications are confidential OpenID Connect clients capable of securely storing the client secret and using it to exchange the code for the tokens.
...
The `code_verifier` is decrypted when the user is redirected back to {product-name} and sent to the token endpoint alongside the `code`, client secret, and other parameters to complete the code exchange. 
...
Use the following key points to find the right balance for securing your {product-name} applications:
...
If your application is deployed across multiple domains, set the `quarkus.oidc.authentication.cookie-domain` property so that the session cookie is visible to all protected {product-name} services.
For example, if you have {product-name} services deployed on the following two domains, then you must set the `quarkus.oidc.authentication.cookie-domain` property to `company.net`.
...
This makes {product-name} OIDC endpoints completely stateless.
...
Configure the `quarkus.oidc.token-state-manager.encryption-secret` property if {product-name} authenticates to the OpenId Connect Provider by using one of the following authentication methods:

* mTLS
* `private_key_jwt`, where a private RSA or EC key is used to sign a JWT token

Otherwise, a random key is generated, which can be problematic if the {product-name} application is running in the cloud with multiple pods managing the requests.
...
Users can request a logout by sending a request to the {product-name} endpoint logout path set with a `quarkus.oidc.logout.path` property.
...
If the `quarkus.oidc.logout.post-logout-path` is set then a `q_post_logout` cookie will be created and a matching `state` query parameter will be added to the logout redirect URI and the OpenID Connect Provider will return this `state` once the logout has been completed. It is recommended for the {product-name} `web-app` applications to check that a `state` query parameter matches the value of the `q_post_logout` cookie which can be done for example in a Jakarta REST filter.
...
Here is an example of how to configure a {product-name} app initiated logout flow:
...
However, this is not a problem for {product-name} because the specific logout mechanisms of such OIDC providers only differ in how the logout URL query parameters are named.
...
You can configure {product-name} to support `Back-Channel Logout` as follows:
...
It is like <<back-channel-logout,Back-channel logout>> but the logout steps are executed by the user agent like the browser and not in the background by the OIDP provider.
This option is rarely used.

You can configure {product-name} to support `Front-Channel Logout` as follows:
...
<<user-initiated-logout,User-initiated logout>> will log the user out of the OIDC provider, if it is used as single sign-on, it might not be what you require.
...
By default, logout is based on the expiration time of the ID token issued by the OIDC provider. When the ID token expires, the current user session at the {product-name} endpoint is invalidated and the user is redirected to the OIDC provider again to authenticate. 
...
If you work with a {product-name} OIDC `web-app` application, then it is the {product-name} OIDC code authentication mechanism that is managing the user session lifespan.
...
When the current authenticated user returns to the protected {product-name} endpoint and the ID token associated with the session cookie has expired, then, by default, the user is automatically redirected to the OIDC Authorization endpoint to re-authenticate. 
...
If the ID token expires, the session cookie might not be returned to the {product-name} endpoint during the next user request as the cookie lifespan would have elapsed. 
{product-name} assumes that this request is the first authentication request. 
...
You can further optimize this process by having a simple JavaScript function ping your {product-name} endpoint to periodically emulate the user activity, which minimizes the time frame during which the user might have to be re-authenticated.
...
OAuth2 providers such as GitHub do not return `IdToken`, the fact of the user authentication is implicit and is indirectly represented by the `access` token which represents an authenticated user authorizing the current {product-name} `web-app` application to access some data on behalf of the authenticated user.
...
This approach is similar to the OIDC <<user-info,UserInfo>> approach, with `UserInfo` fetched by {product-name} OIDC on your behalf.

For example, when working with GitHub, the {product-name} endpoint can acquire an `access` token which allows the {product-name} endpoint to request a GitHub profile for the current user.
...
In this case, the session lifespan is set to 5 minutes, which you can can extend further as described in the <<session-management,session management>> section.

This simplifies how you handle an application that supports multiple OIDC providers.
--

The next step is to ensure that the returned access token can be useful and is valid to the current {product-name} endpoint.
...
For well-known OIDC or OAuth2 providers, like Apple, Facebook, GitHub, Google, Microsoft, Spotify, and Twitter, {product-name} can help significantly simplify your application's configuration with the `quarkus.oidc.provider` property.
Here is how you can integrate `quarkus-oidc` with GitHub after you have link:https://docs.github.com/en/developers/apps/building-oauth-apps/creating-an-oauth-app[created a GitHub OAuth application]. Configure your {product-name} endpoint like this:
...
If you prefer to use SPAs and JavaScript APIs such as `Fetch` or `XMLHttpRequest`(XHR) with {product-name} web applications, be aware that OpenID Connect providers might not support cross-origin resource sharing (CORS) for authorization endpoints where the users are authenticated after a redirect from {product-name}. 
This will lead to authentication failures if the {product-name} application and the OpenID Connect provider are hosted on different HTTP domains, ports, or both.

In such cases, set the `quarkus.oidc.authentication.java-script-auto-redirect` property to `false`, which will instruct {product-name} to return a `499` status code and a `WWW-Authenticate` header with the `OIDC` value.
...
If the script engine sets an engine-specific request header itself, then you can register a custom `quarkus.oidc.JavaScriptRequestChecker` bean, which will inform {product-name} if the current request is a JavaScript request. For example, if the JavaScript engine sets a header such as `HX-Request: true` then you can have it checked like this:
...
You can have {product-name} OIDC `web-app` applications access **Google Cloud services** such as **BigQuery** on behalf of the currently-authenticated users who have enabled OpenID Connect (Authorization Code Flow) permissions to such services in their Google developer consoles.
...
`quarkus.oidc.authentication.force-redirect-https-scheme` property may also be used when the {product-name} application is running behind an SSL terminating reverse proxy.
...
{product-name} offers several options from mocking to a local run of an OIDC provider.
