...
public class HttpCompatibilityTest {
    ...
    static final QuarkusUnitTest TEST = new QuarkusUnitTest().setArchiveProducer(() -> ShrinkWrap
            ...
            .addClasses(Util.class, PingPongResource.class, PingPongResource.PingPongRestClient.class,
                    ServletEndpoint.class, VertxWebEndpoint.class, HelloResource.class)
    ...
    /**
     * Same as io.quarkus.micrometer.deployment.binder.UriTagTest. Makes sure we are getting equivalent results in OTel.
    ...
    void testHttpTimerToHistogramCompatibility() {
        ...
        final List<MetricData> clientMetricDataList = metricExporter.getFinishedMetricItems("http.client.requests",
                null);

        Awaitility.await().atMost(10, SECONDS).untilAsserted(() -> {
            final MetricData clientMetricData = clientMetricDataList.get(clientMetricDataList.size() - 1); // get last
                                                                                                           // collected
            assertThat(clientMetricData.getInstrumentationScopeInfo().getName())
                    .isEqualTo("io.opentelemetry.micrometer-1.5");
            assertThat(clientMetricData).hasName("http.client.requests") // in OTel it should be
                    ...
                    .hasHistogramSatisfying(histogram -> histogram.isCumulative().hasPointsSatisfying(
                            ...
                            point -> point.hasCount(1).hasAttributesSatisfying(
                                    ...
                                    equalTo(STATUS, "400")),
                            point -> point.hasCount(1).hasAttributesSatisfying(equalTo(URI, "/pong/{message}"),
                                    equalTo(METHOD, "GET"), equalTo(STATUS, "500")),
                            point -> point.hasCount(6) // 3 sync requests and 3 async requests
                                    .hasAttributesSatisfying(equalTo(URI, "/pong/{message}"), equalTo(METHOD, "GET"),
                                            equalTo(STATUS, "200"))));
        });
    }

    private static void assertServerMetrics(MetricData metricData) {
        assertThat(metricData).hasName("http.server.requests") // in OTel it should be "http.server.request.duration"
                .hasDescription("HTTP server request processing time") // in OTel it should be "Duration of HTTP server
                // requests."
                .hasUnit("ms") // OTel has seconds
                .hasHistogramSatisfying(histogram -> histogram.isCumulative().hasPointsSatisfying(
                        // valid entries
                        point -> point.hasCount(1).hasAttributesSatisfying(
                                ...
                                // "http.response.status_code" and it should use a long key and not a string key
                                equalTo(STATUS, "200")),
                        point -> point.hasCount(1).hasAttributesSatisfying(equalTo(URI, "/vertx/item/{id}/{sub}"),
                                equalTo(METHOD, "GET"), equalTo(STATUS, "200")),
                        point -> point.hasCount(2).hasAttributesSatisfying(equalTo(URI, "/hello/{message}"),
                                equalTo(METHOD, "HEAD"), equalTo(STATUS, "200")),
                        point -> point.hasCount(2).hasAttributesSatisfying(equalTo(URI, "/hello/{message}"),
                                equalTo(METHOD, "GET"), equalTo(STATUS, "200")),
                        point -> point.hasCount(2).hasAttributesSatisfying(equalTo(URI, "/vertx/echo/{msg}"),
                                equalTo(METHOD, "HEAD"), equalTo(STATUS, "200")),
                        point -> point.hasCount(2).hasAttributesSatisfying(equalTo(URI, "/vertx/echo/{msg}"),
                                equalTo(METHOD, "GET"), equalTo(STATUS, "200")),
                        point -> point.hasCount(5) // 3 x 200 + 400 + 500 status codes
                                .hasAttributesSatisfying(equalTo(URI, "/ping/{message}"), equalTo(METHOD, "GET"),
                                        equalTo(STATUS, "200")),
                        point -> point.hasCount(3).hasAttributesSatisfying(equalTo(URI, "/async-ping/{message}"),
                                equalTo(METHOD, "GET"), equalTo(STATUS, "200")),
                        point -> point.hasCount(6) // 3 sync requests and 3 async requests
                                .hasAttributesSatisfying(equalTo(URI, "/pong/{message}"), equalTo(METHOD, "GET"),
                                        equalTo(STATUS, "200")),
                        point -> point.hasCount(1).hasAttributesSatisfying(equalTo(URI, "/pong/{message}"),
                                equalTo(METHOD, "GET"), equalTo(STATUS, "500")),
                        point -> point.hasCount(1).hasAttributesSatisfying(equalTo(URI, "/pong/{message}"),
                                equalTo(METHOD, "GET"), equalTo(STATUS, "400"))));
    }
}
