...
If you did not leave {product-name-first} running in dev mode, start it again:
...
If you did not leave {product-name} running in dev mode, start it again:
...
You've observed some of the metrics that {product-name} captures automatically, and have added a `Counter` and `Timer` that are unique to the application.
