...
NOTE: Serving static resources from a local directory is disabled by default.
To enable it, you must explicitly set `quarkus.http.static-dir.enabled=true`
and configure both `endpoint` and `path`.
