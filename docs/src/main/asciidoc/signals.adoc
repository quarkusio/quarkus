...
A new span is created for each receiver invocation as a child of the emitter's span, so a `publish` emission that reaches multiple receivers results in one child span per receiver.
Each span is named `signal receive` and carries the `signals.signal.type` and `signals.emission.type` attributes.

If the signal is emitted outside of an active trace, no span is created.
