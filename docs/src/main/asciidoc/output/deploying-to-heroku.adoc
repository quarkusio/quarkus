...
:categories: cloud
:summary: Deploy your {product-name-first} applications on Heroku.

In this guide you will learn how to deploy a {product-name} based web application as a web-dyno to Heroku.
...
Heroku can be used in different ways to run a {product-name} application:

* As a plain Java program running in a container defined by Heroku's environment
* As a containerized Java program running in a container defined by the {product-name} build process
* As a containerized native program running in a container defined by the {product-name} build process
...
Heroku picks a random port and assigns it to the container that is eventually running your {product-name} application.
That port is available as an environment variable under `$PORT`.
The easiest way to make {product-name} in all deployment scenarios aware of it is using the following configuration:
...
The first variant uses the {product-name} Maven build to create the _quarkus-app_ application structure containing the runnable "fast-jar" as well as all libraries needed
...
{product-name} needs JDK 11, so we specify that first:
...
We need to add an extension to our project to build container images via the {product-name} Maven plugin:
