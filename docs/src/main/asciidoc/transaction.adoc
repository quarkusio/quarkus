...
public class TransactionExample {
    ...
    public void runnerExample() {
        ...
        int result = QuarkusTransaction.runner().suspendExisting()
                .timeout(10)
                .exceptionHandler((throwable) -> {
                    if (throwable instanceof SomeException) {
                        return RunOptions.ExceptionResult.COMMIT;
...
The second method shows the use of lambda scoped transactions with `QuarkusTransaction.run(...)`/`QuarkusTransaction.call(...)`;
the first example just runs a `Runnable` within a new transaction,
the second example calls a  `Callable` with some specific options.
In particular the `exceptionHandler` method can be used to control if the transaction
...
The third method shows the use of lambda scoped transactions with `QuarkusTransaction.runner()`;
the first example just runs a `Runnable` within a new transaction,
the second calls a `Callable` with some specific options.
In particular the semantic (which must be explicit) is set to `SUSPEND_EXISTING`,
and the `exceptionHandler` method can be used to control if the transaction is rolled back or not on exception.
