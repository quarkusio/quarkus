...
public class OpenApiWithConfigTestCase {
    ...
    static QuarkusUnitTest runner = new QuarkusUnitTest()
            .withApplicationRoot(
                    (jar) -> jar.addClasses(OpenApiResource.class, ResourceBean.class)
                            ...
                            .addAsResource(new StringAsset("mp.openapi.scan.disable=true"
                                    ...
                                    + "\nquarkus.smallrye-openapi.info-title=Bla"), "application.properties"));

    @Test
    public void testOpenAPI() {
        RestAssured.given().header("Accept", "application/json").when().get(OPEN_API_PATH).then()
                .header("Content-Type", "application/json;charset=UTF-8").body("openapi", Matchers.startsWith("3.0"))
                ...
                .body("paths", Matchers.hasKey("/openapi")).body("paths", Matchers.not(Matchers.hasKey("/resource")));
