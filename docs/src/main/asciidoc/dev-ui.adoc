...
Dev UI has one *Observability* entry in the left menu, and it is a dashboard the user composes:
they pick the cards they want the first time they open it, change their mind later with
*Edit dashboard*, and drag the cards into the order that suits them. The choice is kept in the
browser's local storage rather than per application, so it follows the developer from one
...
----
<1> The page that backs the card. An unlisted page is reached from the dashboard rather than
from your extension's card, but a listed page works just as well - all the dashboard needs is a
page whose web component it can import.
...
The card renders your component inline, in a box roughly a third of the page tall, with an *Open*
link through to the full page. Size the component against its host (`height: 100%`, or a flex
column) rather than against the viewport, and let it scroll internally - the user can always
maximize the card to give it the whole dashboard area. Nothing else changes: the component is
imported only when the card is actually on the dashboard, and its JSON-RPC calls, subscriptions
and hot reload behave exactly as they do on the full page.
...
the user adds: a rate for a counter, a dial for a gauge, bucket bars for a histogram, and mean,
max and percentiles for a timer or a distribution summary. If your extension exposes metrics,
simply register them as usual.
...
`MetricsBackendBuildItem`; the core then registers the JSON-RPC service and the metrics signal
once, no matter how many backends are active. See `DevUiMetricsSampler` (Micrometer) and
`DevUiMetricsExporter` (OpenTelemetry) for the two existing implementations.
