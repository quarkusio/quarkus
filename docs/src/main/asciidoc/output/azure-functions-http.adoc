...
In other words, this extension is a bridge from the Azure Functions HttpTrigger and the {product-name-first} family
of HTTP APIs.
...
You can generate the example code from {product-name}'s online application generator at https://code.quarkus.io/d?e=azure-functions-http&cn=code.quarkus.io[this link].
...
{product-name} dev mode works by just running your application as a HTTP endpoint.  In dev mode
...
{product-name} and Azure Functions.
