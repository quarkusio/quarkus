...
curl $APP_NAME.herokuapp.com/hello?name=World
----
...
When you want to use Podman as a drop-in-replacement for Docker, you will have some problems because the Heroku CLI depends on Docker. But there are possible solutions for these problems.
...
====
Instead of doing a normal podman push (OCI format) we must use a workaround in order to push and release our app through Podman and the Heroku CLI in the desired format (v2s2 - Docker Image Manifest Version 2, Schema 2).
[source,bash]
----
...
After that, push and release again (see above).
