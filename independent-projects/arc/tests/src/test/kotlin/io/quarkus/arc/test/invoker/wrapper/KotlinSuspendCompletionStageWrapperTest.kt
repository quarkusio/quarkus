...
class KotlinSuspendCompletionStageWrapperTest {
    @RegisterExtension
        ...
        .beanRegistrars(InvokerHelperRegistrar(MyService::class.java) { bean, factory, invokers ->
            for (name in listOf("helloSync", "helloAsync", "helloThrow")) {
                ...
                invokers[name] = factory.createInvoker(bean, method)
                    ...
                    .withInvocationWrapper(SuspendToCompletionStage::class.java, "wrap")
                    .build()
            }
        })
        .build()
    ...
    fun testSync() {
        ...
        val invoker = helper.getInvoker<MyService, CompletionStage<String>>("helloSync")
        ...
        assertThat(result.toCompletableFuture()).succeedsWithin(Duration.ofSeconds(5))
        assertEquals(1, MyDependency.destroyedCounter.get())
        assertEquals("hello", result.toCompletableFuture().get())
    }
    ...
    fun testAsync() {
        ...
        val helper = Arc.container().instance(InvokerHelper::class.java).get()
        val invoker = helper.getInvoker<MyService, CompletionStage<String>>("helloAsync")
    ...
    fun testSyncThrow() {
        ...
        val helper = Arc.container().instance(InvokerHelper::class.java).get()
        val invoker = helper.getInvoker<MyService, CompletionStage<String>>("helloThrow")

        val result = invoker.invoke(null, arrayOf(null))

        // the invoker offloads the method call to a thread pool, so we need to wait for completion
        assertThat(result.toCompletableFuture()).failsWithin(Duration.ofSeconds(5))
        assertEquals(1, MyDependency.destroyedCounter.get())
        ...
        assertInstanceOf(IllegalArgumentException::class.java, exception.cause)
    }

    class SuspendToCompletionStage {
        companion object {
