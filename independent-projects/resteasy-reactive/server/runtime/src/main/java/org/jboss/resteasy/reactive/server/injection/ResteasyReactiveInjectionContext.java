...
public interface ResteasyReactiveInjectionContext {
     ...
     *         if there is no such context object
     */
    <T> T getContextParameter(Class<T> type);

    /**
     * Gets the bean parameter instance by type via CDI, and registers a cleanup for it. This does not call __inject on
     * it, and it does not work for records (which cannot be gotten via CDI).
     *
     * @throws IllegalStateException
     *         if there is no such context object
     */
