...
If the application raises `quarkus.log.min-level` above `DEBUG`, also set `quarkus.log.category."io.quarkus.vertx.http.runtime.cors.CORSFilter".min-level=DEBUG`, which is a build-time property, otherwise the `DEBUG` records are removed at build time.
See the xref:logging.adoc[logging guide] for details.
