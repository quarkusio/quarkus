...
Oracle has a very weird behavior of committing the uncommitted transactions on connection close.

Which means that when stopping Quarkus for instance, in progress transactions might be committed even if incomplete.

Given that is not the expected behavior and that it could lead to data loss, we added an interceptor that rollbacks any unfinished transactions at connection close.

If this behavior introduced in 3.18 causes issues for your specific workload, you can disable it by setting the `-Dquarkus-oracle-no-automatic-rollback-on-connection-close` system property.
