...
:summary: This guide demonstrates how your {product-name-first} application can utilize SmallRye Reactive Messaging to interact with Apache Pulsar.

This guide demonstrates how your {product-name} application can utilize SmallRye Reactive Messaging to interact with Apache Pulsar.
...
{product-name} has built-in capabilities to deal with JSON Pulsar messages.
...
{product-name} detects them, and if none are found, generates them using appropriate schema types.
...
No need to configure anything, as {product-name} will automatically associate the `quotes` channel to the `quotes` Pulsar topic.
...
{product-name} starts a Pulsar broker automatically, configures the application and shares the Pulsar broker instance between different applications.
