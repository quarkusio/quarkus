...
public class PartTypeMultipartTest {
    ...
    static QuarkusUnitTest TEST = new QuarkusUnitTest().withApplicationRoot(
            (jar) -> jar.addClasses(PartTypeDto.class, PartTypeEnum.class, PartTypeResource.class));
    ...
    public void testMultipartEncoding() throws URISyntaxException {
        MultiPartSpecification multiPartSpecification = new MultiPartSpecBuilder("{ \"key\": \"value\" }")
                .controlName("myMapping").mimeType(MediaType.APPLICATION_JSON).build();

        RestAssured.given().multiPart(multiPartSpecification).multiPart("flag", "true")
                .multiPart("partTypeEnum", "ACTIVE").post("/test/part-type/").then().statusCode(200);
    }
