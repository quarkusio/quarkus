...
NOTE: <<global_variables,Global variables>> are part of the data of the calling template, so an isolated tag template, and any template it includes, cannot reference them directly; `{year}` results in a "key not found" error even if a global variable `year` exists. Use the `global:` namespace instead, for example `{global:year}`.
...
IMPORTANT: Global variables are added to the data of the template instance that is rendered. They are therefore not visible in <<user_tags,user tags>>, which are isolated by default, nor in templates included from an isolated tag. In these templates, always use the `global:` namespace, for example `{global:age}`, or call the tag with `_unisolated`.
