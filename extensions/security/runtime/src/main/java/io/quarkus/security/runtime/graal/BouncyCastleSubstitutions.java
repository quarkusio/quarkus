...
final class BouncyCastlePackages {
    ...
    static final Set<String> PACKAGES = Arrays.stream(Package.getPackages())
            .map(Package::getName).filter(p -> p.startsWith(ORG_BOUNCYCASTLE_CRYPTO_PACKAGE)).collect(Collectors.toSet());
}
