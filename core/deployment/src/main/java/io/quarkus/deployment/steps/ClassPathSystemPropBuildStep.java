...
public class ClassPathSystemPropBuildStep {
    ...
    public void set(List<SetClassPathSystemPropBuildItem> setCPItems, CurateOutcomeBuildItem curateOutcome,
            ClassPathSystemPropertyRecorder recorder) {
        if (setCPItems.isEmpty()) {
            return;
        }
        Collection<ResolvedDependency> runtimeDependencies = curateOutcome.getApplicationModel()
        ...
        List<String> allJarPaths = Stream.concat(parentFirst.stream(), regular.stream())
