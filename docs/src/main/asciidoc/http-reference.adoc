...
Quarkus can serve static files from a local filesystem directory (outside of Java resources).
This is configured via the `quarkus.http.static-dir.*` properties.


To expose the contents of a `public/` path under the `/static` endpoint:
