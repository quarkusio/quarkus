...
[WARNING]
====
This guide is about the REST Client compatible with https://resteasy.dev[RESTEasy Classic] which used to be the default Jakarta REST (formerly known as JAX-RS) implementation until {product-name-first} 2.8.
...
If you already have your {product-name} project configured, you can add the `rest-client` and the `rest-client-jackson` extensions
...
The `getById` method gives our code the ability to get an extension by id from the Code {product-name} API. The client will handle all the networking and marshalling leaving our code clean of such technical details.

The purpose of the annotations in the code above is the following:

* `@RegisterRestClient` allows {product-name} to know that this interface is meant to be available for
...
[NOTE]
====
When a JSON extension is installed such as `quarkus-rest-client-jackson` or `quarkus-rest-client-jsonb`, {product-name} will use the `application/json` media type
...
If a property is specified via both the {product-name} notation and the MicroProfile notation, the {product-name} notation takes a precedence.
====
...
[NOTE]
====
{product-name} REST client provides an embedded hostname verifier strategy to disable the hostname verification called `io.quarkus.restclient.NoopHostnameVerifier`.
====
...
In {product-name}, the REST Client extension and xref:rest-json.adoc[the RESTEasy extension] share the same infrastructure.
...
In {product-name} tests when some service needs to be started before the {product-name} tests are ran, we utilize the `@io.quarkus.test.common.QuarkusTestResource`
...
values that {product-name} will use.
...
<2> The `start` method is invoked by {product-name} before any test is run and returns a `Map` of configuration properties that apply during the test execution.
<3> Launch Wiremock.
