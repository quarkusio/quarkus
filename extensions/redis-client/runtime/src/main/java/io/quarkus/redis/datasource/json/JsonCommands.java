...
/**
 * Allows executing commands from the {@code json} group (requires the Redis stack). See
 * <a href="https://redis.io/commands/?group=json">the json command list</a> for further information about these
 * commands.
 * <p>
 * Redis JSON lets you store, update, and retrieve JSON values in a Redis database, similar to any other Redis data
 * type.
 * <p>
 * Ths API is based on the {@link JsonObject} and {@link JsonArray} types. Some methods also allows direct
 * <em>mapping</em> from and to objects. In the case of deserialization, the {@link Class Class<T>} must be passed.
 *
 * @param <K>
 *        the type of the key
 */
public interface JsonCommands<K> extends RedisCommands {
     ...
     * <p>
     * This method uses the root path ({@code $}). It map the retrieve JSON document to an object of type {@code <T>}.
     *
     * @param key
     *        the key, must not be {@code null}
     * @param clazz
     ...
     *
     * @return the stored JSON object, {@code null} if it does not exist. If no path are passed, this is equivalent to
     *         {@link #jsonGetObject(Object)}. If multiple paths are passed, the produced JSON object contains the
     *         result for each pass as a JSON array.
     **/
     ...
     *        the type of value
     *
     * @return a list with the new sizes of each modified array (in order) or {@code null} (instead of the size) if the
     *         point object was not an array.
     **/
     ...
     *        the type of value
     *
     * @return a list with the first position in the array of each JSON value that matches the path, {@code -1} if not
     *         found in the array, or {@code null} if the matching JSON value is not an array.
     **/
     ...
     *        the type of value
     *
     * @return a list of integer containing for each path, the array's new size or {@code null} if the matching JSON
     ...
     *
     * @return a list of integer containing for each path, the array's length, or {@code null} if the matching JSON
     *         value is not an array.
     **/
     ...
     *
     * @return a list of T including for each path, an instance of T rebuilt from the JSON value, or {@code null} if the
     *         matching JSON value is not an array. Popping an empty array produces {@code null}.
     **/
     ...
     *
     * @return a list containing, for each matching path, the list of keys, or {@code null} if the matching JSON value
     *         is not an object.
     **/
     ...
     *        path the path, path defaults to {@code $} if not provided.
     *
     * @return a list containing, for each path, the length of the object, {@code null} if the matching JSON value is
     *         not an object
     **/
     ...
     *
     * @return a list containing, for each path, the new string length, {@code null} if the matching JSON value is not a
     *         string.
     **/
     ...
     *        path the path, path defaults to {@code $} if not provided.
     *
     * @return a list containing, for each path, the length of the string, {@code null} if the matching JSON value is
     *         not a string. Returns {@code null} if the key or path do not exist.
     **/
     ...
     *        path the path, must not be {@code null}
     *
     * @return a list containing, for each path, the new boolean value, {@code null} if the matching JSON value is not a
     *         boolean.
     **/
     ...
     *        path the path, path defaults to {@code $} if not provided.
     *
     * @return a list containing, for each path, the json type as String (string, integer, number, boolean, object,
     *         array), empty if no match.
     **/
