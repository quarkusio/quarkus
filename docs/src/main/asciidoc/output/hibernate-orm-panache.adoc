...
Hibernate ORM with Panache focuses on making your entities trivial and fun to write in {product-name-first}.
...
{product-name} will generate them for you. You can even extend `PanacheEntity` and take advantage of the default ID it provides.
...
To make it easier to showcase some capabilities of Hibernate ORM with Panache on {product-name} with Dev mode, some test data should be inserted into the database by adding the following content to a new file named src/main/resources/import.sql:
...
NOTE: If you would like to initialize the DB when you start the {product-name} app in your production environment, add `quarkus.hibernate-orm.database.generation=drop-and-create` to the {product-name} startup options in addition to `import.sql`.
...
This is enabled by default if you are using the {product-name} Maven archetype. If you are not using it, add the property `<maven.compiler.parameters>true</maven.compiler.parameters>` to your `pom.xml`.
====
