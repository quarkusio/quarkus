...
{product-name-first} provides first class support for using Flyway as will be explained in this guide.
...
Flyway support relies on the {product-name} datasource config.
...
Now you can start your application and {product-name} will run the Flyway's
migrate method according to your config.

NOTE: With `quarkus.flyway.migrate-at-start=true`, as in the example
above, {product-name} will execute the Flyway migration as part of the
...
TIP: In dev mode {product-name} will automatically restart the application if
...
In cases where Flyway needs to be configured in addition to the configuration options that {product-name} provides, the `io.quarkus.flyway.FlywayConfigurationCustomizer` class comes in handy.
...
<1> Inject the Flyway object if you want to use it directly
<2> Inject Flyway for named datasources using the {product-name} `FlywayDataSource` qualifier
