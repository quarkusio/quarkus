...
public class HibernateOrmDevControllerTestCase {
    ...
    final static QuarkusDevModeTest TEST = new QuarkusDevModeTest().withApplicationRoot((jar) -> jar
            .addClasses(MyEntityWithSuccessfulDDLGeneration.class, HibernateOrmDevInfoServiceTestResource.class)
            .addAsResource("application.properties").addAsResource("import-custom-table-name.sql", "import.sql"));
    ...
    public void infoAvailable() {
        RestAssured.given().param("expectedCreateDDLContent", "EXCEPTION").param("expectedDropDDLContent", "EXCEPTION")
                .when().get("/dev-info/check-pu-info-with-successful-ddl-generation").then().body(is("OK"));
    }
