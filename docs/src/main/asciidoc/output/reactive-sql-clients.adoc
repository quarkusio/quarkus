...
:categories: data
:summary: This guide covers how to use the Reactive SQL Clients in {product-name-first}.
...
IMPORTANT: If you are not familiar with the {product-name} Vert.x extension, consider reading the xref:vertx.adoc[Using Eclipse Vert.x] guide first.
...
If you have an already created project, the `reactive-pg-client` extension can be added to an existing {product-name} project with the `add-extension` command:
...
The Reactive PostgreSQL Client can be configured with standard {product-name} datasource properties and a reactive URL:
...
This code is part of a `@PostConstruct` method and {product-name} invokes it synchronously.
...
Now start {product-name} in dev mode with:
...
<head>
    <meta charset="utf-8"/>
    <title>Reactive PostgreSQL Client - {product-name}</title>
