...
public interface CuckooCommands<K, V> extends RedisCommands {
     ...
     *        the values, must not be {@code null}, must not be empty, must not contain {@code null}
     *
     * @return a list of boolean. For each added value, the corresponding boolean is {@code true} if the value has been
     *         added (non-existing) or {@code false} if the value was already present in the filter.
     */
     ...
     *
     * @return a list of boolean indicating, for each corresponding value, if the value exists in the filter or not.
     */
