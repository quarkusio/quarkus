...
public class FruitService {
    ...
    public void indexBulk(InputStream is) throws IOException { //<5>
        Request request = new Request("POST", "fruits/_bulk?pretty");
        request.setEntity(new ByteArrayEntity(
            is.readAllBytes(), ContentType.create("application/x-ndjson")));
    ...
    public Fruit get(String id) throws IOException {
        ...
        JsonObject json = new JsonObject(responseBody); //<6>
...
<5> We demonstrate `Bulk API` usage via low level `RestClient`. Method operates with `InputStream`, because file provided by call is not valid json.
Read https://www.elastic.co/guide/en/elasticsearch/reference/current/docs-bulk.html#docs-bulk-api-desc[description] to get how input file should be structured.
<6> In order to deserialize the object from Elasticsearch, we again use Vert.x `JsonObject`.
...
public class FruitResource {
    ...
    @ResponseStatus(200)
    public void indexBulk(InputStream is) throws IOException {
        fruitService.indexBulk(is);
    }
