...
{product-name-first} provides the following extensions to set up REST Data with Panache.
...
* `rolesAllowed` - List of the security roles permitted to access the resources. It needs a {product-name} Security extension to be present, otherwise it will be ignored. Default is empty.
...
* `rolesAllowed` - List of the security roles permitted to access this operation. It needs a {product-name} Security extension to be present, otherwise it will be ignored. Default is empty.
...
Thus, the REST Data with Panache interfaces can be annotated with profile conditions (`@io.quarkus.arc.profile.IfBuildProfile` or `@io.quarkus.arc.profile.UnlessBuildProfile`) and/or with property conditions (`io.quarkus.arc.properties.IfBuildProperty` or `io.quarkus.arc.properties.UnlessBuildProperty`) to indicate to {product-name} at build time under which conditions the generated Jakarta REST classes should be included.

In the following example, {product-name} will include the generated resource from the `PeopleResource` interface if and only if the build profile `app1` has been enabled.
...
IMPORTANT: This option is only available when using the RESTEasy Reactive {product-name} extension.

{product-name} can also conditionally disable the generated Jakarta REST Resources based on the value of runtime properties using the `@io.quarkus.resteasy.reactive.server.EndpointDisabled` annotation.

In the following example, {product-name} will exclude the generated resource from the `PeopleResource` interface at runtime if the application has `some.property` configured to `"disable"`.
