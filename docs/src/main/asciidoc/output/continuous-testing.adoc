...
Learn how to use continuous testing in your {product-name-first} Application.
...
{product-name} supports continuous testing, where tests run immediately after code changes have been saved. This allows you to
get instant feedback on your code changes. {product-name} detects which tests cover which code, and uses this information to
...
{product-name} will start in development mode as normal, but down the bottom of the screen you should see the following:
...
return `"hello world"`, and save the file. {product-name} should immediately re-run the test, and you should get output similar
to the following:
