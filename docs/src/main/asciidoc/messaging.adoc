...
This activates a new request context when an `@Incoming` method is invoked, and closes the context once the message is processed (including when a returned `Uni` or `CompletionStage` completes).
