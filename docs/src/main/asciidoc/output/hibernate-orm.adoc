...
:categories: data
:summary: Hibernate ORM is the de facto Jakarta Persistence implementation and offers you the full breath of an Object Relational Mapper. It works beautifully in {product-name-first}.
...
It works beautifully in {product-name}.
...
When using Hibernate ORM in {product-name}, you don't need to have a `persistence.xml` resource to configure it.
...
In {product-name}, you only need to:
...
Other configuration needs have been automated: {product-name} will make some opinionated choices and educated guesses.
...
Also, {product-name} will set many Hibernate ORM configuration settings automatically, and will often use more modern defaults.
...
An `EntityManagerFactory` will be created based on the {product-name} `datasource` configuration as long as the Hibernate ORM extension is listed among your project dependencies.
...
{product-name} will try to check this version against the actual database version on startup,
...
If xref:datasource.adoc#other-databases[your database does not have a corresponding {product-name} extension],
...
There are various optional properties useful to refine your `EntityManagerFactory` or guide guesses of {product-name}.

There are no required properties, as long as a default datasource is configured.

When no property is set, {product-name} can typically infer everything it needs to set up Hibernate ORM
...
{product-name} will raise an exception.
...
It is possible to define multiple persistence units using the {product-name} configuration properties.
...
If you are familiar with using Hibernate ORM in {product-name}, you probably already have injected the `EntityManager` using CDI:
...
Please remember these are not the same property names as the ones used in the {product-name} `{config-file}`, nor will
the same defaults be applied.

[[xml-mapping]]
== XML mapping

Hibernate ORM in {product-name} supports XML mapping.
...
Hibernate ORM in {product-name} relies on compile-time bytecode enhancements to your entities. If you define your entities in the
same project where you build your {product-name} application, everything will work fine.

If the entities come from external projects
or jars, you can make sure that your jar is treated like a {product-name} application library by adding an empty `META-INF/beans.xml` file.

This will allow {product-name} to index and enhance your entities as if they were inside the current project.

[[dev-mode]]
== Hibernate ORM in development mode

{product-name} development mode is really useful for applications that mix front end or services and database access.
...
This is best to perfectly control your environment and works magic with {product-name} live reload mode:
...
These approaches become really powerful when combined with {product-name} configuration profiles.
...
{product-name} comes with default profiles (`dev`, `test` and `prod`).
And you can add your own custom profiles to describe various environments (`staging`, `prod-us`, etc).

The Hibernate ORM {product-name} extension sets some default configurations differently in dev and test modes than in other environments.
...
If you have the xref:flyway.adoc[Flyway extension] installed when running in development mode, {product-name} provides a simple way to turn
...
That's all! Caching technology is already integrated and enabled by default in {product-name}, so it's enough to set which ones are safe to be cached.
...
The caching technology provided within {product-name} is currently quite rudimentary and limited.
...
Rather than enabling caching on mutable data, ideally a better solution would be to use a clustered cache; however at this time {product-name} doesn't provide any such implementation: feel free to get in touch and let this need known so that the team can take this into account.
====
...
In {product-name}, Envers has a dedicated {product-name} Extension `io.quarkus:quarkus-hibernate-envers`; you just need to add this to your project to start using it.
...
{product-name} does not modify the libraries it uses; this rule applies to Hibernate ORM as well: when using
this extension you will mostly have the same experience as using the original library.

But while they share the same code, {product-name} does configure some components automatically and injects custom implementations
...
Typically, you would need to adapt your build scripts to include the Hibernate Enhancement plugins; in {product-name} this is
not necessary as the enhancement step is integrated in the build and analysis of the {product-name} application.
...
You don't need to set this up, {product-name} automatically injects the reference to the Narayana Transaction Manager.
The dependency is included automatically as a transitive dependency of the Hibernate ORM extension.
All configuration is optional; for more details see xref:transaction.adoc[Using Transactions in {product-name}].

Connection pool::
Don't need to choose one either. {product-name} automatically includes the Agroal connection pool;
configure your datasource as in the above examples and it will set up Hibernate ORM to use Agroal.
More details about this connection pool can be found in xref:datasource.adoc[{product-name} - Datasources].
...
Since {product-name} provides out-of-the-box CDI support, injection or programmatic CDI lookup is a better approach.
...
But in {product-name}, this use case doesn't apply as components are injected directly, making JNDI support an unnecessary legacy.
To avoid unexpected use of JNDI, full support for JNDI has been disabled in the Hibernate ORM extension for {product-name}.
This is both a security precaution and an optimization.

=== Other notable differences

Format of `import.sql`::
When importing a `import.sql` to set up your database, keep in mind that {product-name} reconfigures Hibernate ORM so to require a semicolon (`;`) to terminate each statement.
...
The xref:hibernate-orm-panache.adoc[Hibernate ORM with Panache] extension facilitates the usage of Hibernate ORM by providing active record style entities (and repositories) and focuses on making your entities trivial and fun to write in {product-name}.

== Configure your datasource

Datasource configuration is extremely simple, but is covered in a different guide as technically
it's implemented by the Agroal connection pool extension for {product-name}.

Jump over to xref:datasource.adoc[{product-name} - Datasources] for all details.
...
{product-name} currently supports the link:{hibernate-orm-docs-url}#multitenacy-separate-database[separate database] approach, the link:{hibernate-orm-docs-url}#multitenacy-separate-schema[separate schema] approach and the link:{hibernate-orm-docs-url}#multitenacy-discriminator[discriminator] approach.
...
to tell {product-name} it should be used in the default persistence unit.
+
...
will replace the current {product-name} default implementation `io.quarkus.hibernate.orm.runtime.tenant.DataSourceTenantConnectionResolver`.
