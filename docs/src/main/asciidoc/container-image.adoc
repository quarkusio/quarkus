...
The extension `quarkus-container-image-buildpack` is using the buildpacks in order to perfrom container image builds. 
Under the hood buildpacks will use a docker daemon for the actual build. While buildpacks support alternatives to docker, this extension will only work with docker.
...
NOTE: See xref:config.adoc#combine-property-env-var[this] for more information on how to combine properties with environment variables.
