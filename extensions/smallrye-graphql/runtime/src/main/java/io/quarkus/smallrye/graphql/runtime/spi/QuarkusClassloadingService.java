...
public class QuarkusClassloadingService implements ClassloadingService {
    ...
    /*
     * <em>Ugly Hack</em> In Quarkus dev mode, we receive a classloader to use, when doing hot reload This hack is
     * required because using the TCCL would get an outdated version - the initial one. This is because the worker
     * thread on which the handler is called captures the TCCL at creation time and does not allow updating it. In non
     * dev mode, the TCCL is used. TODO: remove this once the vert.x class loader issues are resolved.
     */
