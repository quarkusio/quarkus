...
:categories: cloud
:summary: Deploy a {product-name-first} application to the Microsoft Azure cloud platform.

This guide covers:

* Update {product-name} HTTP Port
...
While {product-name} by default runs on port 8080, most Azure services expect web applications to be running on port 80. Before we continue, go back to your quickstart code and open the file `src/main/docker/Dockerfile.native`.
...
Your container image is now ready to run on Azure. Remember, the {product-name} application is mapped to run on port 80.
...
At this point, you should have your {product-name} container image on your Azure Container Registry. To verify, run the following command:
...
The command above, if run successfully, will give you the address of your container in the Cloud. Access your {product-name} application in the address displayed as output.

For more information and details on ACR authentication and the use of service principals, follow this guide below and remember the Azure Container Registry `loginServer` and the image name of your {product-name} application now hosted on the ACR.
...
Once deployed, the application will be running on whatever port is used to expose the service. By default, {product-name} apps run on port 8080 internally.
...
To deploy your {product-name} Native container image to this service, follow this tutorial:
