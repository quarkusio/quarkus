...
/**
 * Security interceptors run for desired annotated methods prior to security checks. Interceptors should only be run
 * when proactive security is disabled, because they must be run before request is authenticated.
 * <p>
 * These interceptors are very important when {@link io.quarkus.security.spi.runtime.SecurityCheck}s are not run with
 * CDI interceptors, because they provide only way to link request to invoked method prior to checks.
 */
public class EagerSecurityInterceptorStorage {
    ...
    EagerSecurityInterceptorStorage(Map<MethodDescription, Consumer<RoutingContext>> methodToInterceptor,
    ...
    /**
     * This method should be invoked prior to any security check is run if proactive auth is disabled. Class-level
     * security interceptors are applied when security is applied once per class, for example per HTTP upgrade.
     *
     * @param className
     *        with security annotation
     *
