...
public class ConfigTemplateExtensionsTest {
    ...
    static final QuarkusUnitTest config = new QuarkusUnitTest()
            .withApplicationRoot((jar) -> jar.addAsResource(new StringAsset("{config:foo}={config:property('foo')}\n"
                    ...
                    + "{config:integer('foo.bar.baz')}"), "templates/foo.html").addAsResource(
                            new StringAsset("foo=false\nfoo.bar.baz=11\nfoo.bool=true"), "application.properties"));
