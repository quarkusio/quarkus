...
/**
 * Provides the runtime methods to bootstrap Undertow. This class is present in the final uber-jar, and is invoked from
 * generated bytecode
 */
@Recorder
public class UndertowDeploymentRecorder {
    ...
    static {
        ...
        if (maxMemory < 64 * 1024 * 1024) {
            ...
            DEFAULT_DIRECT_BUFFERS = true;
            DEFAULT_BUFFER_SIZE = 1024 * 16 - 20; // the 20 is to allow some space for protocol headers, see
                                                  // UNDERTOW-1209
        }
    ...
    public RuntimeValue<DeploymentInfo> createDeployment(String name, Set<String> knownFiles,
            Set<String> knownDirectories, LaunchMode launchMode, ShutdownContext context, String mountPoint,
            String defaultCharset, String requestCharacterEncoding, String responseCharacterEncoding,
            boolean proactiveAuth, List<String> welcomeFiles, final boolean hasSecurityCapability) {
        ...
        d.addServlet(
                new ServletInfo(ServletPathMatches.DEFAULT_SERVLET_NAME, DefaultServlet.class).setAsyncSupported(true));
        for (HandlerWrapper i : hotDeploymentWrappers) {
            d.addOuterHandlerChainWrapper(i);
        }
        d.addAuthenticationMechanism("QUARKUS",
                new ImmediateAuthenticationMechanismFactory(QuarkusAuthMechanism.INSTANCE));
        ...
        d.addNotificationReceiver(new NotificationReceiver() {
            ...
            public void handleNotification(SecurityNotification notification) {
                if (notification.getEventType() == SecurityNotification.EventType.AUTHENTICATED) {
                    ...
                    Instance<CurrentIdentityAssociation> instance = CDI.current()
    ...
    public RuntimeValue<ServletInfo> registerServlet(RuntimeValue<DeploymentInfo> deploymentInfo, String name,
            Class<?> servletClass, boolean asyncSupported, int loadOnStartup, BeanContainer beanContainer,
        ...
        ServletInfo servletInfo = new ServletInfo(name, (Class<? extends Servlet>) servletClass, factory);
    ...
    public RuntimeValue<FilterInfo> registerFilter(RuntimeValue<DeploymentInfo> info, String name, Class<?> filterClass,
            boolean asyncSupported, BeanContainer beanContainer, InstanceFactory<? extends Filter> instanceFactory)
            throws Exception {
    ...
    public Handler<RoutingContext> startUndertow(ShutdownContext shutdown, ExecutorService executorService,
            DeploymentManager manager, List<HandlerWrapper> wrappers, ServletRuntimeConfig servletRuntimeConfig,
        ...
        UndertowBufferAllocator allocator = new UndertowBufferAllocator(
                servletRuntimeConfig.directBuffers().orElse(DEFAULT_DIRECT_BUFFERS), (int) servletRuntimeConfig
                        .bufferSize().orElse(new MemorySize(BigInteger.valueOf(DEFAULT_BUFFER_SIZE))).asLongValue());
        ...
        return new Handler<RoutingContext>() {
            ...
            public void handle(RoutingContext event) {
                ...
                // Note that we can't add an end handler in a separate HttpCompressionHandler because VertxHttpExchange
                // does set
                    ...
                    public T call(HttpServerExchange exchange, C context) throws Exception {
                        ...
                        if (requestContext.isActive()) {
                            if (currentVertxRequest.getCurrent() == null && exchange != null
                                ...
                                // invoked for the SecurityIdentityAugmentor that was (re)created during permission
                                // checks
    ...
    public void addServletContainerInitializer(RuntimeValue<DeploymentInfo> deployment,
        ...
        deployment.getValue()
                .addServletContainerInitializer(new ServletContainerInitializerInfo(sciClass, handlesTypes));
    }
    ...
    public void addSecurityConstraint(RuntimeValue<DeploymentInfo> deployment,
            SecurityInfo.EmptyRoleSemantic emptyRoleSemantic, TransportGuaranteeType transportGuaranteeType,
            Set<String> rolesAllowed, Set<WebResourceCollection> webResourceCollections) {

        SecurityConstraint securityConstraint = new SecurityConstraint().setEmptyRoleSemantic(emptyRoleSemantic)
                .addRolesAllowed(rolesAllowed).setTransportGuaranteeType(transportGuaranteeType)
    ...
    public void setSessionCookieConfig(ServletSessionConfig config, String name, String path, String comment,
            String domain, Boolean httpOnly, Integer maxAge, Boolean secure) {
        if (name != null) {
            config.setName(name);
        }
        if (path != null) {
    ...
    private static class QuarkusSessionIdGenerator implements SessionIdGenerator {
        ...
        /**
         * Encode the bytes into a String with a slightly modified Base64-algorithm This code was written by Kevin
         * Kelley <kelley@ruralnet.net> and adapted by Thomas Peuss <jboss@peuss.de>
         *
         * @param data
