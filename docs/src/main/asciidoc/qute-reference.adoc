...
By default, keys are _mapped to method names_ from the corresponding message bundle interface.
Set the `quarkus.qute.localized-file-keys` configuration property to `message-key` if the keys should be the _message keys_ used in templates instead, i.e. the keys derived from the strategy defined by `@MessageBundle#defaultKey()` and `@Message#key()`.
For example, if the strategy is `Message.UNDERSCORED_ELEMENT_NAME` then the method `helloName()` is mapped to the key `helloName` by default, and to `hello_name` if message keys are used.
Note that the keys of enum constant messages, such as `myEnum_ON`, are always derived from the method name.
...
----
<1> Each line in a localized file represents a key/value pair. The key must correspond to a method declared on the message bundle interface, either by its name or, if `quarkus.qute.localized-file-keys=message-key` is set, by its message key. The value is the message template.
