...
public class WrongAnnotationUsageProcessor {
    ...
    void detect(ArcConfig config, ApplicationIndexBuildItem applicationIndex,
            CustomScopeAnnotationsBuildItem scopeAnnotations, TransformedAnnotationsBuildItem transformedAnnotations,
        ...
        unsupported
                .add(new UnsupportedAnnotation("javax.annotation.PostConstruct", "@jakarta.annotation.PostConstruct"));
        ...
        for (UnsupportedAnnotation annotation : unsupported) {
            for (AnnotationInstance annotationInstance : index.getAnnotations(annotation.name)) {
                wrongUsages.put(annotationInstance,
                        String.format("%s declared on %s, use %s instead", annotationInstance.toString(false),
                                getTargetInfo(annotationInstance), annotation.correctAnnotation));
            }
        }
        ...
        // Note that a scope annotation is typically defined as @Target({TYPE, METHOD, FIELD}) but it's not forbidden to
        // use ElementType.TYPE_USE
        for (ClassInfo clazz : index.getKnownClasses()) {
            ...
            if (NestingType.ANONYMOUS == nestingType || NestingType.LOCAL == nestingType
                ...
                if (scopeAnnotations.isScopeIn(classLevelAnnotations)) {
                    validationErrors.produce(new ValidationErrorBuildItem(new IllegalStateException(String.format(
                ...
                for (MethodInfo methodInfo : clazz.methods()) {
                    ...
                    Collection<AnnotationInstance> methodAnnotations = transformedAnnotations
                            .getAnnotations(methodInfo);
