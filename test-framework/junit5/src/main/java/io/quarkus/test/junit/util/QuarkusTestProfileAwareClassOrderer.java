...
/**
 * A {@link ClassOrderer} that orders {@link QuarkusTest}, {@link QuarkusIntegrationTest} and {@link QuarkusMainTest}
 * classes for minimum Quarkus restarts by grouping them by their {@link TestProfile}, {@link WithTestResource}, and
 * {@link QuarkusTestResource} annotation(s).
 * <p/>
 * By default, Quarkus*Tests not using any profile come first, then classes using a profile (in groups) and then all
 * other non-Quarkus tests (e.g. plain unit tests).<br/>
 * Quarkus*Tests with {@linkplain WithTestResource#scope() matching resources} or
 * {@linkplain QuarkusTestResource#restrictToAnnotatedClass() restricted} {@code QuarkusTestResource} come after tests
 * with profiles and Quarkus*Tests with only unrestricted resources are handled like tests without a profile (come
 * first).
 * <p/>
 * Internally, ordering is based on prefixes that are prepended to a secondary order suffix (by default the fully
 * qualified name of the respective test class), with the fully qualified class name of the
 * {@link io.quarkus.test.junit.QuarkusTestProfile QuarkusTestProfile} as an infix (if present). The default prefixes
 * are defined by {@code DEFAULT_ORDER_PREFIX_*} and can be overridden in {@code application.properties} via
 * {@code CFGKEY_ORDER_PREFIX_*}, e.g. non-Quarkus tests can be run first (not last) by setting
 ...
 * {@link org.junit.jupiter.api.ClassOrderer.Random org.junit.jupiter.api.ClassOrderer$Random} will order the test
 * classes within one group randomly instead by class name.
 * <p/>
 * {@link #getCustomOrderKey(ClassDescriptor, ClassOrdererContext)} can be overridden to provide a custom order number
 * for a given test class, e.g. based on {@link org.junit.jupiter.api.Tag} or something else.
 * <p/>
 * Limitations:
 * <ul>
...
public class QuarkusTestProfileAwareClassOrderer implements ClassOrderer {
    ...
    public QuarkusTestProfileAwareClassOrderer() {
        ...
        this.prefixQuarkusTestWithProfile = config
                .getOptionalValue(CFGKEY_ORDER_PREFIX_QUARKUS_TEST_WITH_PROFILE, String.class)
    ...
    QuarkusTestProfileAwareClassOrderer(final String prefixQuarkusTest,
            final String prefixQuarkusTestWithMatchingResource, final String prefixQuarkusTestWithProfile,
            final String prefixQuarkusTestWithRestrictedResource, final String prefixNonQuarkusTest,
    ...
    public void orderClasses(ClassOrdererContext context) {
        ...
        if (context.getClassDescriptors().size() <= 1
                || context.getClassDescriptors().get(0).isAnnotated(Nested.class)) {
            return;
        }

        // In many cases (like QuarkusTest), the heavy lifting of understanding profiles and resources has been done
        // elsewhere; we just need to group tests by classloader
        // However, for integration tests and main tests, profiles will not have been read
        long classloaderCount = context.getClassDescriptors().stream().map(d -> d.getTestClass().getClassLoader())
                .distinct().count();

        // TODO this check probably isn't enough, because if there's a mix of QuarkusMain and other tests, we will have
        // more than one classloader, but the others will still need sorting
        // TODO a safer check will be to assume anything loaded with a QuarkusClassLoader is pre-sorted, but to sort
        // anything else.
        // TODO So sort by classloader, and then sort the pile which is in the system (or whatever) classloader
        if (classloaderCount > 1) {

            // If we sort first before applying the classloader sorting, the original order will be preserved within
            // classloader groups
            secondaryOrderer.map(fqcn -> {
                try {
            ...
            context.getClassDescriptors().sort(
                    Comparator.<ClassDescriptor, String> comparing(o -> o.getTestClass().getClassLoader().getName()));
    ...
    private void orderByProfiles(ClassOrdererContext context) {
        ...
        // second pass: apply the actual Quarkus aware ordering logic, using the first pass indices as order key
        // suffixes
        classDescriptors.sort(Comparator.comparing(classDescriptor -> {
            ...
            if (classDescriptor.isAnnotated(QuarkusTest.class)
                ...
                return classDescriptor.findAnnotation(TestProfile.class).map(TestProfile::value)
                        .map(profileClass -> prefixQuarkusTestWithProfile + profileClass.getName() + "@"
                                + secondaryOrderSuffix)
                        .orElseGet(() -> {
                            // TODO it should be possible to re-use the resource key here for (a) less code and (b)
    ...
    private boolean isMetaTestResource(QuarkusTestResource resource, ClassDescriptor classDescriptor) {
        return Arrays.stream(classDescriptor.getTestClass().getAnnotationsByType(QuarkusTestResource.class))
                .map(QuarkusTestResource::value).noneMatch(resource.value()::equals);
    }
    ...
    private boolean isMetaTestResource(WithTestResource resource, ClassDescriptor classDescriptor) {
        return Arrays.stream(classDescriptor.getTestClass().getAnnotationsByType(WithTestResource.class))
                .map(WithTestResource::value).noneMatch(resource.value()::equals);
    }
     ...
     * @param context
     *        for config lookup
     * @param secondaryOrderSuffix
     *        the secondary order suffix that was calculated by the secondary orderer
     *
     * @return optional custom order key for the given test class
     */
    protected Optional<String> getCustomOrderKey(ClassDescriptor classDescriptor, ClassOrdererContext context,
