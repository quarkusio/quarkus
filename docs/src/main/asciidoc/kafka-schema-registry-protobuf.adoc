...
////
= Using Apache Kafka with Schema Registry and Protobuf
include::_attributes.adoc[]
:categories: messaging
:summary: Use Apache Kafka, Protobuf serialized records, and connect to a schema registry.
:topics: messaging,kafka,apicurio,registry,protobuf
:extensions: io.quarkus:quarkus-apicurio-registry-protobuf,io.quarkus:quarkus-messaging-kafka

This guide shows how your Quarkus application can use Apache Kafka, https://protobuf.dev/[Protocol Buffers (Protobuf)] serialized
...
The _Movies_ will be serialized and deserialized using Protobuf.
The schema, describing the _Movie_, is stored in Apicurio Registry.
The same concept applies if you are using the Confluent Protobuf _serde_ and Confluent Schema Registry.
...
The solution is located in the `kafka-protobuf-schema-quickstart` link:{quickstarts-tree-url}/kafka-protobuf-schema-quickstart[directory].

== Creating the Maven Project

First, we need a new project. Create a new project with the following command:

:create-app-artifact-id: kafka-protobuf-schema-quickstart
:create-app-extensions: rest-jackson,messaging-kafka,apicurio-registry-protobuf
include::{includes}/devtools/create-app.adoc[]

[TIP]
====
If you use Confluent Schema Registry, you don't need the `quarkus-apicurio-registry-protobuf` extension.
Instead, you need the `quarkus-confluent-registry-protobuf` extension and a few more dependencies.
See <<confluent>> for details.
====

== Protobuf schema

https://protobuf.dev/[Protocol Buffers (Protobuf)] is a language-neutral, platform-neutral data serialization mechanism developed by Google.
Data structures are described using `.proto` schema files, which are compiled into language-specific classes.

The first thing we need to do is to create a schema describing the `Movie` structure.
Create a file called `src/main/proto/movie.proto` with the schema for our record (Kafka message):
...
option java_package = "org.acme.kafka.quarkus";
...
To compile `.proto` files into Java classes, you need to add the `protobuf-maven-plugin` and the `os-maven-plugin` extension to your `pom.xml`:
...
the `movie.proto` will get compiled to Java classes
placed in the `target/generated-sources/protobuf/java` directory.

Take a look at the https://protobuf.dev/programming-guides/proto3/[Protobuf Language Guide] to learn more about
the Protobuf syntax and supported types.
...
import org.acme.kafka.quarkus.MovieOuterClass.Movie;
...
public class MovieResource {
    ...
    @POST
    public Response enqueueMovie(MovieDto movieDto) {
        LOGGER.infof("Sending movie %s to Kafka", movieDto.title);
...
Note that Protobuf messages are immutable and must be built using the generated builder pattern (`Movie.newBuilder()`).
We use a simple `MovieDto` POJO as the REST input:
...
Now, we need to _map_ the `movies` channel (the `Emitter` emits to this channel) to a Kafka topic and configure the Protobuf serializer.
...
mp.messaging.outgoing.movies.value.serializer=io.apicurio.registry.serde.protobuf.ProtobufKafkaSerializer
...
[TIP]
====
Unlike with Avro, Quarkus does not auto-detect Protobuf serializers, so you must configure the `value.serializer` explicitly.
You still need to configure the `apicurio.registry.auto-register` property.

If you use Confluent Schema Registry, you must define the `value.serializer` as `io.confluent.kafka.serializers.protobuf.KafkaProtobufSerializer`.
...
That data is serialized using Protobuf.
...
public class ConsumedMovieResource {
    ...
    public Multi<String> stream() {
        return movies.map(movie -> {
            Descriptors.FieldDescriptor titleField = movie.getDescriptorForType().findFieldByName("title");
            Descriptors.FieldDescriptor yearField = movie.getDescriptorForType().findFieldByName("year");
            return String.format("'%s' from %s", movie.getField(titleField), movie.getField(yearField));
        });
    }
}
----

NOTE: The Protobuf deserializer may return a `DynamicMessage` instead of the specific generated class.
Therefore, we use the generic `Message` interface and extract fields by name using the Protobuf descriptor API.
...
mp.messaging.incoming.movies-from-kafka.value.deserializer=io.apicurio.registry.serde.protobuf.ProtobufKafkaDeserializer
...
[TIP]
====
Unlike with Avro, you must explicitly define the `value.deserializer` for Protobuf.

If you use Confluent Schema Registry, you must configure `value.deserializer` as `io.confluent.kafka.serializers.protobuf.KafkaProtobufDeserializer`.
====
...
[source,bash]
----
./target/kafka-protobuf-schema-quickstart-1.0.0-SNAPSHOT-runner -Dkafka.bootstrap.servers=localhost:9092
----
...
public class MovieResourceTest {
    ...
    @Test
    public void testProtobufProducerConsumer() throws InterruptedException {
...
If you want to use the Confluent Schema Registry, you need the `quarkus-confluent-registry-protobuf` extension, instead of the `quarkus-apicurio-registry-protobuf` extension.
...
<dependencies>
    ...
    <dependency>
        <groupId>io.quarkus</groupId>
        <artifactId>quarkus-confluent-registry-protobuf</artifactId>
    </dependency>
...
<repositories>
    <!-- io.confluent:kafka-protobuf-serializer is only available from this repository: -->
    <repository>
        <id>confluent</id>
...
dependencies {
    ...
    implementation("io.quarkus:quarkus-confluent-registry-protobuf")
...
In JVM mode, any version of `io.confluent:kafka-protobuf-serializer` can be used.
In native mode, Quarkus supports version `8.x`.
...
The serializer and deserializer classes to use are:
...
* xref:kafka-schema-registry-json-schema.adoc[Using Apache Kafka with Schema Registry and JSON Schema]
