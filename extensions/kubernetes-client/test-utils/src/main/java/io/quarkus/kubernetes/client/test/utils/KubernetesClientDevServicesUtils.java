...
public final class KubernetesClientDevServicesUtils {
    ...
    public static Config createConfig(DevServicesContext context) {
        ...
        String masterUrl = props.get(KUBERNETES_CLIENT_MASTER_URL);
        if (masterUrl == null) {
            throw new ConfigurationException(
                    "Unable to obtain the Kubernetes Client Dev Services master URL; is `"
                            + KUBERNETES_CLIENT_MASTER_URL
                            + "` set? Make sure the Kubernetes Client Dev Services are enabled for this test.");
        }
