...
class TestAnalogs {
    ...
    fun testPanacheEntityBase() {
        ...
        val companionMethods =
            map(PanacheCompanionBase::class, ByteCodeType(PanacheEntityBase::class.java)).methods
    ...
    private fun methods(label: String, methods: List<Method>) {
        println("$label: ")
        methods.toSortedSet(compareBy { it.toString() }).forEach { println(it) }
    }
}
...
class AnalogVisitor(val erasedType: ByteCodeType? = null) : ClassVisitor(Gizmo.ASM_API_VERSION) {
    ...
    override fun visitMethod(
        ...
        if (name != "<init>") {
            ...
            var parameters =
                descriptor.substring(descriptor.indexOf("("), descriptor.lastIndexOf(")") + 1)
            erasedType?.let { type ->
