...
:summary: While you are encouraged to use the Cache extension for your application-level caching, {product-name-first} provides a compatibility layer for Spring Cache in the form of the spring-cache extension.

While users are encouraged to use xref:cache.adoc[{product-name} annotations for caching], {product-name} nevertheless provides a compatibility layer for Spring Cache annotations in the form of the `spring-cache` extension.

This guide explains how a {product-name} application can leverage the well known Spring Cache annotations to enable application data caching for their Spring beans.
...
If you already have your {product-name} project configured, you can add the `spring-cache` extension
...
{product-name} provides compatibility with the following Spring Cache annotations:
