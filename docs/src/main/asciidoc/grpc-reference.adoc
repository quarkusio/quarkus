...
== Disabling the gRPC server

If your application only uses gRPC clients (for example, a command-line application calling a remote gRPC service), you can disable the gRPC server entirely by setting the following property in your `application.properties` file:
...
When disabled, no gRPC server is started and no gRPC server-related beans or health checks are registered, even if the application contains `@GrpcService` beans.

In dev mode, the gRPC server is started even if the application does not implement any gRPC service, to ease incremental development of new services.
...
The `quarkus-grpc` extension depends on the Quarkus HTTP server (the gRPC server reuses it when `quarkus.grpc.server.use-separate-server` is set to `false`).
As a consequence, an HTTP server is started even for client-only applications.
If you don't need the HTTP server either, prevent it from listening with:
