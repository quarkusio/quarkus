...
The solution is located in the `using-oauth2-rbac` {quickstarts-archive-url}[directory].
...
We will use the JSR 250 common security annotations, they are described in the link:security-guide.html[Using Security] guide. 
...
=== Setting up application.properties
 ...
 For part A of step 1, create a `using-oauth2-rbac/src/main/resources/application.properties` with the following content:
...
The solution repository located in the `using-oauth2-rbac` {quickstarts-archive-url}[directory] contains the examples we have worked through in this quickstart guide. 
We suggest that you check out the quickstart solutions and explore the `using-oauth2-rbac` directory to learn more about the {extension-name} extension features.
