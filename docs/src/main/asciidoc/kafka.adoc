...
`@ExactlyOnce` can be combined with database transactions to atomically persist data and produce Kafka messages.
The Kafka transaction wraps the database transaction, so if the Kafka transaction fails, the database transaction is rolled back too.
