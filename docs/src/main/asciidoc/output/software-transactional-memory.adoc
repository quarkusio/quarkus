...
Note that the STM implementation provided by {product-name-first} is based on the https://narayana.io/docs/project/index.html#d0e16066[Narayana STM] implementation. This document isn't meant to be a replacement for that project's documentation, so you may want
to look at it for more detail. However, we will try to focus more on how you can combine some key capabilities
into {product-name} when developing Kubernetes native applications and microservices.
...
a particular focus on why we think they're great for {product-name}, microservices and Kubernetes
...
state from concurrent updates. The {product-name} STM implementation will safely manage any conflicts between
these threads using whatever isolation model has been chosen to protect that specific state
instance (object in the case of {product-name}). In {product-name} STM, there are two isolation implementations,
...
* Another benefit of STM is composability and modularity. You can write concurrent {product-name} objects/services that
...
STM implementations, including the one {product-name} uses, support nested transactions and these allow changes made within
...
file system, which means you don't need to configure anything else with {product-name} such as a database.
...
can build STM-aware applications with {product-name}. However, before we do so there are a few basic concepts
which we need to cover.

Note, as you will see, STM in {product-name} relies on a number of annotations to define behaviours. The lack
...
The STM subsystem needs to be told about which objects it should be managing. The {product-name} (aka Narayana) STM implementation
...
be confusing to use especially in a Kubernetes native environment such as {product-name}, but hopefully
