...
== Native Memory Management
Memory management for {product-name-first} native executables is enabled by GraalVM’s SubstrateVM runtime system.
...
This guide complements the information available in the GraalVM website with further observations particularly relevant to {product-name} applications.

=== Garbage Collectors
The garbage collectors available for {product-name} users are currently Serial GC and Epsilon GC.

==== Serial GC
Serial GC, the default option in GraalVM and {product-name}, is a single-threaded non-concurrent GC, just like HotSpot’s Serial GC.
...
Up to version 2.13, {product-name} used the “space/time” GC collection policy by default,
but starting with version 2.14, it switched to using the “adaptive” policy instead.
The reason why {product-name} initially chose to use "space/time" is because at that time it had considerable performance improvements over "adaptive".
...
As a result, the "adaptive" policy appears to be the best option for most, if not all, {product-name} applications.
...
From a {product-name} perspective, some of the most relevant use cases for this garbage collector are extremely short-lived jobs, e.g. serverless functions.
To build {product-name} native with epsilon GC, pass the following argument at build time:
...
the footprint of {product-name} applications is measured using the resident set size (RSS).
...
to understand what causes {product-name} native executables to consume memory on startup.
...
As first step, build a {product-name} native executable with debug info enabled and a couple of extra flags.
...
Let's measure how much RSS a {product-name} native executable takes on startup on this particular environment:
...
How come this {product-name} native executable consumes ~35MB RSS on startup?
...
Next, run the {product-name} native executable through `perf record` tracing the `mmap` system call:
...
{product-name} creates 12 event loop threads.
...
[TIP]
====
Setting `set scheduler-locking step` when the {product-name} native executable is running through `gdb` makes it easier to debug multi-threaded applications.
...
given that {product-name} native executable runs 18 threads.
...
an approximate estimation can be made on the RSS a {product-name} native executable uses on startup.
...
To do this,
first re-generate a {product-name} native executable removing debug info,
...
Next invoke the bcc/eBPF script giving it the PID of the {product-name} process,
...
2023-02-09 18:02:32,794 INFO  [io.quarkus] (main) code-with-quarkus 1.0.0-SNAPSHOT native (powered by {product-name} 2.16.1.Final) started in 0.011s. Listening on: http://0.0.0.0:8080
...
This debugging guide provides further details on debugging issues in {product-name} native executables that might arise during development or production.
...
This guide builds and executes {product-name} native executables within a Linux environment.
...
Start by creating a new {product-name} project.
...
Some {product-name} configuration options will be used constantly throughout this debugging guide,
...
This option can be very useful in detecting whether the options that you pass at a high level via {product-name} are being passed down to the native executable production,
...
[NOTE]
====
Runtime containers produced by {product-name} to run native executables will not include the tools mentioned above.
...
The {product-name} application demonstrated here is very basic, so there’s not a lot we can explore, but here are some example queries you can run to explore the graph in more detail.
...
that explores the {product-name} Hibernate ORM quickstart using the techniques explained above.
...
{product-name} instructs Mandrel to initialize as much as possible at build time,
...
thus making {product-name} more reliable.
...
{product-name} instructs Mandrel to run them at build-time for the reasons given.
...
In this exercise, we profile the runtime behaviour of some {product-name} application that was compiled to a native executable to determine where the bottleneck is.
...
we are going to generate a native {product-name} application that crashes due to a Segmentation Fault when accessing http://localhost:8080/crash.
...
This is because we didn’t compile the {product-name} application with `-Dquarkus.native.debug.enabled`,
...
Recompiling the {product-name} application with `-Dquarkus.native.debug.enabled` and rerunning it through `gdb` we are now able to get a backtrace making clear what caused the crash.
...
#12 0x00000000008bc055 in io.quarkus.runtime.{product-name}::run(java.lang.Class *, java.util.function.BiConsumer *, java.lang.String[] *) () at io/quarkus/runtime/{product-name}.java:67
#13 0x000000000045c88b in io.quarkus.runtime.{product-name}::run () at io/quarkus/runtime/{product-name}.java:41
#14 io.quarkus.runtime.{product-name}::run () at io/quarkus/runtime/{product-name}.java:120
...
When building a native executable {product-name} requires all classes being referenced by the code, no matter if they are build-time or run-time initialized, to be present in the classpath.
...
For example, building a sample native {product-name} Jakarta Persistence application such as the Hibernate ORM quickstart,
...
To add JFR support to your {product-name} executable, add the application property: `-Dquarkus.native.monitoring=jfr`.
E.g.
