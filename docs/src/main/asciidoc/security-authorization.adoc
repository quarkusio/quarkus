...
=== Referring to Endpoint Context Path

If you use `quarkus.http.root-path` to xref:http-reference.adoc#context-path[Configure Endpoint Context Path] then it needs to be referred to as follows:

[source,properties]
----
quarkus.http.root-path=/root
quarkus.http.auth.permission.authenticated.paths=${quarkus.http.root-path}/service/*
----
