...
import io.quarkus.signals.spi.Signals;
...
public interface Signal<T> {
    ...
    /**
     * Creates a new {@code Signal} instance for the given signal type and qualifiers programmatically.
     * <p>
     * This factory method should only be used when CDI injection is not available, e.g. in a static method or a class that is
     * not a CDI bean. Prefer {@code @Inject Signal<T>} whenever possible.
     *
     * @param <T> the type of the signal object
     ...
     */
    static <T> Signal<T> create(Class<T> type, Annotation... qualifiers) {
        return Arc.requireContainer().instance(Signals.class).get().create(type, qualifiers);
    }
     ...
     *
     * @param <T> the type of the signal object
     * @param type a {@link TypeLiteral} representing the signal type
     * @param qualifiers the specified qualifiers
     * @return the {@code Signal}, never {@code null}
     */
    static <T> Signal<T> create(TypeLiteral<T> type, Annotation... qualifiers) {
        return Arc.requireContainer().instance(Signals.class).get().create(type, qualifiers);
    }
