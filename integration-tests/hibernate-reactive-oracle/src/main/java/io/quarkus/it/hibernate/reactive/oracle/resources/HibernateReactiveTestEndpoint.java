...
import io.quarkus.it.hibernate.reactive.oracle.model.FriesianCow;
...
public class HibernateReactiveTestEndpoint {
    ...
    private Uni<RowSet<Row>> populateDB() {
        return Uni.combine().all().unis(
                oraclePool.query("DELETE FROM Pig").execute(),
                oraclePool.query("DELETE FROM Cow").execute())
                .asTuple()
                .chain(() -> oraclePool.preparedQuery("INSERT INTO Pig (id, name) VALUES (5, 'Aloi')").execute());
    }

    private Uni<String> selectNameFromId(Integer id) {
        return oraclePool.preparedQuery("SELECT name FROM Pig WHERE id = ?").execute(Tuple.of(id)).map(rowSet -> {
            if (rowSet.size() == 1) {
