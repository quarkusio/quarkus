...
/**
 * Utility class that can be used to mock CDI normal scoped beans. This includes beans that are
 * {@link jakarta.enterprise.context.ApplicationScoped} and {@link jakarta.enterprise.context.RequestScoped}. To use
 * this inject the bean into a test, and then invoke the mock method with your mock. Mocks installed in
 * {@link org.junit.jupiter.api.BeforeAll} will be present for every test, while mocks installed within a test are
 * cleared after the test has run. Note that you will likely need to use {@link TestInstance.Lifecycle#PER_CLASS} to
 * have a non-static before all method that can access injected beans. Note that as the bean is replaced globally you
 * cannot use parallel test execution, as this will result in race conditions where mocks from one test are active in
 * another.
 */
public class QuarkusMock {
     ...
     *        The mock object
     * @param instance
     *        The type of the CDI normal scoped bean to replace
     * @param qualifiers
     ...
     *        The mock object
     * @param typeLiteral
     *        TypeLiteral representing the required type
     * @param qualifiers
     *        The CDI qualifiers of the bean to mock
     * @param <T>
     *        The bean type
