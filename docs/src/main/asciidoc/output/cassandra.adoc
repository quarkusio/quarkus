...
:categories: data
:summary: This guide covers how to use the Apache Cassandra NoSQL database in {product-name-first}.
...
link:https://github.com/datastax/cassandra-quarkus[Cassandra {product-name} extension], which allows you to
...
the Cassandra {product-name} extension GitHub repository.
...
The `pom.xml` is importing all the {product-name} extensions and dependencies you need.
...
{product-name}. This will be explained in more detail in the "Reactive Programming" section below.
...
Cassandra {product-name} extension also ships with its own annotation processor:
`cassandra-quarkus-mapper-processor`, which has more capabilities than the driver's. This annotation
processor is the only one suitable for use in a {product-name} application, so check that this is the one
...
The Cassandra {product-name} extension allows you to inject any of the following beans in your own
components:
...
{product-name}. Read its javadocs carefully!
...
The translation to and from JSON is done automatically by the {product-name} RESTEasy Reactive extension, which is
...
{product-name} automatically serves static resources located under the `META-INF/resources` directory. In
...
The
link:https://javadoc.io/doc/com.datastax.oss.quarkus/cassandra-quarkus-client/latest/com/datastax/oss/quarkus/runtime/api/session/QuarkusCqlSession.html[`QuarkusCqlSession`
interface] gives you access to a series of reactive methods that integrate seamlessly with {product-name}
...
NOTE: The Cassandra driver uses the Reactive Streams `Publisher` API for reactive calls. The {product-name}
...
are the same Mutiny types that we met before; they are automatically recognized by the {product-name}
...
If you are using the {product-name} SmallRye Health extension, then the Cassandra client will automatically
...
The Cassandra {product-name} client can provide metrics about the Cassandra session and about individual
...
Micrometer is the recommended metrics framework in {product-name} applications.
...
bean and all DAO beans will be initialized eagerly during application startup, on the {product-name} main
thread. This would eliminate any risk of blocking a Vert.x thread, at the cost of making your
startup time (much) longer.

== Conclusion

Accessing a Cassandra database from a client application is easy with {product-name} and the Cassandra
