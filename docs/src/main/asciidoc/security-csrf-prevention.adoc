...
https://owasp.org/www-community/attacks/csrf[Cross-Site Request Forgery (CSRF)] is an attack that forces an end user to execute unwanted actions on a web application in which they are currently authenticated. The CSRF filter applies to requests using idempotent REST verbs, including `POST`, `PUT`, `PATCH`, `DELETE`.
