 ...
 * fields are injected into the MDC (backed by Vert.x context-local data when running
 * on a Vert.x thread) under reserved keys (all prefixed with {@value #MDC_FIELD_PREFIX})
 * so that log formatters such as {@code quarkus-logging-json} can emit them as
 * first-class JSON fields. The MDC entries are removed synchronously in a
 * {@code finally} block after the log record is created, so they never leak
 * beyond the log call.
 *
...
public class JBossLoggingAccessLogReceiver implements AccessLogReceiver {
    ...
    public static final String DEFAULT_CATEGORY = "io.quarkus.http.access-log";
