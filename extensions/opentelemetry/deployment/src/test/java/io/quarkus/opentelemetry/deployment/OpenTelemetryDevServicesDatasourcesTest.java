...
public class OpenTelemetryDevServicesDatasourcesTest {
    ...
    void devDatasource() {
        ...
        TEST.modifyResourceFile("application.properties", s -> s + "quarkus.datasource.jdbc.telemetry=false\n");
    ...
    private void verifyNumOfInsertedTraces(int expectedSpans, int insertCount) {
        ThrowingRunnable assertInsertCount = () -> RestAssured.get("/hello/greetings-insert-count/" + expectedSpans)
                .then().statusCode(200).body(Matchers.is(Integer.toString(insertCount)));
        Awaitility.await().atMost(Duration.ofMinutes(1)).untilAsserted(assertInsertCount);
    }

    private void createGreeting(String greeting) {
        RestAssured.when().post("/hello/{greeting}", greeting).then().statusCode(200).body(Matchers.is(greeting));
    }
    ...
    public static class HelloResource {
        ...
        public long greetingsInsertCount(@PathParam("expectedSpans") int expectedSpans) {
            ...
            return spans.stream()
                    .filter(span -> "INSERT".equals(span.getAttributes().get(AttributeKey.stringKey("db.operation"))))
                    .count();
        }
