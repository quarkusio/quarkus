...
|Reference
|* Use a noun phrase to summarize the content of the document concisely
...
Prefix:: Use a common prefix to group related documents. For example, documents related to writing and contributing to Quarkus docs share the `doc-` prefix.
...
Minimally, each document should define an ID and a title and include common attributes (`_attributes.adoc`).
...
`:summary: <text>`:: Use the summary to provide a concise (26 words or less) document description.
...
The first paragraph in the main body is treated as the abstract, also called the preamble.
...
[IMPORTANT]
====
Ensure that the first sentence explains the content's value and benefits in 26 words or less.
====
...
Start a new line at the end of each sentence, and split sentences at natural breaks between clauses.
...
For example, if this is a reference, should some content be moved to a concept doc or how-to guide instead?
...
Quarkus documentation is built from a source in a few different environments.
...
|\{code-examples}|Relative path to the directory containing collected example source files
...
|\{imagesdir}|Relative path to the directory containing images
|\{includes}|Relative path to the directory containing partial/reusable content
|===

When cross-referencing content, use the inter-document `xref:` syntax and supply a human-readable label to your link.
...
For consistency and better functionality when documenting and calling anchors in Quarkus documentation, use the `xref` macro approach and specify the desired anchor name in `[]` at the end of the syntax.
...
[source,asciidoc]
--
xref:<target-file>.adoc#<anchord-ID>[<anchor-name>]
--

IMPORTANT: The `[]` part of your `xref` anchors sets up a desired anchor name when the default name, the value of which `asciidoc` sources from chapter headings, file names, or anchors, does not fit in the current context. When specifying the intended anchor or `xref` name, always verify if the displayed name fits the context of your content.
...
<2> define the simplified target file name for copying the file into the `target/asciidoc/examples` directory.

The `\{code-examples}` source attribute references content copied this way. The literal string `{{source}}` in the source file, if present, will be replaced with the path of the source file in the copy.
