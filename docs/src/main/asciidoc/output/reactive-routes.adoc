...
{product-name-first} also offers the possibility to use reactive routes.
...
NOTE: Reactive Routes were initially introduced to provide a reactive execution model for HTTP APIs on top of the xref:quarkus-reactive-architecture.adoc[{product-name} Reactive Architecture].
...
Before going further, let's have a look at the HTTP layer of {product-name}.
{product-name} HTTP support is based on a non-blocking and reactive engine (Eclipse Vert.x and Netty).
...
image:http-architecture.png[alt={product-name} HTTP Architecture]
...
====
The `ReactiveRoutes.asJsonArray` has been deprecated as it is not compatible with the security layer of {product-name}.
====
...
====
The `ReactiveRoutes.asEventStream` has been deprecated as it is not compatible with the security layer of {product-name}.
====
...
====
The `ReactiveRoutes.asJsonStream` has been deprecated as it is not compatible with the security layer of {product-name}.
====
...
It also describes the structure of the {product-name} HTTP layer and how to write filters.
