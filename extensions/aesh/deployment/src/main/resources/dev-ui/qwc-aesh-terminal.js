...
import { websocketPath } from 'build-time-data';
...
export class QwcAeshTerminal extends LitElement {
    ...
    async _loadXtermResources() {
        // Load xterm.js CSS and scripts from the bundled vendor resources
        // (served from the WebSocket runtime module at /aesh/vendor/*)
        if (!document.querySelector('link[href*="xterm"]')) {
            const link = document.createElement('link');
            link.rel = 'stylesheet';
            link.href = '/aesh/vendor/xterm.css';
            document.head.appendChild(link);
        }

        // Load xterm.js
        if (!window.Terminal) {
            await this._loadScript('/aesh/vendor/xterm.js');
        }

        // Load fit addon
        if (!window.FitAddon) {
            await this._loadScript('/aesh/vendor/addon-fit.js');
        }
    }
    ...
    async _connect() {
        ...
        const wsPath = websocketPath || '/aesh/terminal';
