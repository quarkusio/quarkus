...
.Prerequisites
You have a Quarkus Maven project that uses a JDBC datasource.

Hibernate ORM is the de facto JPA implementation. It offers you the full breadth of an object relational mapper (ORM).
It makes complex mappings possible, but it does not make simple and common mappings trivial.
Hibernate ORM with Panache focuses on making your entities trivial and fun to write in Quarkus.

== An example

In Panache, you can write your Hibernate ORM entities as shown in the following example:
...
Notice how much more compact and readable the code is.

NOTE: The `list()` method takes fragments of HQL (JP-QL) queries and contextualizes the rest which provides concise and readable code. What was described above is essentially the active record pattern, sometimes just called the entity pattern. Hibernate with Panache also allows for the use of the more classical repository pattern via PanacheRepository
...
//What specifically do I need to add?
...
. In your `pom.xml` file, add the Panache JPA extension and your JDBC driver extension dependency. Quarkus support the following JDBC driver extensions:
+
...
. Add the relevant configuration properties to the `{config-file}` file.
+
[source,properties]
----
...
To define a Panache entity, extend `PanacheEntity`, annotate it with `@Entity` and add your
...
----
+
. Put all of your JPA column annotations on the public fields. If you need a field to not be persisted, use the
...
Because of field access rewrite, when  users read `person.name` they will actually call your `getName()` accessor,
and similarly for field writes and the setter.
This enables proper encapsulation at runtime because all fields calls will be replaced by the corresponding getter/setter calls.

== Most useful operations

After you have written your entity, here are the most common operations you will be able to do:
