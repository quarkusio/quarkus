...
This guide explains how your {product-name-first} application can expose its API description through an OpenAPI specification and
...
(feel free to take a look to the xref:rest-json.adoc[Writing JSON REST services guide] if your want more details on how to build a REST API with {product-name}).
...
{product-name} provides the https://github.com/smallrye/smallrye-open-api/[Smallrye OpenAPI] extension compliant with the
...
You just need to add the `openapi` extension to your {product-name} application:
...
on a Jakarta REST `Application` class.  Because a Jakarta REST `Application` class is not required in {product-name}, you will
...
Instead of dynamically creating OpenAPI schemas from annotation scanning, {product-name} also supports serving static OpenAPI documents.
...
{product-name} also supports alternative <<open-document-paths,OpenAPI document paths>> if you prefer.
...
====
{product-name} supports various paths to store your OpenAPI document under. We recommend you place it under `META-INF/openapi.yml`.
...
Live reload of static OpenAPI document is supported during development. A modification to your OpenAPI document will be picked up on fly by {product-name}.
====
...
The {product-name} `smallrye-openapi` extension comes with a `swagger-ui` extension embedding a properly configured Swagger UI page.

[NOTE]
====
By default, Swagger UI is only available when {product-name} is started in dev or test mode.
