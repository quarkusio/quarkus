...
/**
 * Represents a Form-based authentication configuration.
 */
public sealed interface Form extends FormAuthConfig permits Form.Builder.FormImpl {
    ...
    /**
     * @return Form-based authentication configuration builder. The mechanism is implicitly enabled.
     */
    static Builder builder() {
        return new Builder().enable();
    }
    ...
    final class Builder {
        ...
        public Builder() {
            this(ConfigProvider.getConfig().unwrap(SmallRyeConfig.class).getConfigMapping(VertxHttpConfig.class),
                    ConfigProvider.getConfig().getValue("quarkus.http.auth.form.enabled", Boolean.class));
        }

        public Builder(VertxHttpConfig vertxHttpConfig, boolean enabled) {
         ...
         */
        public Builder cookieSameSite(CookieSameSite cookieSameSite) {
        ...
        public Form build() {
            return new FormImpl(loginPage, usernameParameter, passwordParameter, errorPage, landingPage, redirectAfterLogin,
                    locationCookie, timeout, newCookieInterval, cookieName, cookiePath, cookieDomain, httpOnlyCookie,
                    cookieSameSite, cookieMaxAge, postLocation, encryptionKey, enabled);
        }

        record FormImpl(Optional<String> loginPage, String usernameParameter, String passwordParameter,
                ...
                Optional<String> encryptionKey, boolean enabled) implements Form {
        }
    }
}
