...
NOTE: When enabling AOT via `quarkus.package.jar.aot.enabled=true`, Quarkus will automatically switch to the `aot-jar` type if none is explicitly configured via `quarkus.package.jar.type`.
...
IMPORTANT: For this approach to work, the integration tests must not be skipped
...
----
# Explicit integration test training
./mvnw build quarkusIntTest buildAotEnhancedImage -Dquarkus.package.jar.aot.enabled=true -Dquarkus.package.jar.aot.phase=INTEGRATION_TESTS
...
.Maven
----
./mvnw verify -Dquarkus.package.jar.aot.enabled=true -Dquarkus.container-image.build=true
----
[source, bash, subs=attributes+, role="secondary asciidoc-tabs-sync-gradle"]
.Gradle
----
...
.Gradle
----
./gradlew build -Dquarkus.package.jar.aot.enabled=true -Dquarkus.package.jar.aot.type=AppCDS
----
