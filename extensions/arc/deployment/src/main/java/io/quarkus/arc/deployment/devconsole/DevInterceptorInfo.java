...
public class DevInterceptorInfo implements Comparable<DevInterceptorInfo> {
    ...
    public static DevInterceptorInfo from(InterceptorInfo interceptor, CompletedApplicationClassPredicateBuildItem predicate) {
        ...
        Map<InterceptionType, List<MethodInfo>> intercepts = new HashMap<>();
        if (interceptor.intercepts(InterceptionType.AROUND_INVOKE)) {
            intercepts.put(InterceptionType.AROUND_INVOKE, interceptor.getAroundInvokes());
        }
        if (interceptor.intercepts(InterceptionType.AROUND_CONSTRUCT)) {
            intercepts.put(InterceptionType.AROUND_CONSTRUCT, interceptor.getAroundConstructs());
        }
        if (interceptor.intercepts(InterceptionType.POST_CONSTRUCT)) {
            intercepts.put(InterceptionType.POST_CONSTRUCT, interceptor.getPostConstructs());
        }
        if (interceptor.intercepts(InterceptionType.PRE_DESTROY)) {
            intercepts.put(InterceptionType.PRE_DESTROY, interceptor.getPreDestroys());
        }
    ...
    private final Map<InterceptionType, List<MethodInfo>> intercepts;
    private final boolean isApplicationBean;

    DevInterceptorInfo(String id, Name interceptorClass, Set<Name> bindings, int priority,
            Map<InterceptionType, List<MethodInfo>> intercepts, boolean isApplicationBean) {
        this.id = id;
    ...
    public Map<InterceptionType, List<MethodInfo>> getIntercepts() {
        return intercepts;
    }
