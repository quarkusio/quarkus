...
:summary: Build and deploy your {product-name-first} App using Ansible


Let’s see how to build and deploy a {product-name} app using https://docs.ansible.com/ansible/latest/index.html[Ansible]. We’ll see how we can automate the entire process, from the code checkout to the application compilation using Maven and then its deployment and start of the service, as a https://systemd.io/[systemd service], on the target system using Ansible and its collection for {product-name}.

The first part, the application code checkout, compilation and packaging on the Ansible (where Ansible runs). We’ll use the getting-started sample application provided in its link:{quickstarts-tree-url}/getting-started[{product-name} QuickStarts directory] as a base for this tutorial. We’ll also leverage the https://galaxy.ansible.com/middleware_automation/quarkus[{product-name} collection] for Ansible, an extension for Ansible that alleviates the boilerplate required and to quickly build and deploy a {product-name} using Ansible.
...
You'll need to https://docs.ansible.com/ansible/latest/installation_guide/intro_installation.html[install Ansible] on your workstation. Once this is done, you can install this extension for Ansible dedicated to {product-name} with the following command:
...
A few tasks performed by the Ansible collection for {product-name} will require administrative privileges on the target (create a group and user account, install packages). If Ansible does run as root, you'll need to add the following options to the `ansible-playbook` command line:
...
With the Ansible collection installed on the controller, you can already directly use a playbook provided with it to build and deploy your {product-name} application:
...
PLAY [Build and deploy a {product-name} app using Ansible] ****************************

TASK [Build the {product-name} from https://github.com/quarkusio/quarkus-quickstarts.git.] ***
...
TASK [middleware_automation.quarkus.quarkus : Ensure {product-name} system group exists on target system] ***
ok: [localhost]

TASK [middleware_automation.quarkus.quarkus : Ensure {product-name} user exists on target system.] ***
ok: [localhost]

TASK [middleware_automation.quarkus.quarkus : Ensure deployement directory exits: /opt/quarkus_deploy.] ***
ok: [localhost]

TASK [middleware_automation.quarkus.quarkus : Set {product-name} app source dir (if not defined).] ***
ok: [localhost]

TASK [middleware_automation.quarkus.quarkus : Deploy application from  to target system] ***
ok: [localhost]

TASK [middleware_automation.quarkus.quarkus : Deploy Systemd configuration for {product-name} app] ***
ok: [localhost]

TASK [middleware_automation.quarkus.quarkus : Perform daemon-reload to ensure the changes are picked up] ***
skipping: [localhost]

TASK [middleware_automation.quarkus.quarkus : Ensure {product-name} app service is running.] ***
ok: [localhost]
...
The Ansible collection for {product-name} does all the `heavy lifting` here. First, it checks out the code from Github and builds the application from its sources. It also ensures the system used for this step does have the required OpenJDK installed. By default, the application is built on the localhost (the Ansible controller), but it can be performed on a remote system if needed. Once the application is built, the collection will take care of the deployment.

Here again, it checks that the appropriate OpenJDK is available on the target system. Then we ensure that the required user and group exist on the target. This is recommended mostly to be able to run the {product-name} app with a regular user, rather than with the root account.
...
[NOTE]
====
By default, the Ansible collection for {product-name} will install and use the OpenJDK 17 available in the Yum repositories of the target host (or the controller). If you want to use a different version of OpenJDK, define the following variable:
...
● getting-started.service - A {product-name} service named getting-started
...
Apr 13 12:48:18 bd71f39642c8 systemd[1]: Started A {product-name} service named getting-started.
...
Apr 13 12:48:19 bd71f39642c8 java[853]: 2023-04-13 12:48:19,284 INFO  [io.quarkus] (main) getting-started 1.0.0-SNAPSHOT on JVM (powered by {product-name} 2.16.6.Final) started in 0.607s. Listening on: http://0.0.0.0:8080
...
[source,yml]
----
- name: "Build and deploy a {product-name} app using Ansible"
  hosts: all
  gather_facts: true
  vars:
  ...
  pre_tasks:
    - name: "Build the {product-name} from {{ quarkus_app_repo_url }}."
      ansible.builtin.include_role:
        name: quarkus
        tasks_from: build.yml
  tasks:
    - name: "Deploy {product-name} app on target."
      ansible.builtin.include_role:
        name: quarkus
...
```
 post_tasks:
    ...
    - name: "Check that {product-name} app is accessible"
      ansible.builtin.uri:
