...
:summary: While you are encouraged to use the {product-name-first} Security layer to secure your a{product-name}ions, Quarkus provides a compatibility layer for Spring Security in the form of the spring-security extension.

While users are encouraged to use Java standard annotations for security authorizations, {product-name} provides a compatibility layer for Spring Security in the form of the `spring-security` extension.

This guide explains how a {product-name} application can leverage the well-known Spring Security annotations to define authorizations on RESTful services using roles.
...
If you already have your {product-name} project configured, you can add the `spring-web`, `spring-security` and `security-properties-file` extensions
...
The {product-name} Maven plugin automatically generated a controller with the Spring Web annotations to define our REST endpoint (instead of the Jakarta REST ones used by default).
...
The easiest way to set up users and roles for our example is to use the `security-properties-file` extension. This extension essentially allows users and roles to be defined in the main {product-name} configuration file - `application.properties`.
...
{product-name} currently only supports a subset of the functionalities that Spring Security provides with more features being planned. More specifically, {product-name} supports the security related features of role-based authorization semantics
...
{product-name} provides support for some of the most used features of Spring Security's `@PreAuthorize` annotation.
