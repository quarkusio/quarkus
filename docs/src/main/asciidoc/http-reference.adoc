...
When the proxy in front of Quarkus supports the link:https://www.haproxy.org/download/1.8/doc/proxy-protocol.txt[PROXY protocol], enable it so that the remote address of a connection is the address of the actual client rather than the address of the proxy:
...
The PROXY protocol is then expected on every listener: HTTP, HTTPS and the Unix domain socket.
A connection that does not start with a PROXY protocol header is rejected.
If only some of the listeners are reached through the proxy, for example when the load balancer passes TLS traffic through with a PROXY header while plain HTTP traffic comes from inside the cluster, restrict the protocol to those listeners:
...
The accepted values are `http`, `https` and `domain-socket`.

IMPORTANT: A listener that expects the PROXY protocol trusts the client address carried by the header. Do not expose such a listener to clients other than the proxy.
