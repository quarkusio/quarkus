...
Quarkus also supports https://jakarta.ee/specifications/jsonb/[JSON-B], so if you prefer JSON-B over Jackson, you can create a project relying on the RESTEasy JSON-B extension instead:

:create-app-artifact-id: rest-json-quickstart
:create-app-extensions: resteasy-jsonb
include::{includes}/devtools/create-app.adoc[]

This command generates a new project importing the RESTEasy/Jakarta REST and https://jakarta.ee/specifications/jsonb/[JSON-B] extensions,
and in particular, adds the following dependency:
