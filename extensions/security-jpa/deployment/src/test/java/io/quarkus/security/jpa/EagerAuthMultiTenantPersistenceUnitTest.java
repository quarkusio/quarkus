...
public class EagerAuthMultiTenantPersistenceUnitTest extends JpaSecurityRealmTest {
    ...
    static final QuarkusUnitTest config = new QuarkusUnitTest().withApplicationRoot((jar) -> jar.addClasses(testClasses)
            .addClass(MinimalUserEntity.class).addClass(CustomHibernateTenantResolver.class)
    ...
    public void testRoutingCtxAccessInsideTenantResolver() {
        ...
        RestAssured.given().auth().preemptive().basic("user", "user").when()
                .get("/jaxrs-secured/roles-class/routing-context").then().statusCode(200);
        ...
        try {
            RestAssured.given().auth().preemptive().basic("user", "user").queryParam("tenant", "two").when()
                    .get("/jaxrs-secured/roles-class").then().statusCode(500);
        } finally {
