...
.Prerequisites
You have a Quarkus Maven project configured with a Java database connectivy (JDBC) datasource and Hibernate ORM.

* For information about creating a Quarkus Maven project, see https://access.redhat.com/documentation/en-us/red_hat_build_of_quarkus/1.3/html-single/creating_quarkus_applications_with_apache_maven/[_Creating Quarkus applications with Apache Maven_].
//need ifdef
* For information about configuring your Quarkus Maven project with a JDBC datasource, see https://quarkus.io/guides/datasource[_Configuring your datasources_].
* For information about configuring your Quarkus Maven project Hibernate ORM, see https://quarkus.io/guides/hibernate-orm[_Using Hibernate ORM and JPA_].

For an example of a comleted Quarkus Maven project that uses the Panache extension, see the https://github.com/quarkusio/quarkus-quickstarts/tree/master/hibernate-orm-panache-quickstart[Hibernate ORM Panache Quickstart].
...
Hibernate not only takes care of the mapping from Java classes to database tables and from Java data types to SQL data types, but also provides data query and retrieval facilities. It can significantly reduce development time otherwise spent with manual data handling in SQL and Java database connectivy (JDBC). Hibernate’s design goal is to relieve you of most common data persistence-related programming tasks by eliminating the need for manual, hand-crafted data processing using SQL and JDBC.
...
The following example demonstrates the use of the flush method to allow a specific action in case of `PersistenceException`:
[source,java]
----
@Transactional
