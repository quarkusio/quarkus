...
See <<caching-of-entities-and-associations>>.
...
By default Quarkus estimates each entry's weight from Hibernate's dehydrated second-level cache state
(strings, byte arrays, and similar variable-size values contribute most of the weight).
