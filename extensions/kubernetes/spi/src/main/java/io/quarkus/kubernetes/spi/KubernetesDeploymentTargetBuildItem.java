...
/**
 * Used to control which Kubernetes targets have their files generated and which get deployed Since these can be
 * generated by various locations, code that needs the "final" set of items, should use the {@code mergeList} method to
 * get a new list with merged items. Furthermore, if priorities need to be taken into account, the merged list should
 * also be sorted
 */
public final class KubernetesDeploymentTargetBuildItem extends MultiBuildItem
    ...
    /**
     * Create a new {@code KubernetesDeploymentTargetBuildItem} that contains the maximum of the two priorities and the
     * logical 'OR' of the {@code enabled} field. {@code name} and {@code kind} must match, otherwise an exception is
     * thrown
     */
    private KubernetesDeploymentTargetBuildItem merge(KubernetesDeploymentTargetBuildItem other) {
        ...
        return new KubernetesDeploymentTargetBuildItem(this.name, this.kind, this.group, this.version,
                Math.max(this.priority, other.getPriority()), this.enabled || other.isEnabled(), this.deployStrategy);
    }
    ...
    public boolean equals(Object o) {
        ...
        return priority == that.priority && enabled == that.enabled && name.equals(that.name) && kind.equals(that.kind)
    ...
    /**
     * Return a new list containing from the input where entries that match {@code name} and {@code kind} as merged
     * together based on {@code merge}
     */
