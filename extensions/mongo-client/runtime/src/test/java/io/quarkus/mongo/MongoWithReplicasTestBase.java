...
import static io.quarkus.mongo.MongoTestBase.getConfiguredConnectionString;
...
public class MongoWithReplicasTestBase {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(MongoWithReplicasTestBase.class);
    ...
    public static void startMongoDatabase() throws IOException {
        ...
        if (uri == null) {
            ...
            LOGGER.info("Using existing Mongo {}", uri);
        }
    }
        ...
        try (MongoClient mongo = MongoClients.create(mo)) {
            ...
            await()
                    ...
                    .until(() -> {
                        ...
                        LOGGER.info("replSetGetStatus: {}", result);
