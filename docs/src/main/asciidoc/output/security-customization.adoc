...
`io.quarkus:quarkus-security` module contains the core {product-name-first} Security classes.
...
If you use Jakarta REST `ContainerRequestFilter` to set a custom Jakarta REST `SecurityContext` then make sure `ContainerRequestFilter` runs in the Jakarta REST pre-match phase by adding a `@PreMatching` annotation to it for this custom security context to be linked with {product-name} `SecurityIdentity`, for example:
...
For manual testing {product-name} provides a convenient config property to disable authorization in dev mode. This property has the exact same effect as the custom `AuthorizationController` shown above, but is only available in dev mode:
...
In {product-name} you can achieve the same at the configuration level only without having to modify the code, for example:
...
[NOTE]
====
Note that while accessing the `SunPKCS11` bridge provider is supported in native image, configuring `SunPKCS11` is currently not supported in native image at the {product-name} level.
====
...
to make sure that {product-name} can propagate it. For more information see the
