...
This can be generated for the first example above by running the command `echo -n scott:MyRealm:jb0ss | md5sum | cut -d ' ' -f 1` from the command line (on macOS, `echo -n scott:MyRealm:jb0ss | md5` produces the same digest).
