 ...
 * <p>
 * For incoming messages, it fetches OpenTelemetry context from the message and attaches to the duplicated context of
 * the message. Consumer methods will be called on this duplicated context, so the OpenTelemetry context associated with
 * the incoming message will be propagated.
 */
@ApplicationScoped
    ...
    public Multi<? extends Message<?>> decorate(Multi<? extends Message<?>> publisher, String channelName,
        ...
        if (isConnector) {
            ...
            multi = multi.invoke(m -> {
                var messageContext = m.getMetadata(LocalContextMetadata.class).map(LocalContextMetadata::context)
                        .orElse(null);
                var otelContext = TracingMetadata.fromMessage(m).map(TracingMetadata::getCurrentContext)
