...
public abstract class JpaSecurityRealmTest {
    ...
    protected static Class<?>[] testClasses = { SingleRoleSecuredResource.class, TestApplication.class,
    ...
    public void testSecureAccessFailure() {
        RestAssured.when().get("/jaxrs-secured/user-secured").then().statusCode(401);
    }

    @Test()
    public void testSecureRoleFailure() {
        RestAssured.given().auth().preemptive().basic("noRoleUser", "noRoleUser").when()
                .get("/jaxrs-secured/user-secured").then().statusCode(403);
    }

    @Test()
    public void testSecureAccessSuccess() {
        RestAssured.given().auth().preemptive().basic("user", "user").when().get("/jaxrs-secured/user-secured").then()
                .statusCode(200);
    }
