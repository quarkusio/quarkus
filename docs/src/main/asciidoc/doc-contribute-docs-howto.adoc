...
:nav-title: Basic authentication <6>
<7>
----
...
<5> Set at least one category to ensure that the content is findable on the link:https://quarkus.io/guides/[Quarkus documentation home page]. Valid category IDs are defined in `docs/src/main/asciidoc/navigation-config.yaml`.
<6> A short sidebar title (40 characters or fewer). Required only when the document title exceeds 40 characters. When omitted, the document title is used automatically.
<7> Insert a blank line after all document attributes and before the abstract.
+
[IMPORTANT]
====
