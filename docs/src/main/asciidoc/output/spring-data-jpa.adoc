...
:summary: While you are encouraged to use Hibernate ORM with Panache for your data layer, {product-name-first} provides a compatibility layer for Spring Data JPA in the form of the spring-data-jpa extension.

While users are encouraged to use Hibernate ORM with Panache for Relational Database access, {product-name} provides a compatibility layer for
...
If you already have your {product-name} project configured, you can add the `spring-data-jpa` extension
...
To make it easier to showcase some capabilities of Spring Data JPA on {product-name}, some test data should be inserted into the database
...
The Jakarta REST resource can also be substituted with a Spring Web controller as {product-name} supports REST endpoint definition using Spring controllers.
...
{product-name} dev mode works with the defined repositories just like with any other {product-name} extension, greatly enhancing your productivity during the dev cycle.
...
{product-name} currently supports a subset of Spring Data JPA's features, namely the most useful and most commonly used features.
...
TIP: In {product-name}, `@Param` is optional when parameter names have been compiled to bytecode (which is active by default in generated projects).
...
An extensive list of examples can be seen in the https://github.com/quarkusio/quarkus/tree/main/integration-tests/spring-data-jpa[integration tests] directory which is located inside the {product-name} source code.
...
** In Spring Data JPA this is done by registering a class that extends `org.springframework.data.jpa.repository.support.SimpleJpaRepository` however in {product-name} this class
is not used at all (since all the necessary plumbing is done at build time). Similar support might be added to {product-name} in the future.
...
The {product-name} team is exploring various alternatives to bridging the gap between the JPA and Reactive worlds.
...
* xref:spring-di.adoc[{product-name} - Extension for Spring DI]
