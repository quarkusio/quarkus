...
To trigger building and deploying a container image you need to enable `quarkus.container.build` and `quarkus.container.deploy` flags respectively.
This can be easily done from the command line:

[source, subs=attributes+]
----
mvn clean package -Dquarkus.container.build=true -Dquarkus.kubernetes.deploy=true
----

=== Building

At the moment this is only possible, using `docker`.
The dockerfiles used are internal (Dockerfiles found under `src/main/docker` are NOT used).

if the `docker` binary is not present in the `PATH` or a docker daemon is not available, this step will be skipped.
