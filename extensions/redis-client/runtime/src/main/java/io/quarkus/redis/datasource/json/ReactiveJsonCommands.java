...
public interface ReactiveJsonCommands<K> extends ReactiveRedisCommands {
     ...
     *
     * @return a uni emitting the stored JSON object, {@code null} if it does not exist. If no path are passed, this is
     *         equivalent to {@link #jsonGetObject(Object)}. If multiple paths are passed, the produced JSON object
     *         contains the result for each pass as a JSON array.
     **/
     ...
     *        the key, must not be {@code null}
     *
     * @return a uni emitting a list containing the list of keys, or {@code null} if the stored value is not a JSON
     *         object.
     **/
    ...
    default Uni<Integer> jsonObjLen(K key) {
        return jsonObjLen(key, null).map(l -> l.get(0));
    }
     ...
     *        path the path, path defaults to {@code $} if not provided.
     *
     * @return a uni emitting a list containing, for each path, the length of the string, {@code null} if the matching
     *         JSON value is not a string. Returns {@code null} if the key or path do not exist.
     **/
     ...
     *        path the path, path defaults to {@code $} if not provided.
     *
     * @return a uni emitting a list containing, for each path, the json type as String (string, integer, number,
     *         boolean, object, array), empty if no match.
     **/
