...
{product-name-first} offers the possibility to use YAML in addition to the standard Java Properties file.
...
NOTE: If both are present, {product-name} prioritizes configuration properties from the YAML file first and then from the
Properties file. However, to avoid confusion, we recommend removing the Properties file.

TIP: {product-name} supports both the  `yml` and `yaml` file extensions.
...
file has to be present in the root of the working directory relative to the {product-name} application runner:
