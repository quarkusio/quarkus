...
Open a xref:dev-ui.adoc[Dev UI] available at `http://localhost:8080/q/dev-ui` and click a **Provider: Keycloak** link on an **OpenID Connect** card in the Dev UI.

.Interacting with Dev UI

. Open the xref:dev-ui.adoc[Dev UI] at `http://localhost:8080/q/dev-ui`.
