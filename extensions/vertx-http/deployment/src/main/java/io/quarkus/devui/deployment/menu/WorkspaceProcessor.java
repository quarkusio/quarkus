...
public class WorkspaceProcessor {
    ...
    private WorkspaceContent readContents(Path filePath) {
        if (filePath != null && Files.exists(filePath) && Files.isReadable(filePath)) {
            try {
                ...
                if (mimeType == null)
                    mimeType = getFileTypeManually(filePath);

                if (mimeType.startsWith("image")) {
                    return new WorkspaceContent(mimeType, Base64.getEncoder().encodeToString(Files.readAllBytes(filePath)));
                } else {
                    return new WorkspaceContent(mimeType, Files.readString(filePath));
                }
                // TODO: Add support for other non-text files (like zip ext)
            } catch (IOException ex) {
                throw new UncheckedIOException(ex);
            }
        }
    ...
    static record WorkspaceContent(String type, String content) {
    }
