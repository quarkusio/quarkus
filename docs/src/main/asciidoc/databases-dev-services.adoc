...
NOTE: If you still decide to configure a database URL, username or password Quarkus Dev Services will still start up, but it will only work correctly if you configure a fixed port for the dev services database to run on.
So smth. like `quarkus.datasource.reactive.url=vertx-reactive:postgresql://localhost:${quarkus.datasource.devservices.port}/postgres` will not work unless you have specified `quarkus.datasource.devservices.port`.
