...
public abstract class CollectDependenciesBase extends ResolverSetupCleanup {
    ...
    public void testCollectedDependencies() throws Exception {
        install(root);
        final List<AppDependency> resolvedDeps = collectRuntimeDeps ? resolver.collectRuntimeDependencies(root.toAppArtifact())
                : resolver.collectDependencies(root.toAppArtifact());
