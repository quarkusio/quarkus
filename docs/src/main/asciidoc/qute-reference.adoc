...
This section is an alternative to `{#with}` which allows to specify multiple named parameters:
[source,html]
----
{#let myParent=item.parent price=item.price}
  <h1>{myParent.name}</h1>
  <p>Price: {price}
{/let}
----
...
The following snippet is equivalent to the previous snippet with `let`:
