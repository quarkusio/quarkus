...
public class HibernateReactiveStatelessTransactionsTest {
    ...
    @Test
    @RunOnVertxContext
    public void testCancellationRollback(UniAsserter asserter) {
        // initialTransactionData.sql
        Long heroId = 100L;
        ...
        // Second update, make sure there's a rollback as the uni is cancelled via timeout
        asserter.assertThat(
                () -> transactionalUpdateNotActuallyExeucting(heroId, "this name won't appear")
                        .ifNoItem().after(Duration.ofMillis(100))
                        .failWith(new TimeoutException("Cancelled"))
                        .onFailure().recoverWithNull()
                        .chain(() -> findHero(heroId)),
                h -> {
                    assertThat(h.name).isEqualTo("updatedNameCommitted");
    ...
    @Transactional
    public Uni<Hero> transactionalUpdateNotActuallyExeucting(Long previousHeroId, String newName) {
        return Uni.createFrom().nothing(); // Returns a Uni that doesn't do anything
    }
