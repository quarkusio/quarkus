...
If you have an already created project, the `quarkus-reactive-pg-client` extension can be added to an existing {project-name} project with the `add-extension` command:

[source,shell]
----
mvn quarkus:add-extension -Dextensions="quarkus-reactive-pg-client"
----
...
----
quarkus.datasource.url=vertx-reactive:postgresql:///quarkus_test
...
Eventually, open your browser and navigate to http://localhost:8080/fruits, you should see:
...
As the simple `query` method, `preparedQuery` eventually returns a `PgRowSet`.
...
A `PgRowSet` does not only hold your data in memory.
It also gives you some information about the data itself, such as:
