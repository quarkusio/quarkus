...
[WARNING]
====
This guide is about https://resteasy.dev[RESTEasy Classic] which used to be the default Jakarta REST (formerly known as JAX-RS) implementation until {product-name-first} 2.8.
...
[NOTE]
====
To improve user experience, {product-name} registers the three Jackson https://github.com/FasterXML/jackson-modules-java8[Java 8 modules] so you don't need to do it manually.
====

{product-name} also supports https://eclipse-ee4j.github.io/jsonb-api/[JSON-B] so, if you prefer JSON-B over Jackson, you can create a project relying on the RESTEasy JSON-B extension instead:
...
[NOTE]
====
When a JSON extension is installed such as `quarkus-resteasy-jackson` or `quarkus-resteasy-jsonb`, {product-name} will use the `application/json` media type
...
In {product-name}, the default Jackson `ObjectMapper` obtained via CDI (and consumed by the {product-name} extensions) is configured to ignore unknown properties
...
Also, {product-name} makes it very easy to configure various Jackson settings via CDI beans.
...
public class CustomObjectMapper {
    ...
    // Replaces the CDI producer for ObjectMapper built into {product-name}
    @Singleton
    ObjectMapper objectMapper(@All List<ObjectMapperCustomizer> customizers) {
        ObjectMapper mapper = myObjectMapper(); // Custom `ObjectMapper`

        // Apply all ObjectMapperCustomizer beans (incl. {product-name})
...
As stated above, {product-name} provides the option of using JSON-B instead of Jackson via the use of the `quarkus-resteasy-jsonb` extension.
...
public class CustomJsonbConfig {
    ...
    // Replaces the CDI producer for JsonbConfig built into {product-name}
    @Dependent
    JsonbConfig jsonConfig(Instance<JsonbConfigCustomizer> customizers) {
        JsonbConfig config = myJsonbConfig(); // Custom `JsonbConfig`

        // Apply all JsonbConfigCustomizer beans (incl. {product-name})
...
{product-name} automatically serves static resources located under the `META-INF/resources` directory.
...
The good news is that {product-name} does that work for you most of the time.
So far, we haven't registered any class, not even `Fruit`, for reflection usage and everything is working fine.

{product-name} performs some magic when it is capable of inferring the serialized types from the REST methods.
When you have the following REST method, {product-name} determines that `Fruit` will be serialized:
...
{product-name} does that for you automatically by analyzing the REST methods at build time
...
It is not possible for {product-name} to determine at build time the type included in the `Response` as the information is not available.
In this case, {product-name} won't be able to automatically register for reflection the required classes.
...
As mentioned above, the issue is that {product-name} was not able to determine the `Legume` class will require some reflection by analyzing the REST endpoints.
...
TIP: The `@RegisterForReflection` annotation instructs {product-name} to keep the class and its members during the native compilation. More details about the `@RegisterForReflection` annotation can be found on the xref:writing-native-applications-tips.adoc#registerForReflection[native application tips] page.
...
{product-name} recommends the usage of https://smallrye.io/smallrye-mutiny[Mutiny] to write reactive and asynchronous code.
...
{product-name} comes with a CORS filter. Read the xref:http-reference.adoc#cors-filter[HTTP Reference Documentation] to learn
how to use it.

== GZip Support

{product-name} comes with GZip support (even though it is not enabled by default). The following configuration knobs allow to configure GZip support.
...
{product-name} provides an extension called `quarkus-resteasy-multipart` to make things easier for you.
...
In {product-name}, RESTEasy can either run directly on top of the Vert.x HTTP server, or on top of Undertow if you have any servlet dependency.
...
In {product-name}, the RESTEasy extension and xref:rest-client.adoc[the REST Client extension] share the same infrastructure.
...
In contrast to Jakarta REST (and RESTeasy) running in a standard servlet-container, {product-name} only supports the deployment of a single Jakarta REST application.
...
In {product-name} all Jakarta REST resources are treated as CDI beans.
...
{product-name} enables the inclusion or exclusion of Jakarta REST Resources, Providers and Features directly thanks to build time conditions in the same that it does for CDI beans.
Thus, the various Jakarta REST classes can be annotated with profile conditions (`@io.quarkus.arc.profile.IfBuildProfile` or `@io.quarkus.arc.profile.UnlessBuildProfile`) and/or with property conditions (`io.quarkus.arc.properties.IfBuildProperty` or `io.quarkus.arc.properties.UnlessBuildProperty`) to indicate to {product-name} at build time under which conditions these Jakarta REST classes should be included.

In the following example, {product-name} includes the endpoint `sayHello` if and only if the build profile `app1` has been enabled.
...
Please note that if a Jakarta REST Application has been detected and the method `getClasses()` and/or `getSingletons()` has/have been overridden, {product-name} will ignore the build time conditions and consider only what has been defined in the Jakarta REST Application.

== Conclusion

Creating JSON REST services with {product-name} is easy as it relies on proven and well known technologies.

As usual, {product-name} further simplifies things under the hood when running your application as a native executable.

There is only one thing to remember: if you use `Response` and {product-name} can't determine the beans that are serialized, you need to annotate them with `@RegisterForReflection`.
