...
A multi-module Maven project is typically organized as a parent (aggregator) module with several child modules, for example one or more shared library modules and one or more Quarkus application modules that depend on them.

[[multi-module-bean-discovery]]
==== Making CDI beans discoverable across modules
...
==== Running dev mode in a multi-module project

<<dev-mode,Development mode>> is started on a single Quarkus application module, not on the aggregator.
Before starting it, build the whole project once so that the other modules the application depends on are compiled:
...
Then start dev mode on the application module only, selecting it from the root of the project with `-pl`:

[source,bash]
----
./mvnw quarkus:dev -pl :my-app
----

Alternatively, run it from within the module directory:
...
Local modules that the application depends on are treated as hot-reloadable dependencies: when you edit their sources and a change is detected, Quarkus recompiles them and redeploys the application, exactly as it does for the application's own sources.
This is why the project must be built at least once first; otherwise dev mode fails with a `Hot reloadable dependency ... has not been compiled yet` error.

Dev mode runs a single Quarkus application at a time.
If your project contains several Quarkus application modules, start each one in its own dev mode session, in a separate terminal, and if needed on a different port using `-Dquarkus.http.port`.
