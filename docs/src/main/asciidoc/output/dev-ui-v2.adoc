...
.Dev UI v2
====
This guide covers the Dev UI v2, which is the default from {product-name-first} 3 onwards. Read xref:dev-ui.adoc[De{product-name}] for Quarkus 2.x.
====

This guide covers the {product-name} Dev UI for xref:building-my-first-extension.adoc[extension authors].

{product-name} ships with a Developer UI, which is available in dev mode (when you start
...
<3> The component should be named in the following format: Qwc (stands for {product-name} Web Component) then Extension Name then Page Title, all concatenated with Camel Case. This will also match the file name format as described earlier. The component should also extend `LitComponent`.
...
When running a local application using the `999-SNAPSHOT` version, the Dev UI will show a `Dev UI` Log in the footer. This is useful to debug all JSON RPC messages flowing between the browser and the {product-name} app.
