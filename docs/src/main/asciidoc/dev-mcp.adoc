...
The primary way to interact with Dev MCP is through the https://github.com/quarkusio/quarkus-agent-mcp[Quarkus Agent MCP] server, which provides a complete developer experience including application lifecycle management, documentation search, and extension-specific coding skills.
Alternatively, you can connect MCP clients directly to the Dev MCP endpoint for advanced use cases.

=== Connecting a coding agent (recommended)

The https://github.com/quarkusio/quarkus-agent-mcp[Quarkus Agent MCP] server is the recommended way to connect AI coding agents to your Quarkus application.
It provides several key benefits:

- **Lifecycle management** — Automatically starts, stops, and restarts your Quarkus application
- **Dev MCP proxy** — Seamlessly forwards all Dev MCP tools and resources from your running application
- **Documentation search** — Provides semantic search across Quarkus documentation matching your project version
- **Extension skills** — Delivers extension-specific coding patterns, testing guidelines, and best practices
- **Build tools** — Includes tools for running tests, managing dependencies, and building your application
...
[TIP]
====
The Quarkus Agent MCP server is the recommended approach for most developers.
Use direct connection only if you need to integrate with a custom MCP client or have specific requirements that the agent doesn't support.
====

=== Advanced: Direct connection to Dev MCP

For advanced use cases, you can connect an MCP client directly to the Dev MCP endpoint on your running Quarkus application.
This approach requires you to manually manage the application lifecycle and provides access only to the Dev MCP tools and resources, without the additional features provided by the Quarkus Agent MCP server.

To use direct connection:

1. Open the **Dev UI** settings dialog and select the Dev MCP tab
2. Enable Dev MCP
3. Use the connection details (default `http://localhost:8080/q/dev-mcp`) shown under the **Direct connection** section
...
After a client connects, it will appear on the Dev MCP tab.
