...
public class JacksonMapperUtil {
    ...
    /**
     * Determine the root type that should be used for serialization of generic types. Returns the appropriate root type
     * or {@code null} if default serialization should be used.
     */
    public static JavaType getGenericRootType(Type genericType, ObjectWriter defaultWriter) {
        ...
        // https://github.com/FasterXML/jackson-databind/issues/336 and
        // https://github.com/FasterXML/jackson-databind/issues/23
        ...
        if (genericType != null) {
            /*
             * 10-Jan-2011, tatu: as per [JACKSON-456], it's not safe to just force root type since it prevents
             * polymorphic type serialization. Since we really just need this for generics, let's only use generic type
             * if it's truly generic.
             */
            if (genericType.getClass() != Class.class) {
                rootType = defaultWriter.getTypeFactory().constructType(genericType);
                /*
                 * 26-Feb-2011, tatu: To help with [JACKSON-518], we better recognize cases where type degenerates back
                 * into "Object.class" (as is the case with plain TypeVariable, for example), and not use that.
                 */
    ...
    public enum SerializationInclude {
        ...
        public static SerializationInclude decode(Object object, SerializerProvider serializerProvider) {
            JsonInclude.Include include = serializerProvider.getDefaultPropertyInclusion(object.getClass())
                    .getValueInclusion();
