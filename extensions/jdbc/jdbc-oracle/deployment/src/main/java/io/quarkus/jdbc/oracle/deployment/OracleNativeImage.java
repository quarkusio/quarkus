...
public final class OracleNativeImage {
    ...
    /**
     * Registers the {@code oracle.jdbc.driver.OracleDriver} so that it can be loaded by reflection, as commonly
     * expected.
     */
    @BuildStep
    void reflection(BuildProducer<ReflectiveClassBuildItem> reflectiveClass,
        ...
        // This is needed when using XA and we use the `@RegisterForReflection` trick to make sure all nested classes
        // are registered for reflection
        additionalIndexedClasses.produce(
                new AdditionalIndexedClassesBuildItem("io.quarkus.jdbc.oracle.runtime.graal.OracleReflections"));
        ...
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("com.sun.jndi.ldap.LdapCtxFactory").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("com.sun.jndi.dns.DnsContextFactory").build());
        reflectiveClass
