...
The generated product component can be further described with the following optional properties, using the same `platform.<member-bom-groupId>.<member-bom-artifactId>.` prefix: `product-name`, `product-version`, `product-description` and `product-type` (defaults to `framework`).
...
=3.20.0                       ← version sub-block, ordered lexicographically
legacy                        ← camel-quarkus-legacy
=3.33.0                       ← further versions reuse the same groupId + prefix
