...
Like we have in JPA/Hibernate, optimistic lock control is now supported, for this you just need to use the `@Version`, and panache does the rest.
...
class Person {
    ...
    lateinit var birth: LocalDate
    lateinit var status: Status
    @Version lateinit var version: Long?
}
----

It is important to notice that you can use any variable name, what matters is the `@Version` annotation, you can have some like:
[source,kotlin]
----
class Person {
    var id: ObjectId? = null; // used by MongoDB for the _id field
    ...
    lateinit var birth: LocalDate
    lateinit var status: Status
    @Version lateinit var customVersion: Long?
}
----

WARNING: This feature has no support yet for reactive mode.
