...
:categories: tooling
:summary: Use the {product-name-first} CLI to create, build, run, and manage ex{product-name}s for Quarkus projects.
...
The {product-name} CLI is available in several developer-oriented package managers such as:
...
If you already use (or want to use) one of these tools, it is the simplest way to install the {product-name} CLI and keep it updated.

In addition to these package managers, the {product-name} CLI is also installable via https://www.jbang.dev[JBang].
...
.JBang
****
The {product-name} CLI is available as a jar installable using https://jbang.dev[JBang].
...
If you have built {product-name} locally, you can use that version:
[source,bash]
----
...
With SDKMAN!, you can also install popular Java tools, including the {product-name} CLI.

[NOTE]
====
Make sure you have https://sdkman.io/jdks[a JDK installed] before installing the {product-name} CLI.
...
To install the {product-name} CLI using SDKMAN!, run the following command:
...
It will install the latest version of the {product-name} CLI.
...
You can use Homebrew to install (and update) the {product-name} CLI.

[NOTE]
====
Make sure you have a JDK installed before installing the {product-name} CLI.
...
To install the {product-name} CLI using Homebrew, run the following command:
...
It will install the latest version of the {product-name} CLI.
This command can also be used to update the {product-name} CLI.
...
You can upgrade the {product-name} CLI with:
...
<2> Upgrade {product-name} CLI to the latest version
****
...
You can use Chocolatey to install (and update) the {product-name} CLI.
...
To install the {product-name} CLI using Chocolatey, run the following command:
...
You can use Scoop to install (and update) the {product-name} CLI.
[NOTE]
====
Make sure you have a JDK installed before installing the {product-name} CLI.
You can install a JDK with `scoop install openjdk17` for Java 17 or `scoop install openjdk11` for Java 11.
====
To install the {product-name} CLI using Scoop, run the following command:
[source,shell]
----
...
Options:
      --refresh         Refresh the local {product-name} extension registry cache
...
Commands:
  create                  Create a new project.
    app                   Create a {product-name} application project.
    cli                   Create a {product-name} command-line project.
    extension             Create a {product-name} extension project
  ...
  registry                Configure {product-name} registry client
    list                  List enabled {product-name} registries
    add                   Add a {product-name} extension registry
    remove                Remove a {product-name} extension registry
  ...
  plugin, plug            Configure plugins of the {product-name} CLI.
    list, ls              List CLI plugins.
    add                   Add plugin(s) to the {product-name} CLI.
    remove                Remove plugin(s) to the {product-name} CLI.
...
Both `quarkus create` and `quarkus extension list` allow you to explicitly specify a version of {product-name} in one of two ways:

1. Specify a specific Platform Release BOM
+
A xref:platform.adoc#quarkus-platform-bom[{product-name} Platform release BOM] is identified by `groupId:artifactId:version` (GAV) coordinates. When specifying a platform release BOM, you may use empty segments to fall back to default values (shown with `quarkus create app --help`). If you specify only one segment (no `:`), it is assumed to be a version.
+
...
The {product-name} CLI can be used to list {product-name} extensions.
...
- `--origins` Display concise information along with the {product-name} platform release origin of the extension.

The behavior of `quarkus ext ls` will vary depending on context.

===== Listing extensions for a Quarkus release

If you invoke the {product-name} CLI from outside of a project, {product-name} will list all the extensions available for the {product-name} release used by the CLI itself.

You can also list extensions for a specific release of {product-name} using `-P` or `-S`, as described in <<specifying-quarkus-version>>.

This mode uses the `--origins` format by default.

===== Listing extensions for a Quarkus project

When working with a {product-name} project, the CLI will list the extensions the current project has installed, using the `--name` format by default.

Use the `--installable` or `-i` option to list extensions that can be installed from the {product-name} platform the project is using.
...
The {product-name} CLI can add one or more extensions to your project with the `add`
subcommand:
...
The {product-name} CLI can remove one or more extensions from your project with the `remove`/`rm`
subcommand:
...
To build your project using the {product-name} CLI (using the default configuration in this example),
...
To start dev mode from the {product-name} CLI, use the `dev` command:
...
2021-05-27 10:15:56,032 INFO  [io.quarkus] ({product-name} Main Thread) code-with-quarkus 1.0.0-SNAPSHOT on JVM (powered by {product-name} 999-SNAPSHOT) started in 1.387s. Listening on: http://localhost:8080
2021-05-27 10:15:56,035 INFO  [io.quarkus] ({product-name} Main Thread) Profile dev activated. Live Coding activated.
2021-05-27 10:15:56,035 INFO  [io.quarkus] ({product-name} Main Thread) Installed features: [cdi, resteasy-reactive, smallrye-context-propagation]
...
The {product-name} CLI allows building container images without tampering with your project configuration (adding / removing container image extensions).
...
Each subcommand corresponds to an image building tool supported by {product-name} and gives access to specific configuration options.
...
The {product-name} CLI embeds a plugin system that can be used to dynamically add commands and subcommands to the CLI.
...
A {product-name} extension may define a list of `cli-plugins` as part of its metadata.
