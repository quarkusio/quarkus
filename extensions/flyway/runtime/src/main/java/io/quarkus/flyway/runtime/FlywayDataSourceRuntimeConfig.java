...
public interface FlywayDataSourceRuntimeConfig {
     ...
     * <li>This schema will be the default for the database connection (provided the database supports this
     * concept).</li>
     * </ul>
     */
    ...
    /**
     * The username that Flyway uses to connect to the database. If no specific JDBC URL is configured, falls back to
     * the datasource username if not specified.
     */
    Optional<String> username();

    /**
     * The password that Flyway uses to connect to the database. If no specific JDBC URL is configured, falls back to
     * the datasource password if not specified.
     */
    Optional<String> password();

    /**
     * Comma-separated case-sensitive list of schemas managed by Flyway. The first schema in the list will be
     * automatically set as the default one during the migration. It will also be the one containing the schema history
     * table.
     */
    Optional<List<String>> schemas();

    /**
     * The name of Flyway's schema history table. By default (single-schema mode), the schema history table is placed in
     * the default schema for the connection provided by the datasource. When the flyway.schemas property is set
     * (multi-schema mode), the schema history table is placed in the first schema of the list.
     */
    ...
    /**
     * true to execute Flyway baseline before migrations This flag is ignored if the flyway_schema_history table exists
     * in the current schema or if the current schema is empty. Note that this will not automatically call migrate, you
     * must either enable baselineAtStart or programmatically call flyway.migrate().
     */
    @WithDefault("false")
    boolean baselineOnMigrate();

    /**
     * true to execute Flyway baseline automatically when the application starts. This flag is ignored if the
     * flyway_schema_history table exists in the current schema. This will work even if the current schema is empty.
     */
    @WithDefault("false")
    ...
    /**
     * Ignore missing migrations when reading the history table. When set to true migrations from older versions present
     * in the history table but absent in the configured locations will be ignored (and logged as a warning), when false
     * (the default) the validation step will fail.
     */
    @WithDefault("false")
    boolean ignoreMissingMigrations();

    /**
     * Ignore future migrations when reading the history table. When set to true migrations from newer versions present
     * in the history table but absent in the configured locations will be ignored (and logged as a warning), when false
     * (the default) the validation step will fail.
     */
    @WithDefault("false")
     ...
     * https://flywaydb.org/documentation/configuration/parameters/ignoreMigrationPatterns for more information). When
     * this configuration is set, the ignoreFutureMigrations and ignoreMissingMigrations settings are ignored. Patterns
     * are comma separated.
     */
