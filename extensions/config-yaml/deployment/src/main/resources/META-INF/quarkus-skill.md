...
`application.properties` and `application.yaml` can exist simultaneously. Properties from both are merged, with `application.properties` taking precedence for duplicate keys.

### Common Pitfalls

- **Profile prefix needs quotes**: `%dev` must be `"%dev"` in YAML — without quotes, YAML parsers may reject the file.
- **Precedence**: `application.properties` overrides `application.yaml` for the same key. System properties and environment variables override both.
