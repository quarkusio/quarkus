    ...
    AnnotationsTransformerBuildItem transformObservedAnnotations() {
        return new AnnotationsTransformerBuildItem(new AnnotationTransformation() {
            ...
            public void apply(TransformationContext ctx) {
                ...
                // When the method has no @Observed of its own, it may inherit the binding from a
                // class-level @Observed. In that case we synthesize a per-method @Observed so that
                // each intercepted business method gets its own name and contextualName defaults.
                if (observed == null) {
                    observed = method.declaringClass().declaredAnnotation(OBSERVED);
                    if (observed == null) {
                        return;
                    }
                    // Only augment CDI business methods (skip constructors, static, private and synthetic).
                ...
                if (kvVal != null && kvVal.asStringArray().length > 0) {
                    ...
                    // Fail the build. The values are flattened key/value pairs, so an odd count means a key without a
                    // value
                    if (kvs.length % 2 != 0) {
                        throw new IllegalStateException(
                                "@Observed.lowCardinalityKeyValues on " + method.declaringClass().name() + "#"
                                        + method.name() + " must contain an even number of values (key/value pairs), but "
                                        + kvs.length + " were provided: " + Arrays.toString(kvs));
                    }
