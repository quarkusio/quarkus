...
public class QuarkusTestExtension extends AbstractJvmQuarkusTestExtension
        ...
        BeforeAllCallback, InvocationInterceptor, AfterAllCallback, ParameterResolver {

    // Be very careful with static variables; they will not be as global as one might think, since tests can be loaded
    // with many different classloaders
    ...
    private ExtensionState doJavaStart(ExtensionContext context, Class<? extends QuarkusTestProfile> profile)
            throws Throwable {
        ...
        try {
            ...
            testResourceManager = TestResourceUtil.TestResourceManagerReflections.createReflectively(
                    testResourceManagerClass, requiredTestClass, profile,
                    ...
                    Optional.ofNullable(startupAction.getDevServicesNetworkId()), testClassLocation);
            ...
            Closeable shutdownTask = new Closeable() {
                ...
                public void close() {
                    ...
                    try {
                        // In a nested class with no tests in the outer profile, the running Quarkus application could
                        // be null
        ...
        try {
            ...
            if (requiredTestClass.getClassLoader().getName().contains("QuarkusClassLoader")) {
                throw new RuntimeException("Internal error. The test class " + requiredTestClass
                        ...
                        + QuarkusClassLoader.class.getClassLoader() + " but was " + requiredTestClass.getClassLoader()
    ...
    private Throwable determineEffectiveException(Throwable e) {
        ...
        if ((e instanceof InvocationTargetException) && (e.getCause() != null)) { // QuarkusTestResourceLifecycleManager.start
                                                                                  // is called reflectively
            effectiveException = e.getCause();
            if ((effectiveException instanceof CompletionException) && (effectiveException.getCause() != null)) { // can
                                                                                                                  // happen
                                                                                                                  // because
                                                                                                                  // instances
                                                                                                                  // of
    ...
    public static String getEndpointPath(ExtensionContext context,
        ...
        if (testHTTPEndpoint != null) {
            ...
            if (endpointPath == null) {
                throw new RuntimeException("Cannot determine HTTP path for endpoint " + value + " for test method "
    ...
    private QuarkusTestExtensionState ensureStarted(ExtensionContext extensionContext) {
        ...
        // TODO if classes are misordered, say because someone overrode the ordering, and there are profiles or
        // resources,
    ...
    public <T> T interceptTestClassConstructor(Invocation<T> invocation,
        ...
        if (currentTestClassStack.size() > 1 && currentTestClassStack.contains(requiredTestClass)
    ...
    private Object runExtensionMethod(ReflectiveInvocationContext<Method> invocationContext,
        ...
        try {
            ...
            // this is needed to support before*** and after*** methods that are part of class that encloses the test
            // class
     ...
     * pulled from Arc, which of course will already have its constructor parameters properly resolved // TODO this
     * comment is probably wrong, we do use the class instance which is passed in
     */
    @Override
