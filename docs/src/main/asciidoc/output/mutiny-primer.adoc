...
It is the primary model to write reactive applications with {product-name-first}.
...
Mutiny is the primary API when dealing with the reactive features from {product-name}.
...
These integrations make Mutiny a prominent and cohesive model for every reactive application developed with {product-name}.
In addition, Mutiny architecture allows fine-grain dead-code elimination which improves the memory usage when compiled to native (such as with {product-name} native mode or GraalVM native image compiler).

== Why another reactive programming API?

Seasoned reactive developers may wonder why {product-name} introduced yet another reactive programming APIs while there are existing ones.
...
**Non-Blocking I/O** - Mutiny is the perfect companion to tame the asynchronous nature of applications with non-blocking I/O (which powers xref:quarkus-reactive-architecture.adoc[{product-name}]).
...
{product-name} is powered by a xref:quarkus-reactive-architecture.adoc#engine[reactive engine], and when developing a reactive application, your code is executed on one of the few I/O threads.
...
Many {product-name} extensions expose Mutiny APIs. In this section, we use the MongoDB extension to illustrate how to use Mutiny.
...
In {product-name} some extensions deal with the subscription for you.
...
Vert.x is an essential part of {product-name}, as it provides its reactive capabilities.

In addition, the whole Vert.x API can be used with {product-name}.
...
The integration of Mutiny in {product-name} goes beyond just the library.
Mutiny exposes hooks that allow {product-name} and Mutiny to be closely integrated:

* Calling `await` or `toIterable` would fail if you are running on an I/O thread, preventing blocking the I/O thread;
* The `log()` operator use the {product-name} logger;
* The default Mutiny thread pool is the {product-name} worker thread pool;
