...
public abstract class ProxyTestBase {
    ...
    public void setUp() {
        ...
        proxyServer1.stubFor(WireMock.get(urlMatching("/.*")).willReturn(aResponse()
                .proxiedFrom("http://localhost:" + appUri.getPort()).withAdditionalRequestHeader("X-Via", PROXY_8181)));
        proxyServer1.start();
        proxyServer2 = new WireMockServer(8182);
        proxyServer2.stubFor(WireMock.get(urlMatching("/.*")).willReturn(aResponse()
                .proxiedFrom("http://localhost:" + appUri.getPort()).withAdditionalRequestHeader("X-Via", PROXY_8182)));
        proxyServer2.start();
        authenticatedProxyServer = new WireMockServer(8183);
        authenticatedProxyServer.stubFor(
                WireMock.get(urlMatching("/.*")).withHeader("Proxy-Authorization", equalTo("Basic YWRtaW46cjAwdA=="))
