...
[NOTE]
====
If you are planning to run the application in a native image mode and parse the documents that may have been created with the charsets other than the standard ones supported in Java such as `UTF-8` then you should configure Quarkus Maven Plugin to get the native image generator include all the charsets available to JVM:
[source,xml]
----
<plugin>
