...
import static io.quarkus.core.actionmaker.Util.ord;
import static io.quarkus.core.actionmaker.Util.plural;
 ...
 * Generates:
 * <ul>
 * <li>{@code Action0} through <code>Action&lt;n&gt;</code> - functional interfaces for typed service actions</li>
 * <li>{@code VoidAction0} through <code>VoidAction&lt;n&gt;</code> - functional interfaces for void service actions</li>
 * <li>{@code ServiceBuilder0} through <code>ServiceBuilder&lt;n-1&gt;</code> - type-safe builder chain interfaces</li>
 * <li>{@code VoidServiceBuilder0} through <code>VoidServiceBuilder&lt;n-1&gt;</code> - builder chain interfaces for void
 * services</li>
 * </ul>
 */
public final class MakeServiceTypes {
     ...
     */
    private static final int MAX_PCNT = 25;
    ...
    public static void main(String[] argsArray) throws IOException {
        ...
        Path outputDir = Path.of(args.getFirst());
        ...
        for (int i = 0; i <= MAX_PCNT; i++) {
            ...
            sources.createSourceFile(PKG, className, sf -> {
                ...
                sf.interface_(className, cc -> {
                    ...
                    Type tee = cc.typeParam("T", tpc -> tpc.docComment(dcc -> dcc.text("the return type of the action")));
                    for (int j = 0; j < cnt; j++) {
                        final int paramIdx = j + 1;
                        cc.typeParam("P" + paramIdx,
                                tpc -> tpc.docComment(dcc -> dcc.text("the type of the " + ord(paramIdx) + " parameter")));
                    }
                    cc.method("run", mc -> {
                        mc.docComment(dcc -> {
                        ...
                        for (int j = 0; j < cnt; j++) {
                            final int paramIdx = j + 1;
                            mc.param("p" + paramIdx, Type.named("P" + paramIdx), pc -> pc.docComment(dcc -> {
        ...
        for (int i = 0; i <= MAX_DEPS; i++) {
            ...
            sources.createSourceFile(PKG, className, sf -> {
                ...
                sf.interface_(className, cc -> {
                    ...
                    // T = service type
                    Type svcTypeParam = cc.typeParam("T", tpc -> tpc.docComment(dcc -> dcc.text("the service type")));
                    // P1..PN = dependency types
                    for (int j = 0; j < depCount; j++) {
                        final int paramIdx = j + 1;
                        cc.typeParam("P" + paramIdx,
                                tpc -> tpc.docComment(dcc -> dcc.text("the type of the " + ord(paramIdx) + " dependency")));
                    }
                    ...
                    if (depCount < MAX_DEPS) {
                        ...
                        cc.method("require", mc -> {
                            ...
                            Type pn1 = mc.typeParam("P" + nextDepCount,
                                    tpc -> tpc.docComment(dcc -> dcc.text("the type of the dependency to add")));
                            ...
                            mc.param("type", Type.of(Class.class).typeArg(pn1),
                                    pc -> pc.docComment(dcc -> dcc.text("the dependency type (must not be {@code null})")));
                        });
                        ...
                        cc.method("require", mc -> {
                            ...
                            mc.param("type", Type.of(Class.class).typeArg(pn1),
                                    pc -> pc.docComment(dcc -> dcc.text("the dependency type (must not be {@code null})")));
                            mc.param("name", Type.of(String.class),
                                    pc -> pc.docComment(dcc -> dcc.text("the dependency name (must not be {@code null})")));
                        });
                    }
                    ...
                    // optional() - retype T to Optional<T> so the terminal action returns Optional<T>
                    {
                        Type[] optTypeArgs = new Type[depCount + 1];
                        optTypeArgs[0] = Type.of(Optional.class).typeArg(svcTypeParam);
                        for (int j = 0; j < depCount; j++) {
                            optTypeArgs[j + 1] = Type.named("P" + (j + 1));
                        }
                        Type optReturn = Type.named(className).typeArg(optTypeArgs);
                        cc.method("optional", mc -> {
                            mc.docComment(dcc -> {
                                dcc.text("Declare that this service may produce no value at run time. ");
                                dcc.text("The terminal {@code action} is retyped to return {@code Optional<T>}: an empty ");
                                dcc.text("result marks the service as present-but-valueless (skipped by {@code consumeAll()} ");
                                dcc.text("consumers and delivered as an empty optional to {@code request()} consumers) ");
                                dcc.text("rather than being treated as a startup failure. May only be called once.");
                                dcc.return_(rc -> rc
                                        .text("this builder, retyped so the action returns {@code Optional<T>}"));
                            });
                            mc.returning(optReturn);
                        });
                    }
                    ...
                    cc.method("action", mc -> {
                        ...
                        mc.param("action", Type.named("Action" + actionArity).typeArg(actionTypeArgs),
                                pc -> pc.docComment(dcc -> dcc.text("the service action (must not be {@code null})")));
                    });
            ...
            sources.createSourceFile(PKG, className, sf -> {
                ...
                sf.interface_(className, cc -> {
                    ...
                    cc.method("action", mc -> {
                        ...
                        mc.param("action", Type.named("VoidAction" + actionArity).typeArg(actionTypeArgs),
                                pc -> pc.docComment(dcc -> dcc.text("the service action (must not be {@code null})")));
                    });
    ...
    private static void generateAfterInterfaceMethods(
            InterfaceCreator cc,
            Type selfType) {
        // after(Enum<?> enumConstant)
        cc.method("after", mc -> {
            mc.docComment(dcc -> {
                dcc.text("Add an ordering-only dependency on an enum constant service.");
                dcc.return_(rc -> rc.text("a builder with the added dependency"));
            });
            mc.returning(selfType);
            mc.param("enumConstant", Type.of(Enum.class).typeArg(Type.WILDCARD),
                    pc -> pc.docComment(dcc -> dcc.text("the enum constant (must not be {@code null})")));
        });
        // after(String name)
        cc.method("after", mc -> {
            mc.docComment(dcc -> {
            ...
            mc.returning(selfType);
            mc.param("name", Type.of(String.class),
                    pc -> pc.docComment(dcc -> dcc.text("the void service name (must not be {@code null})")));
        });
        ...
        cc.method("after", mc -> {
            ...
            mc.returning(selfType);
            mc.param("nameParts", Type.of(List.class).typeArg(Type.of(String.class)),
                    pc -> pc.docComment(dcc -> dcc.text("the void service name parts (must not be {@code null})")));
        });
        // after(Class<?> type)
        cc.method("after", mc -> {
            mc.docComment(dcc -> {
            ...
            mc.returning(selfType);
            mc.param("type", Type.of(Class.class).typeArg(Type.WILDCARD),
                    pc -> pc.docComment(dcc -> dcc.text("the service type (must not be {@code null})")));
        });
        ...
        cc.method("after", mc -> {
            ...
            mc.param("type", Type.of(Class.class).typeArg(Type.WILDCARD),
                    pc -> pc.docComment(dcc -> dcc.text("the service type (must not be {@code null})")));
            mc.param("name", Type.of(String.class),
                    pc -> pc.docComment(dcc -> dcc.text("the service name (must not be {@code null})")));
        });
        ...
        cc.method("after", mc -> {
            ...
            mc.param("type", Type.of(Class.class).typeArg(Type.WILDCARD),
                    pc -> pc.docComment(dcc -> dcc.text("the service type (must not be {@code null})")));
            mc.param("nameParts", Type.of(List.class).typeArg(Type.of(String.class)),
                    pc -> pc.docComment(dcc -> dcc.text("the service name parts (must not be {@code null})")));
        });
    }
    ...
    private static void generateBeforeInterfaceMethods(
            InterfaceCreator cc,
            Type selfType) {
        // before(Enum<?> enumConstant)
        cc.method("before", mc -> {
            mc.docComment(dcc -> {
                dcc.text("Declare that the given enum constant service should start after this service and stop before it.");
    ...
    private static void generateAfterBuildItemInterfaceMethod(
        ...
        cc.method("afterBuildItem", mc -> {
            ...
            mc.param("buildItemClass", Type.of(Class.class).typeArg(
                    ...
                    pc -> pc.docComment(dcc -> dcc.text("the build item class (must not be {@code null})")));
        });
    }
    ...
    private static void generateAtPhaseInterfaceMethod(
            ...
            Type selfType) {
        Type phaseType = Type.named("io.quarkus.core.Phase");
        ...
        cc.method("atPhase", mc -> {
            ...
            mc.returning(selfType);
            mc.param("phase", phaseType, pc -> pc.docComment(dcc -> dcc.text("the runtime phase (must not be {@code null})")));
        });
    }
    ...
    private static void generateServiceBuilderImplsParameterized(
        ...
        for (int i = 0; i <= MAX_DEPS; i++) {
            ...
            // action arity is depCount + 1 (context occupies the first slot)
            final int actionArity = depCount + 1;
            sources.createSourceFile(IMPL_PKG, className, sf -> sf.class_(className, cc -> {
                cc.public_();
                cc.final_();
                ...
                // optional() method impl - mark optional on the shared builder and retype T to Optional<T>
                {
                    Type[] optTypeArgs = new Type[depCount + 1];
                    optTypeArgs[0] = Type.of(Optional.class).typeArg(svcTypeParam);
                    for (int j = 0; j < depCount; j++) {
                    ...
                    Type optReturn = interfaceRawType.typeArg(optTypeArgs);
                    cc.method("optional", mc -> {
                        mc.public_();
                        mc.annotate(Type.of(Override.class));
                        mc.annotate(Type.of(SuppressWarnings.class), ac -> ac.value(Expr.str("unchecked")));
                        mc.returning(optReturn);
                        mc.body(b0 -> {
                            b0.emit($v("sbi").call("markOptional"));
                            b0.return_(Expr.THIS.cast(optReturn));
                        });
                    });
                }
    ...
    private static void generateAfterImplMethods(
        ...
        cc.method("after", mc -> {
            ...
            mc.annotate(Type.of(Override.class));
            mc.returning(selfType);
            Var enumConstant = mc.param("enumConstant", Type.of(Enum.class).typeArg(Type.WILDCARD));
            mc.body(b0 -> {
                b0.emit($v("sbi").call("after", enumConstant));
                b0.return_(Expr.THIS);
            });
        });
        // after(String name)
    ...
    private static void generateBeforeImplMethods(
        ...
        cc.method("before", mc -> {
            ...
            Var enumConstant = mc.param("enumConstant", Type.of(Enum.class).typeArg(Type.WILDCARD));
            mc.body(b0 -> {
                b0.emit($v("sbi").call("before", enumConstant));
                b0.return_(Expr.THIS);
            });
        });
        // before(String name)
    ...
    private static void generateConsumeAllInterfaceMethod(
        ...
        cc.method("consumeAll", mc -> {
            ...
            Type pn1 = mc.typeParam("P" + nextDepCount,
                    tpc -> tpc.docComment(dcc -> dcc.text("the type of the services to consume")));
            ...
            mc.returning(Type.named(nextBuilder).typeArg(nextTypeArgs));
            mc.param("type", Type.of(Class.class).typeArg(pn1),
                    pc -> pc.docComment(dcc -> dcc.text("the service type to consume (must not be {@code null})")));
        });
    }
