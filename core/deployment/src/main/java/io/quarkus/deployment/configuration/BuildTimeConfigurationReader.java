...
public final class BuildTimeConfigurationReader {
    ...
    final class ReadOperation {
        ...
        ReadResult run() {
            ...
            // TODO - This could be generated by the above iteration, but we need to do it before, because the ConfigBuilder.build is invoked before the ReadResult.run.
            // Think in a better alternative
            // Store in a field?
            String[] secretKeys = BuildTimeConfigurationReader.this.discoverSecretKeys();
            ...
            return new ReadResult(objectsByRootClass, specifiedRunTimeDefaultValues, buildTimeRunTimeVisibleValues,
                    ...
                    bootstrapRootsEmpty, secretKeys);
        }
    ...
    public String[] discoverSecretKeys() {
        List<String> secretKeys = new ArrayList<>();
        for (RootDefinition root : allRoots) {
            for (String memberName : root.getMemberNames()) {
                ClassDefinition.ClassMember member = root.getMember(memberName);
                Field field = member.getField();
                if (field.getType().equals(ClearSecret.class)) {
                    // TODO - Needs to recurse Groups and Maps
                    // Check - io.quarkus.deployment.steps.ConfigGenerationBuildStep.handleMembers
                    secretKeys.add("quarkus" + "." + root.getRootName() + "." + memberName);
                }
            }
        }
