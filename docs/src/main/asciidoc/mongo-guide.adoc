...
After this, the quarkus-mongo-client extension has been added to your `pom.xml`.
...
Now create a `org.acme.rest.json.FruitService` that will be the business layer of our application and store/load the fruits from the mongo database.
...
The main property to configure is the URL to access to MongoDB, almost all Mongo configuration can be put on the connection URI so we advise you to do so, you can find more information on the Mongo documentation site : https://docs.mongodb.com/manual/reference/connection-string/
...
As by default, `MongoClient` is configure to access a local Mongo database on port 27017 (the default Mongo port), if you have a local running database on this port, there is nothing more to do before being able to test it!
...
A reactive Mongo Client is included in Quarkus.
Using it is as easy as using the classic Mongo Client. You can rewrite the previous example to use it like the following.
...
One of the drawback of using directly the Mongo Client, is that you need to manually transform your objects to/from a Mongo `Document`.
To facilitate this there is two ways.
...
First you need to create a Bson `Codec` that will tells Bson how to transform your entity to/from a Mongo `Document`.
Here we use a `CollectibleCodec` as our object is retrievable from the database (it has a Mongo identifier), if not we would have used a `Codec` instead.
More information on the codec documentation : https://mongodb.github.io/mongo-java-driver/3.10/bson/codecs.
...
You can use the Mongo client in a native executable.
