...
Picocli provides different ways

> to package your application in such a way that end users can invoke it by its command name. https://picocli.info/#_packaging_your_application[Source]

If you want to distribute a single executable file that runs without installing a JDK see <<Native mode support>>. Notice that this executable will be platform specific.
...
Remember this requires a suitable JDK version to be installed. For a single executable file that runs without installing a JDK see <<Native mode support>>. For even more packaging options check the https://picocli.info/#_packaging_your_application[official Picocli documentation].
