...
Please see the <<token-propagation>> and <<jwt-access-tokens>> sections below for more information on how to configure Auth0 and Quarkus to have authorization code access tokens issued in the JWT format and propagated to service endpoints.
====
...
Please see the following <<token-propagation>> and <<jwt-access-tokens>> sections for a recommended approach of working with Auth0 access tokens.
====
