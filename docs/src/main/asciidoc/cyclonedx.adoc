...
The embedded SBOM additionally records the GraalVM/Mandrel `native-image` builder that produced the executable. A single component describing the builder distribution and version (for example `Mandrel`, `Oracle-GraalVM`, `GraalVM-CE` or `Liberica-NIK`) is added to both the top-level `components` array and, for CycloneDX 1.5 and later, `metadata.tools.components`.
Both entries use the same `purl`, but only the top-level component includes a `bom-ref`, keeping the builder's reference unique within the SBOM.
The top-level component has the following form:

[source,json]
----
{
  "type" : "library",
  "bom-ref" : "pkg:generic/Mandrel@25.0.4.1",
  "group" : "org.graalvm",
  "name" : "Mandrel",
  "version" : "25.0.4.1",
  "description" : "GraalVM native-image builder used to build the native executable",
  "purl" : "pkg:generic/Mandrel@25.0.4.1"
}
----

NOTE: The builder component is added at native image build time and is present *only* in the SBOM embedded in the executable (the `sbom` global symbol). The classpath resource and the SBOM served by the <<sbom-endpoint,REST endpoint>> are not modified, since they are produced before the `native-image` version is known.
