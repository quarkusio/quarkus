...
Here you can find a complete reference to all the available https://github.com/dekorateio/dekorate/blob/master/config.md[configuration options].
Please note that in the link above all property names are prefixed with `dekorate` which is not the case for this extension.
