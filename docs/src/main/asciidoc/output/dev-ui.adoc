...
.Dev UI v1
====
This guide covers the Dev UI v1, which has been replaced in {product-name-first} 3 with xref:dev-ui-v2.adoc[a new Dev UI].
...
{product-name} ships with a new experimental Dev UI, which is available in dev mode (when you start
...
[source,html]
----
<img src="{devRootAppend}/resources/images/quarkus_icon_rgb_reverse.svg" width="40" height="30" class="d-inline-block align-middle" alt="{product-name}"/>
----
...
<6> In order for your template to read custom information from your {product-name} extension, you can use
