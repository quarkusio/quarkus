...
By default, `SecurityIdentity` must be augmented with permissions which can be used to check if this identity passes `@PermissionAllowed` authorization restrictions.
...
public class ProjectResource {
    ...
    boolean canRenameProject(SecurityIdentity identity, String projectName) { <3>
