...
`io.quarkus.oidc.client.OidcClient` is a container of `OidcClient`s - it includes a default `OidcClient` (which can also be injected directly as described above) and named clients which can be configured like this:
...
If you need you can also create new `OidcClient` without having to configure it in the `application.properties`:
