...
If you prefer a programmatic configuration, the same mapping can be added with the `io.quarkus.vertx.http.security.event.HttpSecurity` CDI event:
...
Additionally, the `io.quarkus.vertx.http.security.event.HttpSecurity` CDI event can be used to configure specific authentication mechanisms and policies:
