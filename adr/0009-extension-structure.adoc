...
* Strongly coupled: An extension directly depends on another extension's runtime module (and its `build` on the other extension's `build` module), requiring it to be present.
...
NOTE: Because of the integration nature of the build items located in the `build-spi` module, we recommend keeping `build-spi` as the name.
