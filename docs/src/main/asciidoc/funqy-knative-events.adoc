...
Make sure you have the `stern` tool installed.  See the Knative Tutoriall setup for information on that.  Run stern
...
of `defaultChain` triggers an event that is posted to `configChain` which triggers an event posted to `annoatedChain` then
