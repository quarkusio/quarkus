...
= {project-name} - Using AMQP with Reactive Messaging

This guide demonstrates how your {project-name} application can utilize MicroProfile Reactive Messaging to interact AMQP.
 ...
 * The prices are written to a AMQP queue (prices). The AMQP configuration is specified in the
...
This stream is mapped to a AMQP queue using the `application.properties` file that we will create soon.
...
We need to configure AMQP connector. This is done in the `application.properties` file.
...
More details about this configuration is available on the https://smallrye.io/smallrye-reactive-messaging/#_interacting_using_amqp[SmallRye Reactive Messaging AMQP connector] documentation.
...
If you followed the instructions, you should have AMQP broker running.
