...
public Uni<Response> create(Fruit fruit) {
    return Panache.<Fruit>withTransaction(fruit::persist)
            .replaceWith(inserted -> Response.created(URI.create("/fruits/" + inserted.id)).build());
}
----
...
NOTE: The `.onItem().transform(...)` can be replaced with `.replaceWith(...)`, as `replaceWith` is a shortcut version.
