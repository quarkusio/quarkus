...
NOTE: the Ansible collection we just installed only supports RHEL, Fedora and other Linux distribution using RPMs. Ansible defines these as "RedHat family". Using the content on other platform (Windows, Debian, Ubuntu,...), while not impossible, will require a few adjustments.
...
NOTE: by default, the Ansible collection for Quarkus will install and use the OpenJDK 17 available in the Yum repositories of the target host (or the controller). If you want to use a different version of OpenJDK, define the following variable:
