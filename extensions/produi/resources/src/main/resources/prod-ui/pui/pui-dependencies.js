...
export class PuiDependencies extends LitElement {
    ...
    static styles = css`
        ...
        .badge.runtime { background: var(--lumo-success-color); color: white; }
        .badge.deployment { background: var(--lumo-contrast-40pct); color: white; }
    `;
    ...
    render() {
        if (!this._deps) {
            return html`<span>No dependency information available.</span>`;
        }
        return html`
            <div class="container">
                <div class="search">
                ...
                <vaadin-grid .items="${this._filtered}" class="grid" theme="no-border row-stripes">
                    ...
                    <vaadin-grid-sort-column auto-width header="Scope" path="type"></vaadin-grid-sort-column>
                </vaadin-grid>
            </div>`;
    }
