...
All three are **build-time properties** (fixed at build time), meaning they are resolved during the Quarkus augmentation phase.
...
|`quarkus.vertx.prefer-native-transport` icon:lock[title=Fixed at build time]
|`false`
...
|`quarkus.vertx.native-transport-required` icon:lock[title=Fixed at build time]
|`false`
|Enforce strict native transport loading. When `true` and the requested transport dependency is not on the classpath, the **build fails**. If the dependency is present but the native library cannot load at runtime (e.g., wrong platform), the application fails to start.
...
%dev.quarkus.vertx.prefer-native-transport=false

# Production: require io_uring on Linux
%prod.quarkus.vertx.native-transport-type=io-uring
%prod.quarkus.vertx.native-transport-required=true
----
...
** When `native-transport-required=true`: the build **fails** with a `ConfigurationException`.
** When `native-transport-required=false`: the build logs a warning and continues. The application will fall back to Java NIO at runtime.
* If `prefer-native-transport=true` but no native transport dependency is found, the build logs a warning.
