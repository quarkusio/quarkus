...
public interface KafkaStreamsRuntimeConfig {
     ...
     * {@code kafka.bootstrap.servers}, and if not set either use {@code localhost:9092}.
     */
    ...
    /**
     * A comma-separated list of topic names. The pipeline will only be started once all these topics are present in the
     * Kafka cluster and {@code ignore.topics} is set to false.
     */
    Optional<List<String>> topics();

    /**
     * A comma-separated list of topic name patterns. The pipeline will only be started once all these topics are
     * present in the Kafka cluster and {@code ignore.topics} is set to false.
     */
    Optional<List<String>> topicPatterns();

    /**
     * Timeout to wait for topic names to be returned from admin client. If set to 0 (or negative), {@code topics} check
     * is ignored.
     */
    @WithDefault("10S")
    ...
    /**
     * The schema registry key. Different schema registry libraries expect a registry URL in different configuration
     * properties. For Apicurio Registry, use {@code apicurio.registry.url}. For Confluent schema registry, use
    ...
    /**
     * The security protocol to use See
