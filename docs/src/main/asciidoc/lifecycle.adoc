...
Often an application requires initialization tasks to run before the actual application starts up (before the StartupEvent is fired).
Some extensions (e.g. xref:flyway.adoc[Flyway] and xref:liquibase.adoc[Liquibase]) perform such initialization tasks. There are cases
where users need provide custom initialization tasks.

=== User provided Initialization Tasks

User provided initialization tasks are `Runnable` annotated with the `@PreStart` annotation that indicates that
the run method should be executed as part of the application initialization. An example:
...
@PreStart  <1>
public class UserInitTask implements Runnable { <2>

    public void run() {
       //User inititalization code
    }
}
----
<1> The task is annotated with `@PreStart`
<2> The task implements Runnable


=== Using Initialization directly on methods
An alternative to annotating a `Runnable` class is to annotate a zero argument method. In this case the runnable class will be generated
behind the scenes and the run method will delegate to the method annotated.
...
----
<1> The method is annotated with `@PreStart`
Method with arguments or additional methods under a single class are ignored.

Both styles allow using dependency injection and therefore the user initialization code is able to use CDI beans provided by the user or extensions.

=== Exit after initialization

In some cases, users may choose only to run the initialization code. By setting the configuration options `quarkus.init-and-exit` to `true`,
the execution will stop right after initialization.
