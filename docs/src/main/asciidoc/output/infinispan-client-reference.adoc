...
Infinispan is a distributed, in-memory key/value store that provides {product-name-first} applications with a highly configurable
and independently scalable data layer.
This extension gives you client functionality that connects applications running on {product-name} with remote Infinispan clusters.
...
Run the following command in the base directory of your {product-name} project to add the `infinispan-client` extension:
...
the same {product-name} profile, `configuration-uri` gets the highest preference, over `configuration-resource` and `configuration`.
...
NOTE: In {product-name} the `schemaFileName` and `schemaFilePath` attributes should NOT be set on the `AutoProtoSchemaBuilder` annotation. Setting either attributes causes native runtime errors.
...
When a method annotated with `@CacheResult` is invoked, {product-name} will use the method argument as the cache key and check in the cache whether the method has been already invoked.
...
NOTE: Infinispan Client extension is not able yet to cache `null` values unlike the {product-name}-Cache extension.
...
You can use either the Query DSL or the Ickle Query language with the {product-name} Infinispan client
extension.
...
Infinispan also has a notion of counters and the {product-name} Infinispan client supports them out of
the box.

The {product-name} Infinispan client extension allows for Dependency Injection
...
feature was not tested in a {product-name} environment, and they may or may not work. Please let us
know if you need these added!

[[dev-services]]
== Dev Services for Infinispan

When you use the infinispan-client extension in dev mode or in test, {product-name} automatically starts an Infinispan server
...
{product-name} will share the Infinispan broker if you have multiple applications running in dev mode.
Dev Services for Infinispan implements a _service discovery_ mechanism for your multiple {product-name} applications running in _dev_ mode to share a single broker.
...
To start an Infinispan Server for your unit tests, {product-name} provides one `QuarkusTestResourceLifecycleManager` that relies on link:https://infinispan.org/docs/stable/titles/hotrod_java/hotrod_java.html#junit-testing[Infinispan Server Test Container].
...
For more information about the usage of a `QuarkusTestResourceLifecycleManager` please read xref:getting-started-testing.adoc#quarkus-test-resource[{product-name} test resource].
