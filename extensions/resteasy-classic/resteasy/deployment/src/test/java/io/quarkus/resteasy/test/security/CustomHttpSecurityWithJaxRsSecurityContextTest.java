...
public class CustomHttpSecurityWithJaxRsSecurityContextTest {
    ...
    public void testAugmentedIdentityInSecurityContext() {
        ...
        RestAssured.given().auth().preemptive().basic("user", "user").get("/custom-policy/is-admin").then()
                .statusCode(200).body(Matchers.is("true"));
        RestAssured.given().auth().preemptive().basic("test", "test").get("/custom-policy/is-admin").then()
                .statusCode(200).body(Matchers.is("false"));
    }
