...
:summary: This guide explains how to best measure the footprint of a {product-name-first} application.

This guide covers:

* how we measure memory usage
* how we measure startup time
* which additional flags will {product-name} apply to native-image by default
...
When measuring the footprint of a {product-name} application, we measure https://en.wikipedia.org/wiki/Resident_set_size[Resident Set Size (RSS)]
...
10:57:32.508
10:57:32.512
2019-04-05 10:57:32,512 INFO  [io.quarkus] (main) {product-name} 0.11.0 started in 0.002s. Listening on: http://127.0.0.1:8080
...
When {product-name} invokes GraalVM `native-image` it will apply some additional flags by default.
...
{product-name} disables this feature by setting `-H:FallbackThreshold=0`: this will ensure you get a compilation failure rather
...
Isolates are a neat feature of GraalVM, but {product-name} isn't using them at this stage.

Disable via `-H:-SpawnIsolates`.

=== Disable auto-registration of all Service Loader implementations

{product-name} extensions can automatically pick the right services they need, while GraalVM's native-image defaults to include
...
If you're to investigate some differences in detail make sure to check what {product-name} is invoking exactly: when the build
