...
The solution is located in the `elytron-security-jdbc` {quickstarts-tree-url}/elytron-security-jdbc[directory].
...
mvn io.quarkus:quarkus-maven-plugin:{quarkus-version}:create \
    ...
    -DprojectArtifactId=elytron-security-jdbc \
    -Dextensions="elytron-security-jdbc, jdbc-h2, resteasy"
----

[NOTE]
====
Don't forget to add the database connector library of choice. Here we are using H2 as identity store.
====
...
[source,properties]
----
quarkus.datasource.url=jdbc:h2:mem:elytron-security-jdbc;DB_CLOSE_DELAY=-1;DB_CLOSE_DELAY=-1;INIT=RUNSCRIPT FROM 'classpath:import.sql' <1>
quarkus.datasource.driver=org.h2.Driver
quarkus.datasource.username=sa
quarkus.datasource.password=sa
----

In our context, we are using H2 in-memory mode as identity store and we load `import.sql` to init the database with users and roles.
