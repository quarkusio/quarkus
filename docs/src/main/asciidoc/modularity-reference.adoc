...
== `AddedDependencyBuildItem`

`AddedDependencyBuildItem` is a `MultiBuildItem` produced by extensions.
...
----
@BuildStep
AddedDependencyBuildItem extraDependency() {
    return new AddedDependencyBuildItem(
...
These are used with `AddedDependencyBuildItem` to control how the dependency is wired.
