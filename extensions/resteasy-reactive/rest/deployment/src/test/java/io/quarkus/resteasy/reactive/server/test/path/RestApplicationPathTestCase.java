...
public class RestApplicationPathTestCase {
    ...
    static QuarkusUnitTest test = new QuarkusUnitTest().withConfigurationResource("empty.properties")
            ...
            .withApplicationRoot((jar) -> jar.addClasses(HelloResource.class, BarApp.class, BaseApplication.class));

    /**
     * Using @ApplicationPath will overlay/replace `quarkus.rest.path`. Per spec: <quote> Identifies the application
     * path that serves as the base URI for all resource URIs provided by Path. May only be applied to a subclass of
     * Application. </quote> This path will also be relative to the configured HTTP root
     */
    @ApplicationPath("/bar")
