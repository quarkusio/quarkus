...
The `io.quarkus.vertx.http.security.CORS` builder allows you to configure complete CORS filter configuration.
For example, here is how you can specify allowed origins:
...
public class CorsProgrammaticConfig {
    void configure(@Observes HttpSecurity httpSecurity) {
        httpSecurity.cors(CORS.origins("https://example.com", "https://example.io"));
    }
}
----
