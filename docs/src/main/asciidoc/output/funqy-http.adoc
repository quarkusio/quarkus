...
HTTP features like cache-control and conditional GETs.  You may want to consider using {product-name-first}'s
...
To write Funqy HTTP functions, simply include the `quarkus-funqy-http` dependency into your {product-name} `pom.xml` file:
...
This starts your functions in {product-name} dev mode.
