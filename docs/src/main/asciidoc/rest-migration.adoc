...
NOTE: Users migrating multipart resources to Quarkus REST should be aware of the configuration parameter `quarkus.http.limits.max-form-attribute-size`, as this poses an upper limit to the size of each non-file part.
File parts are bounded by `quarkus.http.limits.max-body-size` and, when set, by `quarkus.http.limits.max-multipart-file-size`.
Any request with a part size exceeding these configuration values will result in HTTP status code 413.
