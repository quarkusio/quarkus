...
In addition to the configuration properties listed above, Quarkus supports programmatic set up during the runtime as in the example below:
...
public class FormConfiguration {
    ...
    void configure(@Observes Form form) { <1>
        form
                .enable()
                .httpOnlyCookie()
...
----
<1> Observe the `io.quarkus.vertx.http.security.event.Form` CDI event and customize the configuration provided in the `application.properties` file.
