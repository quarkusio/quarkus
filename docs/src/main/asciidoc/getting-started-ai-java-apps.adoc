...
Create the `AiPoemService.java` interface `src/main/java/org/acme/AiPoemService.java`:
...
Create the `Poems.java` class `src/main/java/org/acme/Poems.java`:
...
You then need only set the API key and the base URL properties for the LLM; in this case, you can use the value `demo` to get limited demo access to the LLM which is sufficient for this application in `src/main/resources/application.properties`:
