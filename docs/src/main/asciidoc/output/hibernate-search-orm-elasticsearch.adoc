...
If you already have your {product-name-first} project configured, you can add the `hibernate-search-orm-elasticsearch` extension
...
to tell {product-name} it should be used by Hibernate Search in the default persistence unit.
+
...
(and configure {product-name} to use it, more on that later).
...
----
<1> Annotate the configurer implementation with the `@SearchExtension` qualifier
to tell {product-name} it should be used in the default persistence unit, for all Elasticsearch indexes (by default).
+
The annotation can also target a specific persistence unit (`@SearchExtension(persistenceUnit = "nameOfYourPU")`),
...
As usual, we can configure everything in the {product-name} configuration file, `application.properties`.
...
which means {product-name} will start a PostgreSQL database and Elasticsearch cluster automatically.
In production mode, however,
you will want to start a PostgreSQL database and Elasticsearch cluster manually,
which is why we provide {product-name} with this connection info in the `prod` profile (`%prod.` prefix).
...
{product-name} supports a feature called Dev Services that allows you to start various containers without any config.
In the case of Elasticsearch this support extends to the default Elasticsearch connection.
What that means practically, is that if you have not configured `quarkus.hibernate-search-orm.elasticsearch.hosts` {product-name} will automatically
 ...
 * start your {product-name} application with:
+
...
In the context of {product-name} and to build microservices, we thought the latter would make more sense.
Thus, we focused our efforts on it.

We don't have plans to support the Lucene backend in {product-name} for now.
