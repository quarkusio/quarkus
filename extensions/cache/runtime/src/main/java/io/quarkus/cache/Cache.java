...
public interface Cache {
     ...
     * @throws NullPointerException if the key is {@code null}
     * @throws CacheException if an exception is thrown during a cache value computation
     * @throws UnsupportedOperationException if per-item expiration is not supported by this cache implementation
     *         and {@code expiresAfter} is not {@code null}
     */
    @CheckReturnValue
    default <K, V> Uni<V> get(K key, Function<K, V> valueLoader, Duration expiresAfter) {
        if (expiresAfter != null) {
            throw new UnsupportedOperationException(
                    "Per-item expiration is not supported by this cache implementation");
        }
