...
public class StaticResourcesDevModeTest extends AbstractStaticResourcesTest {
    ...
    @RegisterExtension
    final static QuarkusDevModeTest test = new QuarkusDevModeTest().withApplicationRoot(
    ...
    void shouldChangeContentOnModification() {
        RestAssured.when().get("/static-file.html").then()
                .body(Matchers.containsString("This is the title of the webpage!")).statusCode(200);
        RestAssured.when().get("/").then().body(Matchers.containsString("This is the title of the webpage!"))
                .statusCode(200);
        test.modifyResourceFile("META-INF/resources/static-file.html", s -> s.replace("webpage", "Andy"));
        RestAssured.when().get("/static-file.html").then()
                .body(Matchers.containsString("This is the title of the Andy!")).statusCode(200);
        RestAssured.when().get("/").then().body(Matchers.containsString("This is the title of the webpage!"))
                .statusCode(200);
    }
}
