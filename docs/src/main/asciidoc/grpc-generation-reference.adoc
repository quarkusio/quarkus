...
IMPORTANT: In `application.properties`, the `:` between the group id and the artifact id must be escaped as `\:`, as in the example above.
An unescaped `:` ends the key in the properties format, so the include or exclude patterns would not apply to the dependency.
The code generation fails with a message pointing at the property when it finds such a key.
