...
Please refer to xref:virtual-threads.adoc[Writing simpler reactive REST services with {product-name-first} Virtual Thread support]
to read more about Java virtual threads in general and the {product-name} Virtual Thread support.
====

By default, the {product-name} gRPC extension invokes service methods on an event-loop thread.
...
The idea behind {product-name} Virtual Thread support for gRPC services is to offload the service method invocation on virtual threads, instead of running it on an event-loop thread or a worker thread.
...
or to use the {product-name} Dev mode, insert the following to the `quarkus-maven-plugin` configuration:
