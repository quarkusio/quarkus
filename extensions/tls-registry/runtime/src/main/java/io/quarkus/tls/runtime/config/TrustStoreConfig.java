...
public interface TrustStoreConfig {
    ...
    /**
     * Enforce certificate expiration. When enabled, the certificate expiration date is verified and the certificate (or
     * any certificate in the chain) is rejected if it is expired.
     */
    @WithDefault("WARN")
    ...
    /**
     * The credential provider configuration for the trust store. A credential provider offers a way to retrieve the
     * trust store password. Note that the credential provider is only used if the password is not set in the
     * configuration.
     */
