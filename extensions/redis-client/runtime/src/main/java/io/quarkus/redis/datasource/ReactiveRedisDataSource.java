 ...
 * <p>
 * This class provides access to various <em>groups of methods</em>. Each method execute a Redis {@code command}. Groups
 * and methods are type-safe. The decomposition follows the Redis API group.
 * <p>
...
public interface ReactiveRedisDataSource {
    ...
    /**
     * Retrieves a {@link ReactiveRedisDataSource} using a single connection with the Redis Server. The connection is
     * acquired from the pool and released when the {@code Uni} returned by {@code function} produces a {@code null}
     * item or a failure.
     *
     * @param function
     *        the function receiving the single-connection data source and producing {@code null} when the
     *        connection can be released.
     */
    Uni<Void> withConnection(Function<ReactiveRedisDataSource, Uni<Void>> function);

    /**
     * Retrieves a {@link RedisDataSource} enqueuing commands in a Redis Transaction ({@code MULTI}). Note that
     * transaction acquires a single connection, and all the commands are enqueued in this connection. The commands are
     * only executed when the passed block emits the {@code null} item.
     * <p>
     * The results of the commands are retrieved using the produced {@link TransactionResult}.
     * <p>
     * The user can discard a transaction using the {@link TransactionalRedisDataSource#discard()} method. In this case,
     * the produced {@link TransactionResult} will be empty.
     *
     * @param tx
     *        the consumer receiving the transactional redis data source. The enqueued commands are only executed at
     ...
     *        the end of the block.
     * @param watchedKeys
     *        the keys to watch during the execution of the transaction. If one of these key is modified before the
     *        completion of the transaction, the transaction is discarded.
     */
    Uni<TransactionResult> withTransaction(Function<ReactiveTransactionalRedisDataSource, Uni<Void>> tx,
            String... watchedKeys);
     ...
     * </pre>
     * <p>
     * The {@code preTxBlock} returns a {@link Uni Uni&lt;I&gt;}. The produced value is received by the {@code tx}
     * block, which can use that value to execute the appropriate operation in the transaction. The produced value can
     * also be retrieved from the produced {@link OptimisticLockingTransactionResult}. Commands issued in the
     * {@code preTxBlock } must used the passed (single-connection) {@link ReactiveRedisDataSource} instance.
     * <p>
     * If the {@code preTxBlock} throws an exception or emits a failure, the transaction is not executed, and the
     * returned {@link OptimisticLockingTransactionResult} is empty.
     * <p>
     * This construct allows implementing operation relying on optimistic locking. The results of the commands are
     * retrieved using the produced {@link OptimisticLockingTransactionResult}.
     * <p>
     * The user can discard a transaction using the {@link TransactionalRedisDataSource#discard()} method. In this case,
     * the produced {@link OptimisticLockingTransactionResult} will be empty.
     *
     * @param tx
     *        the consumer receiving the transactional redis data source. The enqueued commands are only executed at
     *        the end of the block.
    ...
    /**
     * Execute the command <a href="https://redis.io/commands/select">SELECT</a>. Summary: Change the selected database
     * for the current connection Group: connection Requires Redis 1.0.0
     * <p>
    ...
    /**
     * Execute the command <a href="https://redis.io/commands/flushall">FLUSHALL</a>. Summary: Remove all keys from all
     * databases Group: server Requires Redis 1.0.0
     **/
    Uni<Void> flushall();
     ...
     *        the class of the keys
     * @param fieldType
     *        the class of the fields included in the message exchanged on the streams
     * @param valueType
     *        the class of the values included in the message exchanged on the streams
     * @param <K>
     *        the type of the redis key
     * @param <F>
     *        the type of the fields (map's keys)
     ...
     *
     * @param <V>
     *        the type of the values added into the Bloom filter
     *
