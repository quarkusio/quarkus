...
:summary: While you are encouraged to use the Scheduler or Quartz extensions to schedule tasks, {product-name-first} provides a compatibility layer for Spring Scheduled in the form of the spring-scheduled extension.

While users are encouraged to use xref:scheduler.adoc#standard-scheduling[regular {product-name} scheduler], {product-name} provides a compatibility layer for Spring Scheduled in the form of the `spring-scheduled` extension.

This guide explains how a {product-name} application can leverage the well known Spring Scheduled annotation to configure and schedule tasks.
...
If you already have your {product-name} project configured, you can add the `spring-scheduled` extension
...
<3> Use the Spring `@Scheduled` annotation with a cron-like expression to instruct {product-name} to schedule this method run.  In this example we're scheduling a task to be executed at 10:15am every day.
...
[source,properties]
----
# The syntax used by Spring for cron expressions is the same as which is used by regular {product-name} scheduler.
cron.expr=*/5 * * * * ?
fixedRate.expr=1000
----
...
{product-name} supports the use of property expressions in the `application.properties` file so to externalize the configuration of the tasks you should store the properties in the `application.properties` file and use the
...
{product-name} currently only supports a subset of the functionalities that Spring @Scheduled provides with more features being planned.
