...
final class ServiceBuilderImpl<T> {
    ...
    /**
     * Per-build-step record of which services have been defined and in which phase, shared with the
     * owning {@link ActionBuilderImpl} and consulted by {@link ActionBuilderImpl#getRecorderProxy(Class, String)}.
     * The mapping is three-state, keyed by service key:
     * <ul>
     * <li>absent ({@code get} returns {@code null}) — no {@code action()}/{@code actionAsync()} has been
     * called for this service key in this build step; requesting a recorder proxy for it is an error</li>
     * <li>{@code Boolean.TRUE} — the service is defined and static-init; its recorder proxy must report
     * {@code __static$$init() == true} and its value must be retained across the phase boundary</li>
     * <li>{@code Boolean.FALSE} — the service is defined and runtime; its recorder proxy reports
     * {@code __static$$init() == false} and needs no cross-phase retention</li>
     * </ul>
     * Entries are inserted by {@link #doAction} once the phase is final, so consumers must call
     * {@code getRecorderProxy()} after the corresponding {@code action()}.
     */
    private final Map<String, Boolean> definedServiceStaticInit;
     ...
     * @param definedServiceStaticInit per-build-step map recording each defined service key's phase
     *        ({@code true} for static-init); populated at {@link #doAction} time and consulted by
     *        {@link ActionBuilderImpl#getRecorderProxy(Class, String)}
     */
    ServiceBuilderImpl(
            Class<T> serviceType,
            ...
            Map<String, Boolean> definedServiceStaticInit) {
        ...
        this.definedServiceStaticInit = definedServiceStaticInit;
    }
    ...
    /**
     * Add an optional ordering-only dependency on an enum constant service.
     ...
     *
     * @param enumConstant the enum constant (must not be {@code null})
     */
    public void after(Enum<?> enumConstant) {
        Assert.checkNotNullParam("enumConstant", enumConstant);
        dependencies
                .add(new Dependency(enumConstant.getDeclaringClass(), List.of(enumConstant.name()), Dependency.FL_OPTIONAL));
    }
    ...
    public void after(List<String> nameParts) {
        Assert.checkNotNullParam("nameParts", nameParts);
    ...
    public void before(Class<?> type) {
        Assert.checkNotNullParam("type", type);
        if (type == Phase.class) {
            throw phaseDependencyException();
        }
    ...
    /**
     * Declare that the given enum constant service should start after this
     * service and stop before it. If the target service does not exist,
     * this declaration is silently ignored.
     *
     * @param enumConstant the enum constant (must not be {@code null})
     */
    public void before(Enum<?> enumConstant) {
        if (enumConstant == null) {
            throw new IllegalArgumentException("enumConstant must not be null");
        }
        if (enumConstant.getDeclaringClass() == Phase.class) {
            throw phaseDependencyException();
        }
        beforeKeys.add(LambdaTransliterator.serviceKey(enumConstant.getDeclaringClass(), List.of(enumConstant.name())));
    }
    ...
    // ── optional() — the service may produce no value at runtime ──

    /**
     * Mark this service as optional: at runtime its action returns {@code Optional<T>}, and an empty result
     * marks the service as present-but-valueless (skipped by {@code consumeAll()} consumers and delivered as an
     * empty optional to {@code request()} consumers) rather than being treated as an error.
     *
     * @throws IllegalStateException if this service has already been marked optional
     */
    public void markOptional() {
        if (optional) {
            throw new IllegalStateException("optional() may only be called once on a service builder");
        }
        optional = true;
    }
    ...
    public void doAction(Serializable action, boolean async) {
        ...
        if (optional && async) {
            throw new IllegalStateException(
                    "optional() services do not support async actions");
        }
        String serviceKey = LambdaTransliterator.serviceKey(serviceType, serviceNameParts);
        // record this service's phase so a later getRecorderProxy() in the same build step can
        // resolve the correct proxy phase and guarantee cross-phase retention for static-init
        definedServiceStaticInit.put(serviceKey, isStaticInit);
        // Automatically make the synthetic phase service depend on this service
        if (serviceType != Phase.class) {
            beforeKeys.add(LambdaTransliterator.serviceKey(Phase.class, List.of(phase.name())));
        }
        TransliteratedAction.ActionService extracted = LambdaTransliterator.extract(
                ...
                async, optional, isStaticInit, buildStepName);
        if (isStaticInit) {
    ...
    private static void checkServiceType(Class<?> type) {
        if (type.getTypeParameters().length > 0) {
            throw new IllegalArgumentException("Generic types cannot be used as service types: " + type.getName());
        }
    }

    private IllegalStateException phaseDependencyException() {
        return new IllegalStateException("Invalid dependency: service '"
                + LambdaTransliterator.serviceKey(serviceType, serviceNameParts)
                + "' cannot declare a 'before' dependency on Phase. Synthetic phase services act as phase completion markers, "
                + "and targeting a phase's start boundary is not supported under Quarkus's concurrent startup model.");
    }
}
