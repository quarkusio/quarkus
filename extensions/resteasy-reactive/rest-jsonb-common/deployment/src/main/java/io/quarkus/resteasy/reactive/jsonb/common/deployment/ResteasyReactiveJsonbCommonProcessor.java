...
public class ResteasyReactiveJsonbCommonProcessor {
    ...
    private static final List<String> VERTX_SERIALIZERS = List.of(VertxJson.JsonObjectSerializer.class.getName(),
            VertxJson.JsonArraySerializer.class.getName());

    private static final List<String> VERTX_DESERIALIZERS = List.of(VertxJson.JsonObjectDeserializer.class.getName(),
            VertxJson.JsonArrayDeserializer.class.getName());

    @BuildStep
    public void registerVertxJsonSupport(BuildProducer<JsonbSerializerBuildItem> serializers,
    ...
    public void beans(BuildProducer<AdditionalBeanBuildItem> additionalBean) {
        ...
        additionalBean.produce(AdditionalBeanBuildItem.builder().addBeanClass(JsonbMessageBodyReader.class.getName())
                .addBeanClass(JsonbMessageBodyWriter.class.getName()).setUnremovable().build());
    }
    ...
    public static void additionalProviders(BuildProducer<MessageBodyReaderBuildItem> additionalReaders,
        ...
        additionalReaders.produce(
                new MessageBodyReaderBuildItem.Builder(JsonbMessageBodyReader.class.getName(), Object.class.getName())
                        .setMediaTypeStrings(Collections.singletonList(MediaType.APPLICATION_JSON)).setBuiltin(true)
                        .setRuntimeType(runtimeType).build());
        additionalWriters.produce(
                new MessageBodyWriterBuildItem.Builder(JsonbMessageBodyWriter.class.getName(), Object.class.getName())
                        ...
                        .setBuiltin(true).setRuntimeType(runtimeType).build());
    }
}
