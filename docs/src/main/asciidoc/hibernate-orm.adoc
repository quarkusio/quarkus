...
+
By default, this script is picked up in dev and test modes only.
It is executed regardless of how the database schema is managed:
...
+
Use `quarkus.hibernate-orm.data-management.init-script` to use other files,
and `quarkus.hibernate-orm.data-management.strategy` to disable its execution (`none`)
or to enable it in production (`create`).
...
By default, it is only picked up in dev and test modes.
+
Use `quarkus.hibernate-orm.schema-management.init-script` to use other files,
or to execute the script in production too.
...
NOTE: The `quarkus.hibernate-orm.sql-load-script` configuration property is deprecated in favor of the properties above.
...
* `quarkus.hibernate-orm.schema-management.init-script` and `quarkus.hibernate-orm.data-management.init-script`
  are set to `no-file` for all profiles except the `dev` and `test` ones,
  so `import.sql` and `data.sql` are not executed.
* `quarkus.hibernate-orm.data-management.strategy` is set to `none` for all profiles except the `dev` and `test` ones,
  so even an explicitly configured data init script is not executed.

You can override them in your `application.properties` explicitly
(e.g. `%prod.quarkus.hibernate-orm.data-management.init-script = data.sql`
and `%prod.quarkus.hibernate-orm.data-management.strategy = create`)
