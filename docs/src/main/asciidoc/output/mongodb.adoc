...
:categories: data
:summary: This guide covers how to use MongoDB in {product-name-first}.
...
If you already have your {product-name} project configured, you can add the `mongodb-client` extension
...
WARNING: By default, {product-name} will restrict the use of JNDI within an application, as a precaution to try and mitigate any future vulnerabilities similar to Log4Shell.
...
{product-name} supports a feature called Dev Services that allows you to create various datasources without any config. In the case of MongoDB this support extends to the default MongoDB connection.
What that means practically, is that if you have not configured `quarkus.mongodb.connection-string`, {product-name} will automatically start a MongoDB container when running tests or in dev mode,
...
A reactive MongoDB Client is included in {product-name}.
...
{product-name} takes care of registering the `CodecProvider` for you as a CDI bean of `@Singleton` scope.
...
the way a POJO is mapped to a MongoDB collection and this codec is initialized automatically by {product-name}
...
{product-name} will automatically register all the classes annotated with `@BsonDiscriminator` with the POJO codec.

The POJO Codec have enhanced generic support via `PropertyCodecProvider`,
{product-name} will automatically register any `PropertyCodecProvider` with the POJO Codec (these classes are automatically made CDI beans of `@Singleton` scope).
...
The xref:mongodb-panache.adoc[MongoDB with Panache] extension facilitates the usage of MongoDB by providing active record style entities (and repositories) like you have in xref:hibernate-orm-panache.adoc[Hibernate ORM with Panache] and focuses on making your entities trivial and fun to write in {product-name}.
...
But if you can't use it, you can start a MongoDB database using one of the two `QuarkusTestResourceLifecycleManager` that {product-name} provides.
...
[WARNING]
====
Currently, {product-name} doesn't support link:https://docs.mongodb.com/manual/core/security-client-side-encryption/[Client-Side Field Level Encryption] in native mode.
====
