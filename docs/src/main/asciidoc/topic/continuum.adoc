...
You can use a HTTP client directly, but, it's often a boring task and the code can become quickly repetitive.
...
But you may be wondering where is configured the URL as it's not in the code.
Remember, it must not be in the code, because the url is likely depending on the environment.
...
Application requirements have changed drastically in the last few years.
...
Generating a _server-sent event_ response with Quarkus is just simple:
...
However, HTTP characteristics does not allow implement https://www.reactivemanifesto.org/[reactive systems], where all the components interact using asynchronous messages passing.
...
The `@Incoming` and `@Outgoing` annotations come from https://www.smallrye.io/smallrye-reactive-messaging[Reactive Messaging].
...
Stream-based manipulation can either use Rx Java 2 (s in the previous snippet) or MicroProfile Reactive Streams Operators.

== Functions as a Service and Serverless

Thanks to their stellar startup time and low memory usage, you can implement a Quarkus functions used in a serverless environment.
