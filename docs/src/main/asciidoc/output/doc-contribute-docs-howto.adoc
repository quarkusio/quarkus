...
Contribute to the documentation by using the recommended diataxis content types, steps, workflow, and style guidance to ensure the content successfully renders on the {product-name-first} website portal.

{product-name} docs use link:https://asciidoc.org/[AsciiDoc] markup.

== Prerequisites

* You have an editor or IDE that provides syntax highlighting and previews for AsciiDoc, either natively or with a plugin.
* You have reviewed the following {product-name} contributor resources:
** The link:https://github.com/quarkusio/quarkus/blob/main/CONTRIBUTING.md#documentation[Documentation] section of the {product-name} "Contributing" guide.
** The xref:{doc-guides}/doc-reference.adoc[{product-name} style and content guidelines] for the required syntax, preferred style, and other conventions.
** The xref:{doc-guides}/doc-concept.adoc[{product-name} documentation content types].
...
- AsciiDoc files are in the `src/main/asciidoc` directory within the `docs` module of the link:{quarkus-base-url}/tree/main/docs[{product-name} GitHub repository].
- The {product-name} doc templates are located in the `src/main/asciidoc/_templates` directory within the `docs` module of the link:{quarkus-base-url}/tree/main/docs[{product-name} GitHub repository].
...
- The link:https://quarkus.io/guides/[{product-name} documentation] menu page, also known as the doc index page, is sourced in the link:https://github.com/quarkusio/quarkusio.github.io[quarkusio.github.io] repository.


== Create Quarkus content in AsciiDoc

To ensure that your content shows up correctly on the link:https://quarkus.io/guides/[{product-name} documentation home page], use the following steps:

. Decide on a xref:doc-concept.adoc[Diataxis] content type that best fits the content that you are contributing.
+
TIP: To help you decide, see the content type descriptions in xref:{doc-guides}/doc-reference.adoc#titles-and-headings[Titles and headings] on the "About {product-name} documentation" page.
...
<2> For information about how to create a good title for each content type, see xref:{doc-guides}/doc-reference.adoc#titles-and-headings[Titles and headings] on the "{product-name} style and content guidelines" page.
...
<5> Set at least one category to ensure that the content is findable on the link:https://quarkus.io/guides/[{product-name} documentation home page]. For a list of {product-name} categories, see xref:{doc-guides}/doc-reference.adoc#document-attributes-and-variables[Document attributes and variables] on the "{product-name} style and content guidelines" page.
...
[IMPORTANT]
====
The first sentence of the abstract must explain the value and some benefit of the content in less than 27 words because this automatically displays on the link:https://quarkus.io/guides/[{product-name} guides homepage].
There must also be a line break before and after the abstract.
====

For more information about the minimum header requirements, see xref:{doc-guides}/doc-reference.adoc#document-structure[Document structure] on the "{product-name} style and content guidelines" page.
...
* `{prerequisites-no-cli}` removes `* Optionally the xref:cli-tooling.adoc[{product-name} CLI] if you want to use it`.
...
As content evolves, you might want to restructure an existing piece of {product-name} content into one or more content types and retire the existing AsciiDoc source file.

If you are retiring or renaming a published {product-name} AsciiDoc source file, ensure that the restructure does not break existing bookmarks and links to the original content.
Configure a URL redirect in the link:https://github.com/quarkusio/quarkusio.github.io/[{product-name}.io Website] GitHub repository by using the following steps:
...
Our builds use https://vale.sh[Vale] to check grammar, style, and word usage in the English versions of our documents. We created our own Vale style ruleset to ensure that content aligns with the preferred {product-name} style guidelines.

[NOTE]
====
* The {product-name} configuration for Vale is in `docs/.vale.ini`.
* The {product-name} style rules are in YAML format in the `docs/.vale/styles` directory.
====
...
It verifies both {product-name} document metadata and Vale style rules.
...
[source,shell]
----
# Run from the {product-name} project root
...
Submit your proposed changes to the core {product-name} docs by {gh-pull-requests-fork}[creating a pull request] against the `main` branch of the {product-name} repository from your own {gh-about-forks}[repository fork].
...
For more information about managing pull requests, see link:https://github.com/quarkusio/quarkus/blob/main/COMMITTERS.adoc[Information for {product-name} Committers].

=== Automatic style checking on the PR diff

The Vale linter job automatically runs when a PR is created or updated.
If your content updates do not align with the key style or terminology preferences of the {product-name} community, the updated line on the diff tab gets annotated with the Vale recommendations.
To ensure that your content gets approved, fix the linter errors, warnings, and suggestions.

We welcome your feedback on the {product-name} documentation style guidelines.
...
After your PR is merged to `main` and the branch is synchronized with the link:https://github.com/quarkusio/quarkusio.github.io/tree/develop/_generated-doc[{product-name}.io] website repository, you can preview the resulting build output on the link:https://quarkus.io/version/main/guides/[Main branch (SNAPSHOT)] documentation page of the {product-name} site.
