...
|Connection status: 'X' if the client closed the connection before the response was completed, '-' otherwise |          | `%{CONNECTION_STATUS}`
|===

NOTE: When a request is logged with the connection status `X`, the response was never sent, so the status code (`%s`) and the bytes sent (`%b`, `%B`) describe the response as it was when the connection closed, and the response time (`%D`, `%T`) is the time until the connection closed.
