...
At this point, Quarkus uses the OpenTelemetry Dev Service to run Jaeger-all-in-one container to collect, process and export telemetry data, which you can display.
To utilize Dev Service you should not set `quarkus.otel.exporter.otlp.traces.endpoint` property. Jaeger-all-in-one includes the Jaeger agent, an OTel collector, and the query service/UI.

[NOTE]
====

For production you will need to configure your own tracer.

====

If using your own tracer do not forget to configure your OTLP gRPC endpoint in `application.properties` as follows:
