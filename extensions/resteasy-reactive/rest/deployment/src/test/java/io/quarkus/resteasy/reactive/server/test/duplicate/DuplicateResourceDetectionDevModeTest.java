...
public class DuplicateResourceDetectionDevModeTest {
    ...
    static QuarkusDevModeTest TEST = new QuarkusDevModeTest().setArchiveProducer(
            () -> ShrinkWrap.create(JavaArchive.class).addClasses(GreetingResource3.class, GreetingResource4.class))
            .setAllowFailedStart(true);

    @Test
    public void testRestarts() {
        // The build should fail initially
        when().get("/hello-resteasy").then().statusCode(500);
        ...
        when().get("/hello-resteasy").then().statusCode(200);
        when().get("/hello-resteasy2").then().statusCode(200);
