...
class AeshWebSocketProcessor {
    ...
    void remoteTransport(AeshWebSocketConfig config,
        ...
        if (config.enabled()) {
            producer.produce(new AeshRemoteTransportBuildItem("websocket", config.path()));
        }
    }
    ...
    void disableEndpointIfNotEnabled(AeshWebSocketConfig config,
        ...
        } else if (!config.healthEnabled() || !capabilities.isPresent(Capability.SMALLRYE_HEALTH)) {
            transformers.produce(new AnnotationsTransformerBuildItem(
                    AnnotationTransformation.forClasses()
    ...
    void configureWebSocketPath(AeshWebSocketConfig config,
        ...
        transformers.produce(new AnnotationsTransformerBuildItem(
                AnnotationTransformation.forClasses()
                        .whenClass(c -> c.name().equals(ENDPOINT_CLASS))
                        .transform(ctx -> {
                            // Remove the existing @WebSocket and re-add it with the
                            // configured path while preserving all other annotation values.
                            for (AnnotationInstance ai : ctx.annotations()) {
                                if (ai.name().equals(WEB_SOCKET)) {
                                    ctx.remove(a -> a.name().equals(WEB_SOCKET));
                                    java.util.List<AnnotationValue> values = new java.util.ArrayList<>();
                                    for (AnnotationValue v : ai.values()) {
                                        if (!"path".equals(v.name())) {
                                            values.add(v);
                                        }
                                    }
                                    values.add(AnnotationValue.createStringValue("path", path));
                                    ctx.add(AnnotationInstance.create(WEB_SOCKET, ctx.declaration(),
                                            values.toArray(new AnnotationValue[0])));
                                    break;
                                }
                            }
                        })));
    ...
    @BuildStep
    HealthBuildItem addHealthCheck(AeshWebSocketConfig config, Capabilities capabilities) {
