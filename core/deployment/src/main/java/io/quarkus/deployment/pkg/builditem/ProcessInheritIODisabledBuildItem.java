...
/**
 * A build item, which indicates that the {@link ProcessBuilder#inheritIO()} will not work for processes launched by
 * build steps and instead the build step will have to explicitly stream the newly launched process' STDOUT/STDERR, if
 * the data generated on the STDOUT/STDERR of the launched process needs to be made available
 *
...
public final class ProcessInheritIODisabledBuildItem extends SimpleBuildItem {
    ...
    /**
     * Generates a {@link List<Consumer<BuildChainBuilder>> build chain builder} which creates a build step producing
     * the {@link ProcessInheritIODisabledBuildItem} build item
     */
