...
This guide explains how to write REST Services with RESTEasy Reactive in {product-name-first}.
...
{product-name} and consequently moving a lot of work to build time.
...
Moreover, if `quarkus.http.body.delete-uploaded-files-on-end` is set to true, {product-name} will delete the uploaded file when the HTTP response is sent. If the setting is disabled,
...
database, and allows {product-name} to serve more requests until your book is ready to
...
[NOTE]
====
{product-name} will automatically find all the classes annotated with `@XmlRootElement` and then bound them to the JAXB context.
====
...
public class CustomJaxbContext {
    ...
    // Replaces the CDI producer for JAXBContext built into {product-name}
    @Singleton
    @Produces
    JAXBContext jaxbContext() {
        // ...
...
[IMPORTANT]
====
Note that if you provide your custom JAXB context instance, you will need to register the classes you want to use for the XML serialization. This means that {product-name} will not update your custom JAXB context instance with the auto-discovered classes.
====
...
{product-name} comes with a CORS filter at the HTTP layer level.
...
or any of the xref:quarkus-reactive-architecture.adoc#quarkus-extensions-enabling-reactive[{product-name} Reactive extensions] for example:
...
In {product-name}, the RESTEasy Reactive extension and xref:rest-client-reactive.adoc[the REST Client Reactive extension] share the same infrastructure.
...
In the following example, {product-name} includes the `ResourceForApp1Only` Resource class if and only if the build profile `app1` has been enabled.
...
{product-name} can also conditionally disable Jakarta REST Resources based on the value of runtime properties using the `@io.quarkus.resteasy.reactive.server.EndpointDisabled` annotation.

In the following example, {product-name} will exclude `RuntimeResource` at runtime if the application has `some.property` configured to `"disable"`.
