...
public class ObservedInterceptorTest {
    ...
    static final QuarkusExtensionTest TEST = new QuarkusExtensionTest()
            .setArchiveProducer(
                    () -> ShrinkWrap.create(JavaArchive.class)
                            .addClasses(ObservedBean.class, ObservedTypeBean.class, TestRegistryProducer.class,
    ...
    @Inject
    ObservedTypeBean beanTypeAnnotated;
    ...
    @Test
    void syncMethodProducesSpanOnTypeAnnotatedBean() {
        String result = beanTypeAnnotated.syncMethod();
        assertThat(result).isEqualTo("hello");
    ...
    @Test
    void customNameIsUsedOnTypeAnnotatedBean() {
        beanTypeAnnotated.customNameMethod();
    ...
    @Test
    void asyncMultiProducesSingleSpan() {
        List<String> result = bean.asyncMulti().collect().asList().await().indefinitely();
        assertThat(result).containsExactly("one", "two", "three");

        // A Multi emits several items but the observation must be started/stopped once per
        // subscription (on stream termination), so exactly one span and one timer recording.
        List<SpanData> spans = spanExporter.getFinishedSpanItems(1);
        SpanData span = spans.get(0);
        assertThat(span.getName()).contains("asyncMulti");
        assertThat(span.getStatus().getStatusCode()).isEqualTo(StatusCode.OK);

        Timer timer = simpleMeterRegistry.find("asyncMulti").timer();
        assertThat(timer).isNotNull();
        assertThat(timer.count()).isEqualTo(1);
    }

    @Test
    void asyncMultiWithErrorProducesErrorSpan() {
        try {
            bean.asyncMultiWithError().collect().asList().await().indefinitely();
        } catch (RuntimeException expected) {
            // expected
        }
        ...
        List<SpanData> spans = spanExporter.getFinishedSpanItems(1);
        SpanData span = spans.get(0);
        assertThat(span.getName()).contains("asyncMultiWithError");
        assertThat(span.getStatus().getStatusCode()).isEqualTo(StatusCode.ERROR);
        assertThat(span.getEvents()).isNotEmpty();

        Timer timer = simpleMeterRegistry.find("asyncMultiWithError").timer();
        assertThat(timer).isNotNull();
        assertThat(timer.count()).isEqualTo(1);
    }
    ...
    @Test
    void lowCardinalityKeyValuesFromAnnotationOnTypeAnnotatedBean() {
        beanTypeAnnotated.withKeyValues();
    ...
    public static class ObservedBean {
        ...
        @Observed
        public Multi<String> asyncMulti() {
            return Multi.createFrom().items("one", "two", "three");
        }

        @Observed
        public Multi<String> asyncMultiWithError() {
            return Multi.createFrom().<String> failure(new RuntimeException("test error"));
        }
    ...
    @Observed
    @ApplicationScoped
    public static class ObservedTypeBean {
