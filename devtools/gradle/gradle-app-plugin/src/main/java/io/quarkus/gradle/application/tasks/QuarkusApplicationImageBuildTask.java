...
/**
 * Builds the normal container image for a named application build.
 * <p>
 * The task consumes the corresponding image-reference preflight receipt, verifies that Quarkus reports the preflight
 * reference, and writes a result receipt. It always executes and is not build-cacheable because it mutates external
...
public abstract class QuarkusApplicationImageBuildTask extends QuarkusApplicationImageTask {
    ...
    /**
     * Creates a task whose operation kind defaults to {@link ImageOperation#BUILD}.
     */
    ...
    /**
     * Builds the image and verifies its reported reference against the preflight result.
     */
    @TaskAction
