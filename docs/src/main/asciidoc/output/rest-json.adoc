...
[TIP]
====
This is an introduction to writing JSON REST services with {product-name-first}.
...
{product-name} also supports https://eclipse-ee4j.github.io/jsonb-api/[JSON-B] so, if you prefer JSON-B over Jackson, you can create a project relying on the RESTEasy Reactive JSON-B extension instead:
...
[NOTE]
====
When a JSON extension is installed such as `quarkus-resteasy-reactive-jackson` or `quarkus-resteasy-reactive-jsonb`, {product-name} will use the `application/json` media type
...
{product-name} automates the registration of Jackson's Mixin support, via the `io.quarkus.jackson.JacksonMixin` annotation.
