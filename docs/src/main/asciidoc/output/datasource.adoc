...
{product-name-first} provides a unified configuration model to define datasources for Java Database Connectivity (JDBC) and Reactive database drivers.

{product-name} uses link:https://agroal.github.io/[Agroal] and link:https://vertx.io/[Vert.x] to provide high-performance, scaleable data source connection pooling for JDBC and reactive drivers.
The `jdbc-\*` and `reactive-*` extensions provide build time optimizations and integrate configured data sources with {product-name} features like security, health checks, and metrics.

For more information on consuming and using a reactive datasource, consult the {product-name} xref:reactive-sql-clients.adoc[Reactive SQL clients] guide.

Additionally, refer to the {product-name} xref:hibernate-orm.adoc[Hibernate ORM] guide for information on consuming and using a JDBC datasource.
...
{product-name} simplifies database configuration by offering the Dev Services feature, enabling zero-config database setup for testing or running in dev mode.
In dev mode, the suggested approach is to use DevServices and let {product-name} handle the database for you, whereas for production mode, you provide explicit database configuration details pointing to a database managed outside of {product-name}.
...
In Dev mode, if you do not provide any explicit database connection details, {product-name} automatically handles the database setup and provides the wiring between the application and the database.
...
If only one viable extension is available, {product-name} assumes this is the correct one.
If a driver has been added to the test scope, {product-name} automatically includes the driver in testing.
...
----
+
{product-name} deduces the JDBC driver class it needs to use from the specified value of the `db-kind` database platform attribute.
+
...
+
{product-name} currently includes the following built-in database kinds:
+
* DB2: `db2`
* Derby: `derby`
* H2: `h2`
...
[NOTE]
====
You can use any JDBC driver in a {product-name} app in JVM mode as described in <<other-databases,Using other databases>>.
However, using a non-built-in database kind is unlikely to work when compiling your application to a native executable.

For native executable builds, it is recommended to either utilize the available JDBC {product-name} extensions or contribute a custom extension for your specific driver.
====
...
.. For use with a built-in JDBC driver, choose and add the {product-name} extension for your relational database driver from the list below:
+
...
For more information about configuring JDBC, see <<jdbc-url,JDBC URL format reference>> and <<extensions-and-database-drivers-reference,{product-name} extensions and database drivers reference>>.


[[other-databases]]
===== Custom databases and drivers

If you need to connect to a database for which {product-name} does not provide an extension with the JDBC driver, you can use a custom driver instead. For example, if you are using the OpenTracing JDBC driver in your project.

Without an extension, the driver will work correctly in any {product-name} app running in JVM mode.
However, the driver is unlikely to work when compiling your application to a native executable.
If you plan to make a native executable, use the existing JDBC {product-name} extensions, or contribute one for your driver.
...
{product-name} offers several reactive clients for a use with reactive datasource.
...
NOTE: Even when only one database extension is installed, named databases need to specify at least one build-time property so that {product-name} can detect them.
...
To facilitate the storage of transaction logs in a database by using JDBC, see xref:transaction.adoc#jdbcstore[Configuring transaction logs to be stored in a datasource] section of the xref:transaction.adoc[Using transactions in {product-name}] guide.

==== Named datasources

When using Dev Services, the default datasource will always be created, but to specify a named datasource, you need to have at least one build time property so {product-name} can detect how to create the datasource.
...
When compiling a {product-name} application to a native image, the MySQL support for JMX and Oracle Cloud Infrastructure (OCI) integrations are disabled as they are incompatible with GraalVM native images.
...
The following tables list the built-in `db-kind` values, the corresponding {product-name} extensions, and the JDBC drivers used by those extensions.
...
|===
|Database kind |{product-name} extension |Drivers
...
[options="header",cols="^10%,^25%,65%"]
|===
|Database kind |{product-name} extension |Driver
