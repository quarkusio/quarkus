        ...
        for (var httpPermission : HttpSecurityConfiguration.get().httpPermissions()) {
            if (httpPermission.shouldApplyToJaxRs() && httpPermission.getAuthMechanisms() != null) {
                ...
                throw new ConfigurationException("""
                        Cannot select '%s' authentication mechanisms with HttpSecurityPolicy applied to JAXRS.
                        Please use the '%s' annotation to select mechanisms instead.
                        """.formatted(httpPermission.getAuthMechanisms().names(), HttpAuthenticationMechanism.class.getName()));
            }
        }
    }
