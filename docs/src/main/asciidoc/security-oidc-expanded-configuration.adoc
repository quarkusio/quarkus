...
|quarkus.oidc.credentials.jwt.token-path ||If the token source is BEARER then this path must point to a JWT bearer token in the file system, or a custom `io.quarkus.oidc.common.ClientAssertionProvider` must be registered as a CDI bean
|====
