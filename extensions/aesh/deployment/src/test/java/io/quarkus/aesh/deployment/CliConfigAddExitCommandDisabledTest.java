...
/**
 * Tests that {@code quarkus.aesh.add-exit-command=false} removes the built-in
 * exit command from console mode.
 */
public class CliConfigAddExitCommandDisabledTest {

    @RegisterExtension
    static final QuarkusUnitTest config = new QuarkusUnitTest()
            .withApplicationRoot(jar -> jar.addClasses(DummyCommand.class))
            .overrideConfigKey("quarkus.aesh.add-exit-command", "false")
    ...
    @Test
    public void testExitCommandNotAvailable() throws Exception {
        ...
        CapturingStubConnection connection = new CapturingStubConnection(data -> {
            ...
            output.add(text);
            if (text.contains("exit:") || text.contains("command not found") || text.contains("$")) {
        ...
        if (stdinHandler != null) {
            stdinHandler.accept("exit\r".chars().toArray());
        }
        ...
        // With exit command disabled, "exit" should not be recognized as a built-in
        String allOutput = String.join("", output);
        Assertions.assertThat(allOutput).doesNotContain("Goodbye");
    }
