...
https://datatracker.ietf.org/doc/draft-ietf-oauth-attestation-based-client-auth/[OAuth 2.0 Attestation-Based Client Authentication] specifies a mechanism that allows OAuth2 client to prove its authenticity verified by a client attester and authenticate without using a client secret.

.Client attestation properties
[options="header"]
|====
|Property name |Default |Description

|quarkus.oidc.credentials.attestation.enabled |false|Enable a custom attestation or default self-attestation
|quarkus.oidc.credentials.attestation.signature-algorithm |ES256|Attestation JWT PoP token signature algorithm
|====

`quarkus.oidc.credentials.attestation.enabled=true` enables an attestation-based client authentication, using either a registered `io.quarkus.oidc.common.ClientAttester` attester or a default self-attestation.

`quarkus.oidc.credentials.attestation.signature-algorithm` specifies an Attestation JWT PoP token signature algorithm, and, in case of the self-attestation, an Attestation JWT signature algorithm.

Currently, only `ES256`, `ES384` and `ES512` signature algorithms are supported. Custom client attesters can chose any other standard JWT token signature algorithm.
...
|quarkus.oidc.allowed-routes |`backchannel-logout,resource-metadata,client-id-metadata,client-attestation`|OIDC routes that tenants can use
|====

To be able to support https://openid.net/specs/openid-connect-backchannel-1_0.html[OIDC back-channel logout], https://datatracker.ietf.org/doc/rfc9728/[OAuth2 Protected Resource Metadata] and https://datatracker.ietf.org/doc/html/draft-ietf-oauth-client-id-metadata-document[OAuth Client ID Metadata Document] requests, as well as https://datatracker.ietf.org/doc/draft-ietf-oauth-attestation-based-client-auth/[OAuth 2.0 Attestation-Based Client Authentication] in a self-attestation mode, `quarkus-oidc` must register routes that require dedicated HTTP handlers for all static and dynamic OIDC tenants that must support all or one of these features.
...
* `client-attestation` - enables an HTTP route for OIDC tenants to authenticate to OIDC providers with the <<client-attestation,client attestation tokens>>
