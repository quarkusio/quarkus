...
public class HibernateOrmDevInfoServiceTestResource {
    ...
    public String checkPuInfoWithSuccessfulDDLGeneration() {
        ...
        assertThat(pu.getManagedEntities()).hasSize(1).element(0)
                .returns(MyEntityWithSuccessfulDDLGeneration.class.getName(), HibernateOrmDevInfo.Entity::getClassName)
                .returns(MyEntityWithSuccessfulDDLGeneration.TABLE_NAME, HibernateOrmDevInfo.Entity::getTableName);

        assertThat(pu.getNamedQueries()).hasSize(1).element(0)
        ...
        assertThat(pu.getNamedNativeQueries()).hasSize(1).element(0)
                .returns("MyEntity.nativeFindAll", HibernateOrmDevInfo.Query::getName)
                .returns("SELECT e FROM MyEntityTable e ORDER BY e.name", HibernateOrmDevInfo.Query::getQuery)
                .returns(false, HibernateOrmDevInfo.Query::isCacheable);

        assertThat(pu.getCreateDDL()).contains("create table MyEntityTable")
                .contains("INSERT INTO MyEntityTable(id, name) VALUES(1, 'default sql load script entity');");
        assertThat(pu.getDropDDL()).contains("drop table if exists MyEntityTable");
    ...
    public String checkPuInfoWithFailingDDLGeneration() {
        ...
        assertThat(pu.getName()).isEqualTo(PersistenceUnitUtil.DEFAULT_PERSISTENCE_UNIT_NAME);
        assertThat(pu.getManagedEntities()).hasSize(1).element(0)
                .returns(MyEntityWithFailingDDLGeneration.class.getName(), HibernateOrmDevInfo.Entity::getClassName)
                .returns(MyEntityWithFailingDDLGeneration.TABLE_NAME, HibernateOrmDevInfo.Entity::getTableName);

        assertThat(pu.getCreateDDL()).contains("MyEntityTable").contains(TypeWithUnsupportedSqlCode.UNSUPPORTED_SQL_CODE
                + " (UNKNOWN(" + TypeWithUnsupportedSqlCode.UNSUPPORTED_SQL_CODE + "))");
