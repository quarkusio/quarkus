...
This reference guide demonstrates how your {product-name-first} application can utilize SmallRye Reactive Messaging to interact with Apache Kafka.
...
{product-name} provides support for Apache Kafka through https://smallrye.io/smallrye-reactive-messaging/[SmallRye Reactive Messaging] framework.
...
{product-name} automatically associates _orphan_ channels to the (unique) connector found on the classpath.
...
Continuing from the previous minimal configuration, your {product-name} application can receive message payload directly:
...
See the xref:quarkus-reactive-architecture.adoc[{product-name} Reactive Architecture documentation] for further details on this topic.
...
The second one, used also with other reactive features of {product-name}, uses the default worker pool and preserves the order.
...
.@RunOnVirtualThread
====
For running the blocking processing on Java _virtual threads_, see the xref:messaging-virtual-threads.adoc[{product-name} Virtual Thread support with Reactive Messaging documentation].
====
...
Starting with SmallRye Fault Tolerance 5.2.0 (available since {product-name} 2.1.0.Final), it is not necessary.
...
Let's explore briefly different producer/consumer patterns and how to implement them using {product-name}:
...
{product-name} generates a UUID that changes between two executions (including in dev mode).
...
[NOTE]
====
{product-name} autodetects batch types for incoming channels and sets batch configuration automatically.
...
{product-name} provides following state store implementations:
...
When {product-name} detects the use of `KafkaTransactions` for an outgoing channel it configures these properties on the channel,
...
{product-name} has built-in capabilities to deal with JSON Kafka messages.
...
{product-name} has built-in support for JSON serialization and deserialization based on Jackson.
...
When using SmallRye Reactive Messaging with Kafka (`io.quarkus:quarkus-smallrye-reactive-messaging-kafka`), {product-name} can often automatically detect the correct serializer and deserializer class.
...
and your configuration indicates that the `generated-price` channel uses the `smallrye-kafka` connector, then {product-name} will automatically set the `value.serializer` to Kafka's built-in `IntegerSerializer`.
...
and your configuration indicates that the `my-kafka-records` channel uses the `smallrye-kafka` connector, then {product-name} will automatically set the `key.deserializer` to Kafka's built-in `LongDeserializer`, as well as the `value.deserializer` to `ByteArrayDeserializer`.
...
and your configuration indicates that the `price-create` channel uses the `smallrye-kafka` connector, then {product-name} will automatically set the `value.serializer` to Kafka's built-in `DoubleSerializer`.
...
If you find you need to do this, please file a bug in the link:https://github.com/quarkusio/quarkus/issues[{product-name} issue tracker] so we can fix whatever problem you have.

[[serialization-generation]]
== JSON Serializer/deserializer generation
{product-name} automatically generates serializers and deserializers for channels where:
...
{product-name} provides several health checks for Kafka.
...
Then, create a {product-name} Test Resource as follows:
...
Create a {product-name} Test using the test resource created above:
...
{product-name} Kafka extension supports
...
{product-name} exposes all Kafka related application properties, prefixed with `kafka.` or `KAFKA_` inside a configuration map with `default-kafka-broker` name.
...
{product-name} automatically considers the method as _blocking_. Indeed, writing to a database using classic Hibernate is blocking. So, {product-name} calls the method on a worker thread you can block (and not an I/O thread).
...
To reduce the amount of log written by the Kafka client, {product-name} sets the level of the following log categories to `WARNING`:
...
Then, you can configure the {product-name} application to connect to the broker as follows:
...
To allow your {product-name} application to use that secret, add the following line to the `application.properties` file:
...
Then, you can configure the {product-name} application to connect to the schema registry as follows:
...
If your {product-name} application is deployed on a Kubernetes or OpenShift cluster with link:https://github.com/redhat-developer/service-binding-operator[Service Binding Operator] and link:https://github.com/redhat-developer/app-services-operator/tree/main/docs[OpenShift Application Services] operators installed,
...
you can configure the {product-name} application to generate `ServiceBinding` resources for those services:
...
For this example {product-name} build will generate the following `ServiceBinding` resources:
...
This guide has shown how you can interact with Kafka using {product-name}.
It utilizes SmallRye Reactive Messaging to build data streaming applications.

If you want to go further, check the documentation of https://smallrye.io/smallrye-reactive-messaging[SmallRye Reactive Messaging], the implementation used in {product-name}.
