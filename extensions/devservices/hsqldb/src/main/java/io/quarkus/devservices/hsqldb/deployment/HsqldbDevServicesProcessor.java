        ...
        return new DevServicesDatasourceProviderBuildItem(DatabaseKind.HSQLDB, new DevServicesDatasourceProvider() {
            ...
            public RunningDevServicesDatasource startDatabase(
                ...
                try {
                    ...
                    String connectionUrl = "jdbc:hsqldb:tcp://localhost:" + tcpServer.getPort() + "/mem:" + effectiveDbName
                            ...
                            new Closeable() {
                                ...
                                public void close() throws IOException {
                                    ...
                                    if (tcpServer.isRunning(false)) {
                                        ...
                                        tcpServer.stop();
                                        LOG.info("Dev Services for HSQLDB shut down; server status: " + tcpServer.getStatus());
                                    } else {
                                        LOG.info("Dev Services for HSQLDB was NOT shut down as it appears it was down already; server status: " + tcpServer.getStatus());
                                    }
                                }
                            });
