...
You can secure your {product-name-first} application endpoints with HTTP Basic authentication.
{product-name} provides a built-in authentication mechanism for Basic authentication.
...
The following table outlines some limitations of using HTTP Basic authentication to secure your {product-name} applications:
...
The risk of exposing credentials as plain text increases if a load balancer terminates HTTPS because the request is forwarded to {product-name} over HTTP.
Furthermore, in multi-hop deployments, the credentials can be exposed if HTTPS is used between the client and the first {product-name} endpoint only, and the credentials are propagated to the next {product-name} endpoint over HTTP.
...
|The {product-name} application must validate that usernames, passwords, and roles are managed securely.
...
For more information about how you can secure your {product-name} applications by using Basic authentication, see the following resources:
