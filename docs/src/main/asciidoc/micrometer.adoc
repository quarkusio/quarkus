...
Use a custom `@Produces` method to create and configure a customized `MeterRegistry` if you need to. Use the
`@AlternatePriority` annotation to disable the registry provided by the extension.
