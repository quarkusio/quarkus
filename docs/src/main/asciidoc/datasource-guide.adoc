...
In {project-name}, the out of the box datasource implementation is Agroal.
...
You can add it explicitly, or transitively by adding the `quarkus-hibernate-orm` dependency.

If you want to add the dependency explicitly:
...
If you'd like to add Hibernate ORM instead, and get Agroal transitively:
...
Do so by adding one of these dependencies to your pom.xml file:
...
This is done in `{config-file}`.
The minimum necessary for a configured database is as follows:

[source,properties]
--
include::../../../../extensions/agroal/deployment/src/test/resources/application-default-datasource.properties[basic]
--
...
|java.time.Duration
|PT3M, PT56S, for more information on the format, see link:https://docs.oracle.com/javase/8/docs/api/java/time/Duration.html#parse-java.lang.CharSequence-[Duration.parse]
