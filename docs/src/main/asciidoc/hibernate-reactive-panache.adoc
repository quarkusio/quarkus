...
[NOTE]
====
Do not mix `@Transactional` with `@WithSession` or `@WithSessionOnDemand` in the same application.
...
To run a test in a transaction and roll back any database changes when the test completes, annotate the test class or method with `io.quarkus.test.TestReactiveTransaction` instead of `@RunOnVertxContext`.
