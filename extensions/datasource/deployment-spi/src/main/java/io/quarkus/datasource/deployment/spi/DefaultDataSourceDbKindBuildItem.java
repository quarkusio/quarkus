...
/**
 * A build item that represents the "quarkus.datasource.db-kind" value. This is generated by specific extensions that
 * are meant to take away the burden of configuring anything datasource related from the user.
 */
public final class DefaultDataSourceDbKindBuildItem extends MultiBuildItem {
    ...
    public String getScope(CurateOutcomeBuildItem curateOutcomeBuildItem) {
        if (scope == null) {
            Map.Entry<String, String> artifact = ArtifactInfoUtil.groupIdAndArtifactId(callerClass,
                    curateOutcomeBuildItem);
            for (ResolvedDependency i : curateOutcomeBuildItem.getApplicationModel().getDependencies()) {
                if (i.getArtifactId().equals(artifact.getValue()) && i.getGroupId().equals(artifact.getKey())) {
                    scope = i.getScope();
                    break;
                }
            }
    ...
    public static Optional<String> resolve(Optional<String> configured,
            List<DefaultDataSourceDbKindBuildItem> defaultDbKinds, boolean enableImplicitResolution,
