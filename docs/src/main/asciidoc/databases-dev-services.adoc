...
The container image used by Dev Services is determined by the database extension you include in your project.
For example, including `quarkus-jdbc-postgresql` will use a PostgreSQL image, while `quarkus-jdbc-mysql` will use a MySQL image.
