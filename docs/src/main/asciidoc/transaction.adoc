...
All reaper properties are optional — when not set, the Narayana defaults are used:
...
| `quarkus.transaction-manager.reaper.cancel-wait-period` | `500ms`
| `quarkus.transaction-manager.reaper.cancel-fail-wait-period` | `500ms`
