...
You can also configure your Quarkus application using a hierarchical YAML file thanks to the `quarkus-config-yaml` extension.

First, you need to add the extension to your project:
...
Then, create the `src/main/resources/application.yaml` file with a content corresponding to your desired configuration.

[WARNING]
====
The configuration values from `application.yaml` will take precedence over the ones from `application.properties` if both files are present in your application.
But if a configuration key is present in `application.properties` and not in `application.yaml`, the value from `application.properties` will be used.
====

Here's an example of a Quarkus `application.properties` configuration file, followed by its `application.yaml` equivalent:
