...
https://neo4j.com[Neo4j] is a graph database management system developed by Neo4j, Inc.
Neo4j is a native graph database focused not only on the data itself, but especially on the relations between data.
...
The solution is located in the `neo4j-quickstart` directory.
...
public CompletionStage<Response> delete(@PathParam("id") Long id) {
    ...
    return session
        .writeTransactionAsync(tx ->
            tx.runAsync("MATCH (a:Fruit) WHERE id(a) = $id DETACH DELETE a", Values.parameters("id", id))
        )
...
The make life a bit easier, we will use https://projectreactor.io[Project Reactor] for this.
