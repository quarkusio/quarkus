...
NOTE: The https://github.com/quarkusio/spring-quarkus-perf-comparison[spring-quarkus-perf-comparison] repository contains the runnable scripts used for our official benchmarks. The sections below describe the methodology; the repository has the exact, up-to-date commands.
...
It is important to measure the startup time to first request (TTFR) to most accurately reflect how long a framework needs to start.
...
We record a nanosecond timestamp immediately before launching the application, then poll the application's HTTP endpoint until it returns a 200 response. The difference is the Time to First Request (TTFR).

The https://github.com/quarkusio/spring-quarkus-perf-comparison/blob/main/scripts/perf-lab/scripts/time-to-1st-request.sh[`time-to-1st-request.sh`] script in the benchmark repository implements this precisely. It records the start time using nanosecond precision:
...
It may not include time spent loading shared libraries, JVM bootstrap, or the time until the application can actually serve a request.
