...
public interface PackageConfig {
    ...
    interface JarConfig {
        ...
        interface AotConfig {
            ...
            /**
             * When AOT file generation is enabled, if this property is set, then the JVM used to generate the AOT file
             * will be the JVM present in the container image. The builder image is expected to have the 'java' binary
             * on its PATH.
             * This flag is useful when the JVM to be used at runtime is not the same exact JVM version as the one used to build
             * the jar.
            ...
            /**
             * Whether creation of the AOT file generation should run in a container if available.
             *
             * <p>
             * Normally, if either a suitable container image to use to create the AOT file
             * can be determined automatically or if one is explicitly set using the
             * {@code quarkus.package.jar.aot.builder-image} setting, the AOT archive is generated by
             * running the JDK contained in the image as a container.
             *
             * <p>
             * If this option is set to {@code false}, a container will not be used to generate the
             * AOT archive. Instead, the JDK used to build the application is also used to create the
             * archive. Note that the exact same JDK version must be used to run the application in this
             * case.
             *
             * <p>
             * Ignored if {@code quarkus.package.jar.aot.enabled} is set to {@code false}.
             */
            @WithDefault("true")
            boolean useContainer();
             ...
             * If {@link Type#AUTO} is used, Quarkus will generate an AOT file for JDK25+, otherwise it will generate an AppCDS
             * file.
             * TODO: support Semeru as well
             */
            Optional<Type> type();
