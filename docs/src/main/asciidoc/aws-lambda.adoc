...
By default, the native executable embeds the certificates from the default trust store present at build time.

You might have to rebuild your native executable from time to time if you want to keep them up to date.
You can also specify a custom trust store by using a custom `bootstrap` script.

To do this, first create a directory `src/main/zip.native/` with your build.  Next create a shell script file called `bootstrap`
within `src/main/zip.native/`, like below.
An example called `bootstrap-example.sh` is created automatically in your build folder (`target` or `build` depending on your build tool).
...
Next you must copy and tweak the `cacerts` from your GraalVM distribution (or use your very own) into `src/main/zip.native/`.
...
To extract the required file, you must start up a Docker container in the background, and attach to that container to copy the artifacts.
...
Then you can extract the `cacerts` file from the started container:
...
If you included a custom `bootstrap` script and `cacerts` file, your final archive will look like this:
...
To create a lambda container image that uses the native executable we'll need to do things a little differently. In this case, we won't need to use the `lambda/java` base image from AWS, but instead we'll use a special image that assumes that the runtime environment for the lambda is provided. The example below creates such a container. It assumes that a Maven build has been executed (such as `mvn package -Dnative=true`) and has generated the native binary into the `target/` directory. The binary needs to be named `bootstrap` and be placed in `/var/runtime/`:
