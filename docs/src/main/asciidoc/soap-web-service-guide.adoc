...
In order to support only SOAP client, register endpoint URL and the service endpoint interface (same than the server) with configuration:
...
Then inject the client to used it:
