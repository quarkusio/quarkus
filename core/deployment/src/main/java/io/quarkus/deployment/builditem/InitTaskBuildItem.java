...
/**
 * Represents an initialization task for the application. Often extensions perform some sort of initialization as part
 * of the application startup. There are cases where we want to externalize the initialization (e.g. in a pipeline).
 * Often the task is run using the same artifact as the application but using a different command or arguments. In the
 * later case it might be desirable to pass additional environment variables to both the init tasks (to enable init) and
 * the application (to disable the init).
 */
public final class InitTaskBuildItem extends MultiBuildItem {
    ...
    public InitTaskBuildItem withImage(String image) {
        return new InitTaskBuildItem(name, Optional.of(image), command, arguments, taskEnvVars, appEnvVars,
     ...
     * same environment variables as the application.
     *
    ...
    /**
     * Flag for tasks that need to share the file system with the application. Often tasks need to access resources,
     * configured via the file system (e.g. local config files, Kubernetes service binding etc). In other cases, tasks
     * may need to produce files needed by the application. This flag expresses that the task should share the file
