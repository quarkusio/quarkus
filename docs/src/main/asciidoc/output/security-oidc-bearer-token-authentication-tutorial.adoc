...
Here, you use the {product-name-first} OpenID Connect (OIDC) extension to secure a Jakarta REST application using Bearer token authentication.
...
which is an implementation of OIDC for {product-name}.

If you already have your {product-name} project configured, you can add the `oidc` extension
...
Configure the {product-name} OpenID Connect (OIDC) extension by setting the following configuration properties in the `src/main/resources/application.properties` file.
...
See the xref:security-keycloak-admin-client.adoc[{product-name} Keycloak Admin Client] guide for more information.
...
* xref:security-overview.adoc[{product-name} Security]
