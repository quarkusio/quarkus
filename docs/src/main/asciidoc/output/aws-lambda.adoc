...
:categories: cloud
:summary: This guide explains how you can deploy {product-name-first}-based AWS Lambdas.

The `quarkus-amazon-lambda` extension allows you to use {product-name} to build your AWS Lambdas.
Your lambdas can use injection annotations from CDI or Spring and other {product-name} facilities as you need them.

{product-name} lambdas can be deployed using the Amazon Java Runtime, or you can build a native executable and use
Amazon's Custom Runtime if you want a smaller memory footprint and faster cold boot startup time.

{product-name}'s integration with lambdas also supports {product-name}'s Live Coding development cycle.  You can
bring up your {product-name} lambda project in dev or test mode and code on your project live.
...
Create the {product-name} AWS Lambda maven project using our Maven Archetype.
...
The `quarkus.lambda.handler` property tells {product-name} which lambda handler to deploy. This can be overridden
...
handler bootstraps {product-name} and wraps your actual handler so that injection can be performed.
...
NOTE: If you are building on a non-Linux system, you will need to also pass in a property instructing {product-name} to use a docker build as Amazon
...
the {product-name} AWS Lambda extension boots up a mock AWS Lambda event server in {product-name} Dev and Test mode.
This mock event server simulates a true AWS Lambda environment.

While running in {product-name} Dev Mode, you can feed events to it by doing an HTTP POST to `http://localhost:8080`.
...
using rest-assured.  {product-name} starts up a separate Mock Event server under port 8081.
The default port for Rest Assured is automatically set to 8081 by {product-name}, so you can invoke
on this endpoint.
...
to take advantage of it.  Otherwise, the {product-name} JUnit integration should be sufficient for most of your needs.
...
as GraalVM will only include the dependencies when explicitly declared.  {product-name}, by default enables this functionality on extensions that implicitly require it.
For further information, please consult the xref:native-and-ssl.adoc[{product-name} SSL guide]
...
NOTE: {product-name} now has extensions for DynamoDB, S3, SNS and SQS (more coming).
Please check link:{amazon-services-guide}[those guides] on how to use the various AWS Services with {product-name}, as opposed to wiring manually like below.
...
there is some additional work to bundle the `function.zip`, as below.  For more information, please see the xref:native-and-ssl.adoc[{product-name} Native SSL Guide].
...
For a regular JVM distribution you need to base your image off the official AWS Java base images. Below is an example of a Dockerfile that would create a container image from your {product-name} Lambda project. It assumes that `mvn package` has been executed and binaries are available in the `target/` directory:
...
To use Alexa with {product-name} native, you need to use the https://github.com/quarkiverse/quarkus-amazon-alexa[{product-name} Amazon Alexa extension hosted at the Quarkiverse Hub].
