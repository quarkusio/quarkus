...
:summary: Provide users with initial code for extensions when generating {product-name-first} applications on code.quarkus.io{product-name}l the Quarkus tooling. This guide explains how to create and configure a Codestart for an extension.

This guide explains how to create and configure a {product-name} Codestart for an extension.

== Description

"Extension Codestarts" is the name we give to our {product-name} extension quickstart code generation system. It aims to provide a personalized getting started experience with {product-name}.
A {product-name} extension is able to provide one or more well-defined codestarts which will contain the resources and code required/recommended to start using that particular extension.

Extension codestarts are applied by default when using the {product-name} tooling (if the chosen extensions contain any):

* https://code.quarkus.io[code.quarkus.io, window="_blank"] (find the extensions tagged with [code])
* The {product-name} Maven plugin:
+
...
* project: The project skeleton (e.g. a {product-name} project)
...
* code: Any {product-name} extension can provide starter code. The user can decide to activate it or not.
...
- In the {product-name} core repository, the extension codestarts are all in the same https://github.com/quarkusio/quarkus/tree/main/devtools/project-core-extension-codestarts/src/main/resources/codestarts/quarkus/extension-codestarts[module, window="_blank"].
...
In addition, {product-name} also provides the following ways to initialize a new extension project with a Codestart:
...
.Maven
****
{product-name} provides the <<building-my-first-extension.adoc#maven-setup,`create-extension` Maven Mojo>> to initialize an extension project.
...
Here is a step-by-step guide to write an extension codestart. You may also watch the https://www.youtube.com/watch?v=lLyVDqVK8cE[{product-name} Insight #99] with a live-coding session.

As was mentioned previously, the base project files (pom.xml, Dockerfiles, ...) are already generated by the base codestarts provided by the {product-name} core. Thanks to this, we can only focus on the starter code specific to the extension.
...
In this tutorial a Codestart project is created from a {product-name} project and adding the needed templates.
...
quarkus.maven-plugin.group-id = {product-name} Maven plugin groupId
quarkus.maven-plugin.artifact-id = {product-name} Maven plugin artifactId
quarkus.maven-plugin.version = {product-name} Maven plugin version
quarkus.gradle-plugin.id = {product-name} Gradle pluginId
quarkus.gradle-plugin.version = {product-name} Gradle plugin version
quarkus.version = {product-name} version
...
NOTE: The `{#include readme-header /}` will use a template located in the {product-name} project codestart which displays standard info from the `codestart.yml` metadata.

[#app-config]
=== application config application.yml

As a convention, you should always provide the {product-name} configuration as a yaml file (`base/src/main/resources/application.yml`).
...
NOTE: The `{#include index-entry /}` will use a template located in the {product-name} project codestart which displays standard info from the `codestart.yml` metadata.
...
NOTE: Before all the tests, the extension will generate {product-name} projects in the specified languages with the given codestart using mocked data and real data. You can find those generated projects in the `target/quarkus-codestart-test` directory. You can open the `real-data` ones in your IDE or play with them using the terminal. *The real data is the easiest way to iterate on your extension codestart development.*
...
The https://github.com/quarkusio/quarkus/blob/main/integration-tests/devtools/src/test/java/io/quarkus/devtools/codestarts/quarkus/ConfigYamlCodestartTest.java[ ConfigYamlCodestartTest, window="_blank"] is a good example in {product-name} core.
...
* https://github.com/quarkusio/quarkus/tree/main/independent-projects/tools/devtools-common/src/main/java/io/quarkus/devtools/codestarts/quarkus[{product-name} implementation of the Codestart generator, window="_blank"]
