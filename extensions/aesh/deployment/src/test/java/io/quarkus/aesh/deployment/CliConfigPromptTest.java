...
/**
 * Tests that a custom {@code quarkus.aesh.prompt} value appears in remote
 * connection output.
 */
public class CliConfigPromptTest {

    @RegisterExtension
    static final QuarkusUnitTest config = new QuarkusUnitTest()
            .withApplicationRoot(jar -> jar.addClasses(DummyCommand.class))
            .overrideConfigKey("quarkus.aesh.prompt", "myapp> ")
    ...
    @Test
    public void testCustomPromptAppearsInOutput() throws Exception {
        ...
        boolean received = promptLatch.await(10, TimeUnit.SECONDS);
        connection.close();
        handler.join(10_000);

        String allOutput = String.join("", output);
        Assertions.assertThat(received)
                .as("Expected custom prompt 'myapp>' in output. Got: %s", allOutput)
                .isTrue();
    }
    ...
    /**
     * StubConnection that captures stdout output as String data.
     */
