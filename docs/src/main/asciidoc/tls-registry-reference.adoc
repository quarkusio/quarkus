...
Quarkus exposes two properties to control PQC key exchange, both of which apply to the HTTP server, HTTP client, REST client, and mailer.
...
`client-negotiated`:: The server advertises PQC groups and prefers them, but accepts connections from clients that do not support them, falling back to classical key exchange.
Use this when you want PQC where available without breaking existing clients.

`strict`:: The server only accepts connections that use a post-quantum key exchange group.
Clients that do not negotiate a PQC group are rejected.
Use this when all clients are known to support PQC.

[IMPORTANT]
====
`pqc-enforcement-policy` does more than enforce PQC: it controls which SSL engine Vert.x selects.
Setting `pqc-enforcement-policy` to `client-negotiated` or `strict` activates the native OpenSSL engine (requires OpenSSL 3.5+), which negotiates PQC key exchange groups.
With the default `relaxed` policy, Vert.x uses the JDK SSL engine, which ignores any PQC key-exchange groups before JDK 27 — meaning that configuring PQC groups without also setting the enforcement policy results in no PQC key exchange at all.
On JDK 27+, the JDK natively supports `X25519MLKEM768` and PQC key exchange works without changing the enforcement policy.

Quarkus logs a warning at startup if you configure a PQC group with `relaxed` policy to help catch this misconfiguration.
====

[NOTE]
====
When using `client-negotiated` or `strict`, `quarkus.tls.key-exchange-groups` must include at least one PQC-capable group such as `X25519MLKEM768`.
Vert.x validates this at startup and throws an exception if no compatible engine or group is configured.
====

[WARNING]
====
`strict` enforcement will reject any client that does not support the configured PQC group, including standard browsers and TLS clients that have not yet adopted post-quantum extensions.
...
If you are running on JDK 27+, or on JDK 25+ once the PQC backport ships (expected October 2026), you can still enable PQC key exchange by setting `quarkus.tls.key-exchange-groups` — the negotiation will work correctly without policy enforcement.
For full enforcement, OpenSSL 3.5+ via the native SSL engine is required.
====

[IMPORTANT]
====
PQC enforcement (`client-negotiated` and `strict`) relies on a native OpenSSL 3.5+ library.
...
On those platforms, use `relaxed` with JDK 27+ for PQC key exchange.
====
