...
public class ApplicationYamlHotDeploymentTest {
    ...
    static final QuarkusDevModeTest test = new QuarkusDevModeTest().withApplicationRoot((jar) -> jar
            .addAsResource("application.yaml").addAsResource("application.yml").addClass(FooResource.class));

    @Test
    public void testConfigReload() {
        RestAssured.when().get("/foo").then().statusCode(200).body(is("AAAA"));

        RestAssured.when().get("/foo2").then().statusCode(200).body(is("CCCC"));

        test.modifyResourceFile("application.yaml", s -> s.replace("AAAA", "BBBB"));

        RestAssured.when().get("/foo").then().statusCode(200).body(is("BBBB"));

        test.modifyResourceFile("application.yml", s -> s.replace("CCCC", "DDDD"));

        RestAssured.when().get("/foo2").then().statusCode(200).body(is("DDDD"));
    }
}
