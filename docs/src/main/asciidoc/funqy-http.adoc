...
----
== Customising HTTP Responses

For HTTP requests, the Response can be customized using `HttpResponse` class. Here's a simple example:

[source, java]
----
@Funq
public HttpResponse funqHttpResponse(String name) {
    ...
    return new HttpResponse(404, "User not found");
}
----

If the response body is of complex type such as an object, it will be converted to json and sent as the response body.
...
public class MyFunctions {
    @Funq
    public HttpResponse saveFamily(String name) {
        ...
        return new HttpResponse(201, family);
    }
}
----
