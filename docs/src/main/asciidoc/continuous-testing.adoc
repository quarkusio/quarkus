...
In addition to the built-in hotkeys above, you can define your own aliases for the Quarkus dev mode terminal.
Aliases are read from `~/.quarkus/console-aliases.txt`, which is created automatically the first time you run an
...
For example, to bind `t` to re-running tests with output enabled:
...
If the aliased command contains spaces, it must be quoted:

[source]
----
alias ll=config set quarkus.log.console.format="%d{HH:mm:ss,SSS} %-5p [%c{3.}] (%t) %s%e%n"
----

There are two ways an alias can be used, depending on its name:

* *Single-character aliases* (e.g. `alias x=...`) become additional hotkeys, exactly like the built-in `[r]`, `[s]`,
`[h]` commands above. They are also listed under a "User Defined Aliases" heading when you press `h`.
* *Multi-character aliases* are available from the full interactive Quarkus Terminal. Press `:` while dev mode is
running to enter this mode, where normal Aesh alias resolution applies.

NOTE: Alias names that collide with a built-in hotkey (e.g. `r`, `s`, `h`) are ignored in favor of the built-in
command.
