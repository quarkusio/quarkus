...
:summary: This guide demonstrates how your {product-name-first} application can utilize the SmallRye Health extension.

This guide demonstrates how your {product-name} application can use https://github.com/smallrye/smallrye-health/[SmallRye Health]
...
If you already have your {product-name} project configured, you can add the `smallrye-health` extension
...
As we have started our {product-name} application in dev mode simply repeat the request 
...
Congratulations! You've created your first {product-name} health check procedure. Let's 
...
because we are running {product-name} in dev mode you can add `database.up=true` in 
...
For example, `quarkus-agroal` that is used to manage {product-name} datasource(s) automatically register a readiness health check
...
The {product-name} `smallrye-health` extension ships with `health-ui` and enables it by default in dev and test modes, but it can also be explicitly configured for production mode as well.
...
All that is needed to enable the SmallRye Health features in {product-name} is:

:devtools-wrapped:
* adding the `smallrye-health` {product-name} extension to your project using the 
