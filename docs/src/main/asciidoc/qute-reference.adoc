...
If the tag template declares the type of a parameter with a <<typesafe_expressions,parameter declaration>>, such as `{@org.acme.User it}` or `{@java.lang.Integer size=10}`, then the arguments passed by the callers of the tag are validated against these declarations at build time.
The build fails if the type of an argument is known and it is not assignable to the declared type; a widening numeric conversion, e.g. an `int` argument for a `java.lang.Long` parameter, is accepted.
Arguments whose type cannot be determined, arguments that are not declared in the tag template and declared parameters that are not passed by the caller are not validated.
