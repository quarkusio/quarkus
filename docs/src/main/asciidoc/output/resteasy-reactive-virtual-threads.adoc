...
----
<1> Instructs {product-name-first} to invoke this method on a virtual thread
...
1. {product-name} creates the virtual thread to invoke your endpoint (because of the `@RunOnVirtualThread` annotation).
...
7. The result is captured by {product-name} and written in the HTTP response
