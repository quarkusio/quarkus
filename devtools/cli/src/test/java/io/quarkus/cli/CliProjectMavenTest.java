...
public class CliProjectMavenTest {
    ...
    public void testCreateAppDefaults() throws Exception {
        ...
        Assertions.assertTrue(project.resolve("mvnw").toFile().exists(), "Wrapper should exist by default");
    ...
    public void testCreateAppWithoutDockerfiles() throws Exception {
        ...
        Assertions.assertTrue(result.stdout.contains("SUCCESS"),
                "Expected confirmation that the project has been created." + result);
        Assertions.assertFalse(Files.exists(project.resolve("src/main/docker")), "Docker folder should not exist");
    }
    ...
    public void testCreateAppOverrides() throws Exception {
        ...
        CliDriver.Result result = CliDriver.execute(workspaceRoot, "create", "app", "--verbose", "-e", "-B",
                "--no-wrapper", "--package-name=custom.pkg", "--output-directory=" + nested,
                "--app-config=" + String.join(",", configs), "--data=" + String.join(",", data),
                "-x rest,micrometer-registry-prometheus", "silly:my-project:0.1.0");
        ...
        result = CliDriver.execute(workspaceRoot, "create", "app", "--dry-run", "--verbose", "-e", "-B",
                "--output-directory=" + nested, "silly:my-project:0.1.0");
        Assertions.assertEquals(CommandLine.ExitCode.OK, result.exitCode, "Expected OK return code. " + result);
        Assertions.assertTrue(result.stdout.contains("WARN"),
                "Expected a warning that the directory already exists. " + result);

        String greetingResource = CliDriver
                .readFileAsString(project.resolve("src/main/java/custom/pkg/GreetingResource.java"));
    ...
    public void testBuildOptions() throws Exception {
        ...
        Assertions.assertTrue(result.stdout.contains(" clean"), "mvn command should specify 'clean'\n" + result);
        ...
        Assertions.assertTrue(result.stdout.contains("-Dnative"), "mvn command should specify -Dnative\n" + result);

        Assertions.assertTrue(result.stdout.contains("--offline"), "mvn command should specify --offline\n" + result);

        // 2 --no-clean --no-tests
        result = CliDriver.execute(project, "build", "-e", "-B", "--dry-run", "--no-clean", "--no-tests");

        Assertions.assertFalse(result.stdout.contains(" clean"), "mvn command should not specify 'clean'\n" + result);
    ...
    public void testDevTestOptions() throws Exception {
        ...
        result = CliDriver.execute(project, "dev", "-e", "--dry-run", "--clean", "--tests", "--debug", "--suspend",
                "--debug-mode=listen", "--offline");

        Assertions.assertEquals(CommandLine.ExitCode.OK, result.exitCode,
                "Expected OK return code. Result:\n" + result);
        Assertions.assertTrue(result.stdout.contains("MAVEN"), "quarkus command should specify 'MAVEN'\n" + result);
        ...
        Assertions.assertTrue(result.stdout.contains("-Dsuspend"), "mvn command should specify '-Dsuspend'\n" + result);

        Assertions.assertTrue(result.stdout.contains("--offline"), "mvn command should specify --offline\n" + result);

        // 2 --no-clean --no-tests --no-debug
        result = CliDriver.execute(project, "dev", "-e", "--dry-run", "--no-clean", "--no-tests", "--no-debug");
        ...
        result = CliDriver.execute(project, "dev", "-e", "--dry-run", "--no-suspend", "--debug-host=0.0.0.0",
                "--debug-port=8008", "--debug-mode=connect", "--", "arg1", "arg2");

        Assertions.assertEquals(CommandLine.ExitCode.OK, result.exitCode,
                "Expected OK return code. Result:\n" + result);
        Assertions.assertTrue(result.stdout.contains("MAVEN"), "quarkus command should specify 'MAVEN'\n" + result);

        Assertions.assertTrue(result.stdout.contains("-DdebugHost=0.0.0.0 -Ddebug=client -DdebugPort=8008"),
        ...
        result = CliDriver.execute(project, "test", "-e", "--dry-run", "--clean", "--debug", "--suspend",
                "--debug-mode=listen", "--offline", "--filter=FooTest");
        Assertions.assertEquals(CommandLine.ExitCode.OK, result.exitCode,
                "Expected OK return code. Result:\n" + result);
        ...
        result = CliDriver.execute(project, "test", "-e", "--dry-run", "--once", "--offline", "--filter=FooTest");
        Assertions.assertEquals(CommandLine.ExitCode.OK, result.exitCode,
                "Expected OK return code. Result:\n" + result);
    ...
    public void testCreateArgPassthrough() throws Exception {
        ...
        CliDriver.Result result = CliDriver.execute(workspaceRoot, "create", "--verbose", "-e", "-B", "--dryrun",
                "--no-wrapper", "--package-name=custom.pkg", "-x rest", "-x micrometer", "--output-directory=" + nested,
        ...
        Assertions.assertTrue(result.stdout.contains("MAVEN"), "Should contain MAVEN, found: " + result.stdout);
    ...
    public void testCreateArgJava17() throws Exception {
        CliDriver.Result result = CliDriver.execute(workspaceRoot, "create", "app", "-e", "-B", "--verbose", "--java",
                "17");
    ...
    public void testCreateNameDescription() throws Exception {
        CliDriver.Result result = CliDriver.execute(workspaceRoot, "create", "app", "--name", "My name",
