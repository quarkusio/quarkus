...
public class CombinedFormBasicAuthTestCase {
    ...
    public void testBasicBasedAuthSuccess() {
        RestAssured.given().auth().preemptive().basic("admin", "admin").redirects().follow(false).when().get("/admin")
                .then().assertThat().statusCode(200).body(equalTo("admin:/admin"));
    ...
    public void testBasicAuthFailure() {
        CookieFilter cookies = new CookieFilter();
        RestAssured.given().auth().preemptive().basic("admin", "wrongpassword").filter(cookies).redirects()
                .follow(false).get("/admin").then().assertThat().statusCode(401)
