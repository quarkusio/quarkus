...
:summary: This guide explains how to start using gRPC in your {product-name-first} application.

This page explains how to start using gRPC in your {product-name} application.
While this page describes how to configure it with Maven, it is also possible to use Gradle.

Let's imagine you have a regular {product-name} project, generated from the https://code.quarkus.io[{product-name} project generator].
...
Add the {product-name} gRPC extension to your build file:
...
NOTE: Your `proto` file must not contain `option java_generic_services = true;`. https://developers.google.com/protocol-buffers/docs/reference/java-generated?hl=en#service[Generic services are deprecated] and are not compatible with {product-name} code generation plugins.
...
The {product-name} code generation mechanism lets you control the scope of dependencies to scan for possible imports by setting the `quarkus.generate-code.grpc.scan-for-imports` property to one of the following:
...
Another thing to take note as well is that {product-name}' gRPC support currently includes 3 different types of gRPC usage:
...
With {product-name}, implementing a service requires to implement the generated service interface based on Mutiny, a Reactive Programming API integrated in {product-name}, and expose it as a CDI bean. 
...
Like any other {product-name} applications, you can package it with: `mvn package`.
...
Alternatively to using {product-name} code generation to generate stubs for `proto` files, you can also use
`protobuf-maven-plugin`.
...
NOTE: This configuration instructs the `protobuf-maven-plugin` to generate the default gRPC classes and classes using Mutiny to fit with the {product-name} development experience.
