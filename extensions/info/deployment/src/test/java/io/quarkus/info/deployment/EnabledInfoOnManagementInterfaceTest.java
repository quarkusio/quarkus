...
public class EnabledInfoOnManagementInterfaceTest {
    ...
    public void test() {
        ...
        when().get("http://localhost:9001/q/info").then().statusCode(200).body("os", is(notNullValue()))
                .body("os.name", is(notNullValue())).body("java", is(notNullValue()))
                .body("java.version", is(notNullValue())).body("build", is(notNullValue()))
                .body("build.time", is(notNullValue())).body("git", is(notNullValue()))
