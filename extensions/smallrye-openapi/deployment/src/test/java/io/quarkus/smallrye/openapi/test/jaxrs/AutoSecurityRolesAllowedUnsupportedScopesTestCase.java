...
/**
 * Run same tests as {@link AutoSecurityRolesAllowedTestCase}, but with OpenAPI version 3.0.2 that only allowed security
 * requirement scopes for Oauth2 and OpenID Connect schemes.
 */
public class AutoSecurityRolesAllowedUnsupportedScopesTestCase {

    @RegisterExtension
    static QuarkusUnitTest runner = new QuarkusUnitTest()
            .withApplicationRoot((jar) -> jar.addClasses(ResourceBean.class, OpenApiResourceSecuredAtClassLevel.class,
                    OpenApiResourceSecuredAtClassLevel2.class, OpenApiResourceSecuredAtMethodLevel.class,
                    OpenApiResourceSecuredAtMethodLevel2.class).addAsResource(new StringAsset("""
    ...
    void testAutoSecurityRequirement() {
        ...
        RestAssured.given().header("Accept", "application/json").when().get("/q/openapi").then().log().body().and()
                ...
                .body("paths.'/resource2/test-security/classLevel/4'.get.security", defaultSecurity).and()
