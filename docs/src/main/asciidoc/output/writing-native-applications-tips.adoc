 ...
 * in the context of an extension, {product-name-first} offers a lot of infrastructure to simplify all of this.
...
{product-name} automatically includes the resources present in `META-INF/resources` (the web resources) but, outside this directory, you are on your own.
...
By default, {product-name} initializes all classes at build time.
...
Which is more often than not caused by {product-name} initializing at build time a class with a static `Random`/`SplittableRandom` field,
...
The vast majority of {product-name} XML users don’t need to deserialize images,
so there shouldn’t be a need for users applications to include Java AWT code,
unless they specifically configure {product-name} to add the JAXB AWT code to the native executable.
...
then {product-name} could choose not to include that module unless the user specifically requested the need to serialize/deserialize XML files containing images at build time.
...
{product-name} marks all code to be initialized at build time by default.
...
will always return the same value when executed as a {product-name} native executable.
...
Supporting native in a {product-name} extension is even easier as {product-name} provides a lot of tools to simplify all this.

[WARNING]
====
Everything described here will only work in the context of {product-name} extensions, it won't work in an application.
====

=== Register reflection

{product-name} makes registration of reflection in an extension a breeze by using `ReflectiveClassBuildItem`, thus eliminating the need for a JSON configuration file.

To register a class for reflection, one would need to create a {product-name} processor class and add a build step that registers reflection:
...
In the context of an extension, {product-name} eliminates the need for a JSON configuration file by allowing extension authors to specify a `NativeImageResourceBuildItem`:
...
{product-name} simplifies things by allowing extensions authors to simply register a `RuntimeInitializedClassBuildItem`. A simple example of doing so could be:
...
Very similarly, {product-name} allows extensions authors to register a `NativeImageProxyDefinitionBuildItem`. An example of doing so is:
