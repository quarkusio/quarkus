...
The {product-name-first} gRPC extension integ{product-name}PC in Quarkus application.
It:

* supports implementing gRPC services
* supports consuming gRPC services
* integrates with the reactive engine from {product-name} as well as the reactive development model
...
{product-name} gRPC is based on https://vertx.io/docs/vertx-grpc/java/[Vert.x gRPC].
