...
/**
 * Tests that sub-commands listed in {@code @CommandDefinition(groupCommands=...)}
 * are not promoted to top-level CLI commands in console mode.
 * <p>
 * Without this fix, the AeshProcessor would add {@code commands} to all
...
public class ConsoleGroupCommandTest {
    ...
    static final QuarkusProdModeTest config = new QuarkusProdModeTest()
            ...
            // Console mode is detected (multiple commands commands)
    ...
    public void testConsoleModeBuildWithGroupCommands() {
        ...
        // 1. commands was added to StandaloneCmd (standalone @CommandDefinition)
        // 2. commands was NOT added to RunSub and StatusSub (sub-commands of ToolGroup)
        // 3. ToolGroup (explicit commands) is registered as a top-level command
    ...
    @CommandDefinition(name = "tool", description = "Tool group", groupCommands = { RunSub.class, StatusSub.class })
