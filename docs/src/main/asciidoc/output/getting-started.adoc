...
:summary: Discover how to create your first {product-name-first} application.
:numbered:
:sectnums:
:sectnumlevels: 4

Learn how to create a Hello World {product-name} app.
This guide covers:
...
The easiest way to create a new {product-name} project is to open a terminal and run the following command:
...
You will find the import of the {product-name} BOM, allowing you to omit the version of the different {product-name} dependencies.
...
* the {product-name} Gradle plugin
* an `enforcedPlatform` directive for the {product-name} BOM
...
====
With {product-name}, there is no need to create an `Application` class. It's supported, but not required. In addition, only one instance
...
2019-02-28 17:05:22,635 INFO  [io.qua.dep.QuarkusAugmentor] (main) {product-name} augmentation completed in 288ms
2019-02-28 17:05:22,770 INFO  [io.quarkus] (main) {product-name} started in 0.668s. Listening on: http://localhost:8080
...
Dependency injection in {product-name} is based on ArC which is a CDI-based dependency injection solution tailored for {product-name}' architecture.
If you're new to CDI then we recommend you to read the xref:cdi.adoc[Introduction to CDI] guide.

{product-name} only implements a subset of the CDI features and comes with non-standard features and specific APIS, you can learn more about it in the xref:cdi-reference.adoc[Contexts and Dependency Injection guide].
...
`quarkus:dev` runs {product-name} in development mode. This enables live reload with background compilation, which means
...
{product-name} supports https://junit.org/junit5/[JUnit 5] tests.
...
The test port can be controlled via the `quarkus.http.test-port` config property. {product-name} also creates a system
...
{product-name} heavily utilizes https://github.com/wildfly/jandex[Jandex] at build time, to discover various classes or annotations. One immediately recognizable application of this, is CDI bean discovery.
As a result, most of the {product-name} extensions will not work properly if this build time discovery isn't properly setup.

This index is created by default on the project on which {product-name} is configured for, thanks to our Maven and Gradle plugins.
...
By default, when a {product-name} application starts (in regular or dev mode), it will display an ASCII art banner. The banner can be disabled by setting `quarkus.banner.enabled=false` in `application.properties`,
...
Various {product-name} extensions contribute non-application endpoints that provide different kinds of information about the application.
...
If the application contains the `quarkus-info` extension, then {product-name} will by default expose the `/q/info` endpoint which provides information about the build, java version, version control, and operating system. The level of detail of the exposed information is configurable.
...
This guide covered the creation of an application using {product-name}.
...
If you are interested in reactive, we recommend the xref:getting-started-reactive.adoc[Getting Started with Reactive guide], where you can see how to implement reactive applications with {product-name}.
