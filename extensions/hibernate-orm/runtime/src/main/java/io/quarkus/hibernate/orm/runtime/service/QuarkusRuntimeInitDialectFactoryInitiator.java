    ...
    public QuarkusRuntimeInitDialectFactoryInitiator(String persistenceUnitName,
        ...
        if (database.startOffline() && database.versionCheckEnabled().filter(v -> v.booleanValue()).isPresent()) {
            throw new PersistenceException(
                    "Version check `quarkus.hibernate-orm.database.version-check.enabled` cannot be enabled while using offline mode `quarkus.hibernate-orm.database.start-offline=true`");
        }

        this.versionCheckEnabled = database.versionCheckEnabled()
                ...
                .orElse(!database.startOffline() && recordedConfig.getExplicitDialect().isEmpty());
    }
