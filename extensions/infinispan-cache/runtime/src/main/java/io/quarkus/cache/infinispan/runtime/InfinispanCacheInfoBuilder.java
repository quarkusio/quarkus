...
public class InfinispanCacheInfoBuilder {
    ...
    public static Set<InfinispanCacheInfo> build(Set<String> cacheNames,
            InfinispanCachesBuildTimeConfig buildTimeConfig, InfinispanCachesConfig runtimeConfig) {
        if (cacheNames.isEmpty()) {
            return Collections.emptySet();
        } else {
            Set<InfinispanCacheInfo> result = HashSetFactory.<InfinispanCacheInfo> getInstance()
