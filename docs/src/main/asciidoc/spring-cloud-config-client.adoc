...
== Connect to Config Server through OIDC

By default, the extension supports basic authentication. You can get support for OIDC by adding the following extensions:

[source,xml]
----
<dependency>
    <groupId>io.quarkus</groupId>
    <artifactId>quarkus-spring-cloud-config-client-oidc</artifactId>
</dependency>
<dependency>
    <groupId>io.quarkus</groupId>
...
Once added, the extension will use the configured OIDC client. See the link:security-openid-connect-client[OpenID Connect Client] guide for more information about OIDC configuration.
