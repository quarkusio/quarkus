...
To set up a multi-module maven project we need to adjust the configuration given above slightly. Add or replace the following elements in your root `pom.xml` of the parent project:
...
<4> Remove the `build` step from the plugin, we will add this to the submodule.
<5> Set up the submodules

Next create the `pom.xml` files for the submodules in there respective subfolders:
[source,xml,subs=attributes+]
----
<project>
    [...]
...
<1> Link to the parent and make sure it's the correct version by using the `revision` property.
