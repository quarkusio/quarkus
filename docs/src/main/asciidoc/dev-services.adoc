...
The OpenTelemetry Dev Service will be enabled when the `quarkus-opentelemetry` extension is present in your application, and
the exporter server address for traces has not been explicitly configured. More information can be found in the
xref:opentelemetry-dev-services.adoc[OpenTelemetry Dev Services Guide].
