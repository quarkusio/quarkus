...
mode, run `mvn quarkus:test`, run `gradle quarkusTest` with the `io.quarkus` Gradle plugin, or run
`./gradlew quarkusApplicationContinuousTest --continuous` with the preview
xref:gradle-application-plugin.adoc#gradle-application-plugin-continuous-testing[`io.quarkus.application` Gradle plugin].
...
These options, when used with `maven quarkus:[dev|test]` or the `io.quarkus` plugin's `gradle quarkus[Dev|Test]` tasks, work just like they work with `mvn test` or `gradle test`, except that they filter the set of tests executed during continuous testing.
For the `io.quarkus.application` Gradle plugin, pass Gradle's `--tests` option to the xref:gradle-application-plugin.adoc#gradle-application-plugin-continuous-testing[dev or continuous-test task].
