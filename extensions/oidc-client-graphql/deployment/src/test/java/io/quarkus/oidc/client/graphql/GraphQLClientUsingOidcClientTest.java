...
public class GraphQLClientUsingOidcClientTest {
    ...
    private static final Class<?>[] testClasses = { ProtectedResource.class, GraphQLClientResource.class,
            AnnotationTypesafeGraphQLClient.class, DefaultTypesafeGraphQLClient.class };

    @RegisterExtension
    static final QuarkusDevModeTest test = new QuarkusDevModeTest().withApplicationRoot(
            (jar) -> jar.addClasses(testClasses).addAsResource("application.properties", "application.properties"));

    @Test
    public void typesafeClientAnnotation() {
        // OidcClient selected via @OidcClient("clientName")
        RestAssured.when().get("/oidc-graphql-client/annotation-typesafe").then().statusCode(200).body(equalTo("jdoe"));
    }
    ...
    public void typesafeClientWithDefault() {
        ...
        RestAssured.when().get("/oidc-graphql-client/default-typesafe").then().statusCode(200).body(equalTo("alice"));
    ...
    public void dynamicClientWithDefault() {
        ...
        RestAssured.when().get("/oidc-graphql-client/default-dynamic").then().statusCode(200).body(equalTo("alice"));
    }
