...
https://www.ietf.org/archive/id/draft-ietf-oauth-attestation-based-client-auth-09.html[OAuth 2.0 Attestation-Based Client Authentication] allows clients to authenticate without a pre-shared secret.

When attestation is enabled, Quarkus OIDC generates a key pair, requests an Attestation JWT from a default or registered Client Attester, generates a Proof of Possession JWT that proves it owns a private key and is cryprographically bound to the Attestation JWT and sends both JWTs as HTTP headers:
...
The signature algorithm defaults to `ES256`. To use a different algorithm:
...
It exposes its public key via a JWKS endpoint at `/.well-known/attestation-jwks` for the default tenant, or `/.well-known/attestation-jwks/<tenant-id>` for named OIDC tenants.
...
public class CustomClientAttester implements ClientAttester {
    ...
    public Uni<String> attest(ClientAttestationContext context) {
        // context.clientId() - the OIDC client ID
        // context.keyOwnershipProof() - cryptographic material for proving key ownership to external Client Attester
        // return Uni.createFrom().item(
        //        callExternalAttestationService(context.clientId(), context.keyOwnershipProof()));
    }
}
----
