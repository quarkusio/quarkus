...
import io.quarkus.runtime.configuration.Services;
...
public class VertxHttpRecorder {
    ...
    private static final Handler<HttpServerRequest> ACTUAL_ROOT = new Handler<>() {
        ...
        vertx.deployVerticle(new Supplier<Verticle>() {
            ...
            public Verticle get() {
                return new WebDeploymentVerticle(httpMainServerOptions, httpMainSslServerOptions, httpMainDomainSocketOptions,
                        insecureRequestStrategy, httpConfiguration, connectionCount, registry, startEventsFired);
            }
                ...
                public void handle(AsyncResult<HttpServer> event) {
                    if (event.cause() != null) {
                        ...
                        if (https) {
                            ...
                            Services.register("quarkus://vertx-https", "https", options.getHost(), actualHttpsPort);
                        } else {
                            actualHttpPort = actualPort;
                            validateHttpPorts(actualHttpPort, actualHttpsPort);
                            Services.register("quarkus://vertx-http", "http", options.getHost(), actualHttpPort);
                        }
