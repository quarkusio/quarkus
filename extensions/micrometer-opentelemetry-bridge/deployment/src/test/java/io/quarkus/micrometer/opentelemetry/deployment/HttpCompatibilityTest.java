...
public class HttpCompatibilityTest {
    ...
    void testHttpTimerToHistogramCompatibility() {
        ...
        metricExporter.assertCountDataPointsAtLeast("http.server.requests", null, 10);
                        ...
                        .hasPointsSatisfying(
                                ...
                                point -> point.hasCount(1)
                                        .hasAttributesSatisfying(
                                                // FIXME "uri" not following conventions and should be "http.route"
                                                equalTo(URI, "/vertx/item/{id}"),
                                                //   FIXME method not following conventions and should be "http.request.method"
                                                equalTo(METHOD, "GET"),
                                                //   FIXME status_code not following conventions and should be
        ...
        final MetricData clientMetricData = clientMetricDataList.stream()
                .max(Comparator.comparingInt(data -> data.getData().getPoints().size()))
                .get();

        clientMetricDataList.forEach(System.out::println);
