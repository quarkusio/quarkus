...
/**
 * Agroal registers an interposed synchronization which validates that connections have been released. Components such
 * as hibernate release connections in an interposed synchronization. Therefore, we must ensure that Agroal runs last.
 * <p>
 * This wrapper re-orders interposed synchronizations as follows: [other, hibernate-orm, agroal].
 * <p>
 * Synchronizations are placed into groups according to their package name and the groups are ordered which means that
 * all hibernate synchronizations run before Agroal ones and all other synchs run before the hibernate ones.
 * <p>
