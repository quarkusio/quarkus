...
class MavenEffectiveModelResolverTest {
    ...
    @Test
    void shouldResolveProfilePluginConfigurationWithMavenXmlService() throws Exception {
        var resolver = new MavenEffectiveModelResolver(new InMemoryPomResolver(Map.of(
                new GAV("org.acme", "app", "1.0"), pom("""
                        <project>
                          <modelVersion>4.0.0</modelVersion>
                          ...
                          <profiles>
                            <profile>
                              <id>merge-plugin-configuration</id>
                              <activation>
        ...
        try {
            ...
            assertThat(effectiveModel.getBuild().getPlugins())
                    .filteredOn(plugin -> plugin.getArtifactId().equals("maven-compiler-plugin"))
                    .singleElement()
                    .satisfies(plugin -> assertThat(plugin.getConfiguration().toString())
                            .contains("<release>17</release>", "<parameters>true</parameters>"));
        } finally {
