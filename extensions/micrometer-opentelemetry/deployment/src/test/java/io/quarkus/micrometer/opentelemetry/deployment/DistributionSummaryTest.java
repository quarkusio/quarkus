...
public class DistributionSummaryTest {
    ...
    static final QuarkusUnitTest TEST = new QuarkusUnitTest().setArchiveProducer(() -> ShrinkWrap
            .create(JavaArchive.class).addClasses(ManualHistogramBean.class)
    ...
    void histogramTest() {
        ...
        assertThat(testSummary).hasDescription("This is a test distribution summary").hasUnit("things")
                .hasHistogramSatisfying(histogram -> histogram.hasPointsSatisfying(
                        points -> points.hasSum(555.5).hasCount(4).hasAttributes(attributeEntry("tag", "value"))));

        MetricData textSummaryMax = exporter.getFinishedMetricItem("testSummary.max");
        assertNotNull(textSummaryMax);
        assertThat(textSummaryMax).hasDescription("This is a test distribution summary")
                .hasDoubleGaugeSatisfying(gauge -> gauge.hasPointsSatisfying(
                        point -> point.hasValue(500).hasAttributes(attributeEntry("tag", "value"))));

        MetricData testSummaryHistogram = exporter.getFinishedMetricItem("testSummary.histogram"); // present when SLOs
                                                                                                   // are set
        assertNotNull(testSummaryHistogram);
        assertThat(testSummaryHistogram).hasDoubleGaugeSatisfying(gauge -> gauge.hasPointsSatisfying(
                point -> point.hasValue(1).hasAttributes(attributeEntry("le", "1"), attributeEntry("tag", "value")),
                point -> point.hasValue(2).hasAttributes(attributeEntry("le", "10"), attributeEntry("tag", "value")),
                point -> point.hasValue(3).hasAttributes(attributeEntry("le", "100"), attributeEntry("tag", "value")),
                point -> point.hasValue(4).hasAttributes(attributeEntry("le", "1000"),
    ...
    public static class ManualHistogramBean {
        ...
        public void recordHistogram() {
            DistributionSummary summary = DistributionSummary.builder("testSummary")
                    .description("This is a test distribution summary").baseUnit("things").tags("tag", "value")
                    .serviceLevelObjectives(1, 10, 100, 1000).distributionStatisticBufferLength(10).register(registry);
