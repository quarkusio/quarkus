...
public interface OidcTenantConfig extends OidcClientCommonConfig {
    ...
    interface Token {
         ...
         * claim to be present and set to `org_xyz`. On the other hand, if the configuration property was set to value
         * `quarkus.oidc.token.required-claims.org_id=org_xyz,org_abc`, the `org_id` claim would need to have both
         * `org_xyz` and `org_abc` claim values. Strings and arrays are the only supported types.
