...
Mandrel's main goal is to provide a way to build native executables specifically designed to support {product-name-first}.

* Mandrel releases are built from a code base derived from the upstream Oracle GraalVM CE code base,
with only minor changes but some significant exclusions that are not necessary for {product-name} native apps.
...
The reason for these exclusions is to provide a better level of support for the majority of {product-name} users.
...
[INFO] [io.quarkus.deployment.QuarkusAugmentor] {product-name} augmentation completed in 70686ms
[INFO]
...
2023-05-05 10:55:52,068 INFO  [io.quarkus] (main) getting-started 1.0.0-SNAPSHOT native (powered by {product-name} 3.0.2.Final) started in 0.009s. Listening on: http://0.0.0.0:8081
...
[TIP]
====
By default, {product-name} waits for 60 seconds for the native image to start before automatically failing the native tests. This
...
Quite often one only needs to create a native Linux executable for their {product-name} application (for example in order to run in a containerized environment) and would like to avoid
...
To this end, {product-name} provides a very convenient way of creating a native Linux executable by leveraging a container runtime such as Docker or podman.
...
[TIP]
====
By default, {product-name} automatically detects the container runtime.
...
These are regular {product-name} config properties, so if you always want to build in a container
...
By far the easiest way to create a container-image from your {product-name} application is to leverage one of the container-image extensions.
...
* `quarkus.container-image.build=true` instructs {product-name} to create a container-image using the final application artifact (which is the native executable in this case)

See the xref:container-image.adoc[Container Image guide] for more details.

=== Manually using the micro base image

You can run the application in a container using the JAR produced by the {product-name} Maven Plugin.
...
[NOTE]
.{product-name} Micro Image?
====
The {product-name} Micro Image is a small container image providing the right set of dependencies to run your native application.
...
{product-name} provides a distroless base image that you can use in your `Dockerfile`.
...
{product-name} provides the `quay.io/quarkus/quarkus-distroless-image:2.0` image.
...
{product-name} can compress the produced native executable using UPX.
...
Sources for third party jar dependencies, including {product-name} source code,
...
Please see the {product-name} Native Reference Guide for more detailed information on these monitoring options.
