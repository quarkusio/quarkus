...
public class DevModeConstraintValidationTest {
    ...
    static final QuarkusDevModeTest TEST = new QuarkusDevModeTest()
            .withApplicationRoot((jar) -> jar.addClasses(TestBean.class, DevModeTestResource.class,
                    ClassLevelConstraint.class, ClassLevelValidator.class, DependentTestBean.class));

    @Test
    public void testClassConstraintHotReplacement() {
        RestAssured.given().header("Content-Type", "application/json").when().body("{}").post("/test/validate").then()
                .body(containsString("ok"));

        TEST.modifySourceFile("TestBean.java", s -> s.replace("// <placeholder1>",
                "@io.quarkus.hibernate.validator.test.devmode.ClassLevelConstraint"));
    ...
    public void testMethodConstraintHotReplacement() {
        ...
        RestAssured.given().when().get("/test/mymessage").then().body(containsString("mymessage"));
        ...
        RestAssured.given().header("Content-Type", "application/json").when().get("/test/mymessage").then()
    ...
    public void testNewBeanHotReplacement() {
        ...
        TEST.modifySourceFile("DevModeTestResource.java", s -> s.replace("@Valid TestBean", "@Valid NewTestBean"));
    ...
    public void testNewConstraintHotReplacement() {
        ...
        TEST.modifySourceFile("TestBean.java", s -> s.replace("// <placeholder2>", "@NewConstraint"));
