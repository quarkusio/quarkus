...
[NOTE]
====
The `quarkus-reactive-transactions` extension is currently in preview status. The API and behavior may change in future versions.
====

To use `@Transactional` with Hibernate Reactive, you need to add the `quarkus-reactive-transactions` extension:
...
.build.gradle
----
implementation("io.quarkus:quarkus-reactive-transactions")
----
...
public class FruitResource {
    ...
    public Uni<Fruit> createFruit(Fruit fruit) {
        return persistFruit(fruit);
    }

    @PUT
    @Path("/{id}")
    public Uni<Fruit> updateFruit(Long id, @QueryParam("name") String newName) {
        return updateFruitName(id, newName);
    }

    @Transactional <2>
    public Uni<Fruit> findFruit(Long id) {
    ...
    @Transactional
    public Uni<Fruit> updateFruitName(Long id, String newName) {
        return session.find(Fruit.class, id)
                .map(fruit -> {
...
<2> Use `@Transactional` to define transaction boundaries for read operations
<3> Use `@Transactional` for write operations - the transaction will be committed automatically
