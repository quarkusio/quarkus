...
:extensions: io.quarkus:quarkus-resteasy-client,io.quarkus:quarkus-resteasy-client-jackson,io.quarkus:quarkus-resteasy-client-jsonb
...
:create-app-artifact-id: quarkus-rest-client-quickstart
:create-app-extensions: quarkus-rest-jackson,quarkus-resteasy-client-jackson
include::{includes}/devtools/create-app.adoc[]

This command generates the Maven project with a REST endpoint and imports:

* the `quarkus-rest-jackson` extension for the REST server support. Use `rest` instead if you do not wish to use Jackson;
* the `quarkus-resteasy-client-jackson` extension for the REST client support. Use `rest-client` instead if you do not wish to use Jackson

If you already have your Quarkus project configured, you can add the `quarkus-resteasy-client-jackson` extension
...
[NOTE]
====
When the `quarkus-resteasy-client-jackson` extension is installed, Quarkus will use the `application/json` media type
...
More information about this annotation can be found on the javadoc of link:https://javadoc.io/doc/io.quarkus/quarkus-resteasy-client/latest/io/quarkus/rest/client/reactive/ClientQueryParam.html[`@ClientQueryParam`].
...
More information about this annotation can be found on the javadoc of link:https://javadoc.io/doc/io.quarkus/quarkus-resteasy-client/latest/io/quarkus/rest/client/reactive/ClientFormParam.html[`@ClientFormParam`].
...
Using the configuration above, calling the `getById` method of `ExtensionsService` with a value of `io.quarkus:quarkus-resteasy-client` would result in an HTTP GET request being made to `https://stage.code.quarkus.io/api/extensions?id=io.quarkus:quarkus-resteasy-client`.
...
public class ExtensionsResourceTest {
    ...
    public void testExtensionsIdEndpoint() {
        given()
            .when().get("/extension/id/io.quarkus:quarkus-resteasy-client")
            .then()
            .statusCode(200)
            .body("$.size()", is(1),
                "[0].id", is("io.quarkus:quarkus-resteasy-client"),
...
public void testExtensionIdAsyncEndpoint() {
    given()
        .when().get("/extension/id-async/io.quarkus:quarkus-resteasy-client")
        .then()
        .statusCode(200)
        .body("$.size()", is(1),
            "[0].id", is("io.quarkus:quarkus-resteasy-client"),
...
Open your browser to http://localhost:8080/extension/id/io.quarkus:quarkus-resteasy-client.
...
public class WireMockExtensions implements QuarkusTestResourceLifecycleManager {  // <2>
    ...
    public Map<String, String> start() {
        ...
        wireMockServer.stubFor(get(urlEqualTo("/extensions?id=io.quarkus:quarkus-resteasy-client"))   // <4>
                .willReturn(aResponse()
                        .withHeader("Content-Type", "application/json")
                        .withBody(
                            "[{" +
                            "\"id\": \"io.quarkus:quarkus-resteasy-client\"," +
                            "\"name\": \"REST Client\"" +
                            "}]"
                        )));
...
<3> Launch Wiremock.
<4> Configure Wiremock to stub the calls to `/extensions?id=io.quarkus:quarkus-resteasy-client` by returning a specific canned response.
