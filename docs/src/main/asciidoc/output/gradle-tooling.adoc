...
:summary: Develop and build your {product-name-first} application with Gradle
:devtools-no-maven:
...
To scaffold a Gradle project you can either use the xref:cli-tooling.adoc[{product-name} CLI] or the {product-name} Maven plugin:
...
_For more information about how to install the {product-name} CLI and use it, please refer to xref:cli-tooling.adoc[the {product-name} CLI guide]._
****
...
[NOTE]
====
{product-name} project scaffolding automatically installs the Gradle wrapper (`./gradlew`) in your project.
...
But you can pass a partial name and {product-name} will do its best to find the right extension.
...
{product-name} can't draw the 'pretty' test output so falls back to just logging the output.
...
{product-name} extension dependencies are divided into the runtime extension dependencies that end up on the application runtime
classpath and the deployment (or build time) extension dependencies that are resolved by {product-name} only at application build time to create
the build classpath. Application developers are expected to express dependencies only on the runtime artifacts of {product-name} extensions.

To enable the use-case of building and testing a {product-name} application offline, the plugin includes the `quarkusGoOffline` task that could be called from the command line like this:
...
{product-name} integration tests (annotated with `@QuarkusIntegrationTest`) will run on the artifact produced by {product-name}.
...
{product-name} Gradle plugin supports the generation of Uber-Jars by specifying a `quarkus.package.type` argument as follows:
...
There are multiple configuration sources that influence {product-name} builds, mentioned in the order of their priority.
The {product-name} build uses the `prod` configuration profile:
...
[WARNING]
The above priorities have changed in {product-name} plugin starting with 3.0. Older versions of the {product-name} Gradle plugin
preferred `application.properties` over settings in the Gradle build.

[NOTE]
The {product-name} Gradle plugin uses the "standard" {product-name} mechanisms to load and parse configurations. Support for
`application.(yaml|yml)` has been added in {product-name} 3.0 in addition to `application.properties`. Also new in 3.0
is that all mechanisms available via SmallRye Config, are implicitly also now available for the {product-name} Gradle
plugin.


[TIP]
Use the `quarkusShowEffectiveConfig` task to show the effective configuration options used for a {product-name} build. If you
...
{product-name} application builds are ran in isolated processes using Gradle's worker API. This includes the {product-name}
application build and {product-name} code generation. This is necessary to properly pass the configuration from the
`quarkus` extension and Gradle project properties to {product-name}' code generator / application builder.

The JVM settings of the processes performing the code generation and/or {product-name} build can be configured as follows.
...
The {product-name} plugin leverages the Gradle mechanisms of up-to-date checks and the build cache. The build cache can be
...
The {product-name} Gradle plugin cares about _what_ is cached _in which environment_ (CI or local development). Big artifacts
like uber-jars and native binaries are not cached in CI, but are cached in local development environments.

TIP: The {product-name} Gradle plugin detects a _CI environment_, if the `CI` environment variable is present.

How the various {product-name} package types are cached in non-CI and CI environments is described in the following table.
Note that even if a task's output is not _cached_, the _up-to-date_ checks still apply.

NOTE: The {product-name} application build is split across three tasks. The `quarkusBuild` taskl is responsible to _provide_
a built {product-name} application. The tasks `quarkusDependenciesBuild` and `quarkusAppPartsBuild` are considered internal
tasks (may change at any time w/o prior notice). See below for details.

[cols="2,5,1,1"]
|===
|{product-name} package type|Notes|Caching (non-CI)|Caching in CI
|`fast-jar`, `jar`
...
|The `quarkusBuild` task is responsible for building the {product-name} application.
...
lower than the cost of re-building a {product-name} application. This means, that The {product-name} Gradle plugin allows caching
...
The {product-name} Gradle plugin works with builds that have the
link:https://docs.gradle.org/current/userguide/configuration_cache.html[Gradle's configuration cache] enabled, but
the configuration cache is disabled for the {product-name} tasks. This means, that the {product-name} plugin does not break such
Gradle builds.
