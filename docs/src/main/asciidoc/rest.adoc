...
The size of every non-file part in a multipart request must conform to the value of `quarkus.http.limits.max-form-attribute-size`, for which the default is 2048 bytes.
File parts are only bounded by `quarkus.http.limits.max-body-size` (the limit for the whole request, 10 MB by default) unless `quarkus.http.limits.max-multipart-file-size` is set, in which case every file part must conform to it individually.
Any request with a part of a size exceeding these limits will result in HTTP status code 413.
Note that `quarkus.http.limits.max-body-size` needs to be raised to accept large uploads at all.
