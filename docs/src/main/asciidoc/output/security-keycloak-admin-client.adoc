...
The {product-name-first} Keycloak Admin Client and its reactive twin support Keycloak Admin Client which can be used to configure a running Keycloak server.

This guide demonstrates how you can leverage the xref:cdi-reference.adoc[{product-name} ArC] and inject the admin client to your {product-name} application, as well as how to create it directly in the application code.
...
If you already have your {product-name} project configured, you can add the `keycloak-admin-client-reactive` and `resteasy-reactive-jackson` extensions
...
[source,properties]
----
# {product-name} based Keycloak distribution
