...
public class SmallRyeGraphQLDevUIProcessor {
    ...
    CardPageBuildItem createCard(NonApplicationRootPathBuildItem nonApplicationRootPathBuildItem) {
        ...
        PageBuilder schemaPage = Page.externalPageBuilder("GraphQL Schema").icon("font-awesome-solid:diagram-project")
                .url(schemaPath, schemaPath);

        // GraphiQL UI
        String uiPath = nonApplicationRootPathBuildItem.resolvePath(graphQLConfig.ui().rootPath());
        PageBuilder uiPage = Page.externalPageBuilder("GraphQL UI").icon("font-awesome-solid:table-columns")
                .url(uiPath + "/index.html?embed=true", uiPath);

        // Learn
        PageBuilder learnLink = Page.externalPageBuilder("Learn more about GraphQL")
                .icon("font-awesome-solid:graduation-cap").doNotEmbed().url("https://graphql.org/");

        PageBuilder assistantPage = Page.assistantPageBuilder().title("Generate clients")
