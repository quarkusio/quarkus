...
In dev mode, when the OpenTelemetry extension is present, an *Observability* section
appears in the Dev UI (`/q/dev-ui`) menu with a *Traces* view. It captures finished
spans into a small in-memory ring buffer and renders them grouped by trace as a
live waterfall — no containers or Dev Services required.
