...
public abstract class AbstractPermissionsAllowedTestCase {
    ...
    public static void setupUsers() {
        TestIdentityController.resetRoles().add("admin", "admin", "admin").add("user", "user").add("viewer", "viewer");
    }
    ...
    public void testStringPermission2RequiredPermissionsNonBlocking_MetaAnnotation() {
        ...
        // admin must have both 'create' and 'update' in order to succeed
        RestAssured.given().auth().basic("admin", "admin").post("/permissions-non-blocking/meta-create-update").then()
                .statusCode(200).body(Matchers.equalTo("done"));

        // user has only 'update', therefore should fail
        RestAssured.given().auth().basic("user", "user").post("/permissions-non-blocking/meta-create-update").then()
                .statusCode(403);
    }

    @Test
    public void testStringPermissionOneOfPermissionsAndActions() {
        // invokes GET /permissions/admin endpoint that requires one of 2 permissions: read:resource-admin,
        // read:resource-user
    ...
    public void testStringPermissionOneOfPermissionsAndActionsNonBlocking() {
        // invokes GET /permissions-non-blocking/admin endpoint that requires one of 2 permissions: read:resource-admin,
        // read:resource-user
        ...
        // viewer has 'read:resource-viewer', therefore fails
        RestAssured.given().auth().basic("viewer", "viewer").get("/permissions-non-blocking/admin").then()
                .statusCode(403);
    }
    ...
    public void testBlockingAccessToIdentityOnIOThread() {
        ...
        RestAssured.given().auth().basic("user", "user").get("/permissions/admin/security-identity").then()
                .statusCode(200).body(Matchers.equalTo("user"));

        // admin lack 'get-identity', therefore fails
        RestAssured.given().auth().basic("admin", "admin").get("/permissions/admin/security-identity").then()
                .statusCode(403);
        ...
        // admin lack 'get-identity', therefore fails
        RestAssured.given().auth().basic("admin", "admin").get("/permissions-non-blocking/admin/security-identity")
                .then().statusCode(403);
    }
    ...
    public void testCustomPermissionNonBlocking() {
        ...
        RestAssured.given().auth().basic("admin", "admin").param("greeting", "hello")
                .get("/permissions-non-blocking/custom-permission").then().statusCode(200)
    ...
    public void testCustomPermission() {
        ...
        // we send 'hi' => pass
        RestAssured.given().auth().basic("admin", "admin").param("greeting", "hi").get("/permissions/custom-permission")
                .then().statusCode(403);
    }
    ...
    public void testCustomPermissionWithAdditionalArgs() {
        ...
        reqAutodetectedExtraArgs("admin", "Ostrava").statusCode(200).body(Matchers.equalTo("so long Nelson 3 Ostrava"));
        // user has permission with place 'Prague'
        reqAutodetectedExtraArgs("user", "Prague").statusCode(200).body(Matchers.equalTo("so long Nelson 3 Prague"));
        ...
        // admin has permission with place 'Ostrava'
        reqExplicitlyMarkedExtraArgs("admin", "Ostrava").statusCode(200)
                .body(Matchers.equalTo("so long Nelson 3 Ostrava"));
        // user has permission with place 'Prague'
        reqExplicitlyMarkedExtraArgs("user", "Prague").statusCode(200)
    ...
    private static ValidatableResponse reqAutodetectedExtraArgs(String user, String place) {
        return RestAssured.given().auth().basic(user, user).pathParam("goodbye", "so long").header("toWhom", "Nelson")
                .cookie("day", 3).body(place).post("/permissions-non-blocking/custom-perm-with-args/{goodbye}").then();
    }

    private static ValidatableResponse reqExplicitlyMarkedExtraArgs(String user, String place) {
        return RestAssured.given().auth().basic(user, user).pathParam("goodbye", "so long").header("toWhom", "Nelson")
                .cookie("day", 3).body(place).post("/permissions/custom-perm-with-args/{goodbye}").then();
    }
