...
Applications that frequently read same entities can see their performance improved when Hibernate ORM second-level cache is enabled.
To enable second-level cache, select those entities are read most often and annotate them with `@Cacheable`:
...
Then, set `hibernate.cache.use_second_level_cache` to `true` to enable second-level cache.
Also set `javax.persistence.sharedCache.mode` to `ENABLE_SELECTIVE` so that only those entities that are annotated are cached.

By default entities are cached in regions named after their fully qualified name.
The size of each region can be controlled via `hibernate.cache.<region_name>.memory.size` properties.
Currently size is defined as the number of entities stored in this region.

Setting maximum idle time is another way of controlling the boundaries of each cached region.
Entities that exceed maximum idle time without being accessed are evicted from the cache.
The time is specified in number of seconds and it's set via `hibernate.cache.<region_name>.expiration.max_idle` property.
...
Please set use aggressive eviction size or time values in these scenarios to prevent accessing stale data.
--
...
To enable query caching, set `hibernate.cache.use_query_cache` to `true` and mark the query as cacheable:
