...
// this is a prototype; one day, it will become a public API in Jandex itself
class JandexTypeSystem {
    ...
    private Type callSubstitution(String typeVarId, Type typeVar, Function<String, Type> substitution) {
        ...
        if (substituted == null) {
            ...
            DotName erasure = typeVar.name();
            if (erasure.isComponentized() || !erasure.local().startsWith("[")) {
                return ClassType.create(erasure);
            } else {
                return Type.create(erasure, Type.Kind.ARRAY);
            }
        } else {
            return substituted;
        }
