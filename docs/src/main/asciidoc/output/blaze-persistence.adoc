...
standard mechanism as documented in the xref:cdi-reference.adoc#default_beans[{product-name-first} CDI reference].
...
In {product-name}, you just need to:
...
There are various optional properties useful to refine your `EntityViewManager` and `CriteriaBuilderFactory` or guide guesses of {product-name}.
