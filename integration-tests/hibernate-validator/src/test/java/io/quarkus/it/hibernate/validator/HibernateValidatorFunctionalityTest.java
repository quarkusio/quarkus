...
public class HibernateValidatorFunctionalityTest {
    ...
    public void testCustomClassLevelConstraint() throws Exception {
        ...
        RestAssured.when().get("/hibernate-validator/test/custom-class-level-constraint").then()
    ...
    public void testCDIBeanMethodValidation() {
        ...
        RestAssured.when().get("/hibernate-validator/test/cdi-bean-method-validation").then()
                .body(is(expected.toString()));
    }
    ...
    public void testCDIBeanMethodValidationUncaught() {
        ...
        if (isLogChecksPossible()) {
            ...
            assertThat(LogCollectingTestResource.current().getRecords()).extracting(LogCollectingTestResource::format)
                    .hasSize(1);
            assertThat(LogCollectingTestResource.current().getRecords()).element(0).satisfies(record -> {
                assertThat(record.getLevel()).isEqualTo(Level.SEVERE);
                assertThat(LogCollectingTestResource.format(record)).contains(
    ...
    public void testRestEndPointValidation() {
        ...
        // Bad request
        RestAssured.given().get("/hibernate-validator/test/rest-end-point-validation/plop/").then()
        ...
        // Should be ok
        RestAssured.given().get("/hibernate-validator/test/rest-end-point-validation/42/").then().body(is("42"));
    }
    ...
    public void testRestEndPointValidationUsingXmlMediaTypeWithComponents() {
        ...
        RestAssured.given().header("Accept", "application/xml;charset=UTF8")
                .get("/hibernate-validator/test/rest-end-point-validation/plop/").then()
                .statusCode(Response.Status.BAD_REQUEST.getStatusCode()).contentType(ContentType.XML)
                .body("violationReport.parameterViolations.message", containsString("numeric value out of bounds"));
    }
    ...
    public void testRestEndPointReturnValueValidation() {
        ...
        ValidatableResponse response = RestAssured.when()
                ...
                .body(containsString("Error id")).statusCode(Response.Status.INTERNAL_SERVER_ERROR.getStatusCode());
        if (isInternalErrorExceptionLeakedInQuarkusErrorHandlerResponse()) {
            response.body(containsString(ResteasyViolationExceptionImpl.class.getName())) // Exception type
    ...
    public void testRestEndPointInterfaceValidation() {
        RestAssured.when().get("/hibernate-validator/test/rest-end-point-interface-validation/plop/").then()
                .statusCode(400).body(containsString("numeric value out of bounds"));

        RestAssured.when().get("/hibernate-validator/test/rest-end-point-interface-validation/42/").then()
                .body(is("42"));
    }
    ...
    public void testRestEndPointInterfaceValidationWithAnnotationOnImplMethod() {
        RestAssured.when()
                ...
                .then().statusCode(400).contentType(ContentType.TEXT)
                .body(containsString("numeric value out of bounds"));

        RestAssured.when()
                .get("/hibernate-validator/test/rest-end-point-interface-validation-annotation-on-impl-method/42/")
                .then().contentType(ContentType.TEXT).body(is("42"));
    }

    @Test
    public void testRestEndPointInterfaceValidationWithAnnotationOnOverriddenMethod() {
        RestAssured.when().get(
                "/hibernate-validator/test/rest-end-point-interface-validation-annotation-on-overridden-method/plop/")
                .then().statusCode(400).contentType(ContentType.TEXT)
                .body(containsString("numeric value out of bounds"));

        RestAssured.when().get(
                "/hibernate-validator/test/rest-end-point-interface-validation-annotation-on-overridden-method/42/")
                .then().contentType(ContentType.TEXT).body(is("42"));
    }

    @Test
    public void testRestEndPointGenericMethodValidation() {
        RestAssured.when().get("/hibernate-validator/test/rest-end-point-generic-method-validation/9999999/").then()
                .statusCode(400).body(containsString("numeric value out of bounds"));
        ...
        RestAssured.when().get("/hibernate-validator/test/rest-end-point-generic-method-validation/42/").then()
                .body(is("42"));
    }

    @Test
    public void testNoProduces() {
        RestAssured.when().get("/hibernate-validator/test/no-produces/plop/").then().statusCode(400)
                .contentType(ContentType.JSON).body(containsString("numeric value out of bounds"));
    }
    ...
    public void testInjection() throws Exception {
        ...
        RestAssured.when().get("/hibernate-validator/test/injection").then().body(is(expected.toString()));
    }
    ...
    public void testInheritedImplementsConstraints() {
        ...
        RestAssured.when().get("/hibernate-validator/test/test-inherited-implements-constraints").then()
                .body(is(expected.toString()));
    }
    ...
    public void testInheritedExtendsConstraints() {
        ...
        RestAssured.when().get("/hibernate-validator/test/test-inherited-extends-constraints").then()
                .body(is(expected.toString()));
    }

    @Test
    public void testValidationMessageLocale() {
        RestAssured.given().header("Accept-Language", "en-US;q=0.25,hr-HR;q=1,fr-FR;q=0.5").when()
                .get("/hibernate-validator/test/test-validation-message-locale/1").then()
    ...
    public void testValidationMessageInvalidAcceptLanguageHeaderLeadsToDefaultLocaleBeingUsed() {
        RestAssured.given().header("Accept-Language", "invalid string").when()
                .get("/hibernate-validator/test/test-validation-message-locale/1").then()
    ...
    public void testValidationMessageDefaultLocale() {
        RestAssured.given().when().get("/hibernate-validator/test/test-validation-message-locale/1").then()
                .body(containsString("Value is not in line with the pattern"));
    }

    @Test
    public void testManualValidationMessageLocale() {
        RestAssured.given().header("Accept-Language", "en-US;q=0.25,hr-HR;q=1,fr-FR;q=0.5")
                .header("Content-Type", "application/json").when().body("{\"name\": \"b\"}")
                .post("/hibernate-validator/test/test-manual-validation-message-locale").then()
                .body(containsString("Vrijednost ne zadovoljava uzorak"));
    }

    @Test
    public void testHibernateOrmIntegration() {
        RestAssured.when().get("/hibernate-validator/test/test-hibernate-orm-integration").then().statusCode(500);
    }

    @Test
    public void testInheritance() {
        RestAssured.given().get("/books/science").then().statusCode(400).body(containsString("must not be null"));
    }

    @Test
    public void testRestEndPointValidationGroups_parameters() {
        // POST: input id must be null
        RestAssured.given().contentType(ContentType.JSON).body("{\"id\": 1, \"name\": \"b\"}").when()
                .post("/hibernate-validator/test/rest-end-point-validation-groups/").then().statusCode(400)
                .body(containsString("must be null"));
        RestAssured.given().contentType(ContentType.JSON).body("{\"name\": \"b\"}").when()
                .post("/hibernate-validator/test/rest-end-point-validation-groups/").then().statusCode(200)
                .body(containsString("passed"));

        // PUT: input id must not be null
        RestAssured.given().contentType(ContentType.JSON).body("{\"name\": \"b\"}").when()
                .put("/hibernate-validator/test/rest-end-point-validation-groups/").then().statusCode(400)
                .body(containsString("must not be null"));
        RestAssured.given().contentType(ContentType.JSON).body("{\"id\": 1, \"name\": \"b\"}").when()
                .put("/hibernate-validator/test/rest-end-point-validation-groups/").then().statusCode(200)
                .body(containsString("passed"));

        // Also check that constraints using the default group still work
        RestAssured.given().contentType(ContentType.JSON).body("{}").when()
                .post("/hibernate-validator/test/rest-end-point-validation-groups/").then().statusCode(400)
                .body(containsString("must not be null"));
        RestAssured.given().contentType(ContentType.JSON).body("{\"id\":1}").when()
                .put("/hibernate-validator/test/rest-end-point-validation-groups/").then().statusCode(400)
                .body(containsString("must not be null"));
    }
    ...
    public void testRestEndPointValidationGroups_result() {
        ...
        ValidatableResponse response = RestAssured.given().param("simulateDeleted", true).when()
                .get("/hibernate-validator/test/rest-end-point-validation-groups/1/").then().statusCode(500);
        if (isInternalErrorExceptionLeakedInQuarkusErrorHandlerResponse()) {
            response.body(containsString("must be false"));
        }
        RestAssured.given().param("simulateDeleted", false).when()
                .get("/hibernate-validator/test/rest-end-point-validation-groups/1/").then().statusCode(200)
                .body(containsString("\"deleted\":false"));

        // DELETE: deleted must be true
        response = RestAssured.given().param("simulateDeleted", false).when()
                .delete("/hibernate-validator/test/rest-end-point-validation-groups/1/").then().statusCode(500);
        if (isInternalErrorExceptionLeakedInQuarkusErrorHandlerResponse()) {
            response.body(containsString("must be true"));
        }
        RestAssured.given().param("simulateDeleted", true).when()
                .delete("/hibernate-validator/test/rest-end-point-validation-groups/1/").then().statusCode(200)
                .body(containsString("\"deleted\":true"));

        // Also check that constraints using the default group still work
        response = RestAssured.given().param("simulateDeleted", false).param("simulateNullName", true).when()
                .get("/hibernate-validator/test/rest-end-point-validation-groups/1/").then().statusCode(500);
        if (isInternalErrorExceptionLeakedInQuarkusErrorHandlerResponse()) {
            response.body(containsString("must not be null"));
        }
        response = RestAssured.given().param("simulateDeleted", true).param("simulateNullName", true).when()
                .delete("/hibernate-validator/test/rest-end-point-validation-groups/1/").then().statusCode(500);
        if (isInternalErrorExceptionLeakedInQuarkusErrorHandlerResponse()) {
            response.body(containsString("must not be null"));
        }
    ...
    void testClockBasedConstraints() {
        RestAssured.when().get("/hibernate-validator/test/rest-end-point-clock-based-constraints").then()
                .body(is("passed"));
    }

    @Test
    void testConstraintsDefinedInXml() {
        RestAssured.given().when().get("/hibernate-validator/test/constraints-defined-in-xml").then().statusCode(200)
                .body(containsString("failed"), containsString("id (must be greater than 0)"),
