...
Starting with JDK 24, link:https://openjdk.org/projects/leyden/[Project Leyden] introduces powerful AOT caching capabilities (link:https://openjdk.org/jeps/483[JEP 483]) that dramatically reduce Java application startup time.
Quarkus provides first-class support for these features, making it easy to build and deploy AOT-optimized applications.

Unlike traditional AppCDS (Application Class Data Sharing) which only caches class metadata, Leyden's AOT cache can:

* Pre-load and link classes ahead of time
* Store pre-computed verification results

This results in significantly faster startup times - in benchmarks, Quarkus applications have shown startup improvements of up to 5x when combining aot-jar packaging with AOT caching.
...
* Delegates all class loading to the JDK's class loaders (required for AOT to work effectively)
...
[source,bash]
----
./mvnw verify -Dquarkus.package.jar.aot.enabled=true
----
...
./mvnw verify -Dquarkus.package.jar.aot.enabled=true -Dquarkus.package.jar.aot.phase=INTEGRATION_TESTS
...
[TIP]
====
For maximum startup improvement, always combine `aot-jar` packaging with AOT cache generation:
...
[NOTE]
====
Automatic AOT-enhanced container image generation is currently only supported when building with Maven.
Gradle support is planned for a future release.

Additionally, this feature is currently untested on Windows.
====
...
* For container images, the container's JDK must match the build JDK
* Some highly dynamic features (like extensive runtime class generation) may have limited AOT benefits

[#appcds-legacy]
== AppCDS (Legacy)

For JDK versions prior to 25, Quarkus supports link:https://docs.oracle.com/en/java/javase/17/docs/specs/man/java.html#application-class-data-sharing[Application Class Data Sharing (AppCDS)] as a fallback.
AppCDS provides more modest startup improvements compared to Leyden AOT, but works on older JDKs.

[NOTE]
====
If you're on JDK 24+, we recommend using Leyden AOT instead of AppCDS for better startup performance.
====
