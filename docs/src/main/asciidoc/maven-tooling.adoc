...
| `platformGroupId`
| `io.quarkus`
| The group id of the platform you want to use.

| `platformArtifactId`
| `quarkus-bom`
| The artifact id of the platform you want to use.
...
By default at project creation only the extensions that comes with Quarkus core are enabled. It is possible to get access to the full ecosystem
available on `code.quarkus.io` by using the platform called `universe`.

You do that by passing `platformArtifactId` and `platformVersion` to the project create:
...
Once a project are created using a platform the operations dealing with extensions will use that automatically.
...
You can follow the link:building-native-image.html[Build a native executable guide] as well as link:deploying-to-kubernetes.html[Deploying Application to Kubernetes and OpenShift] for more information.
...
link:config.html#configuration-profiles[Configuration Profiles Documentation].
