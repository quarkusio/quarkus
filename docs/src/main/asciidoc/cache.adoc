...
For that reason, an injected bean with a scope narrower than the application (for example a `@RequestScoped` `JsonWebToken`, `Principal` or `SecurityIdentity`) cannot be used as a key argument: what gets injected is a client proxy whose `equals()` and `hashCode()` depend on the active context. Such an argument is rejected with a `CacheException`. Pass a value derived from the bean instead, such as `principal.getName()`, or declare a `CacheKeyGenerator`.
====
