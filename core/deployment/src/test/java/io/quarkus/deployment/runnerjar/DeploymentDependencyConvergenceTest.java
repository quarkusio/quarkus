...
public class DeploymentDependencyConvergenceTest extends BootstrapFromOriginalJarTestBase {
    ...
    protected TsArtifact composeApplication() {
        ...
        var libC10 = install(TsArtifact.jar("lib-c", "1.0").addDependency(libD10).addDependency(libE10));

        var libB10 = install(TsArtifact.jar("lib-b", "1.0"));
        var libB20 = install(TsArtifact.jar("lib-b", "2.0").addDependency(libC10));

        var extA = new TsQuarkusExt("ext-a");
        addToExpectedLib(extA.getRuntime());
        extA.getDeployment().addManagedDependency(libD20).addManagedDependency(libE20).addDependency(libB10);

        return TsArtifact.jar("app").addManagedDependency(platformDescriptor())
                .addManagedDependency(platformProperties()).addManagedDependency(libB20).addManagedDependency(libE30)
                .addDependency(extA);
    }
    ...
    protected void assertAppModel(ApplicationModel model) throws Exception {
        final Set<Dependency> expected = Set.of(
                new ArtifactDependency(ArtifactCoords.jar("io.quarkus.bootstrap.test", "ext-a", "1"),
                ...
                new ArtifactDependency(ArtifactCoords.jar("io.quarkus.bootstrap.test", "lib-b", "2.0"),
                        JavaScopes.COMPILE, DependencyFlags.DEPLOYMENT_CP),
                new ArtifactDependency(ArtifactCoords.jar("io.quarkus.bootstrap.test", "lib-c", "1.0"),
                        JavaScopes.COMPILE, DependencyFlags.DEPLOYMENT_CP),
                new ArtifactDependency(ArtifactCoords.jar("io.quarkus.bootstrap.test", "lib-d", "2.0"),
                        JavaScopes.COMPILE, DependencyFlags.DEPLOYMENT_CP),
                new ArtifactDependency(ArtifactCoords.jar("io.quarkus.bootstrap.test", "lib-e", "3.0"),
                        JavaScopes.COMPILE, DependencyFlags.DEPLOYMENT_CP));
