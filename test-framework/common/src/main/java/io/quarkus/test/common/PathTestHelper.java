...
public final class PathTestHelper {
    private static final Map<String, String> TEST2MAIN_DIR_FRAGMENTS = new HashMap<>();
    static {
        // eclipse
        TEST2MAIN_DIR_FRAGMENTS.put( //eclipse
                "bin" + File.separator + "test",
                "bin" + File.separator + "main");
        // gradle
        TEST2MAIN_DIR_FRAGMENTS.put(
        ...
        if (!isInTestDir(resource)) {
            throw new RuntimeException(
                    "The test class " + testClass + " is not located in any of the directories "
                            + TEST2MAIN_DIR_FRAGMENTS.keySet());
        }
    ...
    public static Path getAppClassLocation(Class<?> testClass) {
        ...
        return TEST2MAIN_DIR_FRAGMENTS.entrySet().stream()
    ...
    private static boolean isInTestDir(URL resource) {
        ...
        return TEST2MAIN_DIR_FRAGMENTS.keySet().stream()
