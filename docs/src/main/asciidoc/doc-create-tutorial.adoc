...
:diataxis-type: tutorial // <4>
:categories: web // <5>
:nav-title: Acme Http requests // <6>
:extension-status: experimental // <7>
----
...
<4> Specify the diataxis type. This tutorial uses `tutorial`.
<5> Specify the `web` category, as our imaginary extension works with Http requests.
<6> A short sidebar title (40 characters or fewer). Required here because the document title exceeds 40 characters.
<7> Our imaginary `acme` extension is experimental, so we include the extension status declaration in the header.
