...
The following xref:test-users-example[`test-users.properties`] file illustrates the format:
...
The user-to-password mappings are specified in the `application.properties` file by properties keys of the form `quarkus.security.users.embedded.users.<user>=<password>`. The following xref:password-example[password example] snippet illustrates the syntax with four user-to-password mappings:
...
The user-to-role mappings are specified in the `application.properties` file by properties keys of the form `quarkus.security.users.embedded.roles.<user>=role1[,role2[,role3[,...]]]`. The following xref:roles-example[roles example] snipped illustrates the syntax with four user-to-role mappings:
