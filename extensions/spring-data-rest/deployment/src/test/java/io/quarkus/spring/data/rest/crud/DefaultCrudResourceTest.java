...
class DefaultCrudResourceTest {
    ...
    @Test
    void shouldGet() {
        given().accept("application/json").when().get("/default-records/1").then().statusCode(200).and()
                .body("id", is(equalTo(1))).and().body("name", is(equalTo("first")));
    }

    @Test
    void shouldNotGetNonExistent() {
        given().accept("application/json").when().get("/default-records/1000").then().statusCode(404);
    }

    @Test
    void shouldGetHal() {
        given().accept("application/hal+json").when().get("/default-records/1").then().statusCode(200).and()
                .body("id", is(equalTo(1))).and().body("name", is(equalTo("first"))).and()
                .body("_links.add.href", endsWith("/default-records")).and()
                .body("_links.list.href", endsWith("/default-records")).and()
                .body("_links.self.href", endsWith("/default-records/1")).and()
                .body("_links.update.href", endsWith("/default-records/1")).and()
                .body("_links.remove.href", endsWith("/default-records/1"));
    }

    @Test
    void shouldNotGetNonExistentHal() {
        given().accept("application/hal+json").when().get("/default-records/1000").then().statusCode(404);
    }

    @Test
    void shouldList() {
        given().accept("application/json").when().get("/default-records").then().statusCode(200).and()
    ...
    void shouldListHal() {
        given().accept("application/hal+json").when().get("/default-records").then().statusCode(200).and()
                ...
                .body("_links.list.href", endsWith("/default-records"));
    }

    @Test
    void shouldCreate() {
        Response response = given().accept("application/json").and().contentType("application/json").and()
                .body("{\"name\": \"test-create\"}").when().post("/default-records").thenReturn();
    ...
    void shouldCreateHal() {
        Response response = given().accept("application/hal+json").and().contentType("application/json").and()
                .body("{\"name\": \"test-create-hal\"}").when().post("/default-records").thenReturn();
        assertThat(response.statusCode()).isEqualTo(201);
    ...
    void shouldCreateAndUpdate() {
        Response createResponse = given().accept("application/json").and().contentType("application/json").and()
                .body("{\"name\": \"test-update-create\"}").when().post("/default-records/").thenReturn();
    ...
    void shouldCreateAndUpdateHal() {
        Response createResponse = given().accept("application/hal+json").and().contentType("application/json").and()
                .body("{\"name\": \"test-update-create-hal\"}").when().post("/default-records/").thenReturn();
        assertThat(createResponse.statusCode()).isEqualTo(201);
    ...
    void shouldCreateAndDelete() {
        Response createResponse = given().accept("application/json").and().contentType("application/json").and()
                .body("{\"name\": \"test-delete\"}").when().post("/default-records").thenReturn();
        assertThat(createResponse.statusCode()).isEqualTo(201);
    ...
    void shouldNotDeleteNonExistent() {
        when().delete("/default-records/1000").then().statusCode(404);
    }
}
