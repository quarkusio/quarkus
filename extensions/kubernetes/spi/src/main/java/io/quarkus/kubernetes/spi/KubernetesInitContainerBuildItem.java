...
/**
 * A Build item for generating init containers. The generated container will have the specified fields and may
 * optionally inherit env vars and volumes from the app container.
 * <p>
...
public final class KubernetesInitContainerBuildItem extends MultiBuildItem implements Targetable {
    ...
    public static KubernetesInitContainerBuildItem create(String name, String image, String imagePullPolicy) {
        return new KubernetesInitContainerBuildItem(name, null, image, DEFAULT_IMAGE_PULL_POLICY,
                Collections.emptyList(), Collections.emptyList(), Collections.emptyMap(), false, false);
    }
    ...
    public KubernetesInitContainerBuildItem(String name, String target, String image, List<String> command,
            List<String> arguments, Map<String, String> envVars, boolean sharedEnvironment, boolean sharedFilesystem) {
        this(name, target, image, DEFAULT_IMAGE_PULL_POLICY, command, arguments, envVars, sharedEnvironment,
                sharedFilesystem);
    }

    private KubernetesInitContainerBuildItem(String name, String target, String image, String imagePullPolicy, // using
            // a
            // string
            // here
            // as we
            ...
            // in the
            // classpath
            List<String> command, List<String> arguments, Map<String, String> envVars, boolean sharedEnvironment,
    ...
    /**
     * Flag for tasks that require access to the environment variables of the application. Often tasks need to access
     * resources, configured via environment variables. This flag expresses that the task should be executed using the
     * same envrironment variables as the application.
     *
    ...
    /**
     * Flag for tasks that need to share filesystem with the application. Often tasks need to access resources,
     * configured via filesystem (e.g. local config files, kubernetes service binding etc). In other cases, tasks may
     * need to produce files needed by the application. This flag expresses that the task should share filesystem with
     * the application.
     *
