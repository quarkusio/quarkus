...
In order to configure Sentry logging, add the `quarkus-logging-sentry` extension to your
application `pom.xml` as illustrated in the following snippet:

.Add the Sentry logging extension to your pom.xml
[source,xml]
----
<build>
  <dependencies>
...
.All errors and warnings occuring in any of the packages will be sent to Sentry with DSN `https://abcd@sentry.io/1234`
[source, properties]
----
