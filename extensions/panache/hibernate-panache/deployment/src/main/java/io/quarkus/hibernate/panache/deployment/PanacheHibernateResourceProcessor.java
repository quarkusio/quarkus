...
public final class PanacheHibernateResourceProcessor {
    ...
    void recordEntityToPersistenceUnit(List<EntityToPersistenceUnitBuildItem> items,
        ...
        for (ClassInfo classInfo : index.getIndex().getAllKnownImplementations(DOTNAME_PANACHE_REPOSITORY_QUERIES)) {
            ...
            if (typeParameters.get(0).kind() == Kind.CLASS) {
                ...
                System.err.println("Recording repo " + repositoryClass + " -> " + entityClass);
            } else {
    ...
    void makePanache2ReposUnremovable(
        ...
        for (ClassInfo classInfo : index.getIndex().getAllKnownImplementations(DOTNAME_PANACHE_REPOSITORY_QUERIES)) {
            System.err.println("Found implementing repo candidate: " + classInfo);
            // Only keep concrete classes
            if (classInfo.isInterface() || classInfo.isAbstract()) {
                continue;
            }
            System.err.println("Found implementing repo bean: " + classInfo);
