...
/**
 * Generates a GraalVM {@link Feature} that embeds the application SBOM into the native image as the
 * {@code sbom} and {@code sbom_length} global symbols, following the
 * <a href="https://www.graalvm.org/jdk24/security-guide/native-image/sbom/">GraalVM SBOM spec</a>.
 * <p>
 * Internal GraalVM APIs ({@code CGlobalDataFactory}, {@code CGlobalDataFeature}, {@code WordFactory})
 * are referenced via {@link ClassMethodDesc} to avoid compile-time dependencies. This targets the
 * GraalVM {@code com.oracle.svm.core.c} global-data API (GraalVM for JDK 24 / 25.0).
 */
public class SbomNativeImageFeatureStep {
    ...
    void generateSbomEmbedFeature(
        ...
        // make the embedded SBOM available as a resource in the native image as well as to the feature below
        //nativeResources.produce(new NativeImageResourceBuildItem(resourceName));
                ...
                mc.body(b0 -> {
                    b0.try_(t -> {
                        t.body(tb -> {
                            ...
                            // the embedded SBOM resource is stored uncompressed, but the GraalVM SBOM spec
