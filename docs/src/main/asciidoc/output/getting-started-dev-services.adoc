...
This tutorial builds on what you learned writing xref:{doc-guides}/getting-started.adoc[your first {product-name-first} application].
...
In your {product-name} terminal, type 'r' to run the tests. You should see
...
you will start seeing failures in the {product-name} logs at this point.
====
...
Stop {product-name} and run `docker ps` again.
You should see nothing running (it may take a few moments for containers to shut down).
{product-name} will automatically stop the container when your application stops.
...
What's going on? By default, in dev mode, with a Dev Services database,
 {product-name} configures Hibernate ORM database generation to be `drop-and-create`.
...
If you provide an `import.sql` file, {product-name} will use that to initialise
...
This tells {product-name} that you don't want it to start a Dev Service,
...
you'll get a red error screen. In the terminal where {product-name} is running.
...
In fact, {product-name} only starts Dev Services in dev and test modes.
...
Notice that there was no need to restart {product-name} for any of these changes.
