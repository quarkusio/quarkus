...
:summary: This guide explains how your Quarkus application can utilize JDK Flight Recorder to provide insight into itself.
:topics: observability,jfr
:extensions: io.quarkus:quarkus-jfr

This guide explains how your Quarkus application can utilize https://openjdk.org/jeps/328[JDK Flight Recorder] (JFR) to provide
insight into itself.
...
:create-app-extensions: resteasy-reactive,quarkus-jfr
...
1. Select the plus button to add a new lane on the left side, then check to display that lane.
1. Select Quarkus as the event type that lane will display and press OK.
...
To avoid this, we recommend to use Request IDs (described below) to filter events so that you only see the information about the requests you want to see.
====
...
There are no configurations for this extension.
