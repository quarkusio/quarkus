...
Both approaches work with Panache entities. However, you must choose one approach and use it consistently throughout your application.
Mixing both transaction management styles in the same application is not supported and will result in an `UnsupportedOperationException`.
