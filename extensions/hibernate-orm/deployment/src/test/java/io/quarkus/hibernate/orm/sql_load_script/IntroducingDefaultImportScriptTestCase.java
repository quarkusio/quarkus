...
public class IntroducingDefaultImportScriptTestCase {
    ...
    static QuarkusDevModeTest runner = new QuarkusDevModeTest().withApplicationRoot((jar) -> jar
            .addAsResource("application.properties").addClasses(SqlLoadScriptTestResource.class, MyEntity.class));

    @Test
    public void testAddNewImportSql() {
        RestAssured.when().get("/orm-sql-load-script/1").then()
                .body(Matchers.is(SqlLoadScriptTestResource.NO_ENTITY_MESSAGE));
