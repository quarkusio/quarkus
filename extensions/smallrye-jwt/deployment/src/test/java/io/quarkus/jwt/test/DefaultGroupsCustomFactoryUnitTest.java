...
public class DefaultGroupsCustomFactoryUnitTest {
    private static Class<?>[] testClasses = { DefaultGroupsEndpoint.class, TestJWTCallerPrincipalFactory.class,
            TokenUtils.class };
    /**
    ...
    static final QuarkusUnitTest config = new QuarkusUnitTest().withApplicationRoot(
            (jar) -> jar.addClasses(testClasses).addAsResource("privateKey.pem").addAsResource("TokenUserGroup.json")
    ...
    public void echoGroupsWithParser() {
        RestAssured.given().auth().oauth2(token).get("/endp/echo-parser").then().assertThat().statusCode(200)
