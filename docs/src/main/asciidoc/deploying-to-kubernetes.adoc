...
A missing `Secret` or `ConfigMap` referenced this way prevents the container from starting.
To import resources that may not exist, list them with `quarkus.kubernetes.env.optional-secrets` and `quarkus.kubernetes.env.optional-configmaps` instead;
the generated references are then marked `optional: true`:

[source,properties]
----
quarkus.kubernetes.env.optional-configmaps=my-config-map
----
