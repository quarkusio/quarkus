...
Dev Services for Kafka implements a _service discovery_ mechanism for your multiple {product-name-first} applications running in _dev_ mode to share a single broker.
...
**kafka-native** provides images of standard Apache Kafka distribution compiled to native binary using {product-name} and GraalVM.
