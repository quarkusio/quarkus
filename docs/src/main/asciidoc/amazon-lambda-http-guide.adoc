...
to begin using Quarkus.  One step remains, however.
...
sufficient.  However, you might choose to always run this plugin to make sure your template file is always correct.
e.g., should implementation details on the Quarkus side ever change, it might be nice to have things automatically update
without having to worry about tracking changes.  The pom can be configured to run automatically by adding the following
...
With these defined, the properties no longer need to be defined when invoking maven.