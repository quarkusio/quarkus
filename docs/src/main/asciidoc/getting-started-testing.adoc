...
* Refactor the parameter values to use "older" types like `java.util.ArrayList` or `java.util.HashSet` or `java.util.HashMap`
* or add the `--add-opens` option to your build scripts to the declaration of the `maven-surefire-plugin` or Gradle's test worker execution.
