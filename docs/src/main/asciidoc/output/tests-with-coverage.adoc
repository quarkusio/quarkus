...
:categories: tooling
:summary: This guide explains how to measure the test coverage of your {product-name-first} application.
:numbered:
:sectnums:
:sectnumlevels: 4
...
Let's start from an empty application created with the {product-name} Maven plugin:
...
This {product-name} extension takes care of everything that would usually be done via the JaCoCo Maven plugin, so no additional
config is required.
...
The {product-name} automatic JaCoCo config will only work for tests that are annotated with `@QuarkusTest`. If you want to check
...
WARNING: Sharing the same value for `quarkus.test.arg-line` might break integration test runs that test different types of {product-name} artifacts. In such cases, the use of Maven profiles is advised.
