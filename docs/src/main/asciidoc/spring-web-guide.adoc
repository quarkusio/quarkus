...
What that means for end users, is that adding arbitrary Spring libraries to the classpath will not be taken into account. Moreover Spring infrastructure
things like `org.springframework.beans.factory.config.BeanPostProcessor` will not be executed.
