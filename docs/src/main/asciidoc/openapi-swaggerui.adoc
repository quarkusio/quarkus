...
These defaults can be changed by applying an `@OpenApiFilter` on the `mp.openapi.filter` class, or a directly extending class. The RunStage and documents this filter will be run for are then taken from the `@OpenApiFilter` annotation instead of the implicit default.

For example:

[source,properties]
----
mp.openapi.filter=com.acme.SharedFilter
----
...
public class SharedFilter implements OASFilter { //<1>
    // ...
}

@OpenApiFilter(documentNames = "user", stages = {OpenApiFilter.RunStage.RUNTIME_PER_REQUEST}) //<2>
public class MyUserFilter extends SharedFilter implements OASFilter { //<3>
    // ...
}
----

<1> `SharedFilter` comes from a shared library and cannot be annotated. Being named by `mp.openapi.filter`, it would normally run during `RUNTIME_STARTUP`, for every document.
<2> Annotating a directly extending class overrides those defaults. Here the filter runs on every request, and only for the document named `user`.
<3> `MyUserFilter` must implement `OASFilter` explicitly, even though `SharedFilter` already does. Otherwise it is not detected, and the defaults of `SharedFilter` continue to apply. This is a known limitation.
