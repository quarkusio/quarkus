...
public class BearerTokenStepUpAuthenticationTest {
    ...
    public void testMaxAgeOnly() {
        // no auth_time claim -> fail
        stepUpMethodLevelRequest(Set.of("admin"), Set.of(), "max-age-only").statusCode(401)
                .header("www-authenticate", containsString("insufficient_user_authentication"))
                .header("www-authenticate", containsString("max_age"))
                .header("www-authenticate", containsString("120"));
        // auth_time + max_age < now -> fail
        stepUpMethodLevelRequest(Set.of("admin"), Set.of(), "max-age-only", 123L).statusCode(401)
                .header("www-authenticate", containsString("insufficient_user_authentication"))
                .header("www-authenticate", containsString("max_age"))
                .header("www-authenticate", containsString("120"));
        // auth_time + max_age > now -> pass
        final long nowSecs = System.currentTimeMillis() / 1000;
        stepUpMethodLevelRequest(Set.of("admin"), Set.of(), "max-age-only", nowSecs).statusCode(200);
    }

    @Test
    public void testMaxAgeAndAcrRequired() {
        // no auth_time claim && no acr -> fail
        stepUpMethodLevelRequest(Set.of("admin"), null, "max-age-and-acr-required").statusCode(401)
                .header("www-authenticate", containsString("insufficient_user_authentication"))
                .header("www-authenticate", containsString("max_age"))
                .header("www-authenticate", containsString("120"))
                .header("www-authenticate", containsString("acr_values"))
                .header("www-authenticate", containsString("myACR"));
        // no auth_time claim but acr is correct -> fail
        stepUpMethodLevelRequest(Set.of("admin"), Set.of("myACR"), "max-age-and-acr-required").statusCode(401)
                .header("www-authenticate", containsString("insufficient_user_authentication"))
                .header("www-authenticate", containsString("max_age"))
                .header("www-authenticate", containsString("120"))
                .header("www-authenticate", containsString("acr_values"))
        ...
        stepUpMethodLevelRequest(Set.of("admin"), Set.of("myACR"), "max-age-and-acr-required", 123L).statusCode(401)
                .header("www-authenticate", containsString("insufficient_user_authentication"))
                .header("www-authenticate", containsString("max_age"))
                .header("www-authenticate", containsString("120"))
                .header("www-authenticate", containsString("acr_values"))
        ...
        final long nowSecs = System.currentTimeMillis() / 1000;
        stepUpMethodLevelRequest(Set.of("admin"), Set.of("wrongACR"), "max-age-and-acr-required", nowSecs).statusCode(401)
                .header("www-authenticate", containsString("insufficient_user_authentication"))
                .header("www-authenticate", containsString("max_age"))
                .header("www-authenticate", containsString("120"))
                .header("www-authenticate", containsString("acr_values"))
        ...
        stepUpMethodLevelRequest(Set.of("admin"), Set.of("myACR"), "max-age-and-acr-required", nowSecs).statusCode(200)
    ...
    private static ValidatableResponse stepUpMethodLevelRequest(Set<String> roles, Set<String> acrValues, String path) {
        return stepUpMethodLevelRequest(roles, acrValues, path, null);
    }

    private static ValidatableResponse stepUpMethodLevelRequest(Set<String> roles, Set<String> acrValues, String path,
            Long authTime) {
        return stepUpRequest(roles, acrValues, "method", path, authTime);
    }

    private static ValidatableResponse stepUpClassLevelRequest(Set<String> roles, Set<String> acrValues, String path) {
        return stepUpRequest(roles, acrValues, "class", path, null);
    }

    private static ValidatableResponse stepUpRequest(Set<String> roles, Set<String> acrValues, String level, String path,
            Long authTime) {
        return RestAssured.given()
                .auth().oauth2(getAccessToken(roles, acrValues, authTime))
                .when().get("/step-up-auth/" + level + "-level/" + path)
                .then();
    }

    private static String getAccessToken(Set<String> roles, Set<String> acrValues) {
        return getAccessToken(roles, acrValues, null);
    }

    private static String getAccessToken(Set<String> roles, Set<String> acrValues, Long authTime) {
        var jwtBuilder = Jwt.preferredUserName("Pete")
                .groups(roles)
        ...
        if (authTime != null) {
            jwtBuilder.claim(Claims.auth_time, authTime);
        }
        return jwtBuilder
    ...
    public static class StepUpAuthMethodLevelResource {
        ...
        @GET
        @AuthenticationContext(maxAge = 120)
        @Path("max-age-only")
        public String maxAgeOnly() {
            return "max-age-only";
        }

        @GET
        @AuthenticationContext(maxAge = 120, value = "myACR")
