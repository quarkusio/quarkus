...
final class AppendBuffer {
    ...
    public int append(byte[] bytes, int off, int len) {
        ...
        final ByteBuf tmpBuf = VertxByteBufAllocator.POOLED_ALLOCATOR.directBuffer(chunkCapacity);
        try {
    ...
    private CompositeByteBuf clearBuffers() {
        var firstBuf = buffer;
        var others = otherBuffers;
        CompositeByteBuf batch = VertxByteBufAllocator.POOLED_ALLOCATOR.compositeBuffer(1 + others.size());
        try {
            buffer = null;
            size = 0;
