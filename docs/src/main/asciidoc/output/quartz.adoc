...
The Maven project also imports the {product-name-first} Quartz extension.

If you already have your {product-name} project configured, you can add the `quartz` extension
...
<2> Use the `@Scheduled` annotation to instruct {product-name} to run this method every 10 seconds and set the unique identifier for this job.
...
You can register `plugins`, `job-listeners` and `trigger-listeners` through {product-name} configuration.
