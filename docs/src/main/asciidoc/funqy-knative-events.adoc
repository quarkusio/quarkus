...
The first step is to setup the broker in our namespace.
...
The second step is to define a Kubernetes/OpenShift service to points to your the docker image you created and pushed
...
You might need to wait a couple of seconds till the curler pod comes up. Run the following to get bash access to the curler pod.
