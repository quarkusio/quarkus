...
The properties obtained from the ConfigMaps and Secrets can then be injected in our application beans. See https://quarkus.io/guides/config#inject-the-configuration[this link] for more information.

For example, we have installed the following ConfigMap and Secret resources in the namespace where we'll deploy our application:
...
Next, we need to configure the Kubernetes Config extension to use these two resources by adding the following configuration:
...
quarkus.kubernetes-config.secrets=appSecret
----

Now, we can inject the properties from the resources using the `@ConfigProperty` annotation:
...
Moreover, in case of some of your resources use the same keys, you will deal with property name conflicts. To solve this issue, you can also inject these properties by specifying the source resource and the name:

[source,java]
----
// From the config map
@ConfigProperty(name = "configMaps/appConfig.hello.message")
String message;

// From the secret
@ConfigProperty(name = "secrets/appSecret.username")
String username;
----
