...
If for some reason you must customize the default `ObjectMapper` bean provided by this extension and used by the Kubernetes Client, you can do so by declaring a bean that implements the `KubernetesClientJsonMapperBuilderCustomizer` interface.

The following code snippet contains an example of a `KubernetesClientJsonMapperBuilderCustomizer` to set the `ObjectMapper` locale:
...
Access to `Serialization.jsonMapper()` should be replaced by the usage of `KubernetesClientJsonMapperBuilderCustomizer` declared beans.
