...
:summary: This guide explains how your {product-name-first} application can utilize OpenTracing to provide distributed tracing for interactive web applications.

This guide explains how your {product-name} application can utilize OpenTracing to provide distributed tracing for
interactive web applications.

[IMPORTANT]
====
xref:opentelemetry.adoc[OpenTelemetry] is the recommended approach to tracing and telemetry for {product-name}.

When {product-name} will upgrade to Eclipse MicroProfile 6, the SmallRye OpenTracing support will be discontinued.
====
...
The solution is located in the `opentracing-quickstart` link:{quickstarts-tree-url}/opentracing-quickstart[directory].
...
If you already have your {product-name} project configured, you can add the `smallrye-opentracing` extension
...
This way, you can have your {product-name} application ready for tracing and toggle JDBC tracing at runtime.
