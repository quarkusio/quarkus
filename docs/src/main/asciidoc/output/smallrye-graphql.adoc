...
This guide demonstrates how your {product-name-first} application can use https://github.com/smallrye/smallrye-graphql/[SmallRye GraphQL],
...
If you already have your {product-name} project configured, you can add the `smallrye-graphql` extension
...
The {product-name} `smallrye-graphql` extension ships with https://github.com/graphql/graphiql/blob/main/packages/graphiql/README.md[GraphiQL] and enables it by default in `dev` and `test` modes,
...
Using the above adaption, `Map` support is added for {product-name} and are mapped to an `Entry<Key,Value>` with an optional key parameter.
