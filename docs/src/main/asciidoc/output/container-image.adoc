...
:categories: cloud
:summary: Learn how to build and push container images with Jib, OpenShift or Docker as part of the {product-name-first} build.

{product-name} provides extensions for building (and pushing) container images. Currently, it supports:
...
The major benefit of using Jib with {product-name} is that all the dependencies (everything found under `target/lib`) are cached in a different layer than the actual application making rebuilds really fast and small (when it comes to pushing).
...
There are cases when additional files (other than ones produced by the {product-name} build) need to be added to a container image.
To support these cases, {product-name} copies any file under `src/main/jib` into the built container image (which is essentially the same
...
When building a multi-module project containing a {product-name} application as one module and various supporting project dependencies as other modules,
{product-name} supports placing these supporting modules in a separate container image layer from the rest of the application dependencies, with the expectation
...
{product-name} supports generating and including an link:https://docs.oracle.com/en/java/javase/17/docs/specs/man/java.html#application-class-data-sharing[Application Class Data Sharing] archive when generating a container image using Jib.
...
The creation of such objects is being taken care of by the {product-name} Kubernetes extension.
...
To build a container image for your project, `quarkus.container-image.build=true` needs to be set using any of the ways that {product-name} supports.
...
To run tests on the resulting image, `quarkus.container-image.build=true` needs to be set using any of the ways that {product-name} supports.
...
To push a container image for your project, `quarkus.container-image.push=true` needs to be set using any of the ways that {product-name} supports.
...
If you are building a container image in order to deploy your {product-name} application as a Linux service with Podman and Systemd, you might want to consider including the https://quarkiverse.github.io/quarkiverse-docs/quarkus-systemd-notify/dev/index.html[{product-name} Systemd Notify Extension] as part of your application, with:
...
Various CI environments provide a ready to use container-image registry which can be combined with the container-image {product-name} extensions in order to
effortlessly create and push a {product-name} application to said registry.
...
(see GitLab's https://docs.gitlab.com/ee/ci/variables/[documentation]) for more information), which can be used in {product-name} like so:
