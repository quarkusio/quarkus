...
public class ResubscriptionTest extends AbstractSignalTest {
    ...
    @Test
    public void testPublishResolvesReceiversLazily() {
        List<String> received = new CopyOnWriteArrayList<>();

        // Create the Uni BEFORE any receiver is registered
        Uni<Void> uni = signal.reactive().publish(new Ping("lazy-pub"));
        ...
        try {
            // Subscribe — the lazily resolved receiver should be found and invoked
            uni.ifNoItem().after(defaultTimeout()).fail()
                    .await().indefinitely();
            assertEquals(1, received.size());
            assertEquals("lazy-pub", received.get(0));
        } finally {
            reg.unregister();
        }
    }
    ...
    @Test
    public void testSendResolvesReceiversLazily() {
        AtomicInteger count = new AtomicInteger();

        // Create the Uni BEFORE any receiver is registered
        Uni<Void> uni = signal.reactive().send(new Ping("lazy-send"));
    ...
    @Test
    public void testRequestResolvesReceiversLazily() {
        // Create the Uni BEFORE any receiver is registered
        Uni<String> uni = signal.reactive().request(new Ping("lazy-req"), String.class);
        ...
        try {
            // Subscribe — the lazily resolved receiver should be found and invoked
            String result = uni.ifNoItem().after(defaultTimeout()).fail()
                    .await().indefinitely();
            assertEquals("reply_lazy-req", result);
        } finally {
            reg.unregister();
        }
    }
