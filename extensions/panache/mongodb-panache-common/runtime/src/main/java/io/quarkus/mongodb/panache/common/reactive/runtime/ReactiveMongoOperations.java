...
public abstract class ReactiveMongoOperations<QueryType, UpdateType> {
    ...
    private static final List<String> UPDATE_OPERATORS = Arrays.asList("$currentDate", "$inc", "$min", "$max", "$mul",
    ...
    private Uni<Void> update(ReactiveMongoCollection collection, Object entity) {
        ...
        if (Panache.getCurrentSession() != null) {
            return collection.replaceOne(Panache.getCurrentSession(), query, entity).onItem().ignore()
                    .andContinueWithNull();
        }
    ...
    private Uni<Void> persistOrUpdate(ReactiveMongoCollection collection, Object entity) {
        ...
        if (id == null) {
            ...
            if (Panache.getCurrentSession() != null) {
                return collection.insertOne(Panache.getCurrentSession(), entity).onItem().ignore()
                        .andContinueWithNull();
            }
            return collection.insertOne(entity).onItem().ignore().andContinueWithNull();
        } else {
            ...
            if (Panache.getCurrentSession() != null) {
                return collection
                        .replaceOne(Panache.getCurrentSession(), query, entity, new ReplaceOptions().upsert(true))
                        .onItem().ignore().andContinueWithNull();
            }
            return collection.replaceOne(query, entity, new ReplaceOptions().upsert(true)).onItem().ignore()
                    .andContinueWithNull();
        }
    }

    private Uni<Void> persistOrUpdate(ReactiveMongoCollection collection, List<Object> entities) {
        // this will be an ordered bulk: it's less performant than an unordered one but will fail at the first failed
        // write
    ...
    public Uni<Optional> findByIdOptional(Class<?> entityClass, Object id) {
        ...
        if (Panache.getCurrentSession() != null) {
            return collection.find(Panache.getCurrentSession(), new Document(ID, id)).collect().first().onItem()
                    .transform(Optional::ofNullable);
        }
        return collection.find(new Document(ID, id)).collect().first().onItem().transform(Optional::ofNullable);
    }
    ...
    /**
     * We should have a query like <code>{'firstname': ?1, 'lastname': ?2}</code> for native one and like
     * <code>firstname = ?1</code> for PanacheQL one.
     */
    ...
    /**
     * We should have a query like <code>{'firstname': :firstname, 'lastname': :lastname}</code> for native one and like
     * <code>firstname = :firstname and lastname = :lastname</code> for PanacheQL one.
     */
    ...
    /**
     * We should have a query like <code>{'firstname': ?1, 'lastname': ?2}</code> for native one and like
     * <code>firstname = ?1 and lastname = ?2</code> for PanacheQL one. As update document needs an update operator, we
     * add <code>$set</code> if none is provided.
     */
    ...
    /**
     * We should have a query like <code>{'firstname': :firstname, 'lastname': :lastname}</code> for native one and like
     * <code>firstname = :firstname and lastname = :lastname</code> for PanacheQL one. As update document needs an
     * update operator, we add <code>$set</code> if none is provided.
     */
    ...
    public Uni<Long> deleteAll(Class<?> entityClass) {
        ReactiveMongoCollection<?> collection = mongoCollection(entityClass);
        if (Panache.getCurrentSession() != null) {
            return collection.deleteMany(Panache.getCurrentSession(), new Document())
    ...
    public Uni<Boolean> deleteById(Class<?> entityClass, Object id) {
        ...
        if (Panache.getCurrentSession() != null) {
            return collection.deleteOne(Panache.getCurrentSession(), query)
                    .map(results -> results.getDeletedCount() == 1);
        }
