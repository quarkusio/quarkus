...
The REST Client also supports the following optional properties:
[source,shell]
----
org.acme.restclient.CountriesService/mp-rest/connectTimeout=1000 # // <1>
org.acme.restclient.CountriesService/mp-rest/readTimeout=5000 # // <2>
----

<1> Timeout specified in milliseconds to wait to connect to the remote endpoint.
<2> Timeout specified in milliseconds to wait for a response from the remote endpoint.
