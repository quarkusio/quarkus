...
In this guide, you will learn how to enable application data caching in any CDI managed bean of your {product-name-first} application.
...
Let's imagine you want to expose in your {product-name} application a REST API that allows users to retrieve the weather forecast for the next three days.
...
We'll do that using a single {product-name} annotation.

[NOTE]
====
In this guide, we use the default {product-name} Cache backend (Caffeine).
...
First, we need to create a new {product-name} project with the following command:
...
If you already have your {product-name} project configured, you can add the `cache` extension
...
Now that your {product-name} application is up and running, let's tremendously improve its response time by caching the external meteorological service responses.
...
Yes, this is one of {product-name} amazing features for developers called `live coding`.
...
Congratulations! You just added application data caching to your {product-name} application with a single line of code!

Do you want to learn more about the {product-name} application data caching abilities?
The following sections will show you everything there is to know about it.

[#annotations-api]
== Caching using annotations

{product-name} offers a set of annotations that can be used in a CDI managed bean to enable caching abilities.
...
When a method annotated with `@CacheResult` is invoked, {product-name} will compute a cache key and use it to check in the cache whether the method has been already invoked.
...
[NOTE]
====
{product-name} is able to also cache `null` values unlike the underlying Caffeine provider.
...
When a method annotated with `@CacheInvalidate` is invoked, {product-name} will compute a cache key and use it to try to remove an existing entry from the cache.
See the <<cache-keys-building-logic>> section of this guide to learn how the cache key is computed.
If the key does not identify any cache entry, nothing will happen.

=== @CacheInvalidateAll

When a method annotated with `@CacheInvalidateAll` is invoked, {product-name} will remove all entries from the cache.
...
All CDI scopes supported by {product-name} can be used on a key generator.
...
{product-name} also offers a programmatic API which can be used to store, retrieve or delete values from any cache declared using the annotations API.
...
The expiration policy of a `CaffeineCache` can be changed while a {product-name} app is running if that policy was initially specified in the {product-name} configuration.
...
Each of the Caffeine caches backing up the {product-name} application data caching extension can be configured using the following
...
See the xref:telemetry-micrometer.adoc[Micrometer metrics guide] to learn how to use Micrometer in {product-name}.
====
...
This approach has an unfortunate side effect: as we said before, {product-name} can also cache
