...
:categories: web
:summary: Learn more about configuring {product-name-first}' Vert.x based HTTP layer - and Undertow if you are using servlets.
:numbered:
:sectnums:
:sectnumlevels: 4
...
This document explains various HTTP features that you can use in {product-name}.
...
{product-name} can be used without Servlet, following this convention allows existing code that places its resources in this
...
By default, {product-name} will serve content from under the root context. If you want to change this you can use the
...
want to use the HTTP root as this affects everything that {product-name} serves.
...
If REST Assured is used for testing and `quarkus.http.root-path` is set then {product-name} will automatically configure the
base URL for use in {product-name} tests, so test URL's should not include the root path.
...
The link:https://quarkus.io/blog/path-resolution-in-quarkus/[Path Resolution in {product-name}] blog post further explains how path resolution works for both user and extension defined paths.
...
In order to have {product-name} support secure connections, you must either provide a certificate and associated key file, or supply a keystore.
...
{product-name} will first try to load the given files as resources, and uses the filesystem as a fallback.
...
As with the certificate/key file combination, {product-name} will first try to resolve the given path as a resource, before attempting to read it from the filesystem.
...
If the type is not provided, {product-name} will try to deduce it from the file extensions, defaulting to type JKS.
...
_Note: in order to remain compatible with earlier versions of {product-name} (before 0.16) the default password is set to "password". It is therefore not a mandatory parameter!_

=== Configure the HTTPS port

By default, {product-name} listens to port 8443 for SSL secured connections and 8444 when running tests.
...
{product-name} startup, so if your object that is being injected is created eagerly before the port has opened the injected
...
{product-name} comes with a CORS filter which implements the `jakarta.servlet.Filter` interface and intercepts all incoming HTTP
requests. It can be enabled in the {product-name} configuration file, `src/main/resources/application.properties`:
...
Having to configure required origins when you start developing a {product-name} application requiring CORS support can be difficult and, in such cases, you may want to allow all origins in dev mode only in order to focus on the actual development first:
...
{product-name} allows users to arbitrarily customize the options of HTTP servers started by {product-name} via the use of `io.quarkus.vertx.http.HttpServerOptionsCustomizer`.
...
----
<1> By making the class a managed bean, {product-name} will take the customizer into account when it starts the Vert.x servers
...
{product-name} could be accessed through proxies that additionally generate headers (e.g. `X-Forwarded-Host`) to keep
information from the client-facing side of the proxy servers that is altered or lost when they are involved.
In those scenarios, {product-name} can be configured to automatically update information like protocol, host, port and URI
...
One can easily add a https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Set-Cookie/SameSite[SameSite] cookie property to any of the cookies set by a {product-name} endpoint by listing a cookie name and a `SameSite` attribute, for example:
