...
To use them with MongoDB with Panache you need to annotate the method that starts the transaction
with the `@MongoTransactional` (or `@ReactiveMongoTransactional` if you use reactive entity or repository) annotation.
...
TIP: If you want to use transaction, use the `@ReactiveMongoTransactional` annotation instead of the default `@MongoTransactional`.
