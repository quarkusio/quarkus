...
Hibernate Reactive with Panache focuses on making your entities trivial and fun to write in {product-name-first}.
...
Hibernate Reactive in {product-name} currently does not support multiple persistence units.
...
NOTE: Note that a Panache entity may not be used from a blocking thread. See also xref:getting-started-reactive.adoc[Getting Started With Reactive] guide that explains the basics of reactive principles in {product-name}.
