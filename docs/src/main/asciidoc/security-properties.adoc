...
It is not recommended to use this authentication in production as, at present, only plain-text and digest hashed passwords are used, and properties files are generally too limited.
...
This realm supports no other digest algorithm.
The hash for the first example above can be generated with the following command:

[source,shell]
----
echo -n scott:MyRealm:jb0ss | md5sum | cut -d ' ' -f 1 <1>
----
<1> On macOS, `md5` can be used in place of `md5sum | cut -d ' ' -f 1`.
...
If plain text is set to false (or omitted) in the config, then passwords must be stored in the form `ALGORITHM ( username : realm : password )`, where the algorithm is the one set by `quarkus.security.users.embedded.algorithm`.
That property defaults to SHA-512, so the hash for the first example above can be generated with the following command:

[source,shell]
----
echo -n scott:MyRealm:jb0ss | sha512sum | cut -d ' ' -f 1 <1>
----
<1> On macOS, `shasum -a 512` can be used in place of `sha512sum`.

Set `quarkus.security.users.embedded.algorithm` to one of `digest-md5`, `digest-sha`, `digest-sha-256`, `digest-sha-384`, `digest-sha-512` or `digest-sha-512-256` to use a different one, and generate the hashes with the matching command.
