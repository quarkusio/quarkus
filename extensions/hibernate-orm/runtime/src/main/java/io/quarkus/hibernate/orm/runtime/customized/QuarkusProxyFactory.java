...
/**
 * This {@link ProxyFactory} implementation is very similar to the {@link ByteBuddyProxyFactory}, except the class
 * definitions of the proxies have been built upfront so to allow usage of enhanced proxies in native images.
 */
public final class QuarkusProxyFactory implements ProxyFactory {
    ...
    public void postInstantiate(String entityName, Class<?> persistentClass, Set<Class<?>> interfaces,
        ...
        ProxyDefinitions.ProxyClassDetailsHolder detailsHolder = proxyClassDefinitions
                .getProxyForClass(persistentClass);
        if (detailsHolder == null) {
            throw new HibernateException("Could not lookup a pre-generated proxy class definition for entity '"
                    + entityName + "'. Falling back to enforced eager mode for this entity!");
        }
    ...
    public HibernateProxy getProxy(Object id, SharedSessionContractImplementor session) throws HibernateException {
        final ByteBuddyInterceptor interceptor = new ByteBuddyInterceptor(entityName, persistentClass, interfaces, id,
