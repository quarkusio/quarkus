...
In dev mode, Quarkus sets defaults for common AWS Lambda reserved environment variables
such as `AWS_LAMBDA_FUNCTION_NAME`, `AWS_LAMBDA_FUNCTION_VERSION`, and `AWS_LAMBDA_FUNCTION_MEMORY_SIZE`
in the forked dev mode process. These values are available through `System.getenv()` and the Lambda `Context` API.
On AWS, the real environment variables always take precedence. Existing shell environment variables are not
overwritten.

By default, the mock function name is derived from the application artifact id. You can override the mock
environment with:

[source, subs=attributes+]
----
quarkus.lambda.mock-environment.function-name=my-function
quarkus.lambda.mock-environment.function-version=$LATEST
quarkus.lambda.mock-environment.function-memory-size=128
quarkus.lambda.mock-environment.log-group-name=/aws/lambda/my-function
quarkus.lambda.mock-environment.log-stream-name=local/dev
----
