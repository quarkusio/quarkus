...
[[vector]]
== Hibernate Vector
The Vector extension to Hibernate ORM provides support for vector similarity search, enabling storage and querying of vector embeddings in databases that support vector types (e.g. https://github.com/pgvector/pgvector[pgvector] for PostgreSQL).
...
To map a vector column, annotate a `float[]` field with `@JdbcTypeCode(SqlTypes.VECTOR)` and specify the column definition:
[source,java]
.Example entity with a vector embedding
----
@Entity
...
NOTE: The `vector` PostgreSQL extension must be installed in your database. When using Dev Services, configure the pgvector image: `quarkus.datasource.devservices.image-name=docker.io/pgvector/pgvector:pg17`.
