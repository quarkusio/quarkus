...
/**
 * The test allowing to make sure that if we exclude a library from an extension, the exclusion will be also applied to
...
public class ExcludeLibDepsTest extends BootstrapFromOriginalJarTestBase {
    ...
    protected void assertAppModel(ApplicationModel appModel) throws Exception {
        ...
        expectedDeployDeps.add(new ArtifactDependency(new GACTV("io.quarkus.bootstrap.test", "ext-a-deployment", "1"),
        ...
        expectedRuntimeDeps.add(new ArtifactDependency(new GACTV("io.quarkus.bootstrap.test", "ext-a-dep-1", "1"),
                "compile", DependencyFlags.RUNTIME_CP, DependencyFlags.DEPLOYMENT_CP));
        expectedRuntimeDeps.add(new ArtifactDependency(new GACTV("io.quarkus.bootstrap.test", "ext-a-dep-2", "1"),
                "compile", DependencyFlags.RUNTIME_CP, DependencyFlags.DEPLOYMENT_CP));
        expectedRuntimeDeps.add(new ArtifactDependency(new GACTV("io.quarkus.bootstrap.test", "ext-a-dep-trans-1", "1"),
                "compile", DependencyFlags.RUNTIME_CP, DependencyFlags.DEPLOYMENT_CP));
        expectedRuntimeDeps.add(new ArtifactDependency(new GACTV("io.quarkus.bootstrap.test", "ext-a-dep-trans-2", "1"),
                "compile", DependencyFlags.RUNTIME_CP, DependencyFlags.DEPLOYMENT_CP));
