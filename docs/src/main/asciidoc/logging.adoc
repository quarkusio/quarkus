...
* In code contextualized with `org.eclipse.microprofile.context.ThreadContext` and run on a thread without a Vert.x context, for example a `contextualRunnable()` submitted to your own executor, or a stage created with `withContextCapture()` that continues on it. The MDC is registered as the `MDC` context type, so it can be excluded with `@ThreadContextConfig(cleared = "MDC")` or `ThreadContext.builder().cleared("MDC")`.
