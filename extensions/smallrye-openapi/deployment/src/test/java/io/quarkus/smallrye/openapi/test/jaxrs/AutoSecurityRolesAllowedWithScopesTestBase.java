...
abstract class AutoSecurityRolesAllowedWithScopesTestBase {
    ...
    static Matcher<Iterable<Object>> schemeArray(String schemeName, Matcher<Iterable<?>> scopesMatcher) {
        return allOf(iterableWithSize(1),
                hasItem(allOf(aMapWithSize(1), hasEntry(equalTo(schemeName), scopesMatcher))));
    }
    ...
    void testAutoSecurityRequirement(String schemeType) {
        RestAssured.given().header("Accept", "application/json").when().get("/q/openapi").then().log().body().and()
                .body("components.securitySchemes.MyScheme",
                        allOf(hasEntry("type", schemeType), hasEntry("description", "Authentication using MyScheme")))
                .and()
                ...
                .body("paths.'/resource2/test-security/methodLevel/1'.get.security",
                        schemeArray("MyScheme", contains("user1")))
                .body("paths.'/resource2/test-security/methodLevel/2'.get.security",
                        schemeArray("MyScheme", contains("user2")))
                ...
                .body("paths.'/resource2/test-security/annotated/documented'.get.security",
                        schemeArray("JWTCompanyAuthentication", emptyIterable()))
                .body("paths.'/resource2/test-security/methodLevel/3'.get.security",
                        schemeArray("MyScheme", contains("admin")))
                .and()
                // OpenApiResourceSecuredAtClassLevel
                .body("paths.'/resource2/test-security/classLevel/1'.get.security",
                        schemeArray("MyScheme", contains("user1")))
                .body("paths.'/resource2/test-security/classLevel/2'.get.security",
                        schemeArray("MyScheme", contains("user2")))
                .body("paths.'/resource2/test-security/classLevel/3'.get.security",
                        schemeArray("MyOwnName", emptyIterable()))
                .body("paths.'/resource2/test-security/classLevel/4'.get.security",
                        schemeArray("MyScheme", contains("admin")))
                .and()
