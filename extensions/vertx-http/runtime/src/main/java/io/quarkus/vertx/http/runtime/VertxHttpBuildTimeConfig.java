...
public interface VertxHttpBuildTimeConfig {
    ...
    /**
     * Configures the engine to require/request client authentication. {@code NONE, REQUEST, REQUIRED}.
     * <p>
     * When set to {@code REQUIRED}, it's recommended to also set `quarkus.http.insecure-requests=disabled` to disable
     * the plain HTTP port. If `quarkus.http.insecure-requests` is not set, but this parameter is set to
     * {@code REQUIRED}, then, `quarkus.http.insecure-requests` is automatically set to `disabled`.
     */
    ...
    /**
     * If this is true then only a virtual channel will be set up for vertx web. We have this switch for testing
     * purposes.
     */
    @WithDefault("false")
    boolean virtual();
     ...
     * <p>
     * * Relative path (Default, `q`) -> Non-application endpoints will be served from
     * `${quarkus.http.root-path}/${quarkus.http.non-application-root-path}`. * Absolute path (`/q`) -> Non-application
     * endpoints will be served from the specified path. * `${quarkus.http.root-path}` -> Setting this path to the same
     * value as HTTP root path disables this root path. All extension-provided endpoints will be served from
     * `${quarkus.http.root-path}`.
     * <p>
     * If the management interface is enabled, the root path for the endpoints exposed on the management interface is
     * configured using the `quarkus.management.root-path` property instead of this property.
     */
    @WithDefault("q")
    ...
    /**
     * If user adds br, then brotli will be added to the list of supported compression algorithms. It implies loading
     * libbrotli native library via JNI and in case of native image, packing the native library into the native image as
     * a resource thus inflating its size. Note that a native shared object library must be available for your platform
     * in Brotli4J project.
     * <p>
     * Client expresses its capability by sending Accept-Encoding header, e.g. Accept-Encoding: deflate, gzip, br Server
     * chooses the compression algorithm based on the client's capabilities and marks it in a response header, e.g.:
