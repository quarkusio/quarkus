...
* OR it is injected statically in application code (e.g. `@Inject DataSource` or `@Inject @DataSource("inventory") AgroalDataSource`)
...
Notably, as soon as a datasource is used by Hibernate ORM or injected in your application,
...
All this applies equally to the default datasource and to named datasource.
You can mix the default datasource and named datasources, or only have a default datasource, or only have named datasources.
