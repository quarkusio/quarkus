...
// * link:{url-quarkusio-guides}http-reference#ssl[Secure connections with SSL/TLS] - Downstream synergy. Needs an attribute
* xref:cross-origin-resource-sharing[Cross-origin resource sharing]
* xref:cross-site-request-forgery-prevention[Cross-site Request Forgery (CSRF) prevention]
* xref:samesite-cookies[SameSite cookies]
* xref:secret-engines[Secrets engines]
* xref:secure-serialization[Secure serialization]
* xref:secure-resources-by-rest-with-panache[Secure auto-generated resources by REST Data with Panache]
...
[[secure-resources-by-rest-with-panache]]
