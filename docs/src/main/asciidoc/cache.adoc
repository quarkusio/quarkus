...
The same applies when a `@CacheResult` method returns a `Uni` that fails asynchronously,
including when its subscription is scheduled with `runSubscriptionOn`.
A retry can compute a new value, and a successful result is cached.
