...
:summary: Develop and build your {product-name-first} application with Maven
:devtools-no-gradle:
...
| `platformVersion`
| The version currently recommended by the xref:extension-registry-user.adoc[{product-name} Extension Registry]
...
| `true`
| Whether {product-name} should use the online registry to resolve extension catalogs. If this is set to false, the extension catalog will be narrowed to the defined (or default) platform BOM.
...
From inside a {product-name} project, you can obtain a list of the available extensions with:
...
However, you can pass a partial name, and {product-name} will do its best to find the right extension.
...
The {product-name} Maven plugin makes use of `javac`,
...
[IMPORTANT]
====
Because the {product-name} Maven plugin itself runs in the JVM started by Maven,
and because some (rare) {product-name} extensions need to load application classes during the build,
...
{product-name} comes with a built-in development mode.
...
It is possible to use development mode remotely, so that you can run {product-name} in a container environment (such as OpenShift)
...
----
<1> This tells {product-name} to use the mutable-jar format. Mutable applications also include the deployment time parts of {product-name},
...
The `mutable-jar` is then built in the same way that a regular {product-name} jar is built, i.e. by issuing:

include::{includes}/devtools/build.adoc[]

Before you start {product-name} on the remote host set the environment variable `QUARKUS_LAUNCH_DEVMODE=true`. If you are
...
that is generated with a new {product-name} application is all you need. If you are using bare metal launch the {product-name} runner
...
In development mode, {product-name} starts by default with debug mode enabled, listening to port `5005` without suspending the JVM.
...
[TIP]
====
You can also run a {product-name} application in debug mode with a suspended JVM using:
...
Usually, dependencies of an application (which is a Maven project) could be displayed using `mvn dependency:tree` command. In case of a {product-name} application, however, this command will list only the runtime dependencies of the application.
Given that the {product-name} build process adds deployment dependencies of the extensions used in the application to the original application classpath, it could be useful to know which dependencies and which versions end up on the build classpath.
...
{product-name} extension dependencies are divided into the runtime extension dependencies that end up on the application runtime classpath and the deployment (or build time) extension dependencies that are resolved by {product-name} only at application build time to create
the build classpath. Application developers are expected to express dependencies only on the runtime artifacts of {product-name} extensions. As a consequence, the deployment extension dependencies aren't visible to Maven plugins that aren't aware of the {product-name}
...
To enable the use-case of building and testing a {product-name} application offline, the `quarkus-maven-plugin` includes the `go-offline` goal that could be called from the command line like this:
...
Native executables make {product-name} applications ideal for containers and serverless workloads.
...
Executing [/Users/starksm/Dev/JBoss/{product-name}/starksm64-quarkus-quickstarts/getting-started-native/target/quarkus-quickstart-runner, -Dquarkus.http.port=8081, -Dtest.url=http://localhost:8081, -Dquarkus.log.file.path=target/quarkus.log]
2019-02-28 16:52:42,020 INFO  [io.quarkus] (main) {product-name} started in 0.007s. Listening on: http://localhost:8080
...
Be aware that a given {product-name} version might not be compatible with all the images available.
====
...
<2> Optionally use a BOM file to omit the version of the different {product-name} dependencies.
<3> Use the {product-name} Maven plugin that will hook into the build process.
<4> Enabling Maven plugin extensions will register a {product-name} `MavenLifecycleParticipant` which will make sure the {product-name} classloaders used during the build are properly closed. During the `generate-code` and `generate-code-tests` goals the {product-name} application bootstrap is initialized and re-used in the `build` goal (which actually builds and packages a production application). The {product-name} classloaders will be properly closed in the `build` goal of the `quarkus-maven-plugin`. However, if the build fails in between the `generate-code` or `generate-code-tests` and `build` then the {product-name} augmentation classloader won't be properly closed, which may lead to locking of JAR files that happened to be on the classpath on Windows OS.
...
TIP: The `fast-jar` packaging results in creating an artifact that starts a little faster and consumes slightly less memory than a legacy {product-name} jar
...
{product-name} Maven plugin supports the generation of Uber-Jars by specifying a `quarkus.package.type=uber-jar` configuration option in your `application.properties`
...
By default, {product-name} will not discover CDI beans inside another module.
...
The reason is that, {product-name} may need to re-resolve application dependencies during the test phase to set up the test classpath for the tests. The original Maven resolver used in previous build phases
will not be available in the test process and, as a conseqence, {product-name} will need to initialize a new one. To make sure the new resolver is initialized correctly, the relevant configuration options
...
{product-name} supports xref:config-reference.adoc#profiles[configuration profiles] in order to provide a specific configuration according to the target environment.
...
However, it is also possible to specify the profile directly in the POM file of the project using project properties, the {product-name} Maven plugin configuration properties or system properties set in the {product-name} Maven plugin configuration.

In order of precedence (greater precedence first):

.1. System properties set in the {product-name} Maven plugin configuration
...
.2. {product-name} Maven plugin configuration properties
...
In that case, it is possible to add as many executions as needed to the {product-name} Maven plugin configuration.

Below is an example of a {product-name} Maven plugin configuration that will produce two builds of the same application: one using the `prod-oracle` profile and the other one using the `prod-postgresql` profile.
...
By default, {product-name} tests in JVM mode are run using the `test` configuration profile. If you are not familiar with {product-name}
...
NOTE: This goal was introduced in {product-name} Maven plugin 2.7.0.Final and can be used in projects that are based on {product-name} version 2.0.0.Final or later.

The {product-name} Maven plugin includes a goal called `info` (currently marked as 'experimental') that logs {product-name}-specific information about the project, such as: the imported {product-name} platform BOMs and the {product-name} extensions found among the project dependencies.
...
NOTE: The report generated by `quarkus:info` is not currently including the {product-name} Maven plugin information, however it's planned to be added in the future releases.
...
[INFO] {product-name} platform BOMs: <1>
...
<1> {product-name} platform BOMs imported in the project (BOMs imported by parent POMs will also be reported)
<2> Direct {product-name} extension dependencies managed by the `quarkus-bom`
<3> Direct {product-name} extension dependencies managed by the `quarkus-camel-bom`
<4> Direct {product-name} extensions dependencies that aren't managed by {product-name} BOMs but found in the {product-name} extension registry

NOTE: `quarkus:info` will also report {product-name} extensions that aren't found in the {product-name} extension registries if those are present among the project dependencies, indicating they have an 'unknown origin'.

[[project-info-misaligned]]
=== Highlighting misaligned versions

`quarkus:info` will also highlight basic {product-name} dependency version misalignments, in case they are detected. For example, if we modify the project mentioned above by removing the `camel-quarkus-rabbitmq` extension from the dependencies and adding a `2.6.3.Final` `<version>` element to the `quarkus-resteasy-reactive` dependency that is managed by the `quarkus-bom` and then run `quarkus:info` again, we'll see something like:

[source,text,subs=attributes+]
----
[INFO] --- quarkus-maven-plugin:{quarkus-version}:info (default-cli) @ code-with-quarkus ---
[WARNING] quarkus:info goal is experimental, its options and output may change in future versions
[INFO] {product-name} platform BOMs:
[INFO]   {quarkus-platform-groupid}:quarkus-bom:pom:{quarkus-version}
...
[INFO]   io.quarkiverse.prettytime:quarkus-prettytime:0.2.1
[INFO]
[WARNING] Non-recommended {product-name} platform BOM and/or extension versions were found. For more details, please, execute 'mvn quarkus:update -Drectify'
----
<1> The `quarkus-camel-bom` import is now reported as 'unnecessary' since none of the {product-name} extensions it includes are found among the project dependencies
<2> The version `2.6.3.Final` of the `quarkus-resteasy-reactive` is now reported as being misaligned with the version managed by the {product-name} platform BOM imported in the project, which is {quarkus-version}
...
The `quarkus:update` goal (currently marked as 'experimental') provided by the {product-name} Maven plugin can be used to check whether there are {product-name}-related updates available for a project, such as: new releases of the relevant {product-name} platform BOMs and non-platform {product-name} extensions present in the project. In a multi-module project the `update` goal is meant to be executed from the main {product-name} application module.
...
NOTE: The {product-name} Maven plugin version isn't currently included in the update report, however it's planned to be added in the future releases.

The way `quarkus:update` works, first, all the direct {product-name} extension dependencies of the project are collected (those that are managed by the {product-name} platform BOMs and those that aren't but found in the {product-name} extension registries). Then the configured {product-name} extension registries (typically the `registry.quarkus.io`) will be queried for the latest recommended/supported {product-name} platform versions and non-platform {product-name} extensions compatible with them. The algorithm will then select the latest compatible combination of all the extensions found in the project, assuming such a combination actually exists. Otherwise, no updates will be suggested.

Assuming we have a project including Kogito, Camel and core {product-name} extensions available in the {product-name} platform based on {product-name} `2.7.1.Final`, the output of the `quarkus:update` would look like:
[source,text,subs=attributes+]
----
...
[INFO] 
[INFO] Recommended {product-name} platform BOM updates: <1>
...
<1> A list of currently recommended {product-name} platform BOM updates

NOTE: Typically, a single project property will be used to manage all the {product-name} platform BOMs but the implementation isn't currently smart enough to point that out and will report updates for each BOM individually.

If we modify the project to remove all the Camel {product-name} extensions from the project, change the version of the `quarkus-resteasy-reactive` extension to `2.6.3.Final` and downgrade `quarkus-prettytime` which is not included in the {product-name} platform BOMs to `0.2.0`, `quarkus:update` will report something like:
...
<1> A list of the currently recommended {product-name} platform BOM updates for the project
<2> Given that the project does not include any Camel {product-name} extensions, the BOM import is recommended to be removed
...
As was mentioned above, `quarkus:info`, besides reporting {product-name} platform and extension versions, performs a quick version alignment check, to make sure the extension versions used in the project are compatible with the imported {product-name} platform BOMs. If misalignments are detected, the following warning message will be logged:
...
When the `rectify` option is enabled, `quarkus:update`, instead of suggesting the latest recommended {product-name} version updates, will log update instructions to simply align the extension dependency versions found in the project with the currently imported {product-name} platform BOMs.
