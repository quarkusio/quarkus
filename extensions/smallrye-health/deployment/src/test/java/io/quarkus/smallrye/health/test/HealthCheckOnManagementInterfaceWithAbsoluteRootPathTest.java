...
class HealthCheckOnManagementInterfaceWithAbsoluteRootPathTest {
    ...
    void testHealth() {
        try {
            ...
            when().get("http://localhost:9001/sante/live").then().body("status", is("UP"), "checks.status",
                    contains("UP"), "checks.name", contains("my-check"));
            when().get("http://localhost:9001/sante/live").then().body("status", is("DOWN"), "checks.status",
