...
{product-name-first} provides a Jakarta Persistence (formerly known as JPA) identity provider, similar to the xref:security-jdbc.adoc[JDBC identity provider], suitable for use with the xref:security-basic-authentication.adoc[Basic] and xref:security-authentication-mechanisms.adoc#form-auth{product-name}ased] Quarkus Security mechanisms, which require a combination of username and password credentials.
...
For an example of practical use of Basic authentication and Jakarta Persistence, see the xref:security-basic-authentication-tutorial.adoc[Secure a {product-name} application with Basic authentication and Jakarta Persistence] tutorial.


== Jakarta Persistence entity specification

{product-name} security offers a Jakarta Persistence integration to collect usernames, passwords, and roles, and store them into Jakarta Persistence database entities.

The following Jakarta Persistence entity specification demonstrates how users' information needs to be stored in a Jakarta Persistence entity and properly mapped so that {product-name} can retrieve this information from a database.
...
When developing applications with {product-name}, you can decide how to manage password storage and hashing. You can choose to keep the default password and hashing settings of {product-name}, or you can hash passwords manually.
