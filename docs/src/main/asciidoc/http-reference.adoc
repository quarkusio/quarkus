...
A request for a directory, such as `/foo/`, is answered with its index page (`index.html` by default, see `quarkus.http.static-resources.index-page`).
By default, the same directory requested without the trailing slash (`/foo`) is not found. `quarkus.http.static-resources.normalize-index-directories`
can change this to `redirect`, which sends a permanent redirect to `/foo/` so that relative links in the index page keep resolving against the directory,
or to `reroute`, which serves the index page at `/foo` as well. The setting applies in the same way in development mode and in a packaged application.
