...
=== Using a value class as the entity identifier

A `@JvmInline` value class can be the type of an entity's `@Id` property and the `Id` type argument of
`PanacheRepositoryBase` or `PanacheCompanionBase`. Kotlin compiles the value class to its underlying type,
which is what Hibernate ORM maps, and the generated `findById` and `deleteById` implementations pass that
underlying value on, so the repository is used with the value class:
...
class Greeting {
    @Id
    var id: GreetingId = GreetingId(UUID.randomUUID())
}

@ApplicationScoped
class GreetingRepository : PanacheRepositoryBase<Greeting, GreetingId>

val greeting = greetingRepository.findById(GreetingId(uuid))
----
