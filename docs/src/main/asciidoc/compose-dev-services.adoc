...
https://docs.docker.com/compose/[Docker Compose] is the reference implementation and most widely used tool for defining and managing multi-container applications,
...
Quarkus detects the presence of a `compose.yml` (or similar) file in the project and automatically starts services when the application is started in dev or test mode.
Extensions providing dev services discover the custom services and use them instead of creating default ones.
...
you can create a `compose.yml` file in the root of the project and define a custom service using Compose:
...
When you run the application in dev mode or execute tests, Compose Dev Services will automatically start the PostgreSQL service defined in the `compose.yml` file,
...
Note that the `compose.yml` file can be customized as needed with multiple services with dependencies to control the startup order,
...
By default, Compose Dev Services looks for files in the project root named `compose.[yml|yaml]`, `docker-compose.[yml|yaml]`.
...
You can configure Compose Dev Services to ignore specific services by adding the `io.quarkus.devservices.compose.ignore` label to the service in your `compose.yml` file:
...
Compose Dev Services respects the health checks defined in your `compose.yml` file.
...
Compose Dev Services starts services in the order they are defined in the `compose.yml` file.
...
== Configuring the Compose Project Name

When the top-level `name` attribute is not specified in the Compose description,
Compose Dev Services uses a generated project name for the Compose project.
You can customize the project name by setting the `quarkus.compose.devservices.project-name` property in the `application.properties` file:

[source, properties]
----
quarkus.compose.devservices.project-name=my-project
----

=== Discovering already started Compose services

If no compose files are found in the project, but `quarkus.compose.devservices.project-name` is set,
Compose Dev Services will try to discover already started services with the specified project name.

The already started compose project can be started using the `docker compose up` or `podman compose up` commands,
or by another Quarkus application running locally with the same project name.
