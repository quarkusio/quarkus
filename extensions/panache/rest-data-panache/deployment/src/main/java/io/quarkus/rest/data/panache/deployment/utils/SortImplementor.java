...
public final class SortImplementor {
    ...
    /**
     * Builds a Panache {@code Sort} instance based on the sort query parameters. Sort query parameters could be either
     * separated by comma or given as separate instances, e.g. '?sort=name,-age&sort=age'. '-' sign before the field
     * name indicates a descending order.
     *
     * @param creator
     *        Bytecode creator instance.
     * @param sortParams
     *        List of sort query strings
     *
     * @return Panache Sort instance
     */
    public ResultHandle getSort(BytecodeCreator creator, ResultHandle sortParams) {
        ResultHandle sort = creator.invokeStaticMethod(ofMethod(Sort.class, "by", Sort.class, String[].class),
                creator.newArray(String.class, 0));
        ResultHandle fieldsIterator = creator.invokeInterfaceMethod(ofMethod(List.class, "iterator", Iterator.class),
                getSortFields(creator, sortParams));
        // Iterate through the sort fields
        BytecodeCreator loopCreator = creator.whileLoop(c -> iteratorHasNext(c, fieldsIterator)).block();
        ResultHandle field = loopCreator.invokeInterfaceMethod(ofMethod(Iterator.class, "next", Object.class),
                fieldsIterator);
    ...
    private void addDescendingSortField(BytecodeCreator creator, ResultHandle sort, ResultHandle field) {
        ...
        creator.invokeVirtualMethod(ofMethod(Sort.class, "and", Sort.class, String.class, Sort.Direction.class), sort,
                fieldToAdd, creator.load(Sort.Direction.Descending));
    }
    ...
    private ResultHandle getSortFields(BytecodeCreator creator, ResultHandle sortParamsList) {
        ...
        ResultHandle sortParamsIterator = creator
                .invokeInterfaceMethod(ofMethod(List.class, "iterator", Iterator.class), sortParamsList);
        ...
        ResultHandle sortParam = loopCreator.invokeInterfaceMethod(ofMethod(Iterator.class, "next", Object.class),
                sortParamsIterator);
        ResultHandle extractedSortFields = extractSortFieldsFromParam(loopCreator, sortParam);
        loopCreator.invokeInterfaceMethod(ofMethod(List.class, "addAll", boolean.class, Collection.class),
                sortFieldsList, extractedSortFields);
     ...
     *        Bytecode creator instance
     * @param sortParam
     *        a single sort query parameter value (e.g. 'name' or 'name,-age')
     *
    ...
    private ResultHandle extractSortFieldsFromParam(BytecodeCreator creator, ResultHandle sortParam) {
        ...
        return creator.invokeStaticMethod(ofMethod(Arrays.class, "asList", List.class, Object[].class),
                sortFieldsArray);
    }
