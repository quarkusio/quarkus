...
public class RestEasyCORSTestCase {
    ...
    static QuarkusUnitTest runner = new QuarkusUnitTest().withApplicationRoot((jar) -> jar
            .addClasses(RootResource.class).addAsResource("cors-config.properties", "application.properties"));
    ...
    public void testCORSPreflightRootResource() {
        ...
        RestAssured.given().header("Origin", origin).header("Access-Control-Request-Method", methods)
                .header("Access-Control-Request-Headers", headers).when().options("/").then()
    ...
    public void testCORSRootResource() {
        String origin = "http://custom.origin.quarkus";
        RestAssured.given().header("Origin", origin).when().get("/").then()
                .header("Access-Control-Allow-Origin", origin).body(Matchers.is("Root Resource"));
    }
}
