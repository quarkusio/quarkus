...
public interface UserResource {
    ...
    /**
     * Return credential types, which are provided by the user storage where user is stored. Returned values can contain for
     * example "password", "otp" etc.
     ...
     *
     * @param credentialId The credential to move
     * @param newPreviousCredentialId The credential that will be the previous element in the list. If set to null, the moved
     *        credential will be the first element in the list.
     */
    @Path("credentials/{credentialId}/moveAfter/{newPreviousCredentialId}")
    @POST
    void moveCredentialAfter(final @PathParam("credentialId") String credentialId,
     ...
     *
     * This is typically supported just for the users backed by user storage providers. See
     * {@link UserRepresentation#getDisableableCredentialTypes()}
    ...
    @PUT
    @Path("execute-actions-email")
    void executeActionsEmail(@QueryParam("client_id") String clientId, @QueryParam("redirect_uri") String redirectUri,
            List<String> actions);
