...
When a {product-name-first} application starts, a config mapping can be registered twice. One time for _STATIC INIT_ and a second
...
The `Server` can be injected as a mock into a {product-name} test class with `@InjectMock`:
...
The `AppConfig` can be injected as a mock into a {product-name} test class with `@Inject`:
