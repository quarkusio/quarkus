...
public class ReloadOtherKeyStoreTest {
    ...
    static final QuarkusExtensionTest config = new QuarkusExtensionTest().setArchiveProducer(
            ...
            .overrideRuntimeConfigKey("quarkus.tls.key-store.other.type", "PKCS12")
            .overrideRuntimeConfigKey("quarkus.tls.key-store.other.path", temp.getAbsolutePath() + "/tls.p12")
            .overrideRuntimeConfigKey("quarkus.tls.key-store.other.password", "password")
            .overrideRuntimeConfigKey("loc", temp.getAbsolutePath())
            .setBeforeAllCustomizer(() -> {
                try {
                    temp.mkdirs();
                    Files.copy(new File("target/certs/test-reload-A-keystore.p12").toPath(),
