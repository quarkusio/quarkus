...
public class MicroprofileConfigYmlHotReloadTest {

    @RegisterExtension
    final static QuarkusDevModeTest TEST = new QuarkusDevModeTest()
            .setArchiveProducer(() -> ShrinkWrap.create(JavaArchive.class).addClass(HotReloadResource.class)
                    .addAsResource("hot-reload-test.yml", MICROPROFILE_CONFIG_YML_FILE));

    @Test
    public void testHotReload() {
        checkResponse("foo");
        TEST.modifyResourceFile(MICROPROFILE_CONFIG_YML_FILE, new Function<String, String>() {
            @Override
