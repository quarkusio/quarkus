...
{product-name-first} Funq{product-name}rt of Quarkus's serverless strategy and aims to provide a portable Java API to write functions
...
Each Funqy Java class is a {product-name} Arc component and supports dependency injection through
...
have to make the decision on what camp you are in.  {product-name} does support REST integration (through Jakarta REST,
...
Lambda.  In this case, we tell them they may be better off using the xref:aws-lambda.adoc[{product-name} AWS Lambda] integration instead.
