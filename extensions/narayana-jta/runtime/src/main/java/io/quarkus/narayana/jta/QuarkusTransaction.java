...
/**
 * A simplified transaction interface. While broadly covering the same use cases as
 * {@link jakarta.transaction.UserTransaction}, this class is designed to be easier to use. The main features it offers
 * over {@code UserTransaction} are:
 * <ul>
 ...
 * <li><b>No Transaction Leaks: </b>Transactions are tied to the request scope, if the scope is destroyed before the
 * transaction is committed the transaction is rolled back. Note that this means this can only currently be used when
 * the request scope is active.</li>
 * <li><b>Per Transaction Timeouts:</b> {{@link BeginOptions#timeout(int)}/{@link TransactionRunnerOptions#timeout(int)}
 * can be used to set the new transactions timeout, without affecting the per thread default.</li>
 * <li><b>Lambda Style Transactions: </b> {@link Runnable} and {@link Callable} instances can be run inside the scope of
 * a new transaction.</li>
 * </ul>
 * <p>
 * Note that any checked exception will be wrapped by a {@link QuarkusTransactionException}, while unchecked exceptions
...
public interface QuarkusTransaction {
     ...
     * <p>
     * This transaction will be tied to the current request scope, if it is not committed when the scope is destroyed
     * then it will be rolled back to prevent transaction leaks.
     */
    static void begin() {
        begin(beginOptions());
    }
     ...
     *
     * @param options
     *        Options that apply to the new transaction
     */
    ...
    /**
     * Returns whether a transaction is associated to the current request scope. The name of this method is misleading.
     * Don't use it.
     *
     * @return {@code true} if a transaction is associated to the current request scope.
     *
     * @deprecated the name of this method was misleading and it shouldn't be used anymore. Use {@link #getStatus()} for
     *             a better semantic instead.
     */
    ...
    /**
     * Starts the definition of a transaction runner, which can then be used to run a task ({@link Runnable},
     * {@link Callable}, ...), with {@link TransactionSemantics#JOIN_EXISTING} semantics:
     * <ul>
     ...
     * }</pre>
     *
     * @return An interface that allow various options of a transaction runner to be customized, or a
     *         {@link Runnable}/{@link Callable} to be executed.
     *
    ...
    /**
     * Starts the definition of a transaction runner, which can then be used to run a task ({@link Runnable},
     * {@link Callable}, ...), with {@link TransactionSemantics#REQUIRE_NEW} semantics:
     * <ul>
     * <li>If an existing transaction is already associated with the current thread then the transaction is suspended,
     * then a new transaction is started which follows all the normal lifecycle rules, and when it's complete the
    ...
    /**
     * Starts the definition of a transaction runner, which can then be used to run a task ({@link Runnable},
     * {@link Callable}, ...), with {@link TransactionSemantics#DISALLOW_EXISTING} semantics:
     * <ul>
     * <li>If a transaction is already associated with the current thread a {@link QuarkusTransactionException} will be
     * thrown,
     * <li>Otherwise a new transaction is started, and follows all the normal lifecycle rules.
     * </ul>
     * <p>
    ...
    /**
     * Starts the definition of a transaction runner, which can then be used to run a task ({@link Runnable},
     * {@link Callable}, ...), with {@link TransactionSemantics#SUSPEND_EXISTING} semantics:
     * <ul>
     ...
     * <li>The exception handler will never be consulted when these semantics are in use, specifying both an exception
    ...
    /**
     * Starts the definition of a transaction runner, which can then be used to run a task ({@link Runnable},
     * {@link Callable}, ...), following the selected {@link TransactionSemantics}.
     * <p>
     * Examples of use:
     *
     * <pre>{@code
     ...
     *
     * @param semantics
     *        The selected {@link TransactionSemantics}.
     *
     * @return An interface that allow various options of a transaction runner to be customized, or a
     *         {@link Runnable}/{@link Callable} to be executed.
     *
    ...
    /**
     * Runs a task in a new transaction with the default timeout. This defaults to
     * {@link Transactional.TxType#REQUIRES_NEW} semantics, however alternate semantics can be requested using
     * {@link #run(RunOptions, Runnable)}.
     *
     * @param task
     *        The task to run in a transaction
     *
     * @deprecated For the same semantics, use {@link #requiringNew()
     *             <code>QuarkusTransaction.requiringNew().run(task)</code>}. {@link #joiningExisting()},
     *             {@link #disallowingExisting()}, {@link #suspendingExisting()} and
    ...
    /**
     * Runs a task in a new transaction with the default timeout. This defaults to
     * {@link Transactional.TxType#REQUIRES_NEW} semantics, however alternate semantics can be specified using the
     ...
     *             {@link #suspendingExisting()} or {@link #runner(TransactionSemantics)} instead.
     */
    @Deprecated
    ...
    /**
     * Calls a task in a new transaction with the default timeout. This defaults to
     * {@link Transactional.TxType#REQUIRES_NEW} semantics, however alternate semantics can be requested using
     * {@link #call(RunOptions, Callable)}.
     * <p>
     ...
     *
     * @deprecated For the same semantics, use {@link #requiringNew()
     *             <code>QuarkusTransaction.requiringNew().call(task)</code>}. {@link #joiningExisting()},
     *             {@link #disallowingExisting()}, {@link #suspendingExisting()} and
     *             {@link #runner(TransactionSemantics)} can also be used for alternate semantics and options.
     */
    @Deprecated
    ...
    /**
     * Calls a task in a new transaction with the default timeout. This defaults to
     * {@link Transactional.TxType#REQUIRES_NEW} semantics, however alternate semantics can be requested using the
     * {@code options} parameter.
     * <p>
     * If the task throws a checked exception it will be wrapped with a {@link QuarkusTransactionException}
     *
