...
public interface TlsConfigurationRegistry {
    ...
    /**
     * Registers a TLS configuration into the registry. Note that only subsequents calls to {@link #get(String)} will
     * return the configuration.
     * <p>
     * The passed configuration is not validated, so it's up to the caller to ensure the configuration is correct.
     *
     * @param name
     *        the name of the configuration, cannot be {@code null}, cannot be {@code <default>}.
     * @param configuration
     *        the configuration cannot be {@code null}.
     */
