...
OpenID Connect extension has to be used if the Quarkus application needs to authenticate the users using OIDC Authorization Code Flow, please read xref:security-oidc-web-authentication-concept.adoc[OpenID Connect authorization code flow mechanism] guide for more information.
