...
`@PostConstruct` annotations will only be performed if the Bean is invoked. Just by being injected it will not run the post construct methods. After the project starts the post construct method is not executed by default, only when any method of the Bean is used for the first time. 
