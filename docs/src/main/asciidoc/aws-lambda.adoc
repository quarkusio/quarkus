...
In dev and test mode, Quarkus provides defaults for common AWS Lambda reserved environment variables
such as `AWS_LAMBDA_FUNCTION_NAME`, `AWS_LAMBDA_FUNCTION_VERSION`, and `AWS_LAMBDA_FUNCTION_MEMORY_SIZE`.
These values are available through the Lambda `Context` API and, in dev mode, through `System.getenv()`.
On AWS, the real environment variables always take precedence.
