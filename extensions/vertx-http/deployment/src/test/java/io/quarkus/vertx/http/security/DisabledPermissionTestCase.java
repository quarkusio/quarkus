...
public class DisabledPermissionTestCase {
    ...
    public void testAnonymousDenied() {
        RestAssured.given().when().get("/default").then().assertThat().statusCode(401);
    }

    @Test
    public void testAnonymousAllowed() {
        RestAssured.given().when().get("/enabled").then().assertThat().statusCode(200).body(equalTo(":/enabled"));
    }

    @Test
    public void testDefaultEnabled() {
        RestAssured.given().auth().preemptive().basic("test", "test").when().get("/default").then().assertThat()
                .statusCode(200).body(equalTo("test:/default"));
    }

    @Test
    public void testDisabled() {
        RestAssured.given().auth().preemptive().basic("test", "test").when().get("/disabled").then().assertThat()
                .statusCode(200).body(equalTo("test:/disabled"));
    }

    @Test
    public void testEnabled() {
        RestAssured.given().auth().preemptive().basic("test", "test").when().get("/enabled").then().assertThat()
                .statusCode(200).body(equalTo("test:/enabled"));
    }

    @Test
    public void testRestricted() {
        RestAssured.given().auth().preemptive().basic("test", "test").when().post("/restricted").then().assertThat()
                .statusCode(403);
    }

    @Test
    public void testUnrestricted() {
        RestAssured.given().auth().preemptive().basic("test", "test").when().get("/unrestricted").then().assertThat()
                .statusCode(200).body(equalTo("test:/unrestricted"));
    }
