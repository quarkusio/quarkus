...
For other cases, there are two declarative ways and a programmatic way to ensure the session is opened:

* The first method is to simply inject the `Mutiny.Session` with CDI.
...
You can annotate the method with `jakarta.transaction.@Transactional` as you would in Hibernate blocking to specify declarative transaction management. Such feature is provided by the `quarkus-reactive-transactions` extension.
...
This method of defining transactions will be superseded by the standard support for `@Transactional`.
Alternatively, you can use the `Panache.withTransaction()` method for the same effect.

[NOTE]
====
You should not mix `@Transactional` with Panache transaction annotations (`@WithTransaction`, `@WithSession`, `@WithSessionOnDemand`) in the same application. Choose one approach and use it consistently throughout your application. Mixing these annotations will result in an `UnsupportedOperationException`.
