 ...
 * command list</a> for further information about these commands. A {@code hash} is a like a {@code Map&lt;F, V&gt;}.
 * This group is parameterized by the type of the key {@code Map&lt;K&gt;}. This is the type of the key in which the
 * hash is stored. {@code &lt;F&gt;} is the type of the key in the map (field). The stored value are of type
 * {@code Map&lt;V&gt;}
 *
 * @param <K>
 *        the type of the key
...
public interface HashCommands<K, F, V> extends RedisCommands {
     ...
     * @param fields
     *        the fields, must not be empty, must not contain {@code null} values
     *
