...
----
<1> `prefix` is optional. If not set then the prefix to be used will be determined by the class name. In this case it would still be `greeting` (since the `Properties` suffix is removed). If the class were named `GreetingExtraProperties` then the resulting default prefix would be `greeting-extra`.
...
----
@Inject
GreetingConfiguation greetingConfigation;
----
