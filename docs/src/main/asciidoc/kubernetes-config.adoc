...
The generated `Role` grants read access to all secrets in the namespace by default.
Two opt-in properties tighten this following the principle of least privilege:

* `quarkus.kubernetes-config.secrets-role-config.secret-names` — restricts the role to specific secrets by name.
  When set, the generated `Role` will only allow access to the listed secrets.
* `quarkus.kubernetes-config.secrets-role-config.generate-name` — prefixes the role name with the application name (e.g. `my-app-view-secrets`).
  Useful when multiple applications share the same namespace, avoiding conflicts over the default `view-secrets` role name.
