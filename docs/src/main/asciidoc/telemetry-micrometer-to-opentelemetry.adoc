...
In dev mode, the Observability *Metrics* view in the Dev UI charts your live metrics.
With the bridge active, Micrometer meters forwarded into the OpenTelemetry SDK, as well
as native OpenTelemetry metrics, are captured once (from OpenTelemetry) — so there are no
duplicate series. See xref:telemetry-micrometer.adoc#dev-ui-metrics[Metrics in the Dev UI]
for details.
