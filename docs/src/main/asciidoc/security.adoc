...
=== Form and JSON Based Authentication

Quarkus provides form based authentication that works in a similar manner to traditional Servlet form based auth, and
JSON based authentication that allows a username and password to be posted to an endpoint in JSON format. Unlike
...
JSON based auth can be use by sending a JSON POST request to the defined endpoint (`/login` by default). This request
must have a `username` and `password` entry in the JSON object, which will be used to authenticate the user. Once the
user is authenticated the encrypted cookie will be used to provide persistent authentication.
...
The following properties can be used to configure the cookie used for form and JSON based auth:

include::{generated-dir}/config/quarkus-vertx-http-config-group-auth-cookie-config.adoc[opts=optional, leveloffset=+1]
...
The following properties can be used to configure JSON based auth:

include::{generated-dir}/config/quarkus-vertx-http-config-group-json-auth-config.adoc[opts=optional, leveloffset=+1]
