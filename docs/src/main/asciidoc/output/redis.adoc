...
:categories: data
:summary: This guide covers how to use a Redis datastore in {product-name-first}.

This guide demonstrates how your {product-name} application can connect to a Redis server using the Redis Client extension.
...
We'll be using the {product-name} Redis extension to connect to interact with Redis.
...
If you already have your {product-name} project configured, you can add the `redis-client` extension
...
At this point, {product-name} uses the Redis Dev Service to run a Redis server and configure the application.
...
To learn more about the {product-name} Redis extension, check xref:redis-reference.adoc[the Redis extension reference guide].
