...
Read about the use of logging API in {product-name-first}, configuring logging output, and using logging adapters to unify the output from other logging APIs.

{product-name} uses the JBoss Log Manager logging backend for publishing application and framework logs.
{product-name} supports the JBoss Logging API as well as multiple other logging APIs, seamlessly integrated with JBoss Log Manager.
...
When using the JBoss Logging API, your application requires no additional dependencies, as {product-name} automatically provides it.
...
In {product-name}, the most common ways to obtain an application logger are by:
...
{product-name} simplifies logging by automatically adding logger fields to classes that use `io.quarkus.logging.Log`.
...
`Log` method calls that are not processed by {product-name} at build time will throw an exception.
...
{product-name} provides different log levels, which helps developers control the amount of information logged based on the severity of the events.

.Log levels used by {product-name}
...
JBoss Logging is built into {product-name} and provides link:https://quarkus.io/developer-joy/[unified configuration] for all <<logging-apis,supported logging APIs>>.
...
This sets a floor level for which {product-name} needs to generate supporting code.
The minimum log level must be set at build time so that {product-name} can open the door to optimization opportunities where logging on unusable levels can be elided.
...
{product-name} uses a pattern-based logging formatter that generates human-readable text logs by default, but you can also configure the format for each log handler by using a dedicated property.
...
Changing the console log format is useful, for example, when the console output of the {product-name} application is captured by a service that processes and stores the log information for later analysis.
...
{product-name} includes several different log handlers: **console**, **file**, and **syslog**.
...
To log events to a file on the application's host, use the {product-name} file log handler.
The file log handler is disabled by default, so you must first enable it.

The {product-name} file log handler supports log file rotation. 
...
The syslog handler in {product-name} follows the link:https://en.wikipedia.org/wiki/Syslog[Syslog] protocol, which is used to send log messages on Unix-like systems.
...
The following examples show some of the ways in which you can configure logging in {product-name}:

.Console DEBUG logging except for {product-name} logs (INFO), no color, shortened time, shortened category prefixes
[source, properties]
----
...
To send logs to a centralized tool such as Graylog, Logstash, or Fluentd, see the {product-name} xref:centralized-log-management.adoc[Centralized log management] guide.
...
{product-name} relies on the JBoss Logging library for all the logging requirements.
...
+
NOTE: This step is unnecessary for libraries that are dependencies of a {product-name} extension where the extension handles it automatically.
+
...
. Verify whether the logs generated by the added library adhere to the same format as the other {product-name} logs.

=== Use MDC to add contextual log information

{product-name} overrides the logging Mapped Diagnostic Context (MDC) to improve the compatibility with its reactive core.
...
In {product-name}, the MDC provider has a specific implementation for handling the reactive context, ensuring that MDC data is propagated during reactive and asynchronous processing.
