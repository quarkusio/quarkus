...
mvn io.quarkus.platform:quarkus-maven-plugin:{quarkus-version}:create \
    ...
    -Dextensions="security-webauthn,reactive-pg-client,resteasy-reactive,hibernate-reactive-panache,test-security-webauthn" \
    -DnoExamples
...
Run your application in DEV mode with `mvn clean quarkus:dev`, which will start a postgres DEV service container,
...
`POST /webauthn/register`: Set up and obtain a registration challenge
...
`POST /webauthn/login`: Set up and obtain a login challenge
...
`POST /webauthn/callback`: Trigger a registration
...
`POST /webauthn/callback`: Trigger a login
...
`GET /webauthn/logout`: Logs you out
...
a JavaScript library to help you talk to the WebAuthn endpoints, at `/webauthn/webauthn.js`. You can set it up like this:
