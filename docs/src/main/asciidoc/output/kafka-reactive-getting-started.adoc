...
:summary: This guide demonstrates how your {product-name-first} application can utilize SmallRye Reactive Messaging to interact with Apache Kafka.

This guide demonstrates how your {product-name} application can utilize SmallRye Reactive Messaging to interact with Apache Kafka.
...
This command creates the project structure and selects two {product-name} extensions we will be using:
...
{product-name} starts a broker for you automatically.
...
If not indicated otherwise, like in this example, {product-name} uses the channel name as topic name.
So, in this example, the application writes into the `quote-requests` topic.
{product-name} also configures the serializer automatically, because it finds that the `Emitter` produces `String` values.
...
{product-name} detects them, and if none are found, generates them using JSON serialization.
...
No need to configure anything, as {product-name} will automatically associate the `quotes` channel to the `quotes` Kafka topic.
...
{product-name} starts a Kafka broker automatically, configures the application and shares the Kafka broker instance between different applications.
...
{product-name} extension for Kafka also allows
