...
:summary: Easy {product-name-first}-based scripting with jbang.
:extension-status: preview

{product-name} provides integration with https://jbang.dev[jbang] which allows you to write Java scripts/applications requiring no Maven nor Gradle to get running.
...
import io.quarkus.runtime.{product-name};
...
Now, run `jbang quarkusapp.java` and you will see JBang resolving this dependency and building the jar with help from {product-name}' JBang integration.
...
INFO: {product-name} augmentation completed in 722ms
Hello World
----
...
Now let us replace the class with one that uses {product-name} features:
...
It's a very simple class with a main method that starts {product-name} with a REST endpoint, returning "hello" to requests on "/hello".
...
Now when you run the application you will see {product-name} start up.
...
INFO: {product-name} augmentation completed in 681ms
__  ____  __  _____   ___  __ ____  ______
 --/ __ \/ / / / _ | / _ \/ //_/ / / / __/
 -/ /_/ / /_/ / __ |/ , _/ ,< / /_/ /\ \
--\___\_\____/_/ |_/_/|_/_/|_|\____/___/
2020-08-30 05:49:03,255 INFO  [io.quarkus] (main) {product-name} {quarkus-version} on JVM started in 0.638s. Listening on: http://0.0.0.0:8080
...
public class quarkusapp {
    ...
    @GET
    public String sayHello() {
        return "hello from {product-name} with jbang.dev";
    }
...
more traditional {product-name} debug port you can use `jbang --debug=5005 quarkusapp.java`.

Note: JBang debugging always suspends thus you need to connect the debugger to have the application run.

== Logging

To use logging in {product-name} scripting with JBang you do as usual, with configuring a logger, i.e.
...
INFO: Running {product-name} native-image plugin on GraalVM Version 20.1.0 (Java Version 11.0.7)
Aug 30, 2020 6:21:16 AM io.quarkus.deployment.pkg.steps.NativeImageBuildStep build
...
INFO: {product-name} augmentation completed in 76010ms
__  ____  __  _____   ___  __ ____  ______
 --/ __ \/ / / / _ | / _ \/ //_/ / / / __/
 -/ /_/ / /_/ / __ |/ , _/ ,< / /_/ /\ \
--\___\_\____/_/ |_/_/|_/_/|_|\____/___/
2020-08-30 06:22:32,012 INFO  [io.quarkus] (main) {product-name} {quarkus-version} native started in 0.017s. Listening on: http://0.0.0.0:8080
...
__  ____  __  _____   ___  __ ____  ______
 ...
 -/ /_/ / /_/ / __ |/ , _/ ,< / /_/ /\ \
--\___\_\____/_/ |_/_/|_/_/|_|\____/___/
2020-08-30 06:23:36,846 INFO  [io.quarkus] (main) {product-name} {quarkus-version} native started in 0.015s. Listening on: http://0.0.0.0:8080
...
If you want to get started with {product-name} or write something quickly, {product-name} Scripting with jbang lets you do that. No Maven, no Gradle - just a Java file. In this guide we outlined the very basics on using {product-name} with JBang; if you want to learn more about what JBang can do, go see https://jbang.dev.
