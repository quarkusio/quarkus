...
native-image --no-server -O0 --verbose -H:IncludeResources=META-INF/persistence.xml -H:IncludeResources=META-INF/services/.* -H:+ReportUnsupportedElementsAtRuntime -H:ReflectionConfigurationFiles=reflectconfig.json -cp "$CLASSPATH":./target/classes com.example.Main com.example.Main
