...
NOTE: Specify trusted proxies by IP address, hostname or CIDR notation, without a port. A port appended to an address, such as `127.0.0.1:8084`, is compared with the source port of the connection from the proxy, which is usually ephemeral, so the entry only matches a proxy that connects from a fixed source port, and a warning is logged for it.
