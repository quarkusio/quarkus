...
* To verify a plain text password against a stored hash, use `BcryptUtil.matches(char[] plainPassword, String hashedPassword)`.
* For custom iterations: `BcryptUtil.bcryptHash(char[] password, int iterationCount)` where higher iterations (12-14) provide more security but slower performance.
====
