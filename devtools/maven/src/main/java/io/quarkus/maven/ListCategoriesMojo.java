...
public class ListCategoriesMojo extends QuarkusProjectMojoBase {
    ...
    /**
     * Select the output format among 'name' (display the name only) and 'full' (includes a verbose name and a
     * description).
     */
    ...
    public void doExecute(final QuarkusProject quarkusProject, final MessageWriter log) throws MojoExecutionException {
        try {
            ListCategories listExtensions = new ListCategories(quarkusProject).format(format);
            ...
            log.info("");
            log.info(ListCategories.LIST_EXTENSIONS_HINT, "`./mvnw quarkus:list-extensions -Dcategory=\"categoryId\"`");
