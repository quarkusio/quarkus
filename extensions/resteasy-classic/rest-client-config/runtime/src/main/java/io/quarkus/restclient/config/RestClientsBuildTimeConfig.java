...
public interface RestClientsBuildTimeConfig {
    ...
    /**
     * If true, the extension will automatically remove the trailing slash in the paths if any. This property is not
     * applicable to the RESTEasy Client.
     */
    ...
    interface RestClientBuildConfig {
        ...
        /**
         * The CDI scope to use for injection. This property can contain either a fully qualified class name of a CDI
         * scope annotation (such as "jakarta.enterprise.context.ApplicationScoped") or its simple name (such as
         * "ApplicationScoped"). By default, this is not set which means the interface is not registered as a bean
         * unless it is annotated with {@link RegisterRestClient}. If an interface is not annotated with
         * {@link RegisterRestClient} and this property is set, then Quarkus will make the interface a bean of the
         * configured scope.
         */
        Optional<String> scope();

        /**
         * If set to true, then Quarkus will ensure that all calls from the REST client go through a local proxy server
         * (that is managed by Quarkus). This can be very useful for capturing network traffic to a service that uses
         * HTTPS.
         * <p>
        ...
        /**
         * This setting is used to select which proxy provider to use if there are multiple ones. It only applies if
         * {@code enable-local-proxy} is true.
         * <p>
    ...
    /**
     * Provides a new {@link RestClientsBuildTimeConfig} with the discovered registered REST Clients configuration only.
     * This should be preferred once REST Clients are discovered and validated to keep only the required configuration.
     * <p>
     ...
     *
     * @param restClients
     *        the discovered registered REST Clients.
     *
    ...
    default SmallRyeConfig getConfig(List<RegisteredRestClient> restClients) {
        return new SmallRyeConfigBuilder().withSources(new ConfigSource() {
