...
import static io.quarkus.deployment.ExtensionLoaderConfig.RuntimeConfigAtDeployment.warn;
                ...
                for (Parameter parameter : methodParameters) {
                    ...
                    if (rawTypeExtends(parameterType, SimpleBuildItem.class)) {
                        ...
                        if (phase.isAvailableAtBuild()) {
                            ...
                            if (isRecorder) {
                                if (extensionLoaderConfig.runtimeConfigAtDeployment().equals(warn)) {
                                    methodParamFns.add((bc, bri) -> {
                                            ...
                                            if (annotation.phase().isReadAtMain() && !isRuntimeValue) {
                                                if (extensionLoaderConfig.runtimeConfigAtDeployment().equals(warn)) {
                                                    loadLog.warn(reportError(parameter, annotation.phase() + " configuration "
                                                            + type.getTypeName()
                                                            + " should be injected in a @Recorder constructor as a Runtime<"
                                                            + type.getTypeName() + ">").getMessage());
                                                } else {
                                                    throw reportError(parameter, annotation.phase() + " configuration "
                                                            + type.getTypeName()
                                                            + " can only be injected in a @Recorder constructor as a Runtime<"
                                                            + type.getTypeName() + ">");
                                                }
                                            }
                                        }
