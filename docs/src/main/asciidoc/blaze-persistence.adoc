...
We recommend that you go right to the completed example and follow along for further explainations.

Clone the Git repository: `git clone {quickstarts-clone-url}`, or download an {quickstarts-archive-url}[archive].

The solution is located in the `blazebit-quickstart` {quickstarts-tree-url}/blazebit-quickstart[directory].
...
* further Blaze-Persistence integrations as needed:
  ...
  - `blaze-persistence-integration-jaxrs-jackson` with `runtime` scope for both
...
<!-- Optional: Used only when a specific integration is needed -->
<dependency>
    <groupId>com.blazebit</groupId>
