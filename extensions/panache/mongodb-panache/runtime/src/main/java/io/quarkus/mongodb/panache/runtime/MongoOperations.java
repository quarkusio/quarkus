...
import io.quarkus.mongodb.panache.transaction.interceptor.MongoTransactionalInterceptor;
...
public class MongoOperations {
    ...
    static ClientSession getSession() {
        ...
        if (instance.isAvailable()) {
            ...
            if (registry.getTransactionStatus() == Status.STATUS_ACTIVE) {
                return (ClientSession) registry.getResource(MongoTransactionalInterceptor.SESSION_KEY);
            }
        }
        return null;
    }
    ...
    private static long executeCount(Class<?> entityClass, Bson query) {
        MongoCollection collection = mongoCollection(entityClass);
        ClientSession session = getSession();
        if (session == null) {
    ...
    private static long executeDelete(Class<?> entityClass, Bson query) {
        MongoCollection collection = mongoCollection(entityClass);
        ClientSession session = getSession();
        if (session == null) {
