...
<<Obtain a registration options>>), which will then be returned to the client which
...
<<Obtain a login options>>), which will then be returned to the client which
...
=== Obtain a registration options

`GET /q/webauthn/register-options?username=<username>&displayName=<displayName>`
...
WebAuthn uses a challenge-response mechanism to protect from replay attacks. Therefore, when invoking the WebAuthn JavaScript API, it is necessary to retrieve a challenge from the server.
This `register-options` endpoint provides a JSON representation of the data that can be used as parameters when calling the WebAuthn JavaScript API for credential registration. It also issues a challenge cookie will be used by the registration step later.
...
To call `/register`, you must send a JSON representation of the return value from WebAuthn JavaScript API `navigator.credentials.create`. The parameters required to call `navigator.credentials.create` can be obtained from the `/register-options` endpoint, and since WebAuthn uses a challenge-response mechanism, it is mandatory to call `/register-options` before `/register` to retrieve the challenge contained in the options.

This uses the challenge cookie set by the registration options endpoint and clears it. It
...
=== Obtain a login options

`GET /q/webauthn/login-options?username=<username>`
...
WebAuthn uses a challenge-response mechanism to protect from replay attacks. Therefore, when invoking the WebAuthn JavaScript API, it is necessary to retrieve a challenge from the server.
This `login-options` endpoint provides a JSON representation of the data that can be used as parameters when calling the WebAuthn JavaScript API for login. It also issues a challenge cookie will be used by the login step later.
...
To call `/login`, you must send a JSON representation of the return value from WebAuthn JavaScript API `navigator.credentials.get`. The parameters required to call `navigator.credentials.get` can be obtained from the `/login-options` endpoint, and since WebAuthn uses a challenge-response mechanism, it is mandatory to call `/login-options` before `/login` to retrieve the challenge contained in the options.

This uses the challenge cookie set by the login options endpoint and clears it. It
...
<script type="text/javascript">
  ...
  const webAuthn = new WebAuthn({
    registerOptionsChallengePath: '/q/webauthn/register-options',
    loginOptionsChallengePath: '/q/webauthn/login-options',
...
In most cases you can keep using the `/q/webauthn/login-options` and `/q/webauthn/register-options` challenge-initiating
