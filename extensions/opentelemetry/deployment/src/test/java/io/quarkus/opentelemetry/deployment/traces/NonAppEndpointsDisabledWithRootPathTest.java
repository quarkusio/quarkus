...
public class NonAppEndpointsDisabledWithRootPathTest {
    ...
    void testHealthEndpointNotTraced() {
        RestAssured.when().get("/quarkus/health").then().statusCode(200).body(containsString("\"status\": \"UP\""));

        RestAssured.when().get("/quarkus/health/live").then().statusCode(200)
                .body(containsString("\"status\": \"UP\""));

        RestAssured.when().get("/quarkus/health/ready").then().statusCode(200)
                .body(containsString("\"status\": \"UP\""));
