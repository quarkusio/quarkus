...
/**
 * The Oracle JDBC driver includes a {@literal META-INF/native-image} which enables a set of global flags we need to
 * control better, so to ensure such flags do not interfere with requirements of other libraries.
 * <p>
 * For this reason, the {@literal META-INF/native-image/native-image.properties} resource is excluded explicitly; then
 * we re-implement the equivalent directives using Quarkus build items.
 * <p>
 * Other resources such as {@literal jni-config.json} and {@literal resource-config.json} are not excluded, so to ensure
 * we match the recommendations from the Oracle JDBC engineering team and make it easier to pick up improvements in
 * these when the driver gets updated.
 * <p>
 * Regarding {@literal reflect-config.json}, we also prefer excluding it for the time being even though it's strictly
 * not necessary: the reason is that the previous driver version had a build-breaking mistake; this was fixed in version
 * 21.3 so should no longer be necessary, but the previous driver had been tested more widely and would require it, so
 * this would facilitate the option to revert to the older version in case of problems.
 */
...
public final class OracleMetadataOverrides {
    ...
    void build(BuildProducer<ReflectiveClassBuildItem> reflectiveClass) {
        ...
        reflectiveClass.produce(
                ReflectiveClassBuildItem.builder("oracle.jdbc.internal.ACProxyable").constructors().methods().build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.jdbc.driver.T4CDriverExtension").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.jdbc.driver.T2CDriverExtension").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.jdbc.driver.ShardingDriverExtension").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.net.ano.Ano").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.net.ano.AuthenticationService").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.net.ano.DataIntegrityService").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.net.ano.EncryptionService").build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.net.ano.SupervisorService").build());
        ...
        // .build());
        reflectiveClass
                .produce(ReflectiveClassBuildItem.builder("oracle.sql.TypeDescriptor").constructors().fields().build());
        reflectiveClass
                .produce(ReflectiveClassBuildItem.builder("oracle.sql.TypeDescriptorFactory").constructors().build());
        reflectiveClass.produce(ReflectiveClassBuildItem.builder("oracle.sql.AnyDataFactory").constructors().build());
    }
    ...
    void runtimeInitializeDriver(BuildProducer<RuntimeInitializedClassBuildItem> runtimeInitialized,
        ...
        // Override: the original metadata marks the drivers as "runtime initialized" but this makes it incompatible
        // with
        ...
        // B) It does actually not compile if you have other JDBC drivers, as other implementations need this class
        ...
        runtimeInitialized
                .produce(new RuntimeInitializedClassBuildItem("oracle.jdbc.datasource.impl.OracleDataSource"));
        ...
        runtimeReinitialized.produce(
                new RuntimeReinitializedClassBuildItem("oracle.jdbc.driver.BlockSource$ThreadedCachingBlockSource"));
        ...
        runtimeInitialized
                .produce(new RuntimeInitializedClassBuildItem("oracle.jdbc.xa.client.OracleXAHeteroConnection"));
        ...
        // the need was spotted by running the native build: GraalVM will complain about these types having initialized
        // fields
        ...
        runtimeInitialized
                .produce(new RuntimeInitializedClassBuildItem("oracle.jdbc.replay.driver.FailoverManagerImpl"));
        runtimeInitialized.produce(new RuntimeInitializedClassBuildItem("oracle.jdbc.diagnostics.AbstractDiagnosable"));
        runtimeInitialized
                .produce(new RuntimeInitializedClassBuildItem("oracle.jdbc.driver.AbstractTrueCacheConnectionPools"));
        runtimeInitialized.produce(new RuntimeInitializedClassBuildItem("oracle.jdbc.diagnostics.CommonDiagnosable"));
        runtimeInitialized
                .produce(new RuntimeInitializedClassBuildItem("oracle.jdbc.replay.driver.TxnFailoverManagerImpl"));
        runtimeInitialized
    ...
    void excludeOracleDirectives(BuildProducer<ExcludeConfigBuildItem> nativeImageExclusions) {
        ...
        nativeImageExclusions
                .produce(new ExcludeConfigBuildItem(DRIVER_JAR_MATCH_REGEX, NATIVE_IMAGE_RESOURCE_MATCH_REGEX));
        nativeImageExclusions
