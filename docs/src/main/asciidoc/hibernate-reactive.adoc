...
[WARNING]
====
There are some limitations with using different pipelines with `@Transactional`, refer to <<transactional-different-pipelines>>
====
...
Reactive transactions are local only and do not support XA transactions. Only a single datasource can participate in a transaction.
