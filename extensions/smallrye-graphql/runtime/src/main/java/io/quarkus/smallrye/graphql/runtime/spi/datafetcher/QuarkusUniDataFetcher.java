...
public class QuarkusUniDataFetcher<K, T> extends UniDataFetcher<K, T> {
        ...
        try (CleanAutoCloseable ac = SmallRyeThreadContext.withThreadContext(threadContext)) {
            vc.executeBlocking(future -> {
                ...
                try {
                    ...
                    Object objectFromMethodCall = uniFromMethodCall.subscribeAsCompletionStage().get();
        ...
        try (CleanAutoCloseable ac = SmallRyeThreadContext.withThreadContext(threadContext)) {
            vc.executeBlocking(future -> {
                ...
                try {
                    ...
                    List<T> objectFromMethodCall = uniFromMethodCall.subscribeAsCompletionStage().get();
                    future.complete(objectFromMethodCall);
                } catch (Exception ex) {
                    future.fail(ex);
                } finally {
