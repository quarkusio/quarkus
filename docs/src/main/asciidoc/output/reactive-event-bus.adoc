...
{product-name-first} allows different beans to interact using asynchronous events, thus promoting loose-coupling.
...
If you have an already created project, the `vertx` extension can be added to an existing {product-name} project with
...
Alternatively, you can use the default {product-name} worker pool using:
...
Now, open your browser to http://localhost:8080/async/{product-name}, and you should see:
