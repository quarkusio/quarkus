...
The project is located in the `rest-client-quickstart` link:{quickstarts-tree-url}/rest-client-quickstart[directory].
...
The important part is the following option that was automatically added by {product-name-first}:
...
{product-name} has an option to disable the SSL support entirely.
...
Creating a native executable using build time certificates essentially means that the root certificates are fixed at image build time, based on the certificate configuration used at build time (which for {product-name} means when you perform a build having `quarkus.package.type=native` set).
...
Using the runtime certificate configuration, supported by GraalVM since 21.3 does not require any special or additional configuration compared to regular java programs or {product-name} in jvm mode. See the https://www.graalvm.org/{graalvm-version}/reference-manual/native-image/dynamic-features/CertificateManagement/#runtime-options[GraalVM documentation] for more information.
