...
Additionally, a warning will be issued to alert you of the problem. For example, if you define both
...
The full list of the Kubernetes configuration options can be found in <<#configuration-reference>>.
...
# Deprecated env-vars, use env.vars instead
...
TIP: Remember that it's possible to quote property names when they contain `.`.
Indeed, without quoting, Quarkus wouldn't be able to tell where your property ends when parsing the property's "path".
So if you need to reference a
`greeting.message` property in one of the configuration files, you can use the following syntax:
`quarkus.whatever."greeting.message".something=Hello`.

include::{generated-dir}/config/quarkus-kubernetes.adoc[opts=optional,leveloffset=+1]
