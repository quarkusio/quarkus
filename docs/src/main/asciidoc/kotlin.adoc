...
NB: For Gradle project setup please see below
...
* All open Kotlin plugin tells the compiler not to mark as final, those classes with the annotations highlighted (customize as required)
...
<5> All open configuration required, as per Maven guide above
...
Alternatively, prefer the use of the compile-time injection with ArC which works without modification of the Java examples.
...
As usual, the application can be packaged using `./mvnw clean package` and executed using the `-runner.jar` file. You can also build the native executable using `./mvnw package -Pnative`, or `./gradlew buildNative` for the platform-local binary.  