...
IMPORTANT: If you have pagination enabled, using JOIN FETCH or others might prevent the query (`namedQuery`) from being reused for the count query. To resolve this you can add an additional query by adding a comma in front of it like this `http://localhost:8080/people?namedQuery=Person.containsInName,Person.countContainsInName&name=ter` which will be used for the pagination count.
