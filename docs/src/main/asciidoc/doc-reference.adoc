...
[source,asciidoc]
----
[id="security-oidc-bearer-token-authentication"] // <1>
= OpenID Connect (OIDC) Bearer token authentication // <2>
\include::_attributes.adoc[] // <3>
:diataxis-type: {type} // <4>
:categories: security // <5>
:subcategories: oidc // <6>
:nav-title: OIDC bearer authentication // <7>
----
...
<6> Optional: specify one or more <<subcategories>> (comma separated) to group the guide within a category.
<7> Optional: a short sidebar title (40 characters or fewer). Required only when the document title exceeds 40 characters. When omitted, the document title is used automatically.

[[doc-header-optional]]
==== Other common document header attributes

`:nav-title: <short title>`:: A short sidebar title (40 characters or fewer). Required only when the document title exceeds 40 characters. When omitted, the document title is used automatically.
`:subcategories: <ids>`:: Optional comma-separated list of subcategory IDs. Groups the guide under a subcategory heading within its parent category. Valid values are defined in `navigation-config.yaml`.
...
Quarkus documentation is grouped into categories defined in `docs/src/main/asciidoc/navigation-config.yaml`.
The configuration file is the source of truth for valid category IDs, display titles, and display order on the landing page and sidebar.

Tag your content to improve findability by adding at least one category to the categories attribute line in the <<document-header,document header>>.
Valid category IDs are listed in `navigation-config.yaml`.
To add multiple categories, use comma-separated values. For example:
...
To add a new category, add an entry to `navigation-config.yaml` and update the relevant guides.

[[subcategories]]
=== Subcategories

Categories can have subcategories that group related guides under a heading.
Valid subcategories and their parent categories are defined in `navigation-config.yaml`.

To assign a guide to a subcategory, add the `:subcategories:` attribute to the document header:
...
The build uses a placement rule to determine where a guide appears based on the relationship between its categories and subcategories.
For details, see `docs/design/author-driven-navigation.md`.
