...
public class GZipResourceTest {
    ...
    @RegisterExtension
    static QuarkusUnitTest test = new QuarkusUnitTest().withApplicationRoot((jar) -> jar.addClasses(GZipResource.class))
            .overrideConfigKey("quarkus.resteasy.gzip.enabled", "true");

    @Test
    public void testGzipEndpoint() {
        RestAssured.given().get("/gzip").then().statusCode(200).header("content-encoding", "gzip")
