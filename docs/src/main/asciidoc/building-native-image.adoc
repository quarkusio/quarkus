...
[TIP]
====
If you prefer to install GraalVM locally or need to target specific platforms, see the <<local-graalvm-setup,Local GraalVM Setup>> section later in this guide.
====
...
The executable built this way will be a 64-bit Linux executable, so depending on your operating system it may not be runnable locally.
However, it's perfect for containerized deployments.

[IMPORTANT]
====
Starting with Quarkus 3.19+, the _builder_ image used to build the native executable is based on UBI 9.
If you plan to build a container image, make sure that the base image in your `Dockerfile` is compatible with UBI 9.
...
If you prefer to install GraalVM locally or need to target specific platforms other than Linux, you can set up GraalVM on your development machine.

=== Prerequisites for Local Setup

In addition to the basic prerequisites, you'll need:
