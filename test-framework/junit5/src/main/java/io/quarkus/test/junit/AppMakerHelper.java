...
public class AppMakerHelper {
    ...
    public static ApplicationModel getGradleAppModelForIDE(Path projectRoot)
    ...
    static PrepareResult prepare(final Class<?> requiredTestClass, CuratedApplication curatedApplication,
        ...
        return new PrepareResult(curatedApplication.createAugmentor(TestBuildChainFunction.class.getName(), props),
                getQuarkusTestProfile(profile), curatedApplication);
    }
    ...
    /**
     * Reads properties from a profile, sets them as system properties, and returns a Runnable which can be invoked to
     * remove them back off of system properties.
     */
    static Runnable setExtraPropertiesRestorably(Class<?> profileClass, QuarkusTestProfile profileInstance) {
        ...
        if (!profileInstance.getEnabledAlternatives().isEmpty()) {
            additional.put("quarkus.arc.selected-alternatives",
                    profileInstance.getEnabledAlternatives().stream().peek((c) -> {
                        try {
        ...
        // TODO investigate whether we can use the config from https://github.com/quarkusio/quarkus/pull/42715 to avoid
        // system properties
        // ... but be aware that this is called twice, and on the first pass through, the classloader might be an
        // all-purpose runtime classloader, and would not be the actual test classloader
    ...
    public static CuratedApplication makeCuratedApplication(Class<?> requiredTestClass, String displayName,
        ...
        if (gradleAppModel != null && gradleAppModel.getApplicationModule() != null) {
            ...
            final String testClassFileName = ClassLoaderHelper.fromClassNameToResourceName(requiredTestClass.getName());
            ...
            testAppModel = BootstrapAppModelFactory.newInstance().setTest(true).setEnableClasspathCache(true)
                    .setProjectRoot(projectRoot).resolveAppModel().getApplicationModel();
        ...
        CuratedApplication curatedApplication = QuarkusBootstrap.builder()
                // .setExistingModel(gradleAppModel) unfortunately this model is not re-usable due to PathTree
                // serialization by Gradle
                .setExistingModel(testAppModel).setBaseName(displayName + " (QuarkusTest)").setIsolateDeployment(true)
                .setMode(QuarkusBootstrap.Mode.TEST).setTest(true).setAuxiliaryApplication(isContinuousTesting)
                .setTargetDirectory(PathTestHelper.getProjectBuildDir(projectRoot, testClassLocation))
                .setProjectRoot(projectRoot).setApplicationRoot(rootBuilder.build()).build().bootstrap();

        if (!curatedApplication.getApplicationModel().getDependencies(DependencyFlags.RUNTIME_CP).iterator()
                .hasNext()) {
            throw new RuntimeException(
    ...
    public static StartupAction getStartupAction(Class<?> testClass, CuratedApplication curatedApplication,
        ...
        try {
            // To check changes here run integration-tests/elytron-resteasy-reactive and SharedProfileTestCase in
            // integration-tests/main
            return prepareResult.augmentAction().createInitialRuntimeApplication();
        } catch (Exception e) {
            // Errors at this point just get reported as org.junit.platform.commons.JUnitException: TestEngine with ID
            // 'junit-jupiter' failed to discover tests
            // Even though a stack trace isn't ideal handling, we want to make sure people have something to try and
            // debug if problems happen
            e.printStackTrace();
            throw e;
