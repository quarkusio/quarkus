...
When a persistence unit is configured at build time, and its datasource is active, the persistence unit is active by default.
Quarkus starts the corresponding Hibernate ORM `SessionFactory` when the application starts.

To deactivate a persistence unit at runtime, either:

* Do not set `quarkus.datasource[.optional name].jdbc.url` or `quarkus.datasource[.optional name].reactive.url` for the corresponding datasource.
* Set `quarkus.datasource[.optional name].active` to `false` for the corresponding datasource.
* Set `quarkus.hibernate-orm[.optional name].active` to `false`.
...
xref:config-reference.adoc#configuration-sources[Setting] `quarkus.datasource."pg".active=true` at runtime
