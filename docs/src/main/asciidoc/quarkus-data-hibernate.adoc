...
Pass `null` or an empty `Order.by()` to leave the query unsorted.
...
`Order.by()` for `Order<Cat>`, which matches the usual "no sort requested" case without null
checks. Both types are accepted by `PanacheQuery.sort()` — pass `null` or an empty
`Order.by()` to leave the query unsorted:
...
public List<Cat> list(Order<Cat> order) {
    return Cat_.managedBlocking().findAll().sort(order).list();
}
----
...
list of cats. Define a `@Find` method on your repository that accepts `PageRequest` and `Order`:
