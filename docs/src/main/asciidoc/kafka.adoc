...
If you are using the `quarkus-smallrye-health` extension, `quarkus-kafka` could automatically add a readiness health check
...
This behavior can be enabled via the property `quarkus.kafka.health.enabled`.
