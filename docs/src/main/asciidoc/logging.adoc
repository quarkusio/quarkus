...
Setting the minimum log level sets the floor level for which Quarkus generates supporting code.
Quarkus does not generate supporting code for lower levels.

Set the minimum log level at build time to enable further build-time optimizations, such as dead-code elimination when building a native image.

.An example from a native executable:

Setting `INFO` as the minimum log level forces checks for lower levels, such as `isTraceEnabled()`, to always return `false`.
