...
public class RedirectTest {
    ...
    static final QuarkusUnitTest TEST = new QuarkusUnitTest().withApplicationRoot(
            (jar) -> jar.addClasses(RedirectingResourceClient302.class, RedirectingResourceClient307.class,
    ...
    void shouldRedirect3Times_whenMax4() {
        RedirectingResourceClient302 client = RestClientBuilder.newBuilder().baseUri(uri).followRedirects(true)
                .property(QuarkusRestClientProperties.MAX_REDIRECTS, 4).build(RedirectingResourceClient302.class);
    ...
    void shouldNotRedirect3Times_whenMax2() {
        RedirectingResourceClient302 client = RestClientBuilder.newBuilder().baseUri(uri).followRedirects(true)
                .property(QuarkusRestClientProperties.MAX_REDIRECTS, 2).build(RedirectingResourceClient302.class);
        assertThat(client.call(3).getStatus()).isEqualTo(302);
    }

    @Test
    void shouldNotRedirectOnPostMethodsByDefault() {
        RedirectingResourceClient302 client302 = RestClientBuilder.newBuilder().baseUri(uri)
                // this property should be ignored in POST
                .followRedirects(true).build(RedirectingResourceClient302.class);
        assertThat(client302.post().getStatus()).isEqualTo(302);

        RedirectingResourceClient307 client307 = RestClientBuilder.newBuilder().baseUri(uri)
                // this property should be ignored in POST
                .followRedirects(true).build(RedirectingResourceClient307.class);
    ...
    void shouldRedirectWhenUsingCustomRedirectHandlerOnPostMethods() {
        RedirectingResourceClient302 client302 = RestClientBuilder.newBuilder().baseUri(uri)
                ...
                .register(EnablePostRedirectHandler.class).build(RedirectingResourceClient302.class);
        ...
        RedirectingResourceClient307 client307 = RestClientBuilder.newBuilder().baseUri(uri)
                ...
                .followRedirects(true)
                // use custom redirect to enable redirection
                .register(EnablePostRedirectHandler.class).build(RedirectingResourceClient307.class);
    ...
    void shouldRedirectWhenRegisterProviderUsingCustomRedirectHandlerOnPostMethods() {
        RedirectingResourceClient302 client = RestClientBuilder.newBuilder().baseUri(uri)
                // this property should be ignored in POST
                .followRedirects(true).build(RedirectingResourceWithRegisterProviderRedirectHandlerClient.class);
    ...
    void shouldRedirectWhenAnnotatedClientRedirectHandlerOnPostMethods() {
        RedirectingResourceClient302 client = RestClientBuilder.newBuilder().baseUri(uri)
                // this property should be ignored in POST
                .followRedirects(true).build(RedirectingResourceWithRedirectHandlerAnnotationClient.class);
        assertThat(client.post().getStatus()).isEqualTo(200);
    }
    ...
    void shouldNotRedirectWhenARedirectHandlerWithMorePriorityIsUsed() {
        RedirectingResourceClient302 client = RestClientBuilder.newBuilder().baseUri(uri)
                // this property should be ignored in POST
                .followRedirects(true).build(RedirectingResourceWithSeveralRedirectHandlerAnnotationsClient.class);
