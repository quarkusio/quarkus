...
Imagine for instance that an application contains a bean named `Tracer`, which needs to be do nothing when in tests or dev-mode, but work in its normal capacity for the production artifact.
...
If instead, it is required that the `Tracer` bean also work in dev-mode and only default to doing nothing for tests, then `@UnlessBuildProfile` would be ideal. The code would look like:
...
public class TracerConfiguration {
    ...
    @Produces
    @UnlessBuildProfile("prod") // this will be enabled for both test and dev build time profiles
    public Tracer noopTracer() {
        return new NoopTracer();
    }
}
