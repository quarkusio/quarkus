...
The RabbitMQ connector allows {product-name-first} applications to send and receive messages using the AMQP 0.9.1 protocol.
...
{product-name} and the RabbitMQ connector support https://www.vaultproject.io/docs/secrets/rabbitmq[Vault's RabbitMQ secrets engine]
...
Then we need to give a read capability to the {product-name} application on path `rabbitmq/creds/my-role`.
[source,bash]
----
...
Now that Vault knows how to create users in RabbitMQ, we need to configure {product-name} to use a credentials-provider
for RabbitMQ.

First we tell {product-name} to request dynamic credentials using a credentials-provider named `rabbitmq`.
----
