...
If you need to configure a deadline for the calls made through a gRPC stub, i.e. a duration of time after which a call that has not completed fails with the status error `DEADLINE_EXCEEDED`,
you can specify it via the `quarkus.grpc.clients."service-name".deadline` configuration property, e.g.:
...
----
<1> Set the deadline of every call made through the injected stubs and clients of this service.

The deadline is computed when each call starts, and it is propagated to the server.
A call that sets its own deadline, for example with `stub.withDeadlineAfter(...)`, keeps that deadline.
