...
A single-page application (SPA) typically uses an authorization code flow to authenticate users and acquire ID and access tokens.
It sends access tokens as bearer tokens to access Quarkus `service` applications.

SPA can support an authorization code flow by using OIDC provider scripts or implement it directly.
If SPA chooses to implement an authorization code flow directly then it can use an experimental OIDC proxy feature.

Both options are described below.

[[single-page-application-scripts]]
==== Use OIDC provider scripts

Using OIDC provider scripts is an easiest option to support an authorization code flow but SPA can only work with a single OIDC provider in this case.

For example, if you work with Keycloak, you can use `keycloak.js` to authenticate users with an authorization code flow and refresh the expired tokens from the SPA:
...
[[single-page-application-oidc-proxy]]
==== Use OIDC proxy

Instead of tying your SPA to a specific OIDC provider by depending on this OIDC provider's script, you may want to consider implementing an authorization code flow directly in the SPA.

SPA redirects users to a configurable OIDC authorization endpoint, accepts the OIDC provider's redirect and acquires tokens by sending the authorization code to a standard OIDC token endpoint.

Quarkus introduces an experimental `quarkus-oidc-proxy` OIDC proxy extension which provides an authorization code flow support for Quarkus OIDC `service` applications. It can help to encapsule all the OIDC provider specific configuration details.

To use OIDC proxy, all you need to do is to add the `quarkus-oidc-proxy` dependency to your Quarkus OIDC `service` application:
...
.build.gradle
----
implementation("io.quarkus:quarkus-oidc-proxy")
----

Now, the SPA can use the OIDC proxy endpoints to authenticate users with an authorization code flow and send the acquired access tokens to the Quarkus OIDC `service` application.
