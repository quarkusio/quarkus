...
public interface PackageConfig {
    ...
    /**
     * The entry point of the application. This can either be a fully qualified name of a standard Java class with a
     * main method, or {@link io.quarkus.runtime.QuarkusApplication}.
     * <p>
     * If your application has main classes annotated with {@link io.quarkus.runtime.annotations.QuarkusMain} then this
     * can also reference the name given in the annotation, to avoid the need to specify fully qualified names in the
     * config.
     */
    Optional<String> mainClass();

    /**
     * The directory into which the output package(s) should be written. Relative paths are resolved from the build
    ...
    /**
     * Setting this switch to {@code true} will cause Quarkus to write the transformed application bytecode to the build
     * tool's output directory. This is useful for post-build tools that need to scan the application bytecode (for
     * example, offline code-coverage tools).
     * <p>
     ...
     * <p>
     * Setting this to {@code true}, however, should be done with a lot of caution and only if subsequent builds are
     * done in a clean environment (i.e. the build tool's output directory has been completely cleaned).
     */
    @WithDefault("false")
    ...
    interface JarConfig {
        ...
        /**
         * List of all the dependencies that have been defined as optional to include into the final package of the
         * application. Each optional dependency needs to be expressed in the following format:
         * <p>
         ...
         * not a part of the syntax specification). The group ID and artifact ID must be present and non-empty.
         * <p>
         ...
         * <p>
         * This parameter is meant to be used in modules where multi-builds have been configured to avoid getting a
         * final package with unused dependencies.
         */
         ...
         * {@linkplain JarType#UBER_JAR uber-JAR output type}. If disabled, the JAR built by the original build system
         * (Maven, Gradle, etc.) will be replaced with the Quarkus-built uber-JAR.
         */
        @WithDefault("true")
        ...
        interface AppcdsConfig {
            /**
             * Whether to automate the creation of AppCDS. Care must be taken to use the same exact JVM version when
             ...
             * on its PATH. This flag is useful when the JVM to be used at runtime is not the same exact JVM version as
             * the one used to build the jar. Note that this property is consulted only when
             * {@code quarkus.package.jar.appcds.enabled=true} and it requires having docker available during the build.
             */
             ...
             * <p>
             * Normally, if either a suitable container image to use to create the AppCDS archive can be determined
             * automatically or if one is explicitly set using the {@code quarkus.<package-type>.appcds.builder-image}
             * setting, the AppCDS archive is generated by running the JDK contained in the image as a container.
             * <p>
             * If this option is set to {@code false}, a container will not be used to generate the AppCDS archive.
             * Instead, the JDK used to build the application is also used to create the archive. Note that the exact
             * same JDK version must be used to run the application in this case.
             * <p>
            ...
            /**
             * Whether to use <a href="https://openjdk.org/jeps/483">Ahead-of-Time Class Loading & Linking</a>
             * introduced in JDK 24.
             */
            @WithDefault("false")
            boolean useAot();
        }
         ...
         * <p>
         * If this is specified a directory of this name will be created in the jar distribution. Users can place jar
         * files in this directory, and when re-augmentation is performed these will be processed and added to the
         * class-path.
         * <p>
         * Note that before reaugmentation has been performed these jars will be ignored, and if they are updated the
         * app should be reaugmented again.
         */
        Optional<String> userProvidersDirectory();

        /**
         * If this option is true then a list of all the coordinates of the artifacts that made up this image will be
         * included in the quarkus-app directory. This list can be used by vulnerability scanners to determine if your
         * application has any vulnerable dependencies. Only supported for the {@linkplain JarType#FAST_JAR fast JAR}
         * and {@linkplain JarType#MUTABLE_JAR mutable JAR} output types.
         */
        @WithDefault("true")
        ...
        interface ManifestConfig {
            ...
            /**
             * Custom manifest attributes to be added to the main section of the MANIFEST.MF file. An example of the
             * user defined property: quarkus.package.jar.manifest.attributes."Entry-key1"=Value1
            ...
            /**
             * Custom manifest sections to be added to the MANIFEST.MF file. An example of the user defined property:
        ...
        enum JarType {
            ...
            /**
             * The "legacy JAR" packaging type. This corresponds to the packaging type used in Quarkus before version
             * 1.12.
             *
            ...
            /**
             * {@return the names for this output type} The first name is the "canonical" name.
             */
             ...
             *         if the string does not correspond to a valid JAR type
             */
