...
NOTE: To avoid your test hanging, you must add to your `src/test/resources/application.properties` file the following: `quarkus.lambda.enable-polling-jvm-mode=true`
...
2. `libsunec.o` must be added to `function.zip`
3. `cacerts` must be added to `function.zip`

To do this, first create a directory `src/main/zip.native/ssl` with your build.  Next create a shell script file called `bootstrap`
...
Next you must copy some files from your Graal distribution into `src/main/zip.native/ssl`.

[source, shell]
----
cp $GRAALVM_HOME/lib/libsunec.o $PROJECT_DIR/src/main/zip.native/ssl
cp $GRAALVM_HOME/lib/security/cacerts $PROJECT_DIR/src/main/zip.native/ssl
----
...
[source]
----
docker cp {container-id-from-above}:/opt/graalvm/jre/lib/amd64/libsunec.so src/main/zip.native/ssl
----

Second, cacerts, the certificate store.  You may need to periodically obtain an updated copy, also.
[source]
----
docker cp {container-id-from-above}:/opt/graalvm/jre/lib/security/cacerts src/main/zip.native/ssl
----
