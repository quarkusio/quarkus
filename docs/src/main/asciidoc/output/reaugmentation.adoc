...
{product-name-first} application configuration may include two types of configuration options: 
...
In order to run the augmentation steps you need the deployment JARs of the used {product-name} extensions.
...
In order to re-augment your {product-name} application with different build time properties, start the application with the desired configuration plus the `quarkus.launch.rebuild` system property set to `true`.
...
The command line above will not launch the application. {product-name} will exit immediately after the application has been re-augmented.
