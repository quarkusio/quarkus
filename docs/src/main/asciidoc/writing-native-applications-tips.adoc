...
If you cannot modify the class (e.g. it is in a 3rd party jar) then you can instead specify the classes
that you actually want to register in the `targets` member of the annotation:
...
setting `registerDependencies` to `true`, in which case Quarkus will attempt to discover all dependencies
and also register them. This should be used with caution though, as it may result in a lot of other classes
being registered.
...
=== Delaying and re-running class initialization
...
to indicate that a class should be initialized at runtime. If the class cannot be modified (e.g. it is part of a 3rd party jar)
then you can place this annotation on a different class in your application, and use the `targets` member to indicate the
actual classes that should be runtime initialized.
...
You can solve this issue by using the `@io.quarkus.runtime.annotations.RegisterProxy` annotation, which takes a list of
interfaces. Using this annotation you can specify the interface combinations that you want to be able to create dynamic
proxies for. Note that order is important, so the order of the annotation values must match the order that the interfaces
