...
/**
 * Configuration for a key store with an arbitrary type, used for key store formats not directly supported
 * by the TLS registry (such as BCFKS or PKCS#11).
 * <p>
 * The loading strategy depends on whether a {@code KeyStoreFactory} CDI bean annotated with
 * {@code @Identifier("<type>")} exists:
 * <ul>
 * <li><strong>Factory:</strong> If a {@code KeyStoreFactory} bean matching the {@link #type()} is found,
 ...
 * In this case, {@link #path()} is required. This works for any file-backed key store type supported by the JVM
 * or a registered security provider (e.g., BCFKS).</li>
 * </ul>
 */
@ConfigGroup
public interface OtherKeyStoreConfig extends OtherKeyStoreConfiguration {
     ...
     * When not set, the password can be retrieved from the credential provider configured on the parent key store.
     */
     ...
     * <p>
     * Used by the standard fallback to select and validate a specific entry.
     * A factory may also read this value but is not required to.
     */
     ...
     * <p>
     * Used by the standard fallback to verify the private key is recoverable.
     * When not set, the password can be retrieved from the credential provider configured on the parent key store.
     */
     ...
     * <p>
     * Ignored by the standard fallback. Set via {@code quarkus.tls.key-store.other.params.<key>=<value>}.
     */
