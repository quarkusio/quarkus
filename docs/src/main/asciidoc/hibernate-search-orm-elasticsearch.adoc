...
Let's create a `src/main/resources/data.sql` file with the following content;
Hibernate ORM executes it automatically in dev and test modes, and in production once we ask for it in the configuration below
(see xref:hibernate-orm.adoc#sql-init-scripts[the corresponding section of the Hibernate ORM guide]):
...
quarkus.hibernate-search-orm.indexing.plan.synchronization.strategy=sync <3>
...
%prod.quarkus.hibernate-orm.data-management.strategy=create
%prod.quarkus.hibernate-search-orm.elasticsearch.hosts=localhost:9200 <4>
----
<1> Let's create a PostgreSQL datasource.
The initial data (see <<automatic-import-script>>) is loaded on startup in dev and test modes by default;
in production, we ask for it explicitly, along with the creation of the schema by Hibernate ORM.
