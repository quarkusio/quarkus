...
This guide explains how to benefit from Java 19+ virtual threads in {product-name-first} application.
...
In {product-name}, the support of virtual thread is implemented using the link:{runonvthread}[@RunOnVirtualThread] annotation.
...
Thus, {product-name} uses an explicit model until the aforementioned issues disappear (as the Java ecosystem matures).
...
It is essential to understand that these issues are not {product-name} limitations or bugs but are due to the current state of the Java ecosystem which needs to evolve to become virtual thread friendly.

NOTE: To learn more about the internal design and choices, check the https://dl.acm.org/doi/10.1145/3583678.3596895[Considerations for integrating virtual threads in a Java framework: a {product-name} example in a resource-constrained environment] paper.
...
It also explains the various development and execution models offered by {product-name}.

The `@RunOnVirtualThread` annotation instructs {product-name} to invoke the annotated method on a **new** virtual thread instead of the current one.
{product-name} handles the creation of the virtual thread and the offloading.
...
Fortunately, {product-name} provides a massive ecosystem that is ready to be used in virtual threads.
Mutiny, the reactive programming library used in {product-name}, and the Vert.x Mutiny bindings provides the ability to write blocking code (so, no fear, no learning curve) which do not pin the carrier thread.

As a result:

1. {product-name} extensions providing blocking APIs on top of reactive APIs can be used in virtual threads.
...
To containerize your {product-name} application that use `@RunOnVirtualThread`, add the following properties in your `application.properties`:
...
To compile a {product-name} applications leveraging `@RunOnVirtualThreads` into native executable, you must be sure to use a GraalVM / Mandrel `native-image` supporting virtual threads, so providing at least Java 19+.
...
To build a container running a {product-name} application using virtual threads compiled into a native executable, you must
...
{product-name} managed virtual threads are named and prefixed with `quarkus-virtual-thread-`.
...
- https://dl.acm.org/doi/10.1145/3583678.3596895[Considerations for integrating virtual threads in a Java framework: a {product-name} example in a resource-constrained environment]