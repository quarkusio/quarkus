...
public class CompressionTest {
    ...
    public void test() throws Exception {
        ...
        // in its test classes
        given().get("/compress").then().statusCode(200).header("content-encoding", is("gzip"))
                .header("content-length", Integer::parseInt, lessThan(TEXT.length())).body(equalTo(TEXT));
        ...
        given().config(RestAssured.config.decoderConfig(
                DecoderConfig.decoderConfig().with().contentDecoders(DecoderConfig.ContentDecoder.DEFLATE)))
                .get("/compress").then().statusCode(200).header("content-encoding", is("deflate"))
                .header("content-length", Integer::parseInt, lessThan(TEXT.length())).body(equalTo(TEXT));

        given().get("/nocompress").then().statusCode(200).header("content-encoding", is(nullValue()))
                .header("content-length", Integer::parseInt, equalTo(TEXT.length())).body(equalTo(TEXT));
    }
