            ...
            if (!matchAny) {
                if (patternField != null) {
                    ...
                    nested.block(namesMatch -> {
                        ObjectOps objectOps = new ObjectOps(namesMatch, name);
                        for (String match : matchNames) {
                            namesMatch.if_(objectOps.equals_(Const.of(match)), matching -> matching.break_(namesMatch));
                        }
                        namesMatch.break_(nested);
                    });

                } else {
                    nested.ifNot(new ObjectOps(nested, name).equals_(Const.of(matchName)), matching -> matching.break_(nested));
                }
            }
                ...
                Expr whenCompleteFun = nested.lambda(BiConsumer.class, lc -> {
                    ...
                    lc.body(accept -> {
                        accept.ifElse(accept.isNull(throwable), success -> {
                            ...
                            LocalVar paramTypes = success.localVar("pt", success.newEmptyArray(Class.class, evaluated.size()));
                            int idx = 0;
                            for (Param p : evaluated) {
                                success.set(paramTypes.elem(idx++), Const.of(classDescOf(p.type.name())));
                            }
                        ...
                        lc.body(accept -> {
                            accept.ifElse(accept.isNull(throwable), success -> {
                                ...
                                LocalVar paramTypes = success.localVar("pt",
                                        success.newEmptyArray(Class.class, evaluated.size()));
                                int idx = 0;
                                for (Param p : evaluated) {
                                    success.set(paramTypes.elem(idx++), Const.of(classDescOf(p.type.name())));
                                }
    ...
    private void implementAppliesTo(ClassCreator valueResolver, MethodInfo method, String matchName, List<String> matchNames,
        ...
        valueResolver.method("appliesTo", mc -> {
            ...
            mc.body(bc -> {
                ...
                Expr testClass = Const.of(classDescOf(box(params.getFirst(ParamKind.BASE).type).name()));
                ...
                if (!matchAny) {
                    if (patternField != null) {
                        ...
                        // Any of the name matches
                        bc.block(nested -> {
                            ObjectOps objectOps = new ObjectOps(nested, name);
                            for (String match : matchNames) {
                                nested.if_(objectOps.equals_(Const.of(match)), namesMatch -> namesMatch.break_(nested));
                            }
                            nested.returnFalse();
                        });

                    } else {
                        bc.ifNot(new ObjectOps(bc, name).equals_(Const.of(matchName)),
                                nameNotMatched -> nameNotMatched.returnFalse());
                    }
                }
                bc.returnTrue();
