...
:summary: This reference guide provides advanced details about the usage and the configuration of the Vert.x instance used by {product-name-first}.
...
This guide is the companion to the xref:vertx.adoc[Using Eclipse Vert.x API from a {product-name} Application] guide.
It provides more advanced details about the usage and the configuration of the Vert.x instance used by {product-name}.
...
We recommend using the Mutiny variant as it integrates with the other reactive APIs provided by {product-name}.
...
Some {product-name} extension already wraps Vert.x libraries.
...
But, first, we need to instruct {product-name} to enable _ssl_ (if the remote API uses HTTPS).
...
JSON Object and JSON Array are both supported as {product-name} HTTP endpoint requests and response bodies (using classic RESTEasy and RESTEasy Reactive).
...
http://localhost:8080/hello/{product-name}/object returns:
...
http://localhost:8080/hello/{product-name}/array returns:
...
You can deploy _verticles_ in {product-name}.
It supports:
...
However, in {product-name}, you can deploy verticles as beans.
Note that in this case, CDI (Arc in {product-name}) is responsible for creating the instance.
...
Vert.x comes with a built-in https://vertx.io/docs/vertx-core/java/#event_bus[event bus] that you can use from your {product-name} application.
...
While you can use the Vert.x API to register consumers, {product-name} comes with declarative support.
...
{product-name} provides a default codec for local delivery.
...
The SockJS bridge provided by Vert.x allows browser applications and {product-name} applications to communicate using the event bus.
...
SockJS negotiates the communication channel between the {product-name} application and the browser.
...
<head>
    <meta charset="utf-8"/>
    <title>SockJS example - {product-name}</title>
