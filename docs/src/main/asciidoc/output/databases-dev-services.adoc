...
When testing or running in dev mode {product-name-first} can provide you with a zero-config database out of the box, a feature we refer to as Dev Services.
...
Don't configure a database URL, username and password - {product-name} will provide the database and you can just start coding without worrying about config.
...
By default, {product-name} uses the default image for the current version of Testcontainers, if you attempt to start {product-name} the resulting failure will tell you the exact image name in use for you to add to the file.
...
[IMPORTANT]
====
When using Dev Services with Hibernate ORM, by default {product-name} will wipe out the database on application startup, which will wipe out the database data on your Docker host's filesystem.
...
All services based on containers are run using Testcontainers but {product-name} is not using the Testcontainers JDBC driver.
...
{product-name} can support *specific* properties sent to the container itself though, e.g. this is the case for `TC_MY_CNF` which allows to override the MariaDB/MySQL configuration file.
...
In the labels tab, we see that {product-name} added the datasource label, which can be very useful in differentiating containers when multiple
