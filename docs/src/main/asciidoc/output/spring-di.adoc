...
:summary: While you are encouraged to use CDI annotations for injection, {product-name-first} provides a compatibility layer for Spring dependency injection in the form of the spring-di extension.

While users are encouraged to use CDI annotations for injection, {product-name} provides a compatibility layer for Spring dependency injection in the form of the `spring-di` extension.

This guide explains how a {product-name} application can leverage the well known Dependency Injection annotations included in the Spring Framework.
...
If you already have your {product-name} project configured, you can add the `spring-di` extension
...
----
As a Spring developer, you might be tempted to add the `@ComponentScan` annotation in order to define specific packages to scan for additional beans. Do note that `@ComponentScan` is entirely unnecessary since {product-name} performs xref:cdi-reference.adoc#bean_discovery[bean discovery] only in `annotated` mode with no visibility boundaries. Moreover, note that the bean discovery in {product-name} happens at build time.
In the same vein, {product-name} does not support the Spring `@Import` annotation.
...
{product-name} also provides support for injecting configuration values using Spring's `@Value` annotation.
...
Please note that the Spring support in {product-name} does not start a Spring Application Context nor are any Spring infrastructure classes run.
...
Regarding the dependency injection in particular, {product-name} uses a Dependency Injection mechanism (called ArC) based on the https://jakarta.ee/specifications/cdi/2.0/cdi-spec-2.0.html[Contexts and Dependency Injection for Java 2.0] specification. If you want to learn more about it, we recommend you to read the xref:cdi.adoc[{product-name} introduction to CDI] and the xref:cdi-reference.adoc#arc-configuration-reference[CDI reference guide]
The various Spring Boot test features are not supported by {product-name}. For testing purposes, please, check the xref:getting-started-testing.adoc[{product-name} testing guide].
...
|@ComponentScan
|
|Doesn't have a one-to-one mapping to a CDI annotation. It is not used in {product-name} because {product-name} does all classpath scanning at build time.
...
{product-name} has more Spring compatibility features. See the following guides for more details:

* xref:spring-web.adoc[{product-name} - Extension for Spring Web]
* xref:spring-data-jpa.adoc[{product-name} - Extension for Spring Data JPA]
* xref:spring-data-rest.adoc[{product-name} - Extension for Spring Data REST]
* xref:spring-security.adoc[{product-name} - Extension for Spring Security]
* xref:spring-cloud-config-client.adoc[{product-name} - Reading properties from Spring Cloud Config Server]
* xref:spring-boot-properties.adoc[{product-name} - Extension for Spring Boot properties]
* xref:spring-cache.adoc[{product-name} - Extension for Spring Cache]
* xref:spring-scheduled.adoc[{product-name} - Extension for Spring Scheduled]
