...
.Select Bearer token authentication mechanism and OIDC tenant with annotation
[source, java]
----
package io.quarkus.websockets.next.test.security;
...
----
<1> Require that opening WebSocket handshake request is authenticated using Bearer token authentication.
Find how to select other authentication mechanisms with annotations in the xref:security-authentication-mechanisms.adoc#use-annotations-for-path-based-auth[Authentication mechanisms in Quarkus] guide.
<2> Use the `io.quarkus.oidc.Tenant` annotation for resolving the OIDC tenant identifiers.
For information about how to use the `io.quarkus.oidc.Tenant` annotation, see the Quarkus xref:security-openid-connect-multitenancy.adoc#annotations-tenant-resolver[Resolve OIDC tenants with annotations] section of the Using OpenID Connect Multi-Tenancy guide.

[source,properties]
----
quarkus.http.auth.proactive=false <1>
----
<1> Ensures that Quarkus matches opening WebSocket handshake request with the `io.quarkus.oidc.Tenant` and `io.quarkus.oidc.BearerTokenAuthentication` annotations before authentication begins.
