...
Other types of keystores provided by registered keystore providers are also supported. Generally it supports any type of keystore that can be loaded via `java.security.KeyStore.getInstance(type, provider)`.

For example a Bouncy Castle FIPS provided keystore where the type of the keystore is `bcfks`:
[source,properties]
----
quarkus.tls.key-store.bcfks.provider=BCFIPS
quarkus.tls.key-store.bcfks.path=server-keystore.bcfks
quarkus.tls.key-store.bcfks.password=secret
quarkus.tls.key-store.bcfks.alias=my-alias
quarkus.tls.key-store.bcfks.alias-password=my-alias-password
----

Note that all of the standard keystore properties are supported via configuration: `path`, `password`, `alias`, `alias-password`, and `provider` (where `provider` is the name of the security provider and the second argument to `java.security.KeyStore.getInstance(type, provider)`).
...
IMPORTANT: The credential provider can only be used with PKCS12, JKS, and generic provider keystores.
...
Other types of truststores provided by registered truststore providers are also supported. Generally it supports any type of truststore that can be loaded via `java.security.KeyStore.getInstance(type, provider)`.

For example a Bouncy Castle FIPS provided truststore where the type of the truststore is `bcfks`:
[source,properties]
----
quarkus.tls.trust-store.bcfks.provider=BCFIPS
quarkus.tls.trust-store.bcfks.path=client-truststore.bcfks
quarkus.tls.trust-store.bcfks.password=secret
quarkus.tls.trust-store.bcfks.alias=my-alias
----

Note that all of the standard truststore properties are supported via configuration: `path`, `password`, `alias`, `alias-password`, and `provider` (where `provider` is the name of the security provider and the second argument to `java.security.KeyStore.getInstance(type, provider)`).
