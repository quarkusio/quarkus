...
Quarkus supports the use of placeholders in the `application.properties` file on build time.
...
will result on the value of the `callable.url` property to be set at build time to
...
Another example would be defining different database servers depending on profiles used
...
For the purpose of this section we used the defined property `remote.host`, it has to be noted that the value could have been a specific one like 
----
application.host=${HOST:localhost}
----
which will use `localhost` as the default value if `HOST` is not set.
