...
public class TextResourceTest {
    ...
    public void fetchDefault() {
        when().get("/text/validate/boom").then().statusCode(400).contentType(ContentType.TEXT);
    }
    ...
    public void shouldGetAcceptLanguageLocaleIfKeyIsUpperCase() {
        given().header("Accept-Language", "de").when().get("/text/validate/boom").then().log().ifValidationFails()
