...
In dev mode, when a metrics backend is present, an Observability *Metrics* section
appears in the Dev UI. It shows live time-series charts of your application metrics
— one chart per metric name, one line per tag combination. Pick which metrics to
chart from the grouped catalog; only selected metrics are captured, to save
resources. Monotonic series (counters, timers) are rendered as a per-interval rate.

The view is active automatically whenever a metrics backend and the Dev UI are
present; it is dev-mode only and adds nothing to production or native builds. You can
...
This view works with the Prometheus registry, the Micrometer OpenTelemetry bridge,
