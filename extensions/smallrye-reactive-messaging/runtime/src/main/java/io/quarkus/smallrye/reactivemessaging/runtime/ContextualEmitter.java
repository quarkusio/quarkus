...
/**
 * Emitter implementation that plays better with context propagation, especially with the request scoped context. The
 * message emit of this emitter makes sure that
 * <p>
 * <ul>
 * <li>the message context is captured when active thread context plan is applied</li>
 * <li>the message is emitted only after the context propagation ends the context switch, destroying the intermediate
 * state for cleared contexts.</li>
 * </ul>
 *
 * @param <T>
 *        the payload type
...
public interface ContextualEmitter<T> extends EmitterType {
     ...
     * <p>
     * A {@link Message} object will be created to hold the payload and the returned {@code Uni} can be subscribed to
     * for triggering the send. When subscribed, a {@code null} item will be passed to the {@code Uni} when the
     * {@code Message} is acknowledged. If the {@code Message} is never acknowledged, then the {@code Uni} will never be
     * completed.
     * <p>
    ...
    /**
     * Sends a message to the channel. Execution will block waiting for the resulting {@code Message} to be acknowledged
     * before returning.
     *
     * @param <M>
     *        the <em>Message</em> type
