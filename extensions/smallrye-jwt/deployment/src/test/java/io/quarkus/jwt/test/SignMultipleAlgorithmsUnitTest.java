...
public class SignMultipleAlgorithmsUnitTest {
    ...
    static final QuarkusUnitTest config = new QuarkusUnitTest()
            .withApplicationRoot((jar) -> jar.addClass(DefaultGroupsEndpoint.class).addAsResource("rs256PrivateKey.jwk")
                    .addAsResource("edEcPrivateKey.jwk").addAsResource("signatureJwkSet.jwk")
    ...
    public void echoGroupsEdDSA() {
        String token = Jwt.issuer("https://server.example.com").upn("alice").groups("User").sign("/edEcPrivateKey.jwk");
        RestAssured.given().auth().oauth2(token).get("/endp/echo").then().assertThat().statusCode(200)
                .body(equalTo("User"));
    }
}
