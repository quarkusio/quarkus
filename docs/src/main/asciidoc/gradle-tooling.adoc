...
By default, Quarkus starts in debug mode, listening to port `5005` without suspending the JVM.
...
So, you can run a Quarkus application in debug mode listening to port `5006` instead of the default port `5005` using:
...
Then, attach your debugger to `localhost:5006`.

[TIP]
====
You can also run `./gradlew quarkusDev -Ddebug` which is a shorthand for `./gradlew quarkusDev -Ddebug=true`.
====
