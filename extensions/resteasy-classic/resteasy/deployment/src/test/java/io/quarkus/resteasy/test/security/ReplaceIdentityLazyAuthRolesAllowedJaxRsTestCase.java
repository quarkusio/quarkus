...
public class ReplaceIdentityLazyAuthRolesAllowedJaxRsTestCase {
    ...
    static QuarkusUnitTest runner = new QuarkusUnitTest().withApplicationRoot((jar) -> jar
            .addClasses(RolesAllowedResource.class, UserResource.class, TestIdentityProvider.class,
                    TestIdentityController.class, SecurityOverrideFilter.class, UnsecuredSubResource.class)
    ...
    public void testRolesAllowedModified() {
        ...
        RestAssured.given().header("user", "admin").header("role", "admin").get("/roles").then().statusCode(200);
        RestAssured.given().auth().basic("user", "user").header("user", "admin").header("role", "admin")
                .get("/roles/admin").then().statusCode(200);
    }
