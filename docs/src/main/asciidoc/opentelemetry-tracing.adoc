...
In cases were utilities or automatic instrumentation is not available manual instrumentation can be used.
...
public static class HelloBean {
    ...
    public HelloBean(Tracer tracer) {
        this.tracer = tracer;
    }
...
----
<1> It is essential that all the logic the span is measuring be well-delimited by a scope.
