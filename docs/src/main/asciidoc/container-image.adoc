...
When `quarkus.container-image.insecure=true`, the generated `ImageStream` resources are annotated with `openshift.io/image.insecureRepository`, so that OpenShift imports their images from insecure registries.
This covers pulling the builder image (or the output image referenced through an external registry) from a registry with a self-signed certificate or plain HTTP.
Pushing the build output to such a registry is a cluster-level setting of the OpenShift build system and is not affected by this option.
