...
If you're using Podman with {product-name-first}, some one-off setup is needed, but once it's done, you can take advantag{product-name}l the Quarkus features.
...
Testcontainers and {product-name} Dev Services also expect the container service they make requests against to be non-interactive.
