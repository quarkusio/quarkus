...
@Singleton
final class SecurityIntegration implements SignalMetadataEnricher, ReceiverInterceptor {

    private static final String QUARKUS_IDENTITY_EXPIRE_TIME = "quarkus.identity.expire-time";
    ...
    public void enrich(EnrichmentContext context) {
        ...
        if (requestContext.isActive()
            ...
            SecurityIdentity securityIdentity = abstractAssociation.getIdentityValue();
    ...
    public Uni<Object> intercept(InterceptionContext context) {
        if (context.signalContext().metadata().get(SECURITY_IDENTITY_KEY) instanceof SecurityIdentity securityIdentity) {
