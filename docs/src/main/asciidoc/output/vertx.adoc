...
:summary: This guide explains how to use Vert.x in {product-name-first} to build reactive applications.

https://vertx.io[Vert.x] is a toolkit for building reactive applications.
As described in the xref:quarkus-reactive-architecture.adoc[{product-name} Reactive Architecture], {product-name} uses Vert.x underneath.

image::quarkus-reactive-core.png[{product-name} Reactive Core,width=50%, align=center]

{product-name} applications can access and use the Vert.x APIs.

This guide presents how you can build a {product-name} application using:
...
IMPORTANT: {product-name} provides other ways to serve static files. This is an example made for the guide.
...
----
<1> Declaring a CDI Bean in the Application scope. {product-name} will create a single instance of this class.
...
Note that some {product-name} extensions are wrapping Vert.x clients and manage them for you.
...
Let's now implement a new HTTP endpoint that queries the Wikipedia API to retrieve the pages about {product-name} in the different languages.
Add the following method to the `VertxResource` class:

[source, java]
----
private static final String URL = "https://en.wikipedia.org/w/api.php?action=parse&page={product-name}&format=json&prop=langlinks";
...
[{"lang":"de","url":"https://de.wikipedia.org/wiki/{product-name}","langname":"German","autonym":"Deutsch","*":"{product-name}"},{"lang":"fr","url":"https://fr.wikipedia.org/wiki/{product-name}","langname":"French","autonym":"français","*":"{product-name}"}]
----

The response indicates that, in addition to the English page, there are a German and a French page about {product-name} on wikipedia.
...
{product-name} provides the `VertxContextSupport#subscribeAndAwait()` method which subscribes to the supplied `io.smallrye.mutiny.Uni` on a Vert.x duplicated context, then blocks the current thread and waits for the result.
...
This guide introduced how you can use Vert.x APIs from a {product-name} application.
It's just a brief overview.
If you want to know more, check the xref:vertx-reference.adoc[reference guide about Vert.x in {product-name}].

As we have seen, the event bus is the connecting tissue of Vert.x applications.
{product-name} integrates it so different beans can interact with asynchronous messages.
This part is covered in the xref:reactive-event-bus.adoc[event bus documentation].

Learn how to implement highly performant, low-overhead database applications on {product-name} with the xref:reactive-sql-clients.adoc[Reactive SQL Clients].
