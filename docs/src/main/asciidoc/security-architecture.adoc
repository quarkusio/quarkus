...
image::security-architecture-overview.png[alt="Flowchart of an HTTP request passing through the Quarkus Security manager, HttpAuthenticationMechanism, IdentityProvider, SecurityIdentityAugmentor, and Authorization to either access granted or access denied", width="60%", align=center]
