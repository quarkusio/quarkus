...
If a CDI scope is declared on a key generator class and if that class has a default qualifier (no qualifier annotation or `@javax.enterprise.inject.Default`), then the key generator will be injected as a CDI bean during the cache key computation.
...
public class CacheConfigManager {
    ...
    public void setExpireAfterAccess(String cacheName, Duration duration) {
        Optional<Cache> cache = cacheManager.getCache(cacheName);
        if (cache.isPresent()) {
            cache.as(CaffeineCache.class).setExpireAfterAccess(duration); <1>
        }
    }
    ...
    public void setExpireAfterWrite(String cacheName, Duration duration) {
        Optional<Cache> cache = cacheManager.getCache(cacheName);
        if (cache.isPresent()) {
            cache.as(CaffeineCache.class).setExpireAfterWrite(duration); <2>
        }
    }
