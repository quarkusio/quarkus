...
If we focus on the dependencies section, you can see one of the 2 extensions allowing the development of REST applications:
...
In order to use dependency injection, we can use ArC which is a CDI-based dependency injection solution tailored for Quarkus architecture - see also link:cdi-reference.html[Contexts and Dependency Injection].
The `quarkus-arc`, dependecy of the `quarkus-resteasy`, is used for this purpose.
