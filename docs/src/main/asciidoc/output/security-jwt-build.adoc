...
Therefore, the secret keys should be preferred for directly encrypting the tokens, for example, when using JWT as cookies where a secret key is managed by the {product-name-first} endpoint with only this endpoint being both a producer and a consumer of the encrypted token.
