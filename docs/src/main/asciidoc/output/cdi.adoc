...
:categories: core
:summary: {product-name-first} DI solution is based on the [Jakarta Contexts and Dependency Injection 4.0](https://jakarta.ee/specifications/cdi/4.0/jakarta-cdi-spec-4.0.html) specification. This guide explains the basics of CDI.
:numbered:
:sectnums:
:sectnumlevels: 4

In this guide we're going to describe the basic principles of the {product-name} programming model that is based on the https://jakarta.ee/specifications/cdi/4.0/jakarta-cdi-spec-4.0.html[Jakarta Contexts and Dependency Injection 4.0, window="_blank"] specification.
...
However, in {product-name} we detect the absence of no-args constructor and "add" it directly in the bytecode.
...
NOTE: There can be other custom scopes provided by {product-name} extensions. For example, xref:transaction.adoc[`quarkus-narayana-jta`] provides xref:transaction.adoc#transaction-scope[`jakarta.transaction.TransactionScoped`].
...
In this guide, we've covered some basic topics of the {product-name} programming model that is based on the https://jakarta.ee/specifications/cdi/4.0/jakarta-cdi-spec-4.0.html[Jakarta Contexts and Dependency Injection 4.0, window="_blank"] specification.
{product-name} implements the CDI Lite specification, but not CDI Full.
...
There are also quite a few <<cdi-reference#nonstandard_features,non-standard features>> and <<cdi-reference.adoc#build_time_apis,{product-name}-specific APIs>>.

TIP: If you wish to learn more about {product-name}-specific features and limitations there is a {product-name} xref:cdi-reference.adoc[CDI Reference Guide].
