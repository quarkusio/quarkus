...
This guide explains how your {product-name-first} application can utilize https://github.com/smallrye/smallrye-jwt/[SmallRye JWT]
...
and provide secured access to the {product-name} HTTP endpoints using Bearer Token Authorization and https://en.wikipedia.org/wiki/Role-based_access_control[Role-Based Access Control].

NOTE: {product-name} OpenID Connect `quarkus-oidc` extension also supports Bearer Token Authorization and uses `smallrye-jwt` to represent the bearer tokens as `JsonWebToken`.
For more information, read the xref:security-oidc-bearer-token-authentication.adoc[OIDC Bearer token authentication] guide.
OpenID Connect extension has to be used if the {product-name} application needs to authenticate the users using OIDC Authorization Code Flow.
...
If you already have your {product-name} project configured, you can add the `smallrye-jwt` extension
...
2020-07-15 16:09:50,883 INFO  [io.quarkus] ({product-name} Main Thread) security-jwt-quickstart 1.0.0-SNAPSHOT on JVM (powered by {product-name} 999-SNAPSHOT) started in 1.073s. Listening on: http://0.0.0.0:8080
2020-07-15 16:09:50,885 INFO  [io.quarkus] ({product-name} Main Thread) Profile dev activated. Live Coding activated.
2020-07-15 16:09:50,885 INFO  [io.quarkus] ({product-name} Main Thread) Installed features: [cdi, mutiny, resteasy-reactive, resteasy-reactive-jackson, security, smallrye-context-propagation, smallrye-jwt, vertx, vertx-web]
----
...
Use Ctrl-C to stop the {product-name} server.
...
Now let's use this to make a secured request to the /secured/roles-allowed endpoint. Make sure you have the {product-name} server still running in dev mode, and then run the following command, making sure to use your version of the generated JWT from the previous step:
...
2019-03-28 14:27:48,839 INFO  [io.quarkus] (main) {product-name} {quarkus-version} started in 0.796s. Listening on: http://[::]:8080
...
2019-03-28 14:31:37,315 INFO  [io.quarkus] (main) {product-name} 0.12.0 started in 0.006s. Listening on: http://[::]:8080
...
What it means from the perspective of `quarkus-smallrye-jwt` which operates as part of the reactive {product-name} security architecture, is that an IO thread entering the link:https://github.com/smallrye/smallrye-jwt[SmallRye JWT] verification or decryption code might block in one of the following cases:
...
If your {product-name} application is running in a docker container, it may share a network interface with a Keycloak docker container launched by DevServices for Keycloak, with the {product-name} application and Keycloak communicating with each other via an internal shared docker network.
...
----
# keycloak.url is set by DevServices for Keycloak,
# {product-name} will access it via an internal shared docker network interface.
mp.jwt.verify.publickey.location=${keycloak.url}/realms/quarkus/protocol/openid-connect/certs
...
* You work with {product-name} extensions that do not support `HTTP`, such as `{product-name} GRPC`.
* You provide an extension-specific `HTTP`, the support of which conflicts with the support of those offered by `quarkus-smallrye-jwt` and `Vert.x HTTP`, such as `{product-name} AWS Lambda`.
