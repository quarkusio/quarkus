...
{product-name-first} Funqy link:https://knative.dev/docs/eventing[Knative Events] builds off of the xref:funqy-http.adoc[Funqy HTTP] extension to allow you to
...
you'll see that it is a typical {product-name} POM that pulls in one Funqy dependency:
...
Funqy also works with {product-name} Dev mode!
...
Dockerfile is a standard {product-name} dockerfile.  No special Knative magic.
