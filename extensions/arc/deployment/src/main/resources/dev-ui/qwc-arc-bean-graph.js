...
export class QwcArcBeanGraph extends LitElement {
    ...
    render() {
        if (this.beanId) {
            const categories = [
                msg('root', { id: 'quarkus-arc-category-root' }),
                msg('dependencies', { id: 'quarkus-arc-category-dependencies' }),
                msg('declaring bean of a producer', { id: 'quarkus-arc-category-declaring-bean-of-producer' }),
                msg('lookup dependency', { id: 'quarkus-arc-category-lookup-dependency' })
            ];
            ...
            return html`<span>${msg('No bean id provided', { id: 'quarkus-arc-no-bean-id-provided' })}</span>`;
        }
    }
        ...
        return html`
            <div class="top-bar">
                ...
                <div>
                    ...
                    <vaadin-button
                        theme="icon"
                        aria-label=${msg('Zoom in', { id: 'quarkus-arc-zoom-in' })}
                        @click=${this._zoomIn}>
                        <vaadin-icon icon="font-awesome-solid:magnifying-glass-plus"></vaadin-icon>
                    </vaadin-button>
                    <vaadin-button
                        theme="icon"
                        aria-label=${msg('Zoom out', { id: 'quarkus-arc-zoom-out' })}
                        @click=${this._zoomOut}>
    ...
    _renderCheckbox(){
        return html`
            <vaadin-checkbox-group
                ...
                <vaadin-checkbox
                    value="0"
                    label=${msg('Simple description', { id: 'quarkus-arc-simple-description' })}>
                </vaadin-checkbox>
            </vaadin-checkbox-group>`;
    }
