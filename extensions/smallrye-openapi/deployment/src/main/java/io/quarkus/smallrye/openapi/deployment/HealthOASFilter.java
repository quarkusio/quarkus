...
/**
 * This adds /health endpoint to the document.
 */
public class HealthOASFilter implements OASFilter {
    private static final String ROOT_PATH = "quarkus.smallrye-health.root-path";
    private static final String LIVENESS_PATH = "quarkus.smallrye-health.liveness-path";
    private static final String READINESS_PATH = "quarkus.smallrye-health.readiness-path";
    private static final List<String> MICROPROFILE_HEALTH_TAG = Collections.singletonList("MicroProfile Health");

    private final Config config;

    public HealthOASFilter(Config config) {
        this.config = config;
    }
    ...
    public void filterOpenAPI(OpenAPI openAPI) {
        ...
        // Health
        String healthUrl = config.getOptionalValue(ROOT_PATH, String.class).orElse("/health");
        paths.addPathItem(healthUrl, createHealthPathItem());

        // Liveness
        String livenessUrl = config.getOptionalValue(LIVENESS_PATH, String.class).orElse("/live");
        paths.addPathItem(healthUrl + livenessUrl, createLivenessPathItem());

        // Readiness
        String readinessUrl = config.getOptionalValue(READINESS_PATH, String.class).orElse("/ready");
        paths.addPathItem(healthUrl + readinessUrl, createReadinessPathItem());
    }
