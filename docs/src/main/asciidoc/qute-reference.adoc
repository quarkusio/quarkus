...
==== Templates Provided by Extensions

An extension can contribute templates at build time, so that they are validated and available in all modes like the templates found in `src/main/resources/templates`:

* Produce a `io.quarkus.qute.deployment.TemplateRootBuildItem` to register an additional template root. The root is a directory inside an application archive, relative to the archive root, for example `web/templates`; a directory outside the application archives cannot be registered as a root.
* Produce a `io.quarkus.qute.deployment.TemplatePathBuildItem` to register a single template whose content is known at build time, wherever it comes from. Use `TemplatePathBuildItem.builder().path("foo.html").content(content).build()`; the path is relative to the template root and must be unique. This is the way to provide templates that live outside `src/main/resources`, for example in a directory at the project root: read the file during the build and pass its content.
