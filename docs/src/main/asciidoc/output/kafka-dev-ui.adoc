...
the {product-name-first} Dev UI is extended with a Kafka broker management UI.
...
IMPORTANT: Kafka Dev UI is part of the {product-name} Dev UI and is only available in development mode.
