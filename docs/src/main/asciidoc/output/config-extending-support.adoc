...
When a {product-name-first} application starts, a `ConfigSource` can be initialized twice. One time for _STATIC INIT_ and a second
time for _RUNTIME INIT_:

==== STATIC INIT

{product-name} starts some of its services during static initialization, and `Config` is usually one of the first things that
...
IMPORTANT: A custom `ConfigSource` is not automatically added during {product-name} _STATIC INIT_. It requires to be marked with
...
NOTE: All {product-name} core converters use the priority value of `200`. To override any {product-name} specific converter, the
