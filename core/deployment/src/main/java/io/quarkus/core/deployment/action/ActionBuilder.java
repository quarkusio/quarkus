...
public interface ActionBuilder {
    ...
    /**
     * Produce a bare {@code T} recorder proxy for a service defined earlier in this same build step,
     * suitable for passing directly as a parameter to a legacy {@code @Record(STATIC_INIT)} or
     * {@code @Record(RUNTIME_INIT)} recorder method, or embedding in a legacy build item.
     * <p>
     * The returned proxy implements {@link BytecodeRecorderImpl.ReturnedProxy ReturnedProxy}
     * and the service type. When the recorder sees this proxy as a method argument, it generates
     * code to load the service value from the startup context. The proxy phase is inferred from
     * the service definition, so it is valid for whichever recorder phase matches that service.
     * <p>
     * The service must be defined (its {@code action()}/{@code actionAsync()} called) earlier in the
     * <em>same</em> build step; phase resolution is per-build-step. Static-init services are retained
     * across the phase boundary automatically, because the downstream consumer and its phase cannot
     * be detected here.
     *
     ...
     * @throws IllegalStateException if the service is not defined in this build step
     */
    <T> T getRecorderProxy(Class<T> serviceType);

    /**
     * Produce a bare {@code T} recorder proxy for a named service defined earlier in this same build step.
     *
     * @param serviceType the service type (must not be {@code null}; must be proxyable)
     * @param serviceName the service name (may be {@code null} for an unnamed service)
     * @param <T> the service type
     * @return a {@code T} proxy for use with legacy recorders
     * @throws IllegalStateException if the service is not defined in this build step
     * @see #getRecorderProxy(Class)
     */
    <T> T getRecorderProxy(Class<T> serviceType, String serviceName);

    // ── Service → Supplier bridge (temporary) ──

    /**
     * Produce a {@link Supplier Supplier&lt;T&gt;} proxy
