...
1. using system properties: `java -jar target/myapp-runner.jar -Dquarkus.database.password=youshallnotpass`
2. using environment variable: `export QUARKUS_DATABASE_PASSWORD=youshallnotpass ; java -jar target/myapp-runner.jar`
...
{project-name} relies on Eclipse Microprofile and inherit his features.
