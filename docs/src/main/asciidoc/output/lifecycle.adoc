...
* Write a {product-name-first} application with a main method
...
By default, {product-name} will automatically generate a main method, that will bootstrap {product-name} and then just wait for
...
public class Main {
    ...
    public static void main(String ... args) {
        ...
        {product-name}.run(args); <2>
    }
}
----
<1> This annotation tells {product-name} to use this as the main method, unless it is overridden in the config
<2> This launches {product-name}

This main class will bootstrap {product-name} and run it until it stops. This is no different to the automatically
...
WARNING: It is not recommenced to do any business logic in this main method, as {product-name} has not been set up yet,
and {product-name} may run in a different ClassLoader. If you want to perform logic on startup use an `io.quarkus.runtime.QuarkusApplication`
as described below.

If we want to actually perform business logic on startup (or write applications that complete a task and then exit)
we need to supply a `io.quarkus.runtime.QuarkusApplication` class to the run method. After {product-name} has been started
the `run` method of the application will be invoked. When this method returns the {product-name} application will exit.

If you want to perform logic on startup you should call `{product-name}.waitForExit()`, this method will wait until a shutdown
is requested (either from an external signal like when you press `Ctrl+C` or because a thread has called `{product-name}.asyncExit()`).
...
public class Main {
    public static void main(String... args) {
        {product-name}.run(MyApp.class, args);
    }
...
See xref:writing-extensions.adoc#bootstrap-three-phases[Three Phases of Bootstrap and {product-name} Philosophy] for more details.
...
{product-name} has 3 different launch modes, `NORMAL` (i.e. production), `DEVELOPMENT` and `TEST`. If you are running `quarkus:dev`
...
{product-name} includes support for graceful shutdown, this allows {product-name} to wait for running requests to finish, up
