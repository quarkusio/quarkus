...
NOTE: The back-channel logout route handler is registered by default. To prevent its registration, remove `backchannel-logout` from the <<allowed-routes-properties,quarkus.oidc.allowed-routes>> build-time property.
...
NOTE: The protected resource metadata route handler is registered by default. To prevent its registration, remove `resource-metadata` from the <<allowed-routes-properties,quarkus.oidc.allowed-routes>> build-time property.
