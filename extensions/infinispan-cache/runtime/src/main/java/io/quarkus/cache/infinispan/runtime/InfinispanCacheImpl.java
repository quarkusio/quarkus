...
/**
 * This class is an internal Quarkus cache implementation using Infinispan. Do not use it explicitly from your Quarkus
 * application.
 */
public class InfinispanCacheImpl extends AbstractCache implements Cache {
    ...
    public <K, V> Uni<V> getAsync(K key, Function<K, Uni<V>> valueLoader) {
        Context context = Vertx.currentContext();
        Executor executor = duplicateContextExecutor(context);
        return Uni.createFrom().completionStage(getFromInfinispanAsync(key, valueLoader, executor))
                .emitOn(new Executor() {
                    ...
                    public void execute(Runnable command) {
                        ...
                        if (context == null) {
                            ...
                            if (ctx == context) {
                                ...
                                // 1) We are not on a context (ctx == null) => we need to switch to the captured
                                // context.
                                // 2) We are on a different context (ctx != null) => we need to switch to the captured
                                // context.
    ...
    private <K, V> CompletionStage<V> getFromInfinispanAsync(K key, Function<K, Uni<V>> valueLoader,
            Executor executor) {
        return remoteCache.getAsync(key).exceptionallyAsync(ex -> ex, executor).thenApplyAsync(new Function() {
            @Override
            public Object apply(Object v1) {
                ...
                valueLoader.apply(key).convert().toCompletionStage().whenCompleteAsync(new BiConsumer<V, Throwable>() {
                    @Override
    ...
    private <K, V> void putIfAbsentInInfinispan(K key, V newValue, CompletableFuture<V> resultAsync,
            Executor executor) {
        remoteCache.putIfAbsentAsync(key, encodeNull(newValue), lifespan, TimeUnit.MILLISECONDS, maxIdle,
                TimeUnit.MILLISECONDS).whenCompleteAsync(new BiConsumer<Object, Throwable>() {
                    @Override
    ...
    public Uni<Void> invalidateIf(Predicate<Object> predicate) {
        ...
        return Uni.createFrom()
                .multi(Multi.createFrom().publisher(entriesPublisher).map(e -> ((Map.Entry<Object, Object>) e).getKey())
                        .filter(key -> predicate.test(key)).onItem()
