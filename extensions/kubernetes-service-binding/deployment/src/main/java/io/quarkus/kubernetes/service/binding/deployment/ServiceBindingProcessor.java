...
/***
 * Processor that handles the generation of ServiceBinding resources. Generated ServiceBinding rules: 1. .metadata.name:
 * When no user configuration is present then the combination [app.name] - [qualifier kind] - [qualifier name] will be
 * used. Examples: - app-postgresql-default - app-mysql-default - app-mongodb-default 2. .spec.service[*].name: Since
 * services are qualified using apiVersion, we don't need to carry over the [qualifier kind]. In this case the name is
 * just [qualifier name].* Notes: The following pairs are equivalent:
 ...
 * quarkus.kubernetes-service-binding.services.persondb.xxx=yyy When service are auto bound the minimal from (e.g.
 * persondb) will be used. Users are still able to tune things using the [kind]-[name] combo to avoid naming clashes.
 */
public class ServiceBindingProcessor {
    ...
    @BuildStep
    public void registerServiceBindingAsOptional(
            BuildProducer<KubernetesOptionalResourceDefinitionBuildItem> optionalKinds) {
    ...
    public List<ServiceBindingRequirementBuildItem> createServiceBindingDecorators(
            ApplicationInfoBuildItem applicationInfo, KubernetesServiceBindingConfig config,
            List<ServiceBindingQualifierBuildItem> qualifiers) {
        ...
        qualifiers.forEach(q -> {
            Optional<ServiceBindingRequirementBuildItem> requirement = createRequirementFromQualifier(applicationName,
                    config, q);
    ...
    /**
     * Create an {@link Optional} {@link ServiceBindingRequirementBuildItem} from a
     * {@link KubernetesServiceBindingConfig} entry.
     *
     ...
     *        The config instance.
     *
     * @return The service binding requirement that corresponds to the matching entry, or empty if no service with
     *         serviceId was found in config.
     */
     ...
     * @param applicationName
     *        The name of the application.
     * @param defaultsLookupKey
     *        The key to use for looking up default apiVersion/kind).
     * @param serviceId
     *        The key of the service.
     * @param resourceName
    ...
    protected static Optional<ServiceBindingRequirementBuildItem> createRequirementFromConfig(String applicationName,
        ...
        if (config.services() != null && config.services().containsKey(serviceId)) {
            ...
            String apiVersion = provided.apiVersion().orElseGet(() -> getDefaultQualifiedKind(defaultsLookupKey)
                    .map(ServiceBindingProcessor::apiVersion).orElse(""));
            String kind = provided.kind().orElseGet(
                    () -> getDefaultQualifiedKind(defaultsLookupKey).map(ServiceBindingProcessor::kind).orElseThrow(
                            () -> new IllegalStateException("Failed to determine bindable service kind.")));
            // When a service is partially or fully configured, use the configured binding or fallback to the
            // application name and service id combination.
            return Optional.of(
                    new ServiceBindingRequirementBuildItem(provided.binding().orElse(applicationName + "-" + serviceId),
    ...
    /**
     * Create an {@link Optional} {@link ServiceBindingRequirementBuildItem} from a
     * {@link ServiceBindingQualifierBuildItem}.
     *
     * @param applicationName
     *        The name of the application.
     * @param config
     *        The config instance.
     * @param qualifier
     *        The qualifier that will be converted to a requirement.
     *
     * @return The service binding requirement that corresponds to the config entry that matches the qualifier, or the
     *         defaults for the qualifier. Returns empty if none of the above was found.
     */
    protected static Optional<ServiceBindingRequirementBuildItem> createRequirementFromQualifier(String applicationName,
        ...
        if (config.services() != null && config.services().containsKey(qualifier.getId())) {
            return createRequirementFromConfig(applicationName, qualifier.getKind(), qualifier.getId(),
                    qualifier.getName(), config);
        } else if (config.services() != null && config.services().containsKey(qualifier.getName())) {
            return createRequirementFromConfig(applicationName, qualifier.getKind(), qualifier.getName(),
                    qualifier.getName(), config);
