...
The configuration file for vale is at `docs/.vale.ini` and the various style and checks defined in the `docs/.vale` directory.
...
If you install the https://vale.sh/docs/vale-cli/installation/[install the Vale CLI], you can run the checks from the `docs` directory:
...
If you want to run it from another directory use `--config=/path/to/quarkus/docs/.vale.ini`.
...
Each has it's own configuration requirements. The Visual Studio Code IDE extension, for example, requires you to tell where valeCLI is installed in workspace settings:
...
The plugin will pick up the `.vale.ini` settings automatically relatively to the documentation files.
