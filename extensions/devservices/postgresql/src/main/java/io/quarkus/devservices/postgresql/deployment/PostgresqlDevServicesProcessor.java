...
public class PostgresqlDevServicesProcessor {
        ...
        return new DevServicesDatasourceProviderBuildItem(DatabaseKind.POSTGRESQL, new DevServicesDatasourceProvider() {
            ...
            public RunningDevServicesDatasource startDatabase(Optional<String> username, Optional<String> password,
                ...
                Supplier<RunningDevServicesDatasource> createDevService = () -> {
                    QuarkusPostgreSQLContainer container = new QuarkusPostgreSQLContainer(
                ...
                List<String> images = List.of(containerConfig.getImageName()
                        .orElseGet(() -> ConfigureUtil.getDefaultImageNameFor("postgresql")), "postgres");
    ...
    private static class QuarkusPostgreSQLContainer extends PostgreSQLContainer {
        ...
        public QuarkusPostgreSQLContainer(Optional<String> imageName, OptionalInt fixedExposedPort,
            ...
            super(DockerImageName.parse(imageName.orElseGet(() -> ConfigureUtil.getDefaultImageNameFor("postgresql")))
            ...
            // The motivation of this custom wait strategy is that Testcontainers fails to start a Postgresql database
            // when it
            ...
            this.waitStrategy = new WaitAllStrategy()
                    .withStrategy(
                            Wait.forLogMessage("(" + READY_REGEX + ")?(" + SKIPPING_INITIALIZATION_REGEX + ")?", 2))
                    .withStrategy(Wait.forListeningPort()).withStartupTimeout(Duration.of(60L, ChronoUnit.SECONDS));
        ...
        public String getEffectiveJdbcUrl() {
            if (useSharedNetwork) {
                ...
                return "jdbc:postgresql://" + hostName + ":" + POSTGRESQL_PORT + "/" + getDatabaseName()
                        + additionalUrlParams;
            } else {
