...
public class FormConfiguration {
    ...
    void configure(@Observes HttpSecurity httpSecurity) {   <1>
        var builder = Form.builder().httpOnlyCookie().loginPage("/my-login.html").errorPage("/my-error.html");
        httpSecurity
                .mechanism(builder.build())
                .path("/custom-config/*").form(builder.loginPage("/custom-login.html").build());  <2>
    }
...
<2> Configure the `/custom-login.html` login page only for requests with the `/custom-config` sub-paths.
