...
:summary: This guide demonstrates how your {product-name-first} application can utilize the SmallRye Fault Tolerance specification through the SmallRye Fault Tolerance extension.
...
applications, {product-name} provides https://github.com/smallrye/smallrye-fault-tolerance/[SmallRye Fault Tolerance], an
...
This command generates a project, importing the extensions for RESTEasy Reactive/Jakarta REST and SmallRye Fault Tolerance.

If you already have your {product-name} project configured, you can add the `smallrye-fault-tolerance` extension
...
Why not check that our application works? Run the {product-name} development server with:
...
Congratulations, you've just made a working (although somewhat unreliable) {product-name} application!

== Adding Resiliency: Retries

Let the {product-name} development server running and in your IDE add the `@Retry` annotation to the `CoffeeResource#coffees()`
...
Hit refresh in your browser. The {product-name} development server will automatically detect the changes
...
All that is needed to enable the fault tolerance features in {product-name} is:

:devtools-wrapped:
* adding the `smallrye-fault-tolerance` {product-name} extension to your project using the `quarkus-maven-plugin`:
+
include::{includes}/devtools/extension-add.adoc[]
...
In {product-name}, you can use the SmallRye Fault Tolerance optional features out of the box.
...
Support for Kotlin is present (assuming you use the {product-name} extension for Kotlin), so you can guard your `suspend` functions with fault tolerance annotations.

Metrics are automatically discovered and integrated.
If your application uses the {product-name} extension for Micrometer, SmallRye Fault Tolerance will emit metrics to Micrometer.
If your application uses the {product-name} extension for SmallRye Metrics, SmallRye Fault Tolerance will emit metrics to MicroProfile Metrics.
