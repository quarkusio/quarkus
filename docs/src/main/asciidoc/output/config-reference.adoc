...
:categories: core
:summary: Learn more about how to configure your {product-name-first} applications.
:numbered:
:sectnums:
:sectnumlevels: 4
...
In this reference guide we're going to describe various aspects of {product-name} configuration. A {product-name} application and
{product-name} itself (core and extensions) are both configured via the same mechanism that leverages
...
TIP: If you're looking for information how to make a {product-name} extension configurable then see the
<<writing-extensions.adoc#configuration,Writing Your Own Extension>> guide.

[[configuration-sources]]
== Config Sources

By default, {product-name} reads configuration properties from multiple sources (by descending ordinal):
...
4. (260) <<application-properties-file,{product-name} Application configuration file>> in `$PWD/config/application.properties`
5. (250) <<application-properties-file,{product-name} Application configuration file>> `application.properties` in classpath
...
* For {product-name} dev mode: `./mvnw quarkus:dev -Dquarkus.datasource.password=youshallnotpass`
...
case, {product-name} relies upon each source’s list of property names. These must be converted back to their most likely
...
The {product-name} Application configuration file is loaded from the classpath resources, for instance
...
TIP: It works in the exact same way as {product-name} Application configuration file `application.properties`. Recommendation
is to use {product-name} `application.properties`.

=== Additional Config Sources

{product-name} provides additional extensions which cover other configuration formats and stores:
...
{product-name} uses https://microprofile.io/project/eclipse/microprofile-config[MicroProfile Config] annotations to inject the
...
Below is an example of a {product-name}-supplied converter:
...
The {product-name} HTTP port will be 9090. If the `dev` profile is active it will be 8181.
...
By default, {product-name} provides three profiles, that activate automatically in certain conditions:
...
The default {product-name} runtime profile is set to the profile used to build the application:
...
{product-name} provides property expressions expansion on configuration values. An expression string is
...
[WARNING]
====
SmallRye Config may provide handlers not fully supported by {product-name}. Currently, only `smallrye-config-crypto` is
supported.
====

== Accessing a generating UUID

The default config source from {product-name} provides a random UUID value.
...
{product-name} itself is configured via the same mechanism as your application. {product-name} reserves the `quarkus.` namespace
for its own configuration. For example to configure the HTTP server port you can set `quarkus.http.port` in
`application.properties`. All the {product-name} configuration properties are xref:all-config.adoc[documented and searchable].

[IMPORTANT]
====
As mentioned above, properties prefixed with `quarkus.` are effectively reserved for configuring {product-name} itself and its
extensions. Therefore, the `quarkus.` prefix should **never** be used for application specific properties.
====

=== Build Time configuration

Some {product-name} configurations only take effect during build time, meaning is not possible to change them at runtime. These
configurations are still available at runtime but as read-only and have no effect in {product-name} behaviour. A change to any
...
{product-name} relies on link:https://github.com/smallrye/smallrye-config/[SmallRye Config] and inherits its features:
