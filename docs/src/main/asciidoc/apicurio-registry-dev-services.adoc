...
If an extension for schema registry, such as `quarkus-apicurio-registry-avro`, `quarkus-confluent-registry-avro`, `quarkus-apicurio-registry-protobuf`, `quarkus-confluent-registry-protobuf`, `quarkus-apicurio-registry-json-schema`, or `quarkus-confluent-registry-json-schema`, is present, Dev Services for Apicurio Registry automatically starts an Apicurio Registry instance in dev mode and when running tests.
