...
{product-name-first} documentation uses link:https://asciidoc.org/[AsciiDoc] markup.

A gradual refactoring effort is underway to reshape {product-name} documentation to make it easier to both read and write. For more information about this transition, see xref:doc-concept.adoc[{product-name} documentation content types].

For more information on contributing to {product-name} docs, see:

- xref:doc-contribute-docs-howto.adoc[Contribute to {product-name} documentation]
- xref:doc-concept.adoc[{product-name} documentation content types]
- xref:doc-reference.adoc[{product-name} style and content guidelines]
...
* Container:: when we discuss {product-name} running in... containers
