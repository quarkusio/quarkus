...
class HealthOpenAPITest {
    ...
    void testOpenApiPathAccessResource() {
        ...
        RestAssured.given().header("Accept", "application/json").when().get(OPEN_API_PATH).then()
                ...
                .body("paths", Matchers.hasKey("/q/health/ready")).body("paths", Matchers.hasKey("/q/health/live"))
                .body("paths", Matchers.hasKey("/q/health/started")).body("paths", Matchers.hasKey("/q/health"))
