...
The {product-name-first} Redis extension allows{product-name}ating Quarkus applications with Redis.
...
* If you are building an imperative (_classic_) {product-name} application integrating with Redis: use `io.quarkus.redis.datasource.RedisDataSource`.
* If you are building a reactive {product-name} application integrating with Redis: use `io.quarkus.redis.datasource.ReactiveRedisDataSource`.
...
{product-name} exposes a high-level API on top of Redis.
...
{product-name} batches all the commands from a single file and sends all the commands.
...
As mentioned above, in dev and test modes, {product-name} tries to import data by looking for the `src/main/resources/import.redis`.
