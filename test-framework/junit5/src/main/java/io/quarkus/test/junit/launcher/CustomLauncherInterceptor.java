...
public class CustomLauncherInterceptor implements LauncherDiscoveryListener, LauncherSessionListener {
    ...
    public void launcherSessionOpened(LauncherSession session) {
        /*
         * For gradle, test class loading happens fairly shortly after this is called, before the formal discovery
         * phase. So we need to intercept the TCCL. However, the Eclipse runner calls this twice, and the second
         * invocation happens after discovery, which means there is no one to unset the TCCL. That breaks integration
         * tests, so we need to add an ugly guard to not adjust the TCCL the second time round in that scenario. We do
         * not do any classloading dance for prod mode tests.
         */
    ...
    private void actuallyIntercept() {
        ...
        // We might not be in the same classloader as the Facade ClassLoader, so use a name comparison instead of an
        // instanceof
    ...
    private void initializeFacadeClassLoader() {
        ...
        // Be aware, this method might be called more than once, for different kinds of invocations; especially for
        // Gradle executions, the executions could happen before the TCCL gets constructed and set by JUnitTestRunner
        // We might not be in the same classloader as the Facade ClassLoader, so use a name comparison instead of an
        // instanceof
        if (currentCl == null || (currentCl != facadeLoader
                && !currentCl.getClass().getName().equals(FacadeClassLoader.class.getName()))) {

            // We don't ever want more than one FacadeClassLoader active, especially since config gets initialised on
            // it.
    ...
    public void launcherDiscoveryStarted(LauncherDiscoveryRequest request) {
        ...
        // If anything comes through this method for which there are non-null classloaders on the selectors, that will
        // bypass our classloading
        // To check that case, the code would be something like this. We could detect and warn early, and possibly even
        // filter that test out, but that's not necessarily a better UX than failing later
        // request.getSelectorsByType(ClassSelector.class).stream().map(ClassSelector::getClassLoader) ... and then
        // check for non-emptiness on that field
    ...
    public void launcherDiscoveryFinished(LauncherDiscoveryRequest request) {
        ...
        if (!isProductionModeTests()) {
            ...
            // If there are user extensions present which implement `ExecutionCondition`, and they call config in
            // `evaluateExecutionCondition`,
            // they need the TCCL to be right for reading config (that is, the app classloader)
            // On the other hand, if the QuarkusTestExtension is registered by a service loader mechanism, it gets
            // loaded after the discovery phase finishes,
            // so needs the TCCL to still be the facade classloader.
            // This compromise does mean you can't use the service loader mechanism to avoid having to use
            // `@QuarkusTest` and also use Quarkus config in your own test extensions, but that combination is very
            // unlikely.
            if (!facadeLoader.isServiceLoaderMechanism()) {
                // Do not close the facade loader at this stage, because discovery finished may be called several times
                // within a single run
                ...
                // So, rather annoyingly and clumsily, reset the TCCL to what it was when the first interception
                // happened
