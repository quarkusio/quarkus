...
public class HalWrapperResourceTest {
    ...
    static final QuarkusUnitTest TEST = new QuarkusUnitTest().withApplicationRoot(
            (jar) -> jar.addClasses(HalWrapperResource.class, TestRecordWithIdAndPersistenceIdAndRestLinkId.class))
    ...
    void shouldGetAllRecordsWithCustomHalMetadata() {
        Response response = given().header(HttpHeaders.ACCEPT, RestMediaType.APPLICATION_HAL_JSON).get(recordsUrl)
                .thenReturn();

        assertThat(response.body().jsonPath().getString("_embedded['collectionName'][0].restLinkId")).isEqualTo("1");
        assertThat(response.body().jsonPath().getString("_embedded['collectionName'][0]._links.self.href"))
                .endsWith("/hal/1");
        assertThat(response.body().jsonPath().getString("_embedded['collectionName'][0]._links.list.href"))
                .endsWith("/hal");
        assertThat(response.body().jsonPath().getString("_embedded['collectionName'][1].restLinkId")).isEqualTo("2");
        assertThat(response.body().jsonPath().getString("_embedded['collectionName'][1]._links.self.href"))
                .endsWith("/hal/2");
        assertThat(response.body().jsonPath().getString("_embedded['collectionName'][1]._links.list.href"))
                .endsWith("/hal");
    ...
    void shouldGetSingleRecordWithCustomHalMetadata() {
        Response response = given().header(HttpHeaders.ACCEPT, RestMediaType.APPLICATION_HAL_JSON).get(recordIdUrl, 1L)
                .thenReturn();
