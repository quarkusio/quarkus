 ...
 * is passed as arguments to the method instead of forcing the author to use a mix of {@code @Context} and programmatic
 * CDI look-ups.
 * <p>
 ...
 * {@code Optional<Response>}, {@code Optional<RestResponse>}, {@code Uni<Void>}, {@code Uni<Response>} or
 * {@code Uni<RestResponse>}.
 * <ul>
 * <li>{@code void} should be used when filtering does not need to perform any blocking operations and the filter cannot
 * abort processing.
 * <li>{@code Response} or {@code RestResponse} should be used when filtering does not need to perform any blocking
 * operations and the filter cannot abort processing - in this case the processing will be aborted if the response is
 * not {@code null}.
 * <li>{@code Optional<Response>} or {@code Optional<RestResponse>} should be used when filtering does not need to
 * perform any blocking operations but the filter might abort processing - in this case processing is aborted when the
 * {@code Optional} contains a {@code Response} payload.
 * <li>{@code Uni<Void>} should be used when filtering needs to perform a non-blocking operation but the filter cannot
 * abort processing. Note that {@code Uni<Void>} can easily be produced using: {@code Uni.createFrom().nullItem()}
 * <li>{@code Uni<Response>} or {@code Uni<RestResponse>} should be used when filtering needs to perform a non-blocking
 * operation and the filter might abort processing - in this case processing is aborted when the {@code Uni} contains a
 ...
 * {@code abortWith} is not allowed. You should use the proper response type if aborting processing is necessary.
 */
...
public @interface ServerRequestFilter {
     ...
     * on a worker thread. This is meant to be set to {@code true} if a filter should be run on the event-loop even if
     * the target Resource method is going to be run on the worker thread. For this to work, this filter must be run
     * before any of the filters when non-blocking is not required.
     */
    boolean nonBlocking() default false;

    /**
     * If set to {@code true}, the filter will be run after the body has been fully read but before any deserialization
     * takes place.
     * <p>
     ...
     *
     * @deprecated use {@link WithFormRead} on your filter to force reading the form values before your filter is
     *             invoked.
     */
    @Deprecated
