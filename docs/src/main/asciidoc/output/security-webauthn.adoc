...
This guide demonstrates how your {product-name-first} application can use WebAuthn authentication instead of 
passwords.
...
If you already have your {product-name} project configured, you can add the `security-webauthn` extension
...
You need to define a bean implementing the `WebAuthnUserProvider` in order to allow the {product-name} WebAuthn
extension to load and store credentials. This is where you tell {product-name} how to turn your data model into the
...
The {product-name} WebAuthn extension comes out of the box with these REST endpoints pre-defined:
...
Because there's so much JavaScript needed to set WebAuthn up in the browser, the {product-name} WebAuthn extension ships with
...
{product-name} WebAuthn endpoints to defer those calls to the worker pool.                
