...
import io.quarkus.qute.debug.agent.completions.resolvers.DefaultValueResolverCollector;
...
public class CompletionSupport {
    ...
    private static final ResolverCollector REFLECTION_VALUE_RESOLVER_COLLECTOR = new ReflectionValueResolverCollector();
    private static final ResolverCollector DEFAULT_VALUE_RESOLVER_COLLECTOR = new DefaultValueResolverCollector();
    ...
    private void fillWithValueResolvers(CompletionContext context) {
        ...
        for (ValueResolver valueResolver : valueResolvers) {
            ...
            if (collector != null && collector.isApplicable(valueResolver, evalContext)) {
                collector.collect(context, valueResolver);
            }
        }
    }

    private ResolverCollector getCollector(ValueResolver valueResolver) {
        String className = valueResolver.getClass().getName();
        if (className.equals("io.quarkus.qute.ReflectionValueResolver")) {
