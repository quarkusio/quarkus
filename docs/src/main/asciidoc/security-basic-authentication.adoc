...
The `io.quarkus.vertx.http.security.event.Basic` CDI event allows to customize the configuration provided in the 'application.properties' during the runtime like in the example below:
...
public class BasicAuthenticationConfiguration {
    ...
    void configure(@Observes Basic basic) {
        basic
            .enable() <1>
            .realm("QuarkusTestRealm"); <2>
    }

}
----
<1> Enable basic authentication.
Downside is that Quarkus doesn't know that the basic authentication is required during the build-time.
For example, if you generate an OpenAPI schema document with the SmallRye OpenAPI extension, prefer the `quarkus.http.auth.basic` configuration property.
<2> Use the `QuarkusTestRealm` authentication realm.
