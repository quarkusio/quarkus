...
For example, the following configuration will allow for configuring the users in both the production where OAuth2 is required and development modes using link:https://quarkus.io/guides/config#configuration-profiles[Configuration Profiles].
...
If you need to add claims to the identity the `io.quarkus.test.security.SecurityAttribute` annotation can be used.

[source,java]
----
@Test
@TestSecurity(user = "testUser", roles = {"admin", "user"},
              attributes = {@SecurityAttribute(key = "birthdate", value = "27-01-2020"),
                            @SecurityAttribute(key = "petName", value = "Tom")})
void someTestMethod() {
...
}
----

This will run the tests with an identity with the given username, roles and claims.
