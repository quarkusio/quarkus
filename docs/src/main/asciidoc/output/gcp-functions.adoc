...
:categories: cloud
:summary: This guide explains how you can deploy {product-name-first}-based Google Cloud Functions.

The `quarkus-google-cloud-functions` extension allows you to use {product-name} to build your Google Cloud Functions.
Your functions can use injection annotations from CDI or Spring and other {product-name} facilities as you need them.
...
This guide walks you through generating a sample project followed by creating multiple functions showing how to implement `HttpFunction`, `BackgroundFunction` and `RawBackgroundFunction` in {product-name}.
...
NOTE: {product-name} supports Cloud Functions gen 1 and gen 2. For an overview of Cloud Functions gen 2 see https://cloud.google.com/functions/docs/2nd-gen/overview[this page] on the Google Cloud Functions documentation. To use gen 2 you must add the `--gen2` parameter.
...
The `quarkus.google-cloud-functions.function` property tells {product-name} which function to deploy. This can be overridden
...
[IMPORTANT]
====
The entry point must always be set to `io.quarkus.gcp.functions.QuarkusHttpFunction` as this is the class that integrates Cloud Functions with {product-name}.
====
...
[IMPORTANT]
====
The entry point must always be set to `io.quarkus.gcp.functions.QuarkusBackgroundFunction` as this is the class that integrates Cloud Functions with {product-name}.
====
...
[IMPORTANT]
====
The entry point must always be set to `io.quarkus.gcp.functions.QuarkusCloudEventsFunction` as this is the class that integrates Cloud Functions with {product-name}.
====
...
IMPORTANT: The `--classpath` parameter needs to be set to the previously packaged JAR that contains your function class and all {product-name} related classes.
...
{product-name} provides built-in support for testing your Google Cloud functions via the `quarkus-test-google-cloud-functions` dependency.
...
1. This is a standard {product-name} test that must be annotated by `@QuarkusTest`.
