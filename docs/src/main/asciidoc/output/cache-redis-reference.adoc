...
:categories: data
:summary: Use Redis as the {product-name-first} cache backend

By default, {product-name} Cache uses Caffeine as backend.
...
When using Redis as the backend for {product-name} cache, each cached item will be stored in Redis:
...
Then, use the `@CacheResult` and others cache annotations as explained in the xref:cache.adoc[{product-name} Cache guide]:
...
When writing to Redis or reading from Redis, {product-name} needs to know the type.
...
At build time, {product-name} tries to deduce the types from the application code, but that decision can be overridden using:
