...
[TIP]
====
If you use Confluent Schema Registry, you need the following dependencies and the Confluent Maven repository added
to your `pom.xml`:
[source,xml]
----
<dependencies>
...
First, let's add test dependencies on REST Client, Awaitility and Strimzi to `pom.xml`. Testcontainers will be pulled
