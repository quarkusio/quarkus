...
This is also the way to send content held in memory, for example a `byte[]` read from a database, as a file part whose name is only known at runtime, which the declarative `@PartFilename` annotation cannot express:

[source, java]
----
ClientMultipartForm form = ClientMultipartForm.create()
        .binaryFileUpload("file", fileName, Buffer.buffer(bytes), MediaType.APPLICATION_OCTET_STREAM);
----
