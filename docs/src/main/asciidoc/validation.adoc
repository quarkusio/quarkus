...
NOTE: An exception mapper is selected by the closest exception type first, and the priority only decides between the mappers of the same exception type.
A mapper for a super type, such as an `ExceptionMapper<Throwable>` or a `@ServerExceptionMapper` method that takes a `Throwable`, does not replace the builtin `ValidationException` mapper, whatever its priority is.
Declare a mapper for `ValidationException`, as shown below, or disable the builtin mapper with `quarkus.rest.exception-mapping.disable-mapper-for`.
