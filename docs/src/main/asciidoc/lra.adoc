...
[source,bash]
----
./mvnw quarkus:add-extension -Dextensions="narayana-lra,resteasy-jackson,rest-client"
----
...
This means that users should also have either `quarkus-resteasy-jackson` and `quarkus-rest-client` or `quarkus-resteasy-reactive-jackson` and `quarkus-rest-client-reactive` dependencies in their application.
====
