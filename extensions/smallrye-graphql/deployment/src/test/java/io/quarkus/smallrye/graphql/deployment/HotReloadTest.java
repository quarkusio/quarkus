...
public class HotReloadTest extends AbstractGraphQLTest {
    ...
    public void testAddAndRemoveFieldChange() {
        ...
        TEST.modifySourceFile("TestPojo.java", s -> s.replace("// <placeholder>", "private String foo = \"bar\";\n"
        ...
        TEST.modifySourceFile("TestPojo.java", s -> s.replace(
        ...
        RestAssured.given().when().accept(MEDIATYPE_JSON).contentType(MEDIATYPE_JSON).body(fooRequest).post("/graphql")
                ...
                .body(CoreMatchers.containsString(
                        "{\"errors\":[{\"message\":\"Validation error (FieldUndefined@[foo/foo]) : Field 'foo' in type 'TestPojo' is undefined\",\"locations\":[{\"line\":7,\"column\":5}],\"extensions\":{\"classification\":\"ValidationError\"}}]}"))
                .and().body(CoreMatchers.not(CoreMatchers.containsString("\"data\":null")));
    ...
    public void testCodeChange() {
        ...
        // Make a code change
        TEST.modifySourceFile("TestResource.java", s -> s.replace("// <placeholder>", "    @Query(\"pong\")\n"
