...
The server part of RESTEasy Reactive (`quarkus-resteasy-reactive` and its dependencies) provide an implementation of the Jakarta REST specification, but leverage {product-name-first}' build time processing
...
{product-name} uses smart defaults when determining the media type of Jakarta REST methods in order to simplify common use cases.
...
The Reactive REST Client (`quarkus-rest-client-reactive` and its dependencies) replace the legacy `quarkus-rest-client` but leverage {product-name}' build time processing
and the unified I/O model provided by Vert.x.
...
A couple good examples of how {product-name} uses this strategy to support both RESTEasy flavors in the core repository can be seen [here](https://github.com/quarkusio/quarkus/pull/21089) and [here](https://github.com/quarkusio/quarkus/pull/20874).

In general, it should not be needed to have two different versions of the custom extension to support both flavors. Such a choice is only strictly necessary if it is desired for the extension consumers (i.e. {product-name} applications) to not have to select a RESTEasy version themselves.
...
Any code that is run as part of a {product-name} application that used the REST Client, can safely use the Reactive REST Client, as all necessary setup for it has been done at the application's static-init phase.
