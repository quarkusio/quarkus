...
:summary: Learn more about how you can send email from a {product-name-first} application with our reactive email client.

This guide demonstrates how your {product-name} application can send emails using an SMTP server.
This is a getting started guide.
Check the xref:mailer-reference.adoc[{product-name} Mailer Reference documentation] for more complete explanation about the mailer and its usage.
...
3. Qute - the {product-name} template engine
    ...
    public void sendEmail() {
        mailer.send(
                Mail.withText("quarkus@quarkus.io",                     // <5>
                    "Ahoy from {product-name}",
                    "A simple email sent from a {product-name} application."
                )
        );
    }
...
<2> Inject the `Mailer` object managed by {product-name}
...
[source, text]
----
INFO  [quarkus-mailer] (executor-thread-0) Sending email Ahoy from {product-name} from null to [quarkus@quarkus.io], text body:
A simple email sent from a {product-name} application.
html body:
<empty>
----
...
{product-name} also offers a reactive API.
...
public Uni<Void> sendEmailUsingReactiveMailer() {       // <3>
    return reactiveMailer.send(                         // <4>
                Mail.withText("quarkus@quarkus.io",
                    "Ahoy from {product-name}",
                    "A simple email sent from a {product-name} application using the reactive API."
                )
        );
}
----
...
[source, text]
----
INFO  [quarkus-mailer] (vert.x-eventloop-thread-11) Sending email Ahoy from {product-name} from null to [quarkus@quarkus.io], text body:
A simple email sent from a {product-name} application using the reactive API.
html body:
<empty>
----
...
The {product-name} mailer is using SMTP, so make sure you have access to an SMTP server.
...
This guide has shown how to send emails from your {product-name} application.
