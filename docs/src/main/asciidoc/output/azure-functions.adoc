...
:categories: cloud
:summary: Integrate {product-name-first} with the Microsoft Azure functions that you have written.

The `quarkus-azure-functions` extension is a simple integration point between Azure Functions
and {product-name}.  It interacts with Azure Functions runtime to bootstrap quarkus and turns any
...
You can generate the example code from {product-name}'s online application generator at https://code.quarkus.io/d?e=azure-functions&cn=code.quarkus.io[this link].

You can also generate this example with the {product-name} CLI:
...
the project is similar to any other {product-name} project.
The `quarkus-azure-functions` extension is the integration point between
{product-name} and Azure Functions.  It registers callback with the Azure Functions runtime to bootstrap
{product-name} and to set up {product-name}/Arc as the function factory for your function classes.
...
deployment is now all done by {product-name}.
...
{product-name} dev mode does not work currently with Azure Functions.
...
{product-name} will use the Azure CLI in the background to authenticate and deploy to Azure.  If you have
...
If deployment is a success, {product-name} will output the endpoint URL of the example function to the console
