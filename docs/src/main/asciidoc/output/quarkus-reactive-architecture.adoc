...
:summary: Learn more about {product-name-first} reactive architecture.

{product-name} is reactive.
It's even more than this: {product-name} unifies reactive and imperative programming.
...
No need to use different stacks, tooling or APIs; {product-name} bridges both worlds.

This page will explain what we mean by _Reactive_ and how {product-name} enables it.
We will also discuss execution and programming models.
Finally, we will list the {product-name} extensions offering reactive facets.
...
{product-name} has been tailored for that.
It provides features that will help you design, implement and operate reactive systems.

== Reactive Applications

{product-name} is not only going to help you build reactive systems.
...
For this reason, {product-name} uses non-blocking I/O, which allows a low number of OS threads to manage many concurrent I/Os.
As a result, {product-name} applications allow for higher concurrency, use less memory, and improve the deployment density.

== How does Quarkus enable Reactive?

Under the hood, {product-name} has a reactive engine.
This engine, powered by Eclipse Vert.x and Netty, handles the non-blocking I/O interactions.

image::quarkus-reactive-core.png[{product-name} Reactive Core,width=50%, align=center]

{product-name} extensions and the application code can use this engine to orchestrate I/O interactions, interact with databases, send and receive messages, and so on.
...
The {product-name} architecture, based on non-blocking I/O and message passing, allows multiple supporting reactive development models that are all different in how they express continuations.
The two main ways to write reactive code with {product-name} are:
...
[TIP]
Find more info about Mutiny and its usage in {product-name} on xref:mutiny-primer.adoc[Mutiny support documentation].
...
{product-name} is inherently reactive thanks to its reactive engine.
But, you, as an application developer, don't have to write reactive code.
{product-name} unifies reactive and imperative.
It means that you can write traditional blocking applications on {product-name}.
But how do you avoid blocking the I/O threads?
{product-name} implements a https://en.wikipedia.org/wiki/Proactor_pattern[proactor pattern] that switches to worker thread when needed.

image::proactor-pattern.png[The proactor pattern in {product-name},width=50%, align=center]

Thanks to hints in your code (such as the `@Blocking` and `@NonBlocking` annotations), {product-name} extensions can decide when the application logic is blocking or non-blocking.
...
{product-name} is pragmatic and versatile.
...
{product-name} offers a large set of reactive APIs and features.
This section lists the most important, but it's not an exhaustive list.
{product-name} adds new features in every release, and the https://github.com/quarkiverse[Quarkiverse] proposes many extensions enabling _Reactive_.

=== HTTP

* RESTEasy Reactive: an implementation of Jakarta REST tailored for the {product-name} architecture.
It follows a reactive-first approach but allows imperative code using the `@Blocking` annotation.
* Reactive Routes: a declarative way to register HTTP routes directly on the Vert.x router used by {product-name} to route HTTP requests to methods.
* Reactive Rest Client: allows consuming HTTP endpoints.
Under the hood, it uses the non-blocking I/O features from {product-name}.
...
* Vert.x : the underlying reactive engine of {product-name}.
