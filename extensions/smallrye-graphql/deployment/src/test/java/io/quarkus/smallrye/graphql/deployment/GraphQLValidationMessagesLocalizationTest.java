...
/**
 * Test for localization of bean validation messages from a GraphQL endpoint, with the language determined by the
 * Accept-Language header passed by a client.
 */
public class GraphQLValidationMessagesLocalizationTest extends AbstractGraphQLTest {
    ...
    public void testAcceptSpanishBlocking() {
        ...
        RestAssured.given().body(query).contentType(MEDIATYPE_JSON).header(new Header("Accept-Language", "es"))
    ...
    public void testDefaultLocaleBlocking() {
        ...
        RestAssured.given().body(query).contentType(MEDIATYPE_JSON).post("/graphql").then().assertThat().statusCode(200)
                .and().body("errors[0].message", containsString(ERROR_MESSAGE_GERMAN));
    }
