...
This reference guide demonstrates how your {product-name-first} application can utilize SmallRye Reactive Messaging to interact with Apache Pulsar.
...
{product-name} provides support for Apache Pulsar through https://smallrye.io/smallrye-reactive-messaging/[SmallRye Reactive Messaging] framework.
...
If a subscription name is not provided {product-name} generates a unique id.
...
The {product-name} Pulsar extension supports 4 strategies:
...
[NOTE]
====
{product-name} auto-detects batch types for incoming channels and sets batch configuration automatically.
...
When using SmallRye Reactive Messaging Pulsar (`io.quarkus:quarkus-smallrye-reactive-messaging-pulsar`), {product-name} can often automatically detect the correct Pulsar Schema to configure.
...
and your configuration indicates that the `generated-price` channel uses the `smallrye-pulsar` connector, then {product-name} will automatically set the `schema` attribute of the  `generated-price` channel to Pulsar Schema `INT32`.
...
and your configuration indicates that the `my-pulsar-consumer` channel uses the `smallrye-pulsar` connector, then {product-name} will automatically set the `schema` attribute to Pulsar `BYTES` Schema.
...
and your configuration indicates that the `price-create` channel uses the `smallrye-pulsar` connector, then {product-name} will automatically set the `schema` to Pulsar `INT64` Schema.
...
In addition to those Pulsar-provided schemas, {product-name} provides following schema implementations _without enforcing validation_ :
...
The {product-name} extension reports startup, readiness and liveness of each channel managed by the Pulsar connector.
...
This guide has shown how you can interact with Pulsar using {product-name}.
