...
public class WebAuthnOriginsTest {
    ...
    public void testLoginRpFromFirstOrigin() {
        RestAssured.given().contentType(ContentType.URLENC).queryParam("username", "foo")
                .get("/q/webauthn/register-options-challenge").then().log().all().statusCode(200)
                .contentType(ContentType.JSON).body("rp.id", Matchers.equalTo("foo"));
    }

    @Test
    public void testWellKnownConfigured() {
        RestAssured.get("/.well-known/webauthn").then().statusCode(200).contentType(ContentType.JSON)
                .body("origins.size()", Matchers.equalTo(2)).body("origins[0]", Matchers.equalTo("http://foo"))
