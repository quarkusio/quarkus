        ...
        return new Consumer<MetricsFactory>() {
            ...
            public void accept(MetricsFactory metricsFactory) {
                ...
                metricsFactory.builder("agroal.awaiting.count")
                        ...
                        .tag("datasource", tagValue).buildGauge(metrics::awaitingCount);

                metricsFactory.builder("agroal.acquire.count")
                        .description("Number of times an acquire operation succeeded.").tag("datasource", tagValue)
                        .buildCounter(metrics::acquireCount);
                metricsFactory.builder("agroal.creation.count").description("Number of created connections.")
                        .tag("datasource", tagValue).buildCounter(metrics::creationCount);
                metricsFactory.builder("agroal.leak.detection.count").description(
                        "Number of times a leak was detected. A single connection can be detected multiple times.")
                        .tag("datasource", tagValue).buildCounter(metrics::leakDetectionCount);
                metricsFactory.builder("agroal.destroy.count").description("Number of destroyed connections.")
                        .tag("datasource", tagValue).buildCounter(metrics::destroyCount);
                metricsFactory.builder("agroal.flush.count")
                        ...
                        .tag("datasource", tagValue).buildCounter(metrics::flushCount);
                metricsFactory.builder("agroal.invalid.count")
                        .description("Number of connections removed from the pool for being idle.")
                        .tag("datasource", tagValue).buildCounter(metrics::invalidCount);
                metricsFactory.builder("agroal.reap.count")
                        .description("Number of connections removed from the pool for being idle.")
                        .tag("datasource", tagValue).buildCounter(metrics::reapCount);
                ...
                metricsFactory.builder("agroal.blocking.time.average")
                        ...
                        .tag("datasource", tagValue).unit("milliseconds")
                ...
                metricsFactory.builder("agroal.creation.time.average")
                        .description("Average time for a connection to be created.").tag("datasource", tagValue)
                        .unit("milliseconds").buildGauge(metrics::creationTimeAverage, convertToMillis);
                metricsFactory.builder("agroal.creation.time.max")
                        .description("Maximum time for a connection to be created.").tag("datasource", tagValue)
                        .unit("milliseconds").buildGauge(metrics::creationTimeMax, convertToMillis);
                metricsFactory.builder("agroal.creation.time.total")
                        .description("Total time waiting for connections to be created.").tag("datasource", tagValue)
                        .unit("milliseconds").buildGauge(metrics::creationTimeTotal, convertToMillis);
            }
        };
    }
}
