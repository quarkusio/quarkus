...
When using Hibernate Reactive in {product-name-first}, you need to:
...
Also, {product-name} will set many Hibernate Reactive configuration settings automatically, and will often use more modern defaults.
...
A `Mutiny.SessionFactory` will be created based on the {product-name} `datasource` configuration as long as the Hibernate Reactive extension is listed among your project dependencies.
...
There are various optional properties useful to refine your session factory or guide {product-name}' guesses.

When no properties are set, {product-name} can typically infer everything it needs to set up Hibernate Reactive
...
If you are familiar with using Hibernate Reactive in {product-name}, you probably already have injected the `Mutiny.SessionFactory` using CDI:
...
NOTE: Prior to {product-name} 3.0 it was also possible to inject a `@RequestScoped` bean for `Mutiny.Session`. However, the lifecycle of a reactive session does not fit the lifecycle of the CDI request context. Therefore, this bean is removed in {product-name} 3.0.
...
{product-name} does not modify the libraries it uses; this rule applies to Hibernate Reactive as well: when using
this extension you will mostly have the same experience as using the original library.

But while they share the same code, {product-name} does configure some components automatically and inject custom implementations
...
Here's a list of things to pay attention when using Hibernate Reactive in {product-name}:
...
by providing active record style entities (and repositories) and focuses on making your entities trivial and fun to write in {product-name}.
