...
{product-name-first} has an integrated pluggable web security layer.
...
Permissions are defined in the {product-name} configuration using permission sets, each specifying a policy for access control.
...
For more information, see link:https://quarkus.io/blog/path-resolution-in-quarkus/[Path Resolution in {product-name}].
...
{product-name} also provides the `io.quarkus.security.PermissionsAllowed` annotation, which authorizes any authenticated user with the given permission to access the resource.
...
NOTE: `@PermissionsAllowed` is not repeatable on the class level due to a limitation with {product-name} interceptors.
For more information, see the xref:cdi-reference.adoc#repeatable-interceptor-bindings[Repeatable interceptor bindings] section of the {product-name} "CDI reference" guide.
...
Later, {product-name} instantiates your custom permission with actual arguments, with which the method annotated with the `@PermissionsAllowed` has been invoked.
...
{product-name} can optionally pass permission actions to the constructor.
...
* xref:security-authentication-mechanisms.adoc#other-supported-authentication-mechanisms[Authentication mechanisms in {product-name}]
