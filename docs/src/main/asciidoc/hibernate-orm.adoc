...
+
By default, the script is executed in dev and test modes only.
Set `quarkus.hibernate-orm.data-management.strategy` to `none` to disable its execution
or to `create` to execute it in production as well,
...
[NOTE]
====
With <<multiple-persistence-units,multiple persistence units>>, the default files (`import.sql`/`data.sql`)
are picked up by each of them, which suits persistence units mapping the same entities to different databases.
If your persistence units map different entities, either (preferably) don't include SQL files with the default names in your app,
or (if you need to) set `quarkus.hibernate-orm."<name>".data-management.init-script`
and `quarkus.hibernate-orm."<name>".schema-management.init-script` explicitly for each persistence unit
...
* `quarkus.hibernate-orm.data-management.strategy` is set to `none` for all profiles except the `dev` and `test` ones,
  so the data init script (`data.sql`) is not executed, even if it is configured explicitly.
