...
[CAUTION]
====
We only support setting `@PersistenceUnit` at the package level,
using the `@PersistenceUnit` annotation at the class level is not supported.
====
...
----
<1> Be careful to use the `@io.quarkus.hibernate.orm.PersistenceUnit` annotation, not the JPA one.
====
