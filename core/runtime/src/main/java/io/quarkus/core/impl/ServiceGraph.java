...
public final class ServiceGraph {
    ...
    /**
     * The service monitor used to track service state transitions.
     */
     ...
     */
    public ServiceGraph() {
        this(new StartupContext(), NoOpServiceMonitor.INSTANCE);
    }
    ...
    public ServiceGraph(StartupContext startupContext) {
        this(startupContext, NoOpServiceMonitor.INSTANCE);
    }

    /**
     * Construct a new service graph with the given startup context and monitor.
     *
     * @param startupContext the startup context for legacy recorder compatibility
     *        (must not be {@code null})
     * @param monitor the service monitor used for JFR and lifecycle metrics (must not be {@code null})
     */
    public ServiceGraph(StartupContext startupContext, ServiceMonitor monitor) {
        this.executor = mainThreadExecutor;
        this.startupContext = startupContext;
        this.monitor = monitor != null ? monitor : NoOpServiceMonitor.INSTANCE;
    }

    /**
     * Get the service monitor for this graph.
     *
     * @return the service monitor (never {@code null})
     */
