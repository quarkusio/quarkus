...
Setting `quarkus.http.auth.basic=true` enables basic authentication for your Quarkus project. When enabled, users must authenticate by entering their username and password.

For more information, see the xref:security-basic-authentication-howto.adoc[Enable basic authentication] how-to topic.
