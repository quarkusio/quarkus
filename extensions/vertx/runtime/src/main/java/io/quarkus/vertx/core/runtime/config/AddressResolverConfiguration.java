...
public interface AddressResolverConfiguration {
     ...
     * <p/>
     * The resolver caches the hosts configuration (configured using `quarkus.vertx.resolver.hosts-path` after it has
     * read it. When the content of this file can change, setting a positive refresh period will load the configuration
    ...
    /**
     * Set the list of DNS server addresses, an address is the IP of the dns server, followed by an optional colon and a
     * port, e.g {@code 8.8.8.8} or {@code 192.168.0.1:40000}. When the list is empty, the resolver will use the list of
     * the system DNS server addresses from the environment, if that list cannot be retrieved it will use Google's
     * public DNS servers {@code "8.8.8.8"} and {@code "8.8.4.4"}.
     **/
    Optional<List<String>> servers();

    /**
     * Set to true to enable the automatic inclusion in DNS queries of an optional record that hints the remote DNS
     * server about how much data the resolver can read per response.
     */
    @WithDefault("false")
     ...
     * <p/>
     * When the search domain list is null, the effective search domain list will be populated using the system DNS
     * search domains.
     */
    Optional<List<String>> searchDomains();

    /**
     * Set the ndots value used when resolving using search domains, the default value is {@code -1} which determines
     * the value from the OS on Linux or uses the value {@code 1}.
     */
    @WithDefault("-1")
    int ndots();

    /**
     * Set to {@code true} to enable round-robin selection of the dns server to use. It spreads the query load among the
     * servers and avoids all lookup to hit the first server of the list.
     */
