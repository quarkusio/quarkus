...
public class DependencySbomMojo extends AbstractMojo {
    ...
    /**
     * File to store the SBOM in. If not configured, the SBOM will be stored in the ${project.build.directory}
     * directory.
     */
    ...
    public void execute() throws MojoExecutionException, MojoFailureException {
        ...
        CycloneDxSbomGenerator.newInstance()
                .setManifest(ApplicationManifest.fromConfig(
                        ApplicationManifestConfig.builder().setApplicationModel(resolveApplicationModel()).build()))
                .setOutputFile(outputFilePath).setFormat(format)
                .setEffectiveModelResolver(EffectiveModelResolver.of(getResolver())).setSchemaVersion(schemaVersion)
                .setIncludeLicenseText(includeLicenseText).generate();
    ...
    private ApplicationModel resolveApplicationModel() throws MojoExecutionException {
        ...
        try {
            modelResolver = new BootstrapAppModelResolver(getResolver()).setRuntimeModelOnly(runtimeOnly);
            if (mode != null) {
    ...
    protected MavenArtifactResolver getResolver() {
        if (resolver == null) {
            resolver = workspaceProvider.createArtifactResolver(
                    BootstrapMavenContext.config().setUserSettings(session.getRequest().getUserSettingsFile())
                            // The system needs to be initialized with the bootstrap model builder to properly
                            // interpolate system properties set on the command line
