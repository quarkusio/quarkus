...
Secure HTTP access to Jakarta REST (formerly known as JAX-RS) endpoints in your application with Bearer token authentication by using the {product-name-first} OpenID Connect (OIDC) extension.

== Overview of the Bearer token authentication mechanism in Quarkus

{product-name} supports the Bearer token authentication mechanism through the {product-name} OpenID Connect (OIDC) extension.
...
The following diagrams outline the Bearer token authentication mechanism in {product-name}:

.Bearer token authentication mechanism in {product-name} with Single-page application
image::security-bearer-token-authorization-mechanism-1.png[alt=Bearer token authentication, width="60%", align=center]

1. The {product-name} service retrieves verification keys from the OpenID Connect provider. The verification keys are used to verify the bearer access token signatures.
2. The {product-name} user accesses the Single-page application.
...
4. The Single-page application uses the access token to retrieve the service data from the {product-name} service.
5. The {product-name} service verifies the bearer access token signature using the verification keys, checks the token expiry date and other claims, allows the request to proceed if the token is valid, and returns the service response to the Single-page application.
6. The Single-page application returns the same data to the {product-name} user.

.Bearer token authentication mechanism in {product-name} with Java or command line client
...
3. The Client uses the access token to retrieve the service data from the {product-name} service.
4. The {product-name} service verifies the bearer access token signature using the verification keys, checks the token expiry date and other claims, allows the request to proceed if the token is valid, and returns the service response to the Client.
...
to access {product-name} `service` applications.
...
[NOTE]
====
`OidcWiremockTestResource` does not work with `@QuarkusIntegrationTest` against Docker containers, because the Wiremock server is running in the JVM running the test, which cannot be accessed from the Docker container running the {product-name} application.
====
...
copy `privateKey.pem` from the `integration-tests/oidc-tenancy` in the `main` {product-name} repository and use a test code similar to the one in the `Wiremock` section above to generate JWT tokens. You can use your own test keys if preferred.
...
`quarkus.oidc.client-id` property identifies an OpenID Connect Client which requested the current bearer token. It can be an SPA application running in a browser or a {product-name} `web-app` confidential client application propagating the access token to the {product-name} `service` application.
...
Note {product-name} `web-app` applications always require `quarkus.oidc.client-id` property.
...
* xref:security-keycloak-admin-client.adoc[{product-name} Keycloak Admin Client]
