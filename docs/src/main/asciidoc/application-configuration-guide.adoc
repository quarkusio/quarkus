...
Then you need to include fully qualified class name of converter in file `META-INF/services/org.eclipse.microprofile.config.spi.Converter`.
...
In some cases you may want to use custom converter to convert type which is already converted
by a different converter. In such case you can use `javax.annotation.Priority` annotation to
change converters precedence and make your custom converter of higher priority then the other
on the list.

By default, if no `@Priority` can be found on a converter, it's registered with priority of 100
and all {project-name} core converters are registered with priority of 200, so depending on which
converter you would like to replace, you need to set higher value.

To demonstrate the idea let us implement custom converter which will take precedence over 
`MicroProfileCustomValueConverter` implemented in previous example.
...
NOTE: This new converter also needs to be listed in a service file.
