...
{product-name-first} includes the `kubernetes-client` extension which enables the use of the https://github.com/fabric8io/kubernetes-client[Fabric8 Kubernetes Client]
in native mode while also making it easier to work with.

Having a Kubernetes Client extension in {product-name} is very useful in order to unlock the power of Kubernetes Operators.
...
Once you have your {product-name} project configured you can add the `kubernetes-client` extension
...
{product-name} configures a Bean of type `KubernetesClient` which can be injected into application code using the well known CDI methods.
...
{product-name} provides multiple integration points for influencing the Kubernetes Client provided as a CDI bean.
...
it allows for arbitrary customizations of the `io.fabric8.kubernetes.client.Config` created by {product-name} (which takes into account the `quarkus.kubernetes-client.*` properties).
...
To make testing against a mock Kubernetes API extremely simple, {product-name} provides the `WithKubernetesTestServer` annotation which automatically launches
...
thus making {product-name} unable to properly determine the Kubernetes model class for which reflection registration is needed:
