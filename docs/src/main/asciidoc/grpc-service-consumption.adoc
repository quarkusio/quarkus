...
The HTTP/2 `:authority` pseudo-header defaults to `localhost` when using a domain socket. You can override it with:

[source,properties]
----
quarkus.grpc.clients.hello.domain-socket-authority=my-service
----
