...
NOTE: An integrator must also provide a corresponding `org.jboss.quarkus.arc.ResourceReferenceProvider` implementation. 
...
In {project-name} we produce a `BeanRegistrarBuildItem` and leverage the `org.jboss.quarkus.arc.processor.BeanConfigurator` API to build a synthetic bean definition.
...
NOTE: See also `org.jboss.quarkus.arc.processor.BuildExtension.Key` to discover the available metadata.
