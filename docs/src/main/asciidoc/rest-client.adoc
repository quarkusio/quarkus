...
[WARNING]
====
An exception returned by your own `ResponseExceptionMapper` or `@ClientExceptionMapper` is thrown as is.
Only the exceptions produced by the default mapper are wrapped in `org.jboss.resteasy.reactive.ClientWebApplicationException`.

If your mapper returns a `jakarta.ws.rs.WebApplicationException` and the exception is not caught, Quarkus REST treats it like an exception thrown by your own endpoint.
In particular, when the exception carries the `Response` received from the remote service, that status, headers and body are sent to your caller.
To avoid leaking upstream responses, return an exception that does not extend `WebApplicationException`, or one that extends `ClientWebApplicationException`, which Quarkus REST reports as an internal server error instead.
====
