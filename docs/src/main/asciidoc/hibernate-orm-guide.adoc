...
WARNING: An active transaction is required for every method that modifies the database. You can create a container managed 
transaction using the `@Transactional` annotation on any CDI bean. We recommend doing so in your
...
or jars, you should build those jars with the Jandex Maven plugin, so that your entities will be indexed and enhanced by Quarkus:
