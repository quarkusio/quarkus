...
////
= Quarkus - Using our Spring Data JPA compatibility layer

include::./attributes.adoc[]

While you are encouraged to use Hibernate ORM with Panache for Relational Database access, Quarkus provides a compatibility layer for
...
Throughout the course of this guide, we will be using the following JPA Entity:
...
Add the following properties to `application.properties` to configure access to a local PostgreSQL.
...
To make it easier to showcase some capabilities of Spring Data JPA on Quarkus, we will ensure that the database contains some test data.
That is done by adding the following content to a new file named `src/main/resources/import.sql`:
...
It is now time to define the repository we will use to access `Fruit`.
...
Additionally we define `findByColor` whose purpose is to return all Fruit entities that match the specified color.

== Update the JAX-RS resource

With the repository in place, the next order of business is to update our JAX-RS resource that will use the `FruitRepository`. Open `FruitResource` and change its contents to:
...
public class FruitResource {
    ...
    @Path("/color/{color}")
    @Produces("application/json")
    public List<Fruit> all(@PathParam("color") String color) {
...
See the link:spring-web-guide.html[Spring Web guide] for more details.

== Update the test

To test the capabilities of `FruitRepository`, go ahead and update the content of `FruitResourceTest` to:
...
Quarkus dev mode works with the defined repositories just like any other Quarkus extension, greatly enhancing your productivity during the dev cycle.
...
== Supported Spring Data JPA functionality

Quarkus currently supports a subset of Spring Data JPA's features that we feel are the most useful and most commonly used.
...
Assume for example we have a `PersonRepository` that shouldn't extend `CrudRepository` but would like to use `save` and `findById` methods which are defined in said interface.
In such a case we would simply create `PersonRepository` like so:
...
An extensive list of examples can be seen in the `integration-tests/spring-data` directory which is located inside the Quarkus source code.
...
is not used at all (since all the necessary plumbing is done at build time).
We are exploring ways to provide similar functionality in the Quarkus support of Spring Data JPA.
...
Quarkus is exploring various alternatives to bridging the gap between the JPA and Reactive worlds.
...
* link:spring-di-guide.adoc[Spring DI]
