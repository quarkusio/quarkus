...
In this guide, we will see how you can get your REST services to use Elasticsearch.

Quarkus provides two ways to access Elasticsearch: via the `RestClient` or via the `RestHighLevelClient` we will call them
the low level and the high level client.
...
The solution for the low level client is located in the `elasticsearch-low-level-client-quickstart` {quickstarts-tree-url}/elasticsearch-low-level-client-quickstart[directory].

The solution for the high level client is located in the `elasticsearch-high-level-client-quickstart` {quickstarts-tree-url}/elasticsearch-high-level-client-quickstart[directory].
...
This command generates a Maven structure importing the RESTEasy/JAX-RS, Jackson, and the Elasticsearch lowl level client extensions.
After this, the quarkus-elasticsearch-low-level-client extension has been added to your `pom.xml`.

If you want to use the high level client instead, replace the `elasticsearch-low-level-client` by the `elasticsearch-high-level-client`.
...
Here we use the low level client, if you want to use the high level client instead follow the instruction in the link:#using-the-high-level-rest-client[Using the High Level Rest Client] instead.
...
As by default, the Elasticsearch client is configured to access a local Elasticsearch on port 9200 (the default Elasticsearch port),
...
Quarkus provide support for the Elasticsearch High Level RestClient but keep in mind that it comes with some caveats:
...
If you are using the `quarkus-smallrye-health` extension, both low level and high level extensions will automatically add a readiness health check
...
You can use the both clients in a native executable.
...
Accessing an Elasticsearch database from a low level or a high level Client is easy with Quarkus as it provides configuration and native support for it.
