...
Quarkus can also serve static resources directly from a local directory on the filesystem.
This is configured via the `quarkus.http.static-dir.*` properties.


To expose the contents of a `static/` path under the `/static` endpoint:
...
For applications with more advanced static hosting needs (CDN, SPA routing, cache-control),
you may prefer serving resources from the application JAR or through a reverse proxy.
