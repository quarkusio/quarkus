...
public class HibernateReactiveDevModeTest {
    ...
    public void testListAllFruits() {
        Response response = given().when().get("/fruits").then().statusCode(200).contentType("application/json")
                .extract().response();
        ...
        given().when().get("/fruits").then().statusCode(500);

        runner.modifySourceFile(Fruit.class, s -> s.replace("desk", "desc"));
        response = given().when().get("/fruits").then().statusCode(200).contentType("application/json").extract()
                .response();
