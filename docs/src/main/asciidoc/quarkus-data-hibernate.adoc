...
- Make it extend `ManagedEntity.AutoLong`
...
public class Cat extends ManagedEntity.AutoLong {
    ...
    public interface Repo extends ManagedRepository.AutoLong<Cat> {
        @Find
...
- Which extends the `ManagedRepository.AutoLong` interface, with the entity in question as type parameter
...
In the example above, we extended the `ManagedEntity.AutoLong` type, and did not define any database identifier, that's
because `ManagedEntity.AutoLong` comes with a default generated database identifier, so you don't have to worry about it. It
...
Naturally, you can also provide your own database identifier explicitly and implement the `ManagedEntity`
interface in your entity, as well as use the `ManagedRepository` interface for your repository, in order to
...
@Entity
public class CatWithId implements ManagedEntity {
    @Id
    public String id;
    public String name;
    public LocalDate birth;
    ...
    public interface Repo extends ManagedRepository<CatWithId, String> {
        @Find
...
In this case, you need to extend the `RecordEntity.AutoLong` class and implement the
`RecordRepository.AutoLong` interfaces:
...
@Entity
public class Cat extends RecordEntity.AutoLong {
    public String name;
    public LocalDate birth;
    public Breed breed;
    ...
    public interface Repo extends RecordRepository.AutoLong<Cat, Long> {
        @Find
        Cat findByName(String name);
...
- Your entity implements `ManagedEntity.Reactive`
...
@Entity
public class Cat extends WithId.AutoLong implements ManagedEntity.Reactive {
    public String name;
    public LocalDate birth;
    public Breed breed;
    ...
    public interface Repo extends ManagedRepository.Reactive<Cat, Long> {
        @Find
...
@Entity
public class Cat extends WithId.AutoLong implements RecordEntity.Reactive {
    public String name;
    public LocalDate birth;
    public Breed breed;
    ...
    public interface Repo extends RecordRepository.Reactive<Cat, Long> {
        @Find
        Uni<Cat> findByName(String name);
...
|`ManagedRepository.Managed<Cat, Long>`

|`StatelessSession`
|`Cat_.statelessBlocking()`
|`RecordRepository.Stateless<Cat, Long>`

|`Mutiny.Session`
|`Cat_.managedReactive()`
|`ManagedRepository.Reactive<Cat, Long>`

|`Mutiny.StatelessSession`
|`Cat_.statelessReactive()`
|`RecordRepository.Reactive<Cat, Long>`
...
public class Cat extends ManagedEntity.AutoLong {
    ...
    public interface TheOtherRepo extends RecordRepository.Reactive<Cat, Long> {
        @Find
        Uni<Cat> findByName(String name);
    }
}
...
|`ManagedEntity.AutoLong`
|
|`ManagedRepository.AutoLong<Entity>`

|`Session` (managed, blocking)
|`Id`
|`WithId<Id>`
|`ManagedEntity`
|`ManagedRepository<Entity, Id>`

|`StatelessSession` (stateless, blocking)
|`Id`
|`WithId<Id>`
|`RecordEntity`
|`RecordRepository<Entity, Id>`

|`Mutiny.Session` (managed, reactive)
|`Id`
|`WithId<Id>`
|`ManagedEntity.Reactive`
|`ManagedRepository.Reactive<Entity, Id>`
...
|`WithId<Id>`
|`RecordEntity.Reactive`
|`RecordRepository.Reactive<Entity, Id>`
