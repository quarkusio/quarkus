...
We record a timestamp immediately before launching the application, then poll the application's HTTP endpoint until it returns a 200 response. The difference is the Time to First Request (TTFR).
