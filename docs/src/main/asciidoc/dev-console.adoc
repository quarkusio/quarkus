...
This guide covers the Quarkus DEV Console.
...
You have to add a file named `dev-templates/embedded.html` in your `deployment`
extension resources:
...
You need to place them in your `/dev-console` resource folder,
like this page for the link:/guides/cache[`quarkus-cache` extension]:
...
package in a `DevConsoleProcessor` class (in your extension's `deployment` module).
...
declare the action in your extension's `deployment` module:
...
----
<1> While you can use any Vert.x handler, the `DevConsolePostHandler` superclass will handle your POST actions
...
NOTE: the flash messages are handled by the `main` DEV template and will result in nice notifications for your
users:
