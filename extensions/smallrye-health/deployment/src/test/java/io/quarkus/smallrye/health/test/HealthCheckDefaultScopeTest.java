...
class HealthCheckDefaultScopeTest {
    ...
    static final QuarkusUnitTest config = new QuarkusUnitTest().withApplicationRoot(
            (jar) -> jar.addClasses(NoScopeCheck.class, NoScopeStereotypeWithoutScopeCheck.class, MyStereotype.class));
    ...
    void testHealth() {
        ...
        try {
            ...
            when().get("/q/health/live").then().body("status", is("UP"), "checks.status", hasItems("UP", "UP"),
                    "checks.name", hasItems("noScope", "noScopeStereotype"));
            when().get("/q/health/live").then().body("status", is("DOWN"), "checks.status", hasItems("DOWN", "DOWN"),
                    "checks.name", hasItems("noScope", "noScopeStereotype"));
        } finally {
            RestAssured.reset();
        }
