...
/**
 * Reproducer for <a href="https://github.com/quarkusio/quarkus/issues/53393">issue #53393</a>
 */
...
public class MainHttpServerTlsPemTrustStoreCertificateReloadTest {
    ...
    static final QuarkusExtensionTest config = new QuarkusExtensionTest()
            ...
            .overrideConfigKey("quarkus.http.ssl.certificate.trust-store-file", temp.getAbsolutePath() + "/ca.crt")
    ...
    @Test
    void testReloadWithPemTrustStoreDoesNotThrowClassCastException()
        ...
        Files.copy(new File("target/certificates/reload-B-ca.crt").toPath(),
                new File(certs, "/ca.crt").toPath(), java.nio.file.StandardCopyOption.REPLACE_EXISTING);

        // Trigger the reload - this is where the ClassCastException occurs (issue #53393)
        TlsCertificateReloader.reload().toCompletableFuture().get(10, TimeUnit.SECONDS);

        // Verify the server is still working after reload
