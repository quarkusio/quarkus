...
| *Observability*
| A dashboard of telemetry cards you compose yourself: application metrics, plus a card per signal (such as OpenTelemetry traces) contributed by the telemetry extensions in use. Only present when at least one such extension is present.
...
As soon as one extension contributes a telemetry signal, Dev UI adds a single *Observability*
entry to the left menu, and that entry is a dashboard the user composes: they pick the cards they
want the first time they open it, change their mind later with *Edit dashboard*, and drag the
cards into the order that suits them. The choice is kept in the browser's local storage rather
than per application, so it follows the developer from one application to the next.

Extensions contribute to that dashboard in two ways.

==== Contributing a signal card

If your extension captures a telemetry signal (traces, logs, events, profiles, ...), produce an
`ObservabilitySignalBuildItem` from `quarkus-devui-deployment-spi` alongside the page that shows
it. The dashboard then offers that page as a card, so there is no second component to write:

[source,java]
----
@BuildStep(onlyIf = IsLocalDevelopment.class)
void tracesPage(BuildProducer<UnlistedPageBuildItem> unlistedPages,
        BuildProducer<ObservabilitySignalBuildItem> signals) {

    UnlistedPageBuildItem page = new UnlistedPageBuildItem(); // <1>
    ...
    signals.produce(new ObservabilitySignalBuildItem(
            "traces", // <2>
            "OpenTelemetry Traces", // <3>
            "font-awesome-solid:diagram-project", // <4>
            "quarkus-opentelemetry/traces", // <5>
            "spanCount")); // <6>
}
----
<1> The page that backs the card. It has to be an unlisted page: the dashboard resolves the page
id against the unlisted pages only, and a signal pointing at a card, menu or footer page is
dropped without an error, leaving you with a signal that never shows up as a card.
<2> A key unique among all signals. It identifies the card in the stored dashboard, so changing
it drops the card from dashboards that already have it.
<3> The title shown on the card and in the picker. Name the backend here, not just the signal:
the picker lists every extension's signals together, so a bare "Traces" is ambiguous.
<4> The icon, from the Dev UI icon set.
<5> The id of the page above. Dev UI derives it as `<namespace>/<title-in-lowercase-with-dashes>`,
where the namespace defaults to your extension's name. Pass `null` if the signal has no page of
its own: the dashboard then knows the feature is present but contributes no card for it (this is
what metrics does, since it expands into one card per meter instead).
<6> Optional name of a JSON-RPC method returning a live count. Reserved for a future count on the
card; pass `null` if you have none.

The card renders your component inline, in a box 320 pixels tall (420 for a card spanning the
full width), with an *Open* link through to the full page. Size the component against its host
(`height: 100%`, or a flex column) rather than against the viewport - the card body scrolls what
does not fit, and the user can always maximize the card to give it the whole dashboard area.
Nothing else changes: the component is imported only when the card is actually on the dashboard,
and its JSON-RPC calls, subscriptions and hot reload behave exactly as they do on the full page.

Extend `ObservabilityCardBase` rather than `QwcHotReloadElement` in that component. Core Dev UI
puts it in the import map under the bare specifier `observability-card-base`; it extends
`QwcHotReloadElement` itself, so you keep `hotReload()`, and adds the affordances every signal
component ends up wanting, currently `exportCsv(rows, filename)` and `exportJson(value, filename)`:
...
Meters need no work at all. Every meter registered with Micrometer or with the OpenTelemetry SDK
is listed in the dashboard's picker automatically, and the dashboard chooses how to draw the ones
the user adds. That choice follows what was actually captured rather than the declared meter
type: a rate for a counter, a dial for a gauge, and mean and max per interval for a timer or a
distribution summary, joined by percentiles when the meter publishes them and replaced by bucket
bars when it publishes a histogram. If your extension exposes metrics, simply register them as
usual.

Writing a new metrics *backend* is the exception. A backend samples meters into the shared
`MetricsTimeSeriesStore` and tells the core Dev UI it did so by producing a
`MetricsBackendBuildItem` naming itself (`new MetricsBackendBuildItem("micrometer")`); the core
then registers the JSON-RPC service and the metrics signal once, no matter how many backends are
active. See `DevUiMetricsSampler` (Micrometer) and `DevUiMetricsExporter` (OpenTelemetry) for the
two existing implementations.

A backend should also say how the same metrics are named once the application exports them, with
the second argument of the build item (`micrometer-prometheus` or `otlp`). That is a different
question from where the Dev UI captured them: the dashboard stores each meter's own name, while a
Grafana dashboard has to query the name the meter carries after export, which the two routes spell
differently. Leave it `null` when the export naming is not known, and the *Export Grafana* button
below is simply not offered for those meters.

==== Exporting the dashboard to Grafana

The dashboard exports itself as a Grafana dashboard, for a Grafana that holds the same telemetry:
the one xref:observability-devservices-lgtm.adoc[the LGTM Dev Service] starts, or any other
reading the same Prometheus and Tempo. *Export Grafana* downloads a dashboard file, which Grafana
imports under *Dashboards -> New -> Import*; it asks which data sources to use, so the file is not
tied to one Grafana.

Each card becomes the panel that draws the same statistic it draws in Dev UI: a counter becomes a
rate, a gauge a Grafana gauge, a timer the mean of each interval with its maximum beside it, a
meter with a histogram a heatmap over its buckets, and the traces card a Tempo panel for this
application's service name. What the panels query depends on how the application exports its
metrics, which is why a card whose meters take an export route Dev UI does not know is left out.
