...
With Hibernate ORM 8, Java Time values use direct JDBC access by default when the database driver supports it.
In particular, `quarkus.hibernate-orm.jdbc.timezone` does not adjust values handled through direct JDBC access:
`OffsetDateTime`, `OffsetTime`, and supported `ZonedDateTime` mappings may retain their offsets even with
`quarkus.hibernate-orm.mapping.timezone.default-storage` set to `normalize` or `normalize-utc`.
When upgrading an application with existing temporal data, verify how the new mappings read that data and migrate it if necessary.
...
With `none` or `callback` alone, validation constraints do not affect the generated DDL.
