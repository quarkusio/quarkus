...
:summary: This guide demonstrates how your {product-name-first} application can utilize SmallRye Reactive Messaging to interact with AMQP.

This guide demonstrates how your {product-name} application can utilize SmallRye Reactive Messaging to interact with AMQP 1.0.
...
This command creates the project structure and select the two {product-name} extensions we will be using:
...
{product-name} has built-in capabilities to deal with JSON AMQP messages.

[NOTE]
.@RegisterForReflection
====
The `@RegisterForReflection` annotation instructs {product-name} to include the class (including fields and methods) when building the native executable.
...
If not indicated otherwise, like in this example, {product-name} uses the channel name as AMQP queue name.
...
{product-name} starts a AMQP broker automatically, configures the application and shares the broker instance between different applications.
...
The `-Dquarkus.native.container-build=true` instructs {product-name} to build Linux 64bits native executables, who can run inside containers.
...
This guide has shown how you can interact with AMQP 1.0 using {product-name}.
