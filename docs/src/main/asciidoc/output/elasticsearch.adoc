...
{product-name-first} provides two ways of accessing Elasticsearch:
...
[WARNING]
====
A third {product-name} extension for the "high level REST Client" used to exist,
...
What that means practically is that, if you have not configured `quarkus.elasticsearch.hosts`, {product-name} will automatically
...
{product-name} supports Hibernate Search with Elasticsearch via the `quarkus-hibernate-search-orm-elasticsearch` extension.
...
Accessing an Elasticsearch cluster from the low level REST client or the Elasticsearch Java client is easy with {product-name} as it provides easy configuration, CDI integration and native support for it.
