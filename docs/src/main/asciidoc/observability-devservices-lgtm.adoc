...
=== LGTM and AI (MCP support)

The grafana/otel-lgtm image integrates with Model Context Protocol (MCP) so AI coding tools (Claude, Cursor, etc.) can query your telemetry data directly.
More info can be found here: https://github.com/grafana/docker-otel-lgtm/blob/main/docs/mcp-integration.md

It's easy to use this with https://docs.quarkiverse.io/quarkus-langchain4j/dev/index.html[Quarkus' LangChain4j integration] - a library that lets you build Java applications that seamlessly connect to LLMs, tools, and embeddings using CDI, configuration, and native-friendly APIs.

You can expose the LGTM MCP tools via properties:
...
[source, log]
----
[io.qu.ob.de.ObservabilityDevServiceProcessor] (build-35) Dev Service Lgtm started, config: {grafana.endpoint=http://localhost:42797, tempo-mcp.endpoint=http://localhost:55678, quarkus.otel.exporter.otlp.endpoint=http://localhost:34711, otel-collector.url=localhost:34711, quarkus.micrometer.export.otlp.url=http://localhost:34711/v1/metrics, quarkus.otel.exporter.otlp.protocol=http/protobuf}
----
