...
[NOTE]
For non JAX-RS inbound request like from Vert.x, REST Client provides a `VertxRequestClientHeadersFactory`.
It looks the `org.eclipse.microprofile.rest.client.propagateHeaders` property propagate the configured headers from `io.vertx.core.http.HttpServerRequest` requests.
