...
:summary: Use Micrometer to collect metrics produced by {product-name-first}, its extensions, and your application.
...
IMPORTANT: Micrometer is the recommended approach to metrics for {product-name}.
...
{product-name} Micrometer extensions are structured in the same way as the Micrometer project. 
The `quarkus-micrometer` extension provides core Micrometer support and runtime integration.
Other {product-name} and Quarkiverse extensions use the {product-name} Micrometer extension to provide support for other monitoring systems.
...
It will bring in the {product-name} Micrometer extension and Micrometer core libraries as dependencies.
...
Most Micrometer registry extensions, like `quarkus-micrometer-registry-statsd`, provide registry-specific configuration objects that are integrated with the {product-name} configuration model.
...
{product-name} provides a variety of way to customize Micrometer.
