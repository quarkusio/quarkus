...
[NOTE]
====
* The Quarkus configuration for vale is `vale.ini`, which is in the `docs/.vale` directory.
...
Each has its own configuration requirements. The Visual Studio Code IDE extension, for example, requires two workspace settings:
