...
/**
 * Generates SBOMs for packaged applications if the corresponding config is enabled. The API around this is still in
...
public class CdxSbomBuildStep {
     ...
     *
     * @param applicationManifestsBuildItem
     *        application manifests
     * @param outputTargetBuildItem
     *        build output
     * @param appModelProviderBuildItem
     *        application model provider
     * @param cdxSbomConfig
     *        CycloneDX SBOM generation configuration
     * @param sbomProducer
     *        SBOM build item producer
     */
    @BuildStep
    public void generate(ApplicationManifestsBuildItem applicationManifestsBuildItem,
            OutputTargetBuildItem outputTargetBuildItem, AppModelProviderBuildItem appModelProviderBuildItem,
            CycloneDxConfig cdxSbomConfig, BuildProducer<SbomBuildItem> sbomProducer) {
        ...
        for (var manifest : applicationManifestsBuildItem.getManifests()) {
            for (var sbom : CycloneDxSbomGenerator.newInstance().setManifest(manifest)
                    ...
                    .setFormat(cdxSbomConfig.format()).setSchemaVersion(cdxSbomConfig.schemaVersion().orElse(null))
                    .setIncludeLicenseText(cdxSbomConfig.includeLicenseText()).generate()) {
