...
To execute SQL statements when Hibernate ORM starts, add SQL scripts to the root of your `resources` directory:

`data.sql`::
Loads data (reference data, test data, ...).
+
This script is executed regardless of how the database schema is managed:
if Hibernate ORM creates the schema (see `quarkus.hibernate-orm.schema-management.strategy`),
the script is executed right after the schema has been created;
otherwise, for example when the schema is managed by xref:flyway.adoc[Flyway] or xref:liquibase.adoc[Liquibase],
the script is executed once Hibernate ORM has started.
+
By default, the script is executed in dev and test modes, and whenever Hibernate ORM creates the schema.
Use `quarkus.hibernate-orm.data-management.strategy` to disable its execution (`none`)
or to execute it in production even though the schema is managed by another tool (`create`),
and `quarkus.hibernate-orm.data-management.init-script` to use other files.
+
[WARNING]
====
When the schema is not created by Hibernate ORM, the script is executed against a database that may already
contain its data, on every start: in dev mode, that means on every live reload.
Make the script idempotent (`MERGE`, `INSERT ... ON CONFLICT`, ...) if you do not want constraint violations
to be reported on each restart, or set `quarkus.hibernate-orm.data-management.strategy` to `none`.
====

`import.sql`::
Completes the schema generated by Hibernate ORM with database objects it cannot generate
(constraints, views, functions, ...).
+
This script is only executed when Hibernate ORM creates the schema,
i.e. when `quarkus.hibernate-orm.schema-management.strategy` is `create` or `drop-and-create`;
it is executed before `data.sql`.
+
Use `quarkus.hibernate-orm.schema-management.init-script` to use other files.

These scripts can contain any SQL statements.
Make sure to terminate each statement with a semicolon.

This is useful to have a data set ready for your tests or demos.

With <<multiple-persistence-units,multiple persistence units>>, the scripts at the root of your `resources` directory
are picked up by each of them, which suits persistence units mapping the same entities to different databases.
If your persistence units map different entities, set `quarkus.hibernate-orm."<name>".data-management.init-script`
and `quarkus.hibernate-orm."<name>".schema-management.init-script` explicitly for each of them
(to `no-file` where no script applies).

[NOTE]
====
The `quarkus.hibernate-orm.sql-load-script` configuration property is deprecated in favor of the properties above,
and cannot be combined with them.
Scripts configured through this deprecated property keep their historical behavior:
they are only executed when Hibernate ORM creates the schema.
To migrate, remove the property and rename the script to `data.sql`
(or to `import.sql` if it completes the schema rather than loading data).
====
...
1. The first choice is to use `quarkus.hibernate-orm.schema-management.strategy=drop-and-create` in conjunction with `data.sql`.
+
That way for every change to your app and in particular to your entities, the database schema will be properly recreated
and your data fixture (stored in `data.sql`) will be used to repopulate it from scratch.
+
This is best to perfectly control your environment and works magic with Quarkus live reload mode:
your entity changes or any change to your `data.sql` is immediately picked up and the schema updated without restarting the application!
In fact, this is the default when using xref:dev-services.adoc[Dev Services].
+
[TIP]
====
You can use a file with a different name than `data.sql` by setting the property `quarkus.hibernate-orm.data-management.init-script` in `application.properties`.
...
If the schema generated by Hibernate ORM needs to be completed with database objects it cannot generate
(constraints, views, functions, ...), put the corresponding SQL statements in `import.sql`
...
+
Note that `data.sql` is still executed on every startup with this approach:
set `quarkus.hibernate-orm.data-management.strategy=none` if that is not what you want.
...
+
Note that `data.sql` is still executed on every startup with this approach, once the schema has been migrated,
and executed again when resetting the database from the Dev UI, once Flyway or Liquibase have reset the schema:
set `quarkus.hibernate-orm.data-management.strategy=none` if that is not what you want.
...
%dev-with-data.quarkus.hibernate-orm.data-management.strategy = none

%prod.quarkus.hibernate-orm.schema-management.strategy = none
%prod.quarkus.hibernate-orm.data-management.strategy = none
----
...
* `quarkus.hibernate-orm.schema-management.strategy` is set to `none` for all profiles except the `dev` and `test` ones,
  so neither Hibernate ORM's schema generation nor the schema init script (`import.sql`) is executed.
* `quarkus.hibernate-orm.data-management.strategy` is set to `none` for all profiles except the `dev` and `test` ones,
  unless Hibernate ORM creates the schema,
  so the data init script (`data.sql`) is only executed in a schema Hibernate ORM just created.

You can override them in your `application.properties` explicitly
(e.g. `%prod.quarkus.hibernate-orm.data-management.strategy = create`)
...
Format of SQL init scripts::
When importing a `data.sql` or `import.sql` script to set up your database, keep in mind that Quarkus reconfigures Hibernate ORM so to require a semicolon (`;`) to terminate each statement.
