...
* roughly 15 min for CICD
* having access to an Azure subscription. Get a free one here [TODO]

This guide will take as input a native application developed in the link:building-native-image-guide.html[building native image guide].
...
If you correctly followed the link:building-native-image-guide.html[building native image guide], you should have a local container image named `quarkus-quickstart/getting-started`. 
...
== 1. Upload Container Image on Azure
...
The command above, if run successfuly, will give you the address of your container in the Cloud. Access your Quarkus application in the address displayed as output.
...
== 1. Deploy to Azure App Service on Linux Containers
