...
The `quarkus.oidc.proxy.proxy-configuration-name=<name>` property configures the name of proxy configuration registered in the Proxy registry (`quarkus.proxy.<name>.*`). The proxy configuration can be used to control OIDC access to HTTP proxies. The `quarkus.proxy.<name>.non-proxy-hosts` list of the named configuration is honored, and the special name `none` disables the proxy. Neither the default `quarkus.proxy.*` configuration nor the JVM `http.proxyHost` and `https.proxyHost` system properties are used unless a name is configured.
