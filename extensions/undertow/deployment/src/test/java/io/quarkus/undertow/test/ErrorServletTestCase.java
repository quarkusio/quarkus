...
public class ErrorServletTestCase {
    ...
    static QuarkusUnitTest runner = new QuarkusUnitTest()
            .withApplicationRoot((jar) -> jar.addClasses(ErrorServlet.class));

    @Test
    public void testHtmlError() {
        RestAssured.when().get("/error").then().log().all().statusCode(500)
    ...
    public void testJsonError() {
        RestAssured.given().accept(ContentType.JSON).when().get("/error").then().statusCode(500)
