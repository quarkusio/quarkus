...
[source,properties]
----
quarkus.kubernetes-service-binding.services.postgresDB.api-version=postgresql.baiju.dev/v1alpha1
quarkus.kubernetes-service-binding.services.postgresDB.kind=Database
quarkus.kubernetes-service-binding.services.postgresDB.name=db-demo
----
...
NOTE: This feature has limited applicability, as currently there are not many operators / custom resources bindable by the Service Binding Operator.

ALSO NOTE: Advanced, features (e.g. `ServiceBiniding` mappings) are not yet supported.
