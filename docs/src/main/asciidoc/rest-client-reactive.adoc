...
=== Using QuarkusMultipartForm

MultipartForm can be built using the Class `QuarkusMultipartForm` which supports customizing the form details like boundary:

`QuarkusMultipartForm` can be programatically created with custom inputs and/or from `MultipartFormDataInput` and/or from custom Resteasy Reactive Input annotated with `@RestForm` if received.
...
public interface MultipartService {
  ...
  Map<String, String> multipart(QuarkusMultipartForm dataParts); // <1>
}
----

<1> input to the method is a custom Generic `QuarkusMultipartForm` which matches external application api contract.


More information about this Class and supported methods can be found on the javadoc of link:https://javadoc.io/doc/io.quarkus.resteasy.reactive/resteasy-reactive-client/latest/org/jboss/resteasy/reactive/client/impl/multipart/QuarkusMultipartForm.html[`QuarkusMultipartForm`].


Build `QuarkusMultipartForm` from `MultipartFormDataInput` programatically

[source, java]
----
public QuarkusMultipartForm buildQuarkusMultipartForm(MultipartFormDataInput inputForm) // <1>
    throws IOException {
  QuarkusMultipartForm multiPartForm = new QuarkusMultipartForm(); // <2>
...
<2> Creating a `QuarkusMultipartForm` object to populate with various dataparts.
<3> Adding InMemory `FileItem` to `QuarkusMultipartForm`
<4> Adding physical `FileItem` to `QuarkusMultipartForm`
<5> Adding any attribute directly to `QuarkusMultipartForm` if not `FileItem`.

Build `QuarkusMultipartForm` from custom Attributes annotated with `@RestForm`
...
/*
 * Generate QuarkusMultipartForm from custom attributes annotated with @RestForm
 */
public QuarkusMultipartForm buildQuarkusMultipartForm(MultiPartPayloadFormData inputForm) { // <1>
...
<1> `MultiPartPayloadFormData` custom Object created to match the API contract for calling service which needs to be converted to `QuarkusMultipartForm`
<2> Adding attribute `jsonPayload` directly to `QuarkusMultipartForm`
<3> Adding `FileUpload` objects to `QuarkusMultipartForm` as binaryFileUpload with contentType.
