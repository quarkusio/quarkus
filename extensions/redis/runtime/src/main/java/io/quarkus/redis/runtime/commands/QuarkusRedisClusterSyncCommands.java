...
public class QuarkusRedisClusterSyncCommands<K, V> implements RedisAdvancedClusterCommands<K, V>, RedisClusterCommands<K, V> {
    private final StatefulRedisClusterConnection<K, V> clusterConnection;
    private final RedisAdvancedClusterAsyncCommands<K, V> asyncCommands;
    private final TimeoutProvider timeoutProvider;

    public QuarkusRedisClusterSyncCommands(StatefulRedisClusterConnection<K, V> clusterConnection,
            RedisAdvancedClusterAsyncCommands<K, V> asyncCommands) {
    ...
    @Override
    public RedisClusterCommands<K, V> getConnection(String s) {
        return getStatefulConnection().getConnection(s).sync();
    }

    @Override
    public RedisClusterCommands<K, V> getConnection(String s, int i) {
        return getStatefulConnection().getConnection(s, i).sync();
    }
    ...
    public NodeSelection<K, V> replicas(Predicate<RedisClusterNode> predicate) {
        return this.readonly((redisClusterNode) -> predicate.test(redisClusterNode)
                && redisClusterNode.is(RedisClusterNode.NodeFlag.MASTER));
    }
    ...
    public NodeSelection<K, V> readonly(Predicate<RedisClusterNode> predicate) {
        AsyncNodeSelection<K, V> readonly = asyncCommands.readonly(predicate);
        return new QuarkusRedisNodeSelection<>(readonly);
    }

    @Override
    public NodeSelection<K, V> nodes(Predicate<RedisClusterNode> predicate) {
        AsyncNodeSelection<K, V> nodes = asyncCommands.nodes(predicate);
        return new QuarkusRedisNodeSelection<>(nodes);
    }

    @Override
    public NodeSelection<K, V> nodes(Predicate<RedisClusterNode> predicate, boolean b) {
        AsyncNodeSelection<K, V> nodes = asyncCommands.nodes(predicate, b);
        return new QuarkusRedisNodeSelection<>(nodes);
    }
    ...
    public Long zadd(K k, ZAddArgs zAddArgs, ScoredValue<V>... scoredValues) {
        return awaitOrCancel(asyncCommands.zadd(k, zAddArgs, scoredValues));
    }
    ...
    @Override
    public String xgroupCreate(XReadArgs.StreamOffset<K> streamOffset, K k) {
        return awaitOrCancel(this.asyncCommands.xgroupCreate(streamOffset, k));
    }
    ...
    public void setAutoFlushCommands(boolean b) {
        asyncCommands.setAutoFlushCommands(b);
    }
