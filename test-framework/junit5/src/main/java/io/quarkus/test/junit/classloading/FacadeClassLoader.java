...
/**
 * JUnit has many interceptors and listeners, but it does not allow us to intercept test discovery in a fine-grained way
 * that would allow us to swap the thread context classloader. Since we can't intercept with a JUnit hook, we hijack
 * from inside the classloader.
 * <p>
 * We need to load all our test classes in one go, during the discovery phase, before we start the applications. We may
 * need several applications and therefore, several classloaders, depending on what profiles are set. To solve that, we
 * prepare the applications, to get classloaders, and file them here.
 * <p>
...
public final class FacadeClassLoader extends ClassLoader implements Closeable {
    ...
    // It would be nice, and maybe theoretically possible, to re-use the curated application? However, the
    // pre-classloadingrewrite version of the codebase did not reuse curated applications between profiles,
    // which suggests it would be a major rewrite to do so. If they are re-used, most things still work, but config when
    // there are multiple profiles does not work; see, for example, integration-tests/smallrye-config
    ...
    /*
     * A 'disposable' loader for holding temporary instances of the classes to allow us to inspect them. It seems kind
     * of wasteful to load every class twice; that's true, but it's been the case (by a different mechanism) ever since
     * Quarkus 1.2 and the move to isolated classloaders, because the test extension would reload classes into the
     * runtime classloader. In the future, https://openjdk.org/jeps/466 might allow us to avoid loading the classes to
     * avoid a double load in the delegating classloader. Whether that's actually better than using a disposable
     * classloader + reflection, I don't know. The solution referenced by
     ...
     * ClassLoaderReplacingLauncherSessionListener.java#L23-L44 does use a similar approach, although they have a
     * default loader rather than a canary loader.
     */
    ...
    // JUnit extensions can be registered by a service loader - see
    ...
    public FacadeClassLoader(ClassLoader parent, boolean isAuxiliaryApplication, CuratedApplication curatedApplication,
            final Map<String, String> profileNames, final Set<String> quarkusTestClasses, final String classesPath) {
        super(parent);
        ...
        URL[] urls = Arrays.stream(classesPath.split(File.pathSeparator)).map(spec -> {
            try {
        ...
        // If this is launched with a launcher, java.class.path may be very minimal - see
        // https://maven.apache.org/surefire/maven-surefire-plugin/examples/class-loading.html
        // Tests using an isolated classloader do not work correctly with continuous testing, but this issue predates
        // the facade classloader - see https://github.com/quarkusio/quarkus/issues/46478
        // What kind of tests use an isolated classloader? Maven will create one (and also a normal classloader) for
        // tests with `@WithFunction`
        // TODO this special casing is pretty fragile and pretty ugly; without it, @WithFunction tests do not pass, such
        ...
        if (launcherClassloader) {
            // If the classloader is isolated, putting the parent into the peeking classloader will just load all
            // classes with the parent, which isn't what's wanted (and causes @WithFunction tests to fail)
        ...
        // In the isolated classloader case, we actually never discover any quarkus tests, and a new instance gets
        // created;
        // but to be safe, initialise our instance variables. We can't use the peekingClassLoader because it can't see
        // JUnit classes, so just use the parent
        // We have to use reflection because the peekingclassloader may have different versions of the JUnit classes
        // than this class, especially when running with gradle
        try {
            ...
            disabledOnOsAnnotation = (Class<? extends Annotation>) annotationLoader
                    .loadClass(DisabledOnOs.class.getName());
            ...
            profileAnnotation = (Class<? extends Annotation>) annotationLoader.loadClass(TestProfile.class.getName());
            ...
            // That means we can bypass a whole bunch of logic.
            log.debug("Could not load annotations for FacadeClassLoader: " + e);
        }
        // We want to see what services are registered, but without going through the service loader, since that results
        // in a huge catastrophe of class not found exceptions
        // as the servoce loader tries to instantiate things in a nobbled loader. Instead, do it in a crude, safe, way
        // by looking for the resource files and reading them.
        try {
            ...
            while (declaredExtensions.hasMoreElements()) {
                ...
                try (InputStream in = url.openStream()) {
                    ...
                    if (QuarkusTestExtension.class.getName().equals(contents)) {
    ...
    public Class<?> loadClass(String name) throws ClassNotFoundException {
        ...
        // If the service loader mechanism is being used, QuarkusTestExtension gets loaded before any extensions which
        // use it. We need to make sure it's on the right classloader.
        if (isServiceLoaderMechanism && (name.equals(QuarkusTestExtension.class.getName()))) {
            try {
                // We don't have enough information to make a runtime classloader yet, but we can make a curated
                // application and a base classloader
        ...
        try {
            ...
            if (profiles != null && !isServiceLoaderMechanism) {
                ...
                if (quarkusTestAnnotation != null) {
                    // If it's not continuous testing, we need to load everything in order to decide if it's a
                    // QuarkusTest
                    try {
                        inspectionClass = peekingClassLoader.loadClass(name);
                    } catch (ClassNotFoundException | NoClassDefFoundError e) {
                    ...
                    if (!inspectionClass.isAnnotation()) {
                        ...
                        if (isServiceLoaderMechanism) {
                            ...
                            // If a service loader was used to register QuarkusTestExtension, every JUnit test is a
                            // QuarkusTest
                            isQuarkusTest = !anns.isEmpty();
                        }

                        // Because (until we do https://github.com/quarkusio/quarkus/issues/45785) we start dev services
                        // for disabled tests when we load them with the quarkus classloader, and those dev services
                        // often fail, put in some
                        // bypasses.
                        // These bypasses are also useful for performance.
                        // Ideally we would check for every way of disabling a test, but we don't want to recreate the
                        // JUnit logic, so just do common ones that might be guarding classes with classloading or
                        // dev-service-starting issues
                        // That does mean our behaviour in this area is slightly inconsistent between annotations; for
                        // some we will augment before giving up and for some we won't
                        boolean isDisabled = AnnotationSupport.isAnnotated(inspectionClass, disabledAnnotation)
                                || isDisabledOnOs(inspectionClass);

                        // If a whole test class has an @Disabled annotation, do not bother creating a quarkus app for
                        // it
                        // Pragmatically, this fixes a LinkageError in grpc-cli which only reproduces in CI, but it's
                        // also probably what users would expect
                        if (!isDisabled) {
                            // There are several ways a test could be identified as a QuarkusTest:
                            // A Quarkus Test could be annotated with @QuarkusTest or with @ExtendWith[...
                            // QuarkusTestExtension.class ] or @RegisterExtension (or the service loader mechanism could
                            // be used)
                            // An @interface isn't a quarkus test, and doesn't want its own application; to detect it,
                            // just check if it has a superclass

                            isQuarkusTest = isQuarkusTest
                                    || AnnotationSupport.isAnnotated(inspectionClass, quarkusTestAnnotation) // AnnotationSupport
                                                                                                                                           // picks up
                                                                                                                                           // cases
                                                                                                                                           // where a
                                                                                                                                           // class is
                            ...
                            if (isQuarkusTest) {
                                // Many integration tests have Quarkus higher up in the hierarchy, but they do not count
                                // as QuarkusTests and have to be run differently
                                isIntegrationTest = !inspectionClass.isAnnotation() && (AnnotationSupport
                                        .isAnnotated(inspectionClass, quarkusIntegrationTestAnnotation));

                                Optional<? extends Annotation> profileDeclaration = AnnotationSupport
                                        .findAnnotation(inspectionClass, profileAnnotation);
                                if (profileDeclaration.isPresent()) {

                                    Method m = profileDeclaration.get().getClass().getMethod(VALUE);
                                    // We can't be specific about what the class extends, because it's loaded with
                                    // another classloader
            ...
            try {
                ...
                if (values.getClass().isArray()) {
                    ...
                    for (int i = 0; i < length; i++) {
                        Object value = Array.get(values, i); // an OS, but we can't cast to it because it's in a
    ...
    private boolean registersQuarkusTestExtensionWithExtendsWith(Class<?> inspectionClass) {
        ...
        // This toString looks sloppy, but we cannot do an equals() because there might be multiple JUnit extensions,
        // and getting the annotation value would need a reflective call, which may not be any cheaper or prettier than
        // doing the string
    ...
    /*
     * What's this for? It's a bit like detecting the location in a privacy test or detecting the lab environment in an
     * emissions test and then deciding how to behave. We're special-casing behaviour for a hard-coded selection of test
     * packages. Yuck! TODO Hopefully, once https://github.com/quarkusio/quarkus/issues/45785 is done, it will not be
     * needed. Some tests, especially in kubernetes-client and openshift-client, check config to decide whether to start
     * a dev service. That happens at augmentation, which happens before test execution. In the old model, the test
     * class would have already been loaded by JUnit first, and it would have had a chance to write config to the system
     * properties. That config would influence whether dev services were started. TODO even without 45785 it might be
     * nice to find a better way, perhaps rewriting the AbstractKubernetesTestResource test resource to work
     * differently?
     */
    private void preloadTestResourceClasses(Class<?> fromCanary) {
        try {
            ...
            // In some projects, these classes are not on the canary classpath. That's fine, we know there's nothing to
            // preload.
    ...
    private boolean registersQuarkusTestExtensionOnField(Class<?> inspectionClass) {
        ...
        try {
            ...
            List<Field> fields = AnnotationSupport.findAnnotatedFields(inspectionClass, registerExtensionAnnotation,
                    f -> f.getType().getName().equals(QuarkusTestExtension.class.getName()));

            return fields != null && !fields.isEmpty();
        } catch (NoClassDefFoundError e) {
            // Under the covers, JUnit calls getDefinedFields, and that sometimes throws class-related in native mode
            // With -Dnative loading the KeycloakRealmResourceManager gives a class not found exception for junit's
            // TestRule
    ...
    private QuarkusClassLoader getQuarkusClassLoader(Class<?> requiredTestClass, Class<?> profile) {
        ...
        try {
            ...
            // We cannot directly access TestResourceUtil as long as we're in the core module, but the app classloaders
            // can.
            // But, chicken-and-egg, we may not have an app classloader yet. However, if we don't, we won't need to
            // worry about restarts, but this instance clearly cannot need a restart

            // TODO do the experiment - can it now work to use the peekingclassloader instead of the keymaker?

            // If we make a classloader with a null profile, we get the problem of starting dev services multiple times,
            // which is very bad (if temporary) - once that issue is fixed, could reconsider
            if (keyMakerClassLoader == null) {
                ...
                // We cannot use the startup action one because it's a base runtime classloader and so will not have the
                // right access to application classes (they're in its banned list)
                final String resourceKey = requiredTestClass != null ? getResourceKey(requiredTestClass, profile)
                        : null;
            ...
            // Exceptions here get swallowed by the JUnit framework and we don't get any debug information unless we
            // print it ourself
            e.printStackTrace();
            throw e;
        } catch (Exception e) {
    ...
    private String getResourceKey(Class<?> requiredTestClass, Class<?> profile)
        ...
        // We have to access TestResourceUtil reflectively, because if we used this class's classloader, it might be an
        // augmentation classloader without access to application classes
        // TODO check this is true, try skipping reflection and also using the peeking loader
        Method method = Class.forName(TestResourceUtil.class.getName(), true, classLoader)
        ...
        try {
            ...
            // When we load the TestResourceUtil loading gets delegated to a base runtime classloader, which cannot see
            // the app classes; so we need to pre-port the profile to its classloader before passing it to it
            Class<?> transliteratedProfile = profile != null ? keyMakerClassLoader.loadClass(profile.getName()) : null;
            // we reload the test resources (and thus the application) if we changed test class and the new test class
    ...
    private QuarkusClassLoader getOrCreateRuntimeClassLoader(String key, Class<?> requiredTestClass, Class<?> profile)
        ...
        Object testConfigProviderResolver = testConfigProviderResolverClass.getDeclaredConstructor().newInstance();
    ...
    public void close() throws IOException {
        ...
        // Null out the keymaker classloader and runtime classloaders, but don't close them, since we assume they will
        // be closed by the test framework closing the owning application
