...
/**
 * Required parts of the configuration used to <em>configure</em> a Quarkus build task, does not contain settings via
 * the {@link io.quarkus.gradle.extension.QuarkusPluginExtension} or any "forced properties".
 * <p>
 * Configuration from system properties, environment, application.properties/yaml/yml, project properties is available
 * in a Gradle task's configuration phase.
 */
final class BaseConfig {
    ...
    Map<String, String> cachingRelevantProperties(List<String> propertyPatterns) {
        ...
        Predicate<Map.Entry<String, ?>> keyPredicate = e -> patterns.stream()
                .anyMatch(p -> p.matcher(e.getKey()).matches());
        return values.entrySet().stream().filter(keyPredicate)
    ...
    /**
     * Reads missing environment variables that have been defined as `cachingRelevantProperties`. This ensures that the
     * configuration cache tracks these variables as inputs and detects changes in them.
     */
    private void readMissingEnvVariables(List<String> cachingRelevantProperties) {
        cachingRelevantProperties.stream().filter(name -> !values.containsKey(name))
