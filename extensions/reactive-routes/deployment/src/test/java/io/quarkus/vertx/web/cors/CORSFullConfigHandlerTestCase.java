...
public class CORSFullConfigHandlerTestCase {
    ...
    public void corsFullConfigTestServlet() {
        given().header("Origin", "http://custom.origin.quarkus").header("Access-Control-Request-Method", "GET")
                ...
                .header("Access-Control-Allow-Headers", "X-Custom").header("Access-Control-Max-Age", "86400");

        given().header("Origin", "http://custom.origin.quarkus").when().get("/test").then().statusCode(200)
                .header("Access-Control-Allow-Origin", "http://custom.origin.quarkus")
                .header("Access-Control-Allow-Methods", "GET,PUT,POST")
                .header("Access-Control-Expose-Headers", "Content-Disposition")
    ...
    public void corsPartialMethodsTestServlet() {
        given().header("Origin", "http://custom.origin.quarkus").header("Access-Control-Request-Method", "DELETE")
                ...
                .statusCode(200).log().headers().header("Access-Control-Allow-Origin", "http://custom.origin.quarkus")
