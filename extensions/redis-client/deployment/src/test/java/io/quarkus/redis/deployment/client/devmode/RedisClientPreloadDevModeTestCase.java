...
public class RedisClientPreloadDevModeTestCase {
    ...
    static QuarkusDevModeTest test = new QuarkusDevModeTest().setArchiveProducer(new Supplier<>() {
        ...
        public JavaArchive get() {
            return ShrinkWrap.create(JavaArchive.class)
                    .addAsResource(new StringAsset("quarkus.vertx.caching=false\n"
    ...
    public void testImportFileUpdateInDevMode() {
        // Verify preloading
        int count = Integer.valueOf(RestAssured.get("/inc/keys").then().statusCode(200).extract().body().asString());

        // Change the import file
        test.modifyResourceFile("import.redis", s -> s + "\nLPUSH list 1 2 3 4 5");
        RestAssured.get("/inc/keys").then().statusCode(200).body(Matchers.equalTo(Integer.toString(count + 1)));
    }
}
