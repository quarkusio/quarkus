...
It relies on the Brotli4J library, which Quarkus does not add to your application unless you ask for it.
...
The version is managed by the Quarkus BOM, so you do not need to specify one.

Then add `br` to the compressors list:
...
If `br` is configured while Brotli4J is missing, the build fails with a message pointing at this dependency.

NOTE: Brotli4J ships its native library in a separate, platform-specific artifact such as `native-linux-x86_64`.
Maven selects the right one automatically through the profiles declared in the `brotli4j` POM.
Build tools that ignore Maven profiles, such as Gradle, need that artifact declared explicitly as well.
