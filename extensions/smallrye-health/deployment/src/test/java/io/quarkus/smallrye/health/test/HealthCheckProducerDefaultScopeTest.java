...
class HealthCheckProducerDefaultScopeTest {
    ...
    static final QuarkusUnitTest config = new QuarkusUnitTest()
            .withApplicationRoot((jar) -> jar.addClasses(HealthCheckProducers.class));
    ...
    void testHealth() {
        ...
        try {
            ...
            when().get("/q/health/ready").then().header("cache-control", "no-store").body("status", is("UP"),
                    "checks.status", hasItems("UP", "UP"), "checks.name", hasItems("alpha1", "bravo1"));
            when().get("/q/health/ready").then().header("cache-control", "no-store").body("status", is("UP"),
                    "checks.status", hasItems("UP", "UP"), "checks.name", hasItems("alpha1", "bravo2"));
        } finally {
            RestAssured.reset();
        }
    }
