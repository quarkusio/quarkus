...
== Accessing the HTTP request during the handshake

The Jakarta WebSocket API does not expose the underlying HTTP request to `ServerEndpointConfig.Configurator#modifyHandshake`,
so information such as the remote address of the client is not reachable through the `HandshakeRequest` parameter.
During the handshake, Quarkus makes the request available through the `io.vertx.ext.web.RoutingContext` and
`io.vertx.core.http.HttpServerRequest` request-scoped beans:
...
NOTE: The request scope activated for the handshake ends when the handshake completes: `@RequestScoped` beans used in
`modifyHandshake` are not the ones used later by the endpoint methods.
