...
If you already have your {product-name-first} project configured, you can add the `hibernate-validator` extension
...
{product-name} automatically serves static resources contained in the `META-INF/resources` directory.
...
While the `ValidatorFactory` is instantiated by {product-name} itself,
...
{product-name} will check if a bean of this type is available and, if so, it will use it instead of instantiating a `ConstraintValidator`.
...
Configuring the `ValidatorFactory` using a `META-INF/validation.xml` file is *NOT* supported in {product-name}.
...
Consequently, the only way to define constraints in {product-name} is by annotating your classes.

=== ValidatorFactory and native executables

{product-name} provides a default `ValidatorFactory` that you can customize using configuration properties.
This `ValidatorFactory` is carefully initialized to support native executables
using a bootstrap that is {product-name}-specific.
...
Thus why you should always use the {product-name}-managed `ValidatorFactory` by injecting an instance of
...
{product-name} returns the `ValidatorFactory` managed by {product-name} when `Validation.buildDefaultValidatorFactory()`
is called.
