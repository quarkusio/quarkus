...
Proactive authentication is enabled in {product-name-first} by default. This means that if an incoming request has a credential then that request will always be authenticated, even if the target page does not require authentication.
...
To disable proactive authentication in {product-name}, set the following attribute in the `application.properties` configuration file:
...
You can use Jakarta REST `ExceptionMapper` to capture {product-name} Security authentication exceptions such as `io.quarkus.security.AuthenticationFailedException`, for example:
...
public class AuthenticationFailedExceptionMapper implements ExceptionMapper<AuthenticationFailedException> {
    ...
    public Response toResponse(AuthenticationFailedException exception) {
        return Response.status(401).header("WWW-Authenticate", "Basic realm=\"{product-name}\"").build();
    }
}
----
...
* xref:security-authentication-mechanisms.adoc[Authentication mechanisms in {product-name}]
