...
public interface Scheduler {
     ...
     * possible to set the {@code forced} start mode with the {@code quarkus.scheduler.start-mode} configuration
     * property. In this case, the scheduler will be started even if no scheduled business methods are found.
     *
     ...
     * <p>
     * Note that this method only returns {@code true} if the job was explicitly paused. I.e. it does not reflect a
     * paused scheduler.
     *
     * @param identity
     *
    ...
    /**
     * @return {@code true} if a scheduler is started the triggers are fired and jobs are executed, {@code false}
     *         otherwise
     *
     * @see #pause()
     * @see #resume()
    ...
    /**
     * Creates a new job definition. The job is not scheduled until the {@link JobDefinition#schedule()} method is
     * called.
     * <p>
     ...
     *
     * @param identity
     *        The identity must be unique for the scheduler
     *
    ...
    interface JobDefinition<THIS extends JobDefinition<THIS>> {
        ...
        /**
         * The schedule is defined either by {@link #setCron(String)} or by {@link #setInterval(String)}. If both
         * methods are used, then the cron expression takes precedence.
         * <p>
         * {@link Scheduled#cron()}
         *
         * @param cron
         ...
         * warning message is logged immediately.
         * <p>
         ...
         *         If the composite scheduler is used and the selected implementation is not available
         *
         * @see Scheduled#executeWith()
         */
         ...
         * <p>
         * In case of CDI, there must be exactly one bean that has the specified class in its set of bean types. The
         * scope of the bean must be active during execution of the job. If the scope is {@link Dependent} then the bean
         * instance belongs exclusively to the specific job definition and is destroyed when the application is shut
         * down. If the bean is not a dependency of any other bean it has to be marked as unremovable; for example
         * annotated with {@link io.quarkus.arc.Unremovable}.
         * <p>
         ...
         *        whether the task must be run on a virtual thread if the JVM allows it.
         *
         * @return self
         */
