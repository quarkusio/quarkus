...
{product-name-first} extensions enhance your application just as projects dependencies do.
The role of the extensions is to leverage {product-name} paradigms to integrate seamlessly a library into {product-name} architecture - e.g. do more things at build time.
This is how you can use your battle-tested ecosystem and take advantage of {product-name} performance and native compilation.
...
* JVM mode vs Native mode
  ** {product-name} is first and foremost a Java framework, that means you can develop, package and run classic JAR applications, that's what we call *JVM mode*.
...
{product-name}' philosophy is to prevent as much as possible slow and memory intensive dynamic code execution by shifting left these actions and eventually do them during the build time.
A {product-name} extension is a Java piece of code acting as an adapter layer for your favorite library or technology.

== Description of a Quarkus extension

A {product-name} extension consists of two parts:
...
following the {product-name} philosophy.
In other words, it applies all the {product-name} optimizations to your application during the build.
...
{product-name} during the augmentation phase from the runtime dependencies of the application.
...
There are three distinct bootstrap phases of a {product-name} application.

* *Augmentation*. During the build time, the {product-name} extensions will load and scan your application's bytecode (including the dependencies) and configuration.
...
* *Static Init*. During the run time, {product-name} will execute first a static init method which contains some extensions actions/configurations.
...
<2> {product-name} requires a recent version of the Maven compiler plugin supporting the annotationProcessorPaths configuration.
<3> The `quarkus-bom` aligns your dependencies with those used by {product-name} during the augmentation phase.
<4> {product-name} requires these configs to run tests properly.
...
At this point, you just need to understand that this class explains to {product-name} how to deploy a feature named `greeting` which is your extension.
In other words, you are augmenting your application to use the `greeting` extension with all the {product-name} benefits (build time optimization, native support, etc.).
...
<3> We add  the `quarkus-extension-maven-plugin` to generate the {product-name} extension descriptor included into the runtime artifact which links it with the corresponding deployment artifact.
<4> We add  the `quarkus-extension-processor` to the compiler annotation processors.

=== Gradle setup

{product-name} does not provide any way to initialize a Gradle project for extensions yet.
...
Luckily, `quarkus-bom` imported by our parent `pom.xml` already includes the Undertow {product-name} extension.
...
{product-name} magic relies on bytecode generation at build time rather than waiting for the runtime code evaluation, that's the role of your extension's `deployment` module.
Calm down, we know, bytecode is hard and you don't want to do it manually. {product-name} proposes a high level API to make your life easier.
...
<1> `feature()` method is annotated with `@BuildStep` which means it is identified as a deployment task {product-name} will have to execute during the deployment.
...
This `BuildItem` will be used by {product-name} to display information to the users when the application is starting.
...
Now, {product-name} will process this new task which will result in the bytecode generation of the Servlet registration at build time.
...
When developing a {product-name} extension, you mainly want to test your feature is properly deployed in an application and works as expected.
That's why the tests will be hosted in the `deployment` module.

{product-name} proposes facilities to test extensions via the `quarkus-junit5-internal` artifact (which should already be in the deployment pom.xml), in particular the `io.quarkus.test.QuarkusUnitTest` runner which starts an application with your extension.

We will use https://rest-assured.io[RestAssured] (massively used in {product-name}) to test our HTTP endpoint.
...
<1> We register a Junit Extension which will start a {product-name} application with the Greeting extension.
...
2021-01-27 10:24:42,506 INFO  [io.quarkus] (main) {product-name} {quarkus-version} on JVM started in 0.470s. Listening on: http://localhost:8081
...
2021-01-27 10:24:43,764 INFO  [io.quarkus] (main) {product-name} stopped in 0.018s
...
Now that you just finished building your first extension you should be eager to use it in a {product-name} application!
...
Then from another directory, use our tooling to create a new `greeting-app` {product-name} application with your new extension:
...
2021-01-27 10:28:07,240 INFO  [io.quarkus] ({product-name} Main Thread) greeting-app 1.0.0-SNAPSHOT on JVM (powered by {product-name} {quarkus-version}) started in 0.531s. Listening on: http://localhost:8080
2021-01-27 10:28:07,242 INFO  [io.quarkus] ({product-name} Main Thread) Profile dev activated. Live Coding activated.
2021-01-27 10:28:07,243 INFO  [io.quarkus] ({product-name} Main Thread) Installed features: [cdi, greeting-extension, servlet]
----

From an extension developer standpoint the Maven publication strategy is very handy and fast but {product-name} wants to go one step further by also ensuring a reliability of the ecosystem for the people who will use the extensions.
...
That's why there is the {product-name} Platform.

*{product-name} Platform*

{product-name} platform is a set of extensions that target the primary use-cases of {product-name} as a development framework and can safely be used in any combination in the same application without creating a dependency conflict.
From an application developer perspective, a {product-name} platform is represented as one or more Maven BOMs, for example `{quarkus-platform-groupid}:quarkus-bom:{quarkus-version}`, `{quarkus-platform-groupid}:quarkus-camel-bom:{quarkus-version}`, etc, whose dependency version constraints were globally aligned so that these BOMs can be imported in the same application in any order without introducing a dependency conflict.

*Quarkiverse Hub*

link:https://github.com/quarkiverse[Quarkiverse Hub] is the GitHub organization that provides repository hosting (including build, CI and release publishing setup) for {product-name} extension projects contributed by the community.

In case you are wondering about creating a new {product-name} extension and adding it to the {product-name} ecosystem so that the {product-name} community can discover it using the {product-name} dev tools (including the xref:cli-tooling.adoc[{product-name} CLI] and https://code.quarkus.io[code.quarkus.io]), the https://github.com/quarkiverse[Quarkiverse Hub] GitHub organization will be a good home for it.
...
- Configure your extension to use the link:https://github.com/quarkusio/quarkus-ecosystem-ci#quarkus-ecosystem-ci[{product-name} Ecosystem CI] to build against the latest {product-name} Core changes;
- Give you the freedom to manage the project and release to Maven Central as you like.

NOTE: Extensions hosted in the Quarkiverse Hub may or may not end up in the {product-name} platform.
...
Creating new extensions may appear to be an intricate task at first but once you understood the {product-name} game-changer paradigm (build time vs runtime) the structure of an extension makes perfectly sense.

As usual, along the path {product-name} simplifies things under the hood (Maven Mojo, bytecode generation or testing) to make it pleasant to develop new features.

== Further reading

- xref:writing-extensions.adoc[Writing your own extension] for the full documentation.
- xref:dev-ui-v2.adoc[{product-name} Dev UI] to learn how to support the Dev UI in your extension
