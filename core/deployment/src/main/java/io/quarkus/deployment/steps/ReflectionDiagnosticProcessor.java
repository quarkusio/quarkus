 ...
 * <p>
 * This is not really user facing config for now, which is why it is just a system property instead of using the
 * standard config mechanism
 */
public class ReflectionDiagnosticProcessor {

    @BuildStep
    public List<GeneratedResourceBuildItem> writeReflectionData(List<ReflectiveClassBuildItem> classes,
            List<ReflectiveMethodBuildItem> methods, List<ReflectiveFieldBuildItem> fields) {
        if (Boolean.getBoolean("quarkus.debug.reflection")) {
            String classNames = classes.stream().map(ReflectiveClassBuildItem::getClassNames)
                    .flatMap(Collection::stream).sorted().distinct().collect(Collectors.joining("\n", "", "\n"));
            ...
            String fieldsNames = fields.stream().map(m -> m.getDeclaringClass() + "#" + m.getName()).sorted().distinct()
