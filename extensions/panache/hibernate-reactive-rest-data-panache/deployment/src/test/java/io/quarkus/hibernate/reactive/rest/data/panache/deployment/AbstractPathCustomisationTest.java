...
public abstract class AbstractPathCustomisationTest {
    ...
    void testGet(String path, String accept) {
        given().accept(accept).when().get(path).then().statusCode(200);
    }
    ...
    void testCreateAndDelete(String path, String accept) {
        ...
        Response response = given().body("{\"id\": \"" + id + "\", \"name\": \"test collection\"}").and()
                .contentType("application/json").and().accept(accept).when().post(path).thenReturn();
    ...
    void testUpdate(String path, String accept) {
        given().body("{\"id\": \"empty\", \"name\": \"updated collection\"}").and().contentType("application/json")
                .and().accept(accept).when().put(path + "/empty").then().statusCode(204);
    }

    static class TestArgumentsProvider implements ArgumentsProvider {
        @Override
        public Stream<? extends Arguments> provideArguments(ExtensionContext context) {
            return Stream.of(Arguments.of("/collections", "application/json"),
