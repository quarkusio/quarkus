...
If you already have your {product-name-first} project configured, you can add the `scheduler` extension
...
<3> Use the `@Scheduled` annotation to instruct {product-name} to run this method every 10 seconds provided a worker thread is available
({product-name} is using 10 worker threads for the scheduler). If it is not available the method invocation should be re-scheduled by default i.e.
