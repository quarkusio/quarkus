...
It produces 2 jar files, one in `/target` and one in `/target/getting-started-1.0-SNAPSHOT`:
...
* `getting-started-1.0-SNAPSHOT/getting-started-1.0-SNAPSHOT-runner.jar` - being an executable _jar_. Be aware that it's not an _über-jar_ as
the dependencies are copied into the `target/getting-started-1.0-SNAPSHOT/lib` directory.

You can run the application using: `java -jar target/getting-started-1.0-SNAPSHOT/getting-started-1.0-SNAPSHOT-runner.jar`

NOTE: The `Class-Path` entry of the `MANIFEST.MF` from the _runner jar_ explicitly lists the jars from the `lib` directory.
So if you want to deploy your application somewhere, you need to copy the contents of the `getting-started-1.0-SNAPSHOT`
directory.
