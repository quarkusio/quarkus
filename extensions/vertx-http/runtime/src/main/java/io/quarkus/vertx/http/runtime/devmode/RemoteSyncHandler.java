...
public class RemoteSyncHandler implements Handler<HttpServerRequest> {
    ...
    <T> Future<T> executeBlocking(Callable<T> action, boolean ordered) {
        return VertxCoreRecorder.getVertx().get().executeBlocking(action, ordered);
    }
