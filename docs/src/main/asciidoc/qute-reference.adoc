...
An operand can be a value expression that uses the <<virtual_methods,infix notation>>, with or without parentheses.

.Infix notation operands example
[source,html]
----
...
NOTE: Inside the `if` section, `or`, `and`, `is`, `eq`, `ne`, `gt`, `ge`, `lt` and `le` are always the operators of the section, so `{#if (item.name or 'unknown') == 'unknown'}` is not an infix call of the `or` virtual method. Use `?:` instead.
