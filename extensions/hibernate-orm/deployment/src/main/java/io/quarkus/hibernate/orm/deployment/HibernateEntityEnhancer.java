 ...
 * io.quarkus.deployment.ProcessorContext#addByteCodeTransformer(java.util.function.Function). This function adapts the
 * Quarkus bytecode transformer API - which uses ASM - to use the Entity Enhancement API of Hibernate ORM, which exposes
 * a simple byte array. N.B. For enhancement the hardcoded tool of choice is the Byte Buddy based enhancer. This is not
 * configurable, and we enforce the ORM environment to use the "noop" enhancer as we require all entities to be enhanced
 * at build time.
 *
...
public final class HibernateEntityEnhancer implements BiFunction<String, ClassVisitor, ClassVisitor> {
    ...
    // Choose this set to include Jakarta annotations, basic Java types such as String and Map, Hibernate annotations,
    // and Panache supertypes:
    private static final CoreTypePool CORE_POOL = new CoreTypePool("java.", "jakarta.",
            "org.hibernate.bytecode.enhance.spi.", "org.hibernate.engine.spi.", "org.hibernate.annotations.",
    ...
    private static class HibernateEnhancingClassVisitor extends QuarkusClassVisitor {
        ...
        public HibernateEnhancingClassVisitor(String className, ClassVisitor outputClassVisitor,
            ...
            // Most often these match - but occasionally they will diverge which is acceptable as Byte Buddy is shading
            // ASM.
        ...
        public void visitEnd() {
            super.visitEnd();
            final ClassWriter writer = (ClassWriter) this.cv; // safe cast: cv is the ClassWriter instance we passed to
