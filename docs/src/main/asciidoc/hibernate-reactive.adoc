...
Declarative transaction management with Vert.x context-based interceptors (`@Transactional`, `@WithTransaction`, `@WithSession`, `@WithSessionOnDemand`) runs each top-level reactive pipeline on a dedicated duplicated Vert.x context, so combining such methods with `Uni.combine().all().unis()` or `Uni.join().all()` is supported.

Each combined branch still uses its own transaction when annotated with `@Transactional` or `@WithTransaction`.
Sequential calls using `.chain()` continue to share a single transaction when using `REQUIRED` semantics.
