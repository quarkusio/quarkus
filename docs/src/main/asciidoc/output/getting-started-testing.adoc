...
Learn how to test your {product-name-first} Application.
This guide covers:
...
NOTE: {product-name} supports Continuous testing, but this is covered by the xref:continuous-testing.adoc[Continuous Testing Guide].
...
While {product-name} will listen on port `8080` by default, when running tests it defaults to `8081`. This allows you to run
...
{product-name} also provides RestAssured integration that updates the default port used by RestAssured before the tests are run,
...
----
<1> This annotation allows you to directly inject the URL of the {product-name} instance, the value of the annotation will be the path component of the URL
...
{product-name} supports this by allowing you to inject CDI beans into your tests via the `@Inject` annotation (in fact, tests in
{product-name} are full CDI beans, so you can use all CDI functionality). Let's create a simple test that tests the greeting
...
public class GreetingServiceTest {
    ...
    public void testGreetingService() {
        Assertions.assertEquals("hello {product-name}", service.greeting("{product-name}"));
    }
}
----
...
As mentioned above {product-name} tests are actually full CDI beans, and as such you can apply CDI interceptors as you would
...
You can use the standard {product-name} `@Transactional` annotation on tests, but this means that the changes your
...
WARNING: While it is possible to use JUnit Jupiter callback interfaces like `BeforeEachCallback`, you might run into classloading issues because {product-name} has
         to run tests in a custom classloader which JUnit is not aware of.

[[testing_different_profiles]]
== Testing Different Profiles

So far in all our examples we only start {product-name} once for all tests. Before the first test is run {product-name} will boot,
then all tests will run, then {product-name} will shut down at the end. This makes for a very fast testing experience however
it is a bit limited as you can't test different configurations.

To get around this {product-name} supports the idea of a test profile. If a test has a different profile to the previously
run test then {product-name} will be shut down and started with the new profile before running the tests. This is obviously
a bit slower, as it adds a shutdown/startup cycle to the test time, but gives a great deal of flexibility.

To reduce the amount of times {product-name} needs to restart, `io.quarkus.test.junit.util.QuarkusTestProfileAwareClassOrderer`
...
If you encounter such warnings, you can get rid of them by removing the {product-name}-supplied `junit-platform.properties` from the classpath via an exclusion:
[source,xml]
----
<dependency>
    <groupId>io.quarkus</groupId>
...
public class MockGreetingProfile implements QuarkusTestProfile { <1>
     ...
     * then {product-name} will only execute tests that are annotated with a {@code @TestProfile} that has at least one of the
...
{product-name} provides the ability to limit test execution to tests with specific  `@TestProfile` annotations.
...
{product-name} supports the use of mock objects using two different approaches. You can either use CDI alternatives to
...
Building on the features provided by `QuarkusMock`, {product-name} also allows users to effortlessly take advantage of link:https://site.mockito.org/[Mockito] for mocking the beans supported by `QuarkusMock`.
...
Building on the features provided by `InjectMock`, {product-name} also allows users to effortlessly take advantage of link:https://site.mockito.org/[Mockito] for spying on the beans supported by `QuarkusMock`.
...
If you are using {product-name} Security, check out the xref:security-testing.adoc[Testing Security] section for information on how to easily test security features of the application.

[#quarkus-test-resource]
== Starting services before the Quarkus application starts

A very common need is to start some services on which your {product-name} application depends, before the {product-name} application starts for testing. To address this need, {product-name} provides `@io.quarkus.test.common.QuarkusTestResource` and `io.quarkus.test.common.QuarkusTestResourceLifecycleManager`.

By simply annotating any test in the test suite with `@QuarkusTestResource`, {product-name} will run the corresponding `QuarkusTestResourceLifecycleManager` before any tests are run.
...
{product-name} provides a few implementations of `QuarkusTestResourceLifecycleManager` out of the box (see `io.quarkus.test.h2.H2DatabaseTestResource` which starts an H2 database, or `io.quarkus.test.kubernetes.client.KubernetesServerTestResource` which starts a mock Kubernetes API server),
...
time (i.e. no JUnit callbacks are invoked) then {product-name} will print a stack trace to the console to help diagnose the hang.
...
be read until {product-name} has started, so the timeout for {product-name} start will be the default of 10 minutes).
...
`@QuarkusIntegrationTest` should be used to launch and test the artifact produced by the {product-name} build, and supports testing a jar (of whichever type), a native image or container image.
Put simply, this means that if the result of a {product-name} build (`mvn package` or `gradle build`) is a jar, that jar will be launched as `java -jar ...` and tests run against it.
...
These tests will **not** work if run in the same phase as `@QuarkusTest` as {product-name} has not yet created the final artifact.
====
...
The Testcontainers library usually return connection strings without respecting the container network, so additional code is needed to provide {product-name} the "correct" connection string using the container's IP on the container network and the _unmapped_ port number.
...
The reason of this restriction is that `@QuarkusTest` starts a {product-name} server for the whole lifetime of the tests execution run,
thus preventing the other tests to start their own {product-name} server.
...
Use this JRE definition as your {product-name} project targeted runtime and the workaround will be applied to any "Run as JUnit" configuration.
...
have a field that implements `io.quarkus.test.common.DevServicesContext.ContextAware` {product-name} will call the
...
which allows you to set up the resource before {product-name} starts (e.g. configure a KeyCloak instance, add data to a database etc).
...
In {product-name}, the component model is built on top CDI.
Therefore, {product-name} provides the `QuarkusComponentTestExtension`, a JUnit extension to ease the testing of components and mocking of their dependencies.
