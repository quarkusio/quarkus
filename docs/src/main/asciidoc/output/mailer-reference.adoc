...
:summary: This reference guide explains in more details the configuration and usage of the {product-name-first} Mailer.

This guide is the companion from the xref:mailer.adoc[Mailer Getting Started Guide].
It explains in more details the configuration and usage of the {product-name} Mailer.
...
If you put the image file into the `META-INF/resources` folder, you should specify the full path to the file, _e.g._ `META-INF/resources/quarkus-logo.png` otherwise {product-name} will look for the file in the root directory.
...
This is the default if you are running {product-name} in `DEV` or `TEST` mode.
...
The {product-name} Mailer is implemented on top of the https://vertx.io/docs/vertx-mail-client/java/[Vert.x Mail Client], providing an asynchronous and non-blocking way to send emails.
...
[NOTE]
Do use Vault, you need the https://github.com/quarkiverse/quarkus-vault[{product-name} Vault] extension.
...
{product-name} mailer supports JKS, PKCS#12 and PEM trust stores.
...
When not set, {product-name} tries to deduce the type from the file name.
...
This use case is perfectly supported in {product-name} and you can configure several mailers:
...
When done, you can configure your {product-name} application by adding the following properties to your `application.properties`:
...
[NOTE]
====
The `quarkus.mailer.auth-methods` configuration option is needed for the {product-name} mailer to support password authentication with Gmail.
