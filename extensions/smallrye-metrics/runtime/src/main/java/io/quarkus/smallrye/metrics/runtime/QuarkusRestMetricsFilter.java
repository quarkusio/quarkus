 ...
 * properly use a response filter in order to finish the request. Moreover, we use the {@code @ServerResponseFilter} to
 * make writing the filter even easier.
 */
public class QuarkusRestMetricsFilter {
    ...
    public void filter(ResourceInfo resourceInfo, ContainerResponseContext responseContext) {
        ...
        FilterUtil.finishRequest(System.nanoTime(), resourceInfo.getResourceClass(),
                resourceInfo.getResourceMethod().getName(), resourceInfo.getResourceMethod().getParameterTypes(),
