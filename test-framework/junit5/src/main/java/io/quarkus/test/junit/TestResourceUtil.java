...
/**
 * Contains methods that are needed for determining how to deal with {@link io.quarkus.test.common.QuarkusTestResource}
 * and {@link io.quarkus.test.common.WithTestResource}
 */
public final class TestResourceUtil {
    ...
    /**
     * This is where we decide if the test resources of the current state vs the ones required by the next test class to
    ...
    static Set<TestResourceManager.TestResourceComparisonInfo> nextTestResources(Class<?> requiredTestClass,
        ...
        if (profileInstance != null) {
            ...
            for (QuarkusTestProfile.TestResourceEntry entry : profileInstance.testResources()) {
                entriesFromProfile.add(new TestResourceManager.TestResourceClassEntry(entry.getClazz(), entry.getArgs(),
                        null, entry.isParallel(), TestResourceScope.MATCHING_RESOURCES));
            }
        }

        return TestResourceManager.testResourceComparisonInfo(requiredTestClass,
                getTestClassesLocation(requiredTestClass), entriesFromProfile);
    }
    ...
    private static QuarkusTestProfile instantiateProfile(Class<? extends QuarkusTestProfile> nextTestClassProfile) {
        if (nextTestClassProfile != null) {
            ...
            // All this reflective classloading is a bit wasteful, so it would be ideal if the implementation was less
            // picky about classloaders (that's not just moving the reflection further down the line)
            // TODO this may not be necessary with changes in FacadeClassLoader, check
            // TODO not only is it not necessary, it actually cannot work because we've lost access to the runtime
            // classloader which we need to load app classes
            try {
    ...
    /**
     * Contains a bunch of utilities that are needed for handling {@link TestResourceManager} via reflection (due to
    ...
    public static final class TestResourceManagerReflections {
        ...
        /**
         * Since {@link TestResourceManager} is loaded from the ClassLoader passed in as an argument, we need to convert
         * the user input {@link QuarkusTestProfile.TestResourceEntry} into instances of
        ...
        /**
         * Corresponds to
         * {@link TestResourceManager#TestResourceManager(Class, Class, List, boolean, Map, Optional, Path)}
         */
        public static Closeable createReflectively(Class<?> testResourceManagerClass, Class<?> testClass,
                Class<?> profileClass, List<TestResourceManager.TestResourceClassEntry> additionalTestResources,
            ...
            try {
                return (Closeable) testResourceManagerClass.getConstructor(Class.class, Class.class, List.class,
                        boolean.class, Map.class, Optional.class, Path.class).newInstance(testClass, profileClass,
                                additionalTestResources, disableGlobalTestResources, devServicesProperties,
                                containerNetworkId, testClassLocation);
         ...
         */
        static Set<TestResourceManager.TestResourceComparisonInfo> testResourceComparisonInfo(
