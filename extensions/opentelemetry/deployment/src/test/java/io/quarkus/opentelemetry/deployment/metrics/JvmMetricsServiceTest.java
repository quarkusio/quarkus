...
public class JvmMetricsServiceTest {
    ...
    void testClassUnloadedMetrics() throws IOException {
        assertMetric("jvm.class.unloaded", "Number of classes unloaded since JVM start.", "{class}",
                MetricDataType.LONG_SUM);
    }

    @Test
    void testClassCountMetrics() {
        assertMetric("jvm.class.count", "Number of classes currently loaded.", "{class}", MetricDataType.LONG_SUM);
    }
    ...
    void testCpuCountMetric() throws IOException {
        assertMetric("jvm.cpu.count", "Number of processors available to the Java virtual machine.", "{cpu}",
                MetricDataType.LONG_SUM);
    }

    @Test
    void testCpuRecentUtilizationMetric() throws IOException {
        assertMetric("jvm.cpu.recent_utilization", "Recent CPU utilization for the process as reported by the JVM.",
                "1", MetricDataType.DOUBLE_GAUGE);
    }

    @Test
    void testGarbageCollectionCountMetric() {
        System.gc();
        assertMetric("jvm.gc.duration", "Duration of JVM garbage collection actions.", "s", MetricDataType.HISTOGRAM);
    }

    @Test
    void testJvmMemoryUsedMetric() throws IOException {
        assertMetric("jvm.memory.used", "Measure of memory used.", "By", MetricDataType.LONG_SUM);
    }

    @Test
    void testJvmMemoryCommittedMetric() throws IOException {
        assertMetric("jvm.memory.committed", "Measure of memory committed.", "By", MetricDataType.LONG_SUM);
    }

    @Test
    void testMemoryLimitMetric() throws IOException {
        assertMetric("jvm.memory.limit", "Measure of max obtainable memory.", "By", MetricDataType.LONG_SUM);
    }
    ...
    void testThreadCountMetric() throws IOException {
        assertMetric("jvm.thread.count", "Number of executing platform threads.", "{thread}", MetricDataType.LONG_SUM);
    }

    private void assertMetric(final String metricName, final String metricDescription, final String metricUnit,
        ...
        metric.getDoubleSumData().getPoints().stream().forEach(point -> {
            assertThat(point.getValue()).withFailMessage("Double" + point.getValue() + " was not an expected result")
                    .isGreaterThan(0);
        });

        metric.getLongSumData().getPoints().stream().forEach(point -> {
            assertThat(point.getValue()).withFailMessage("Long" + point.getValue() + " was not an expected result")
                    .isGreaterThanOrEqualTo(0);
        });

        metric.getDoubleGaugeData().getPoints().stream().forEach(point -> {
            assertThat(point.getValue()).withFailMessage("Double" + point.getValue() + " was not an expected result")
                    .isGreaterThanOrEqualTo(0);
        });
