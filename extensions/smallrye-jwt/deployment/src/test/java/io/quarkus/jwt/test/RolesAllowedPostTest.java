...
public class RolesAllowedPostTest {
    ...
    public void postText() {
        RestAssured.given().auth()
                .oauth2(Jwt.upn("alice").groups("Tester").sign())
                .when()
                .body("principal:")
                .post("/endp/postInjectedPrincipal")
                .then()
                .statusCode(200)
                .body(equalTo("principal:alice"));
    }
    ...
    public void postTextWrongRole() {
        RestAssured.given().auth()
                .oauth2(Jwt.upn("alice").groups("User").sign())
                .when()
                .body("principal:")
                .post("/endp/postInjectedPrincipal")
                .then().statusCode(403);
    ...
    public void postJson() {
        RestAssured.given()
                ...
                .auth().oauth2(Jwt.upn("alice").groups("Tester").sign())
                .when()
                .body("{\"name\":\"alice\"}")
                .post("/endp/postInjectedPrincipalJson")
                .then()
                .statusCode(200)
