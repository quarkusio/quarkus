...
The configured headers are added to the response; a value that is already present, for example set by the endpoint or by the CORS filter, is never replaced.
When several filters match the same path, the values of all of them are sent, and the `order` decides in which order the filters run and therefore in which order the values are added (filters with a lower order run first).
...
A request for `/paths/order` gets two `Cache-Control` headers, `max-age=5000` first and `max-age=1` second.

IMPORTANT: A header that must have a single value, such as `Cache-Control`, cannot be overridden by another filter. Make the `matches` patterns of such filters disjoint, for example `/paths/order` and `/paths/order/.+`, so that a request matches only one of them.
