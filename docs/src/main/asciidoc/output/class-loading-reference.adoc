...
:summary: Learn more about {product-name-first} class loading infrastructure.

This document explains the {product-name} class loading architecture. It is intended for extension
authors and advanced users who want to understand exactly how {product-name} works.

The {product-name} class loading architecture is slightly different depending on the mode that
...
For all other use cases (e.g. tests, dev mode, and building the application) {product-name}
uses the class loading architecture outlined in following section.


== Bootstrapping Quarkus

All {product-name} applications are created by the QuarkusBootstrap class in the `independent-projects/bootstrap` module. This
class is used to resolve all the relevant dependencies (both deployment and runtime) that are needed for the {product-name}
...
to have any of the {product-name} deployment classes on the class path as the curate process will resolve them for you.

This bootstrap process should be the same no matter how {product-name} is launched, just with different parameters passed in.

=== Current Run Modes

At the moment we have the following use cases for bootstrapping {product-name}:
...
One of the goals of this refactor is to have all these different run modes boot {product-name} in fundamentally the same way.
...
{product-name} has a number of ClassLoaders. This shows the relationship between them :

image::classloader-hierarchy.png[alt={product-name} ClassLoader hierarchy,role="center"]
...
ClassLoader (or whatever ClassLoader is responsible for bootstrapping {product-name}). Most extensions do not need to worry about
...
not compatible with {product-name}. To deal with this, extensions can specify artifacts that should never be loaded. This is
...
Note that class loading config is different to normal config, in that it does not use the standard {product-name} config mechanisms
