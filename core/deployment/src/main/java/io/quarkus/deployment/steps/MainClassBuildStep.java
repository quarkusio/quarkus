...
public class MainClassBuildStep {
    ...
    void build(
        ...
        ResultHandle staticGraph = emitServiceGraph(tryBlock, staticPlan, startupContext, file, consolidatedRefs,
                gizmoOutput, substitutions, recordableConstructorBuildItems, loaders, constants, "static", capabilities);
        ...
        ResultHandle runtimeGraph = emitServiceGraph(tryBlock, runtimePlan, startupContext, file, consolidatedRefs,
                gizmoOutput, substitutions, recordableConstructorBuildItems, loaders, constants, "runtime", capabilities);
    ...
    private ResultHandle emitServiceGraph(
        ...
        ResultHandle monitor;
        if (capabilities.isPresent(Capability.JFR)) {
            monitor = code.readStaticField(FieldDescriptor.of("io.quarkus.core.impl.JfrServiceMonitor",
                    "INSTANCE", "io.quarkus.core.impl.JfrServiceMonitor"));
        } else {
            monitor = code.readStaticField(FieldDescriptor.of("io.quarkus.core.impl.NoOpServiceMonitor",
                    "INSTANCE", "io.quarkus.core.impl.NoOpServiceMonitor"));
        }

        if (!plan.hasNodes()) {
            // empty plan: create a graph wrapping the existing startup context
            return code.newInstance(
                    ofConstructor(SERVICE_GRAPH_CLASS, StartupContext.class, "io.quarkus.core.impl.ServiceMonitor"),
                    startupContext, monitor);
        }

        List<ServiceGraphBuilder.NodeDescriptor> nodes = plan.nodes();

        // create the ServiceGraph wrapping the shared startup context
        ResultHandle graph = code.newInstance(
                ofConstructor(SERVICE_GRAPH_CLASS, StartupContext.class, "io.quarkus.core.impl.ServiceMonitor"), startupContext,
                monitor);
    ...
    private ResultHandle emitActionHandle(
        ...
        switch (node.kind()) {
            ...
            case LEGACY_RECORDER -> {
                ...
                targetClass = Application.APP_CLASS_NAME;
                boolean staticInit = phaseName.equals("staticInit");
                generateLegacyWrapperMethod(classCreator, methodName, node.recorders(),
                        staticInit, gizmoOutput, substitutions,
