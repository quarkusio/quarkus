...
----
<1> A gRPC service implementation bean must be annotated with the `@GrpcService` annotation. This annotation is a CDI stereotype that declares the `javax.inject.Singleton` scope. Additionally, it makes sure the CDI request context is always activate during a service call.
