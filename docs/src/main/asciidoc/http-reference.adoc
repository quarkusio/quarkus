...
When logging to a dedicated file, the file is rotated daily by default and the rotated files are kept forever.
Set `quarkus.http.access-log.max-backup-index` to keep only the most recent rotated files and have the older ones deleted after each rotation.
