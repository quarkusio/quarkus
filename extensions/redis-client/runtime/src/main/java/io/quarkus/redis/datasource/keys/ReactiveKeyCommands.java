...
/**
 * Allows executing commands manipulating keys. See <a href="https://redis.io/commands/?group=generic">the generic
...
public interface ReactiveKeyCommands<K> extends ReactiveRedisCommands {
     ...
     *        the key
     *
     * @return the expiration Unix timestamp in seconds, {@code -1} if the key exists but has no associated expire. The
     *         Uni produces a {@link RedisKeyNotFoundException} if the key does not exist.
     **/
     ...
     *        the key
     *
     * @return {@code true} the timeout was removed. {@code false} key does not exist or does not have an associated
     *         timeout.
     **/
     ...
     *        the new key
     *
     * @return {@code true} if {@code key} was renamed to {@code newkey}. {@code false} if {@code newkey} already
     *         exists.
     **/
    Uni<Boolean> renamenx(K key, K newkey);

    /**
     * Execute the command <a href="https://redis.io/commands/scan">SCAN</a>. Summary: Incrementally iterate the keys
     * space Group: generic Requires Redis 2.8.0
     *
     * @return the cursor.
     **/
