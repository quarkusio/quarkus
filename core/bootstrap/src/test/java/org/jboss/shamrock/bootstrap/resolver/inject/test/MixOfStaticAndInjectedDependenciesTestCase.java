 ...
 */
public class MixOfStaticAndInjectedDependenciesTestCase extends CollectDependenciesBase {

    @Override
    protected void setupDependencies() throws Exception {

        final TsArtifact staticA = new TsArtifact("static-a");
        installAsDep(staticA);

        final TsArtifact deploymentA = new TsArtifact("deployment-a");
        install(deploymentA, "runtime");

        final TsArtifact extensionA = TsArtifact.jar("extension-a");
        install(
                extensionA,
                newJar().addFile(
        ...
        final TsArtifact staticB = new TsArtifact("static-b").addDependency(extensionA);
        installAsDep(staticB);

        final TsArtifact staticC = new TsArtifact("static-c");
        installAsDep(staticC);
    }
}
