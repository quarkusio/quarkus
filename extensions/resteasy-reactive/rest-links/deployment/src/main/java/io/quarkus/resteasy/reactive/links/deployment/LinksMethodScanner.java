...
public class LinksMethodScanner implements MethodScanner {
    ...
    private AnnotationInstance getInjectRestLinksAnnotation(MethodInfo method, ClassInfo actualEndpointClass,
        ...
        AnnotationInstance annotationInstance = annotationStore.getAnnotation(method,
                DotNames.INJECT_REST_LINKS_ANNOTATION);
        if (annotationInstance == null) {
            annotationInstance = annotationStore.getAnnotation(method.declaringClass(),
                    DotNames.INJECT_REST_LINKS_ANNOTATION);
            if ((annotationInstance == null) && !actualEndpointClass.equals(method.declaringClass())) {
                annotationInstance = annotationStore.getAnnotation(actualEndpointClass,
                        DotNames.INJECT_REST_LINKS_ANNOTATION);
            }
        }
