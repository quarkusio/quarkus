...
You can enable HTTP `Host` header validation with a `quarkus.management.host-validation.allowed-hosts` property.

For example:

[source, properties]
----
quarkus.management.host-validation.allowed-hosts=myservice.com,www.my-service.com,localhost
----
...
[IMPORTANT]
====
In production and dev modes, when the `quarkus.management.host` property has a valid localhost name, and neither `quarkus.management.host-validation.allowed-hosts` nor `quarkus.management.host-validation.require-localhost` properties are enabled, HTTP `Host` header is expected to contain only a valid localhost name.

Set `quarkus.management.host-validation.require-localhost=false` to avoid this restriction.
====

[NOTE]
====
HTTP `Host` header contains a request URL authority and therefore may also include ports, for example, `localhost:9001`.

The authority ports are not verified during the HTTP `Host` header verification. The port verification is more relevant for the same origin checks done by the <<management-cors>>. For example, if the server is launched at `localhost:9001` then the same origin CORS check passes only if the HTTP `Host` header is `localhost:9001` and fails if it is `localhost:9002`.
====

[NOTE]
====
When the management interface runs behind a proxy that does not rewrite a `Host` header with a host value from either `Forwarded: host` or `X-Forwarded-Host` headers, you can get the management interface's proxy handler do it instead, please see the <<reverse-proxy>> section for more details.
====
