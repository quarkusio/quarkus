...
This guide explains how to benefit from Java virtual threads when writing message processing applications in {product-name-first}.

[TIP]
====
This guide focuses on using virtual threads with Reactive Messaging extensions.
Please refer to xref:virtual-threads.adoc[Writing simpler reactive REST services with {product-name} Virtual Thread support]
to read more about Java virtual threads in general and the {product-name} Virtual Thread support for REST services.
...
The idea behind {product-name} Virtual Thread support for Reactive Messaging is to offload the message processing on virtual threads,
