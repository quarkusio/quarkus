...
:categories: core
:summary: Go more in depth into the {product-name-first} implementation of CDI.
:numbered:
:sectnums:
:sectnumlevels: 4

{product-name} DI solution (also called ArC) is based on the https://jakarta.ee/specifications/cdi/4.0/jakarta-cdi-spec-4.0.html[Jakarta Contexts and Dependency Injection 4.0, window="_blank"] specification.
...
NOTE: Most of the existing CDI code should work just fine but there are some small differences which follow from the {product-name} architecture and goals.
...
However, {product-name} is using a *simplified bean discovery*.
...
* and {product-name} integration code.
...
NOTE: {product-name} extensions may declare additional discovery rules. For example, `@Scheduled` business methods are registered even if the declaring class is not annotated with a bean defining annotation.
...
It may happen that some beans from third-party libraries do not work correctly in {product-name}.
...
{product-name} is using GraalVM to build a native executable.
...
And if {product-name} DI needs to access a private member it *has to use reflection*.
That's why {product-name} users are encouraged __not to use private members__ in their beans.
...
NOTE: {product-name} users are encouraged to always prefer the `@Observes StartupEvent` to `@Initialized(ApplicationScoped.class)` as explained in the xref:lifecycle.adoc[Application Initialization and Termination] guide.
...
In {product-name}, you can skip the `@Inject` annotation completely if the injected field declares at least one qualifier.
...
There is no need to declare dummy constructors for normal scoped bean in {product-name} - they are generated automatically.
...
However, {product-name} can't detect the programmatic lookup performed via the `CDI.current()` static method.
...
{product-name} also provides a middle ground where application beans are never removed whether or not they are unused, while the optimization proceeds normally for non application classes.
To use this mode, set `quarkus.arc.remove-unused-beans` to `fwk` or `framework`.

==== What's Removed?

{product-name} first identifies so-called _unremovable_ beans that form the roots in the dependency tree.
...
{product-name} adds a capability that CDI currently does not support which is to conditionally declare a bean if no other bean with equal types and qualifiers was declared by any available means (bean class, producer, synthetic bean, ...)
This is done using the `@io.quarkus.arc.DefaultBean` annotation and is best explained with an example.

Say there is a {product-name} extension that among other things declares a few CDI beans like the following code does:
...
way {product-name} supports.

[[enable_build_profile]]
=== Enabling Beans for Quarkus Build Profile

{product-name} adds a capability that CDI currently does not support which is to conditionally enable a bean when a {product-name} build time profile is enabled,
...
{product-name} adds a capability that CDI currently does not support which is to conditionally enable a bean when a {product-name} build time property has/has not a specific value,
...
{product-name} has a simplified bean discovery and the content of `beans.xml` is ignored.
...
In {product-name}, you can skip the `@Produces` annotation completely if the producer method is annotated with a scope annotation, a stereotype or a qualifier.
...
And since {product-name} is a build-time oriented stack that allows for additional class transformations, those limitations don't apply anymore.
...
{product-name} however, can overcome these limitations when `quarkus.arc.transform-unproxyable-classes` is set to `true` (which is the default value).
...
{product-name} provides `@io.quarkus.arc.Lock` and a built-in interceptor for this interceptor binding.
...
{product-name} has limited support for `@Repeatable` interceptor binding annotations.
...
Therefore, {product-name} comes with the `io.quarkus.arc.WithCaching` annotation.
...
TIP: It is also possible to clear the cached value via `io.quarkus.arc.InjectableInstance.clearCache()`. In this case, you'll need to inject the {product-name}-specific `io.quarkus.arc.InjectableInstance` instead of `jakarta.enterprise.inject.Instance`.
...
Therefore, a new way was introduced in {product-name} - you can inject a `java.util.List` annotated with the `io.quarkus.arc.All` qualifier.
...
Therefore, {product-name} logs an error message by default.
...
{product-name} incorporates build-time optimizations in order to provide instant startup and low memory footprint.
The downside of this approach is that CDI Portable Extensions cannot be supported.
Nevertheless, most of the functionality can be achieved using {product-name} xref:writing-extensions.adoc[extensions].
