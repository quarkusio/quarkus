...
The code presented is available in this {quickstarts-base-url}[GitHub repository] under the link:{quickstarts-tree-url}/getting-started-reactive-crud[`getting-started-reactive-crud` directory].
...
[NOTE]
====
Wondering why we need to block until the latest query is completed?
...
You might need to read a specific file only present in production, or retrieve configuration data from a proprietary configuration server.
