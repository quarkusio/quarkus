 ...
 * <p>
 * The {@link QuteDummyTemplateGlobalMarker} is used to identify an application archive where a dummy built-in class
 * with template globals is added.
 */
public class TemplateGlobalDevModeTest {

    @RegisterExtension
    static final QuarkusDevModeTest config = new QuarkusDevModeTest().withApplicationRoot(
            root -> root.addClasses(TestRoute.class, MyGlobals.class, QuteDummyTemplateGlobalMarker.class)
                    .addAsResource(new StringAsset("{foo}:{quteDummyFoo}:{testFoo ?: 'NA'}"), "templates/test.html"));

    @Test
    public void testTemplateGlobals() {
        given().get("test").then().statusCode(200).body(Matchers.equalTo("24:bar:NA"));
        ...
        given().get("test").then().statusCode(200).body(Matchers.equalTo("24:bar:baz"));
    }
