...
|quarkus.oidc.credentials.jwt.audience |OIDC token endpoint address (or `quarkus.oidc.auth-server-url` when `quarkus.oidc.credentials.jwt.source` is `spiffe-jwt`)|Audience (`aud`) claim value
