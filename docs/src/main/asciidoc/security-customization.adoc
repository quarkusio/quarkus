...
[WARNING]
====
Native executables that use `BCFIPS` are not supported on a FIPS-enabled operating system, for example a RHEL host booted in FIPS mode.
In that environment the JDK loads the security providers configured for FIPS mode at run time, which a native executable cannot do, and the application fails at startup with:
...
The same application runs in JVM mode in a FIPS-enabled environment, and as a native executable when the operating system is not in FIPS mode.
====
