...
public class BooleanConfigType extends LeafConfigType {
    private static final MethodDescriptor BOOL_VALUE_METHOD = MethodDescriptor.ofMethod(Boolean.class, "booleanValue", boolean.class);
    ...
    public void acceptConfigurationValueIntoGroup(final Object enclosing, final Field field, final NameIterator name, final SmallRyeConfig config) {
        try {
            field.setBoolean(enclosing, config.getOptionalValue(name.toString(), Boolean.class).orElse(config.convert(defaultValue, Boolean.class)).booleanValue());
    ...
    void generateGetDefaultValueIntoEnclosingGroup(final BytecodeCreator body, final ResultHandle enclosing, final MethodDescriptor setter, final ResultHandle config) {
        final ResultHandle value = body.invokeVirtualMethod(BOOL_VALUE_METHOD, body.checkCast(getConvertedDefault(body, config), Boolean.class));
