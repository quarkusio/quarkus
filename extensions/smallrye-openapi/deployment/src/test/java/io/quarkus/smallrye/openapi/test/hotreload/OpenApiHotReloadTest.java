...
public class OpenApiHotReloadTest {
    ...
    public void testAddingAndDeletingEndpoint() {
        RestAssured.get("/q/openapi").then().statusCode(200).body(containsString("/api"));

        TEST.modifySourceFile("MyResource.java", s -> s.replace("// <placeholder>", "@GET @Path(\"foo\")" // Important -
                // must be on
                // the same
                // line
                + "public String greeting() { " + " return \"bonjour\"; " + "}"));
        ...
        RestAssured.get("/q/openapi").then().statusCode(200).body(containsString("/api"))
                .body(not(containsString("/api/foo"))).body(containsString("/api/bar"));

        TEST.modifySourceFile("MyResource.java", s -> s.replace("@GET @Path(\"bar\")", ""));

        RestAssured.get("/q/openapi").then().statusCode(200).body(containsString("/api"))
                .body(not(containsString("/api/foo"))).body(not(containsString("/api/bar")));
    }
    ...
    public void testAddingAndUpdatingResource() {
        ...
        RestAssured.get("/q/openapi").then().statusCode(200).body(containsString("/api"))
                .body(not(containsString("/my-second-api"))).body(containsString("/foo"));
    }
