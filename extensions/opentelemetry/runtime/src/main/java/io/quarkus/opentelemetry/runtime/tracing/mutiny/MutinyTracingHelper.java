...
public class MutinyTracingHelper {
     ...
     * by running on a duplicated context. The span will be closed when the pipeline completes. If there is already a
     * span in the current context, it will be used as parent for the new span.
     * <p>
     * Use as follows: Given this existing pipeline: ```java Uni.createFrom().item("Hello") .onItem().transform(s -> s +
     * " World") .subscribe().with(System.out::println); ``` wrap like this: ```java Uni.createFrom().item("Hello")
     * .onItem().transformToUni(s -> wrapWithSpan(tracer, "mySpan", Uni.createFrom().item(s + " World")))
     * .subscribe().with(System.out::println); ```
     * <p>
     * it also works with multi: ```java Multi.createFrom().items("Alice", "Bob", "Charlie") .onItem().transform(name ->
     * "Hello " + name) .subscribe().with(System.out::println); ``` wrap like this: ```java
     * Multi.createFrom().items("Alice", "Bob", "Charlie") .onItem().transformToUni(s -> wrapWithSpan(tracer, "mySpan",
     * Uni.createFrom().item("Hello " + s) .onItem().transform(name -> "Hello " + name) ))
     * .subscribe().with(System.out::println); ```
     *
     * @param <T>
    ...
    /**
     * see {@link #wrapWithSpan(Tracer, String, Uni)} use this method if you manually want to specify the parent context
     * of the new span or if you want to ensure the new span is a root span.
     *
     * @param <T>
     * @param parentContext
    ...
    public static <T> Uni<T> wrapWithSpan(final Tracer tracer,
            final Optional<io.opentelemetry.context.Context> parentContext, final String spanName,
            final Uni<T> pipeline) {
