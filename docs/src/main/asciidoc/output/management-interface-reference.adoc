...
By default, {product-name-first} exposes the _management_ endpoints under `/q` on the main HTTP server.
...
{product-name} can be accessed through proxies that generate headers (e.g. `X-Forwarded-Host`) to preserve information about the original request.
{product-name} can be configured to automatically update information like protocol, host, port and URI to use the values from those headers.
...
When {product-name} generates the Kubernetes metadata, it checks if the management interface is enabled and configures the probes accordingly.
...
More details about Basic authentication in {product-name} can be found in the xref:security-basic-authentication-howto.adoc[Basic authentication guide].
