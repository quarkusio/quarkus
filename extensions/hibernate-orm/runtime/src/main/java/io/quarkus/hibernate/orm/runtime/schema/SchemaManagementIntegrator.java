...
public class SchemaManagementIntegrator implements Integrator, DatabaseSchemaProvider {
    ...
    public void integrate(Metadata metadata, SessionFactoryImplementor sessionFactory,
        ...
        metadataMap.put(defaultName(sessionFactory.getName()), new Holder(metadata, sessionFactory, serviceRegistry));
    }

    @Override
    public void disintegrate(SessionFactoryImplementor sessionFactory, SessionFactoryServiceRegistry serviceRegistry) {
        metadataMap.remove(defaultName(sessionFactory.getName()));
    }
    ...
    static String defaultName(String name) {
        if (name == null) {
    ...
    public static void recreateDatabase(String name) {
        ...
        if (prop != null && !(prop.toString().equals("none"))) {
            ...
            val.sessionFactory.getServiceRegistry().getService(SchemaManagementTool.class).getSchemaDropper(new HashMap())
                    .doDrop(val.metadata, new SimpleExecutionOptions(), new SimpleSourceDescriptor(),
                            new SimpleTargetDescriptor());
            val.sessionFactory.getServiceRegistry().getService(SchemaManagementTool.class).getSchemaCreator(new HashMap())
