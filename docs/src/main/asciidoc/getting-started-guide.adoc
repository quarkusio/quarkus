...
Note that Shamrock uses JUnit 5 by default, which means that you may need to upgrade the version of the https://maven.apache.org/surefire/maven-surefire-plugin/[Surefire Maven Plugin] that is in use.
This is done in the `<build><plugins>` section of  `pom.xml`:
