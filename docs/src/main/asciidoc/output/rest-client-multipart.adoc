...
[WARNING]
====
This guide is about the multipart support of the REST Client compatible with https://resteasy.dev[RESTEasy Classic] which used to be the default Jakarta REST (formerly known as JAX-RS) implementation until {product-name-first} 2.8.
...
The solution is located in the `rest-client-multipart-quickstart` link:{quickstarts-tree-url}/rest-client-multipart-quickstart[directory].
